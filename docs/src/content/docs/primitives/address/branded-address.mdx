...
- **Zero-overhead** [branded type](/primitives/branded-types) wrapping `Uint8Array` (20 bytes)
...
Runtime-validated 20-byte `Uint8Array`. [Brand](/primitives/branded-types) prevents accidental mixing with other Uint8Arrays.

Defined in: [primitives/Address/BrandedAddress/BrandedAddress.js](https://github.com/evmts/primitives/blob/main/src/primitives/Address/BrandedAddress/BrandedAddress.js)
...
```typescript
import * as BrandedAddress from '@tevm/voltaire/BrandedAddress'
...
import {
  ...
  fromBase64,
  zero
} from '@tevm/voltaire/BrandedAddress'
...
**Tree-shaking note:** `toChecksummed` and `prettyPrint` include keccak256.
...
const create2Addr = calculateCreate2Address(deployerAddr, salt, initCode)
// Uses keccak256
```
...
```typescript
import { toHex, equals, isZero } from '@tevm/voltaire/BrandedAddress'
...
```typescript
import { from, toHex, equals, toU256 } from '@tevm/voltaire/BrandedAddress'
...
```typescript
import { from, toHex, toChecksummed } from '@tevm/voltaire/BrandedAddress'
...
```typescript
import { from, calculateCreateAddress } from '@tevm/voltaire/BrandedAddress'
...
| Method/Function | Keccak256 | RLP | Notes |
|-----------------|-----------|-----|-------|
...
| `toChecksummed`, `prettyPrint` | ✓ | ✗ | EIP-55 checksumming |
...
```typescript
import { Address, BrandedAddress } from '@tevm/voltaire'
import { toHex, equals } from '@tevm/voltaire/BrandedAddress'
...
const branded: BrandedAddress = BrandedAddress.from("0x...")
...
```typescript
import { SIZE, HEX_SIZE } from '@tevm/voltaire/BrandedAddress'
