...
`BrandedGwei` is a branded `BrandedUint` (Uint8Array-based) providing compile-time type safety for Gwei values. Unlike the namespace API (`Gwei.*`), BrandedGwei uses direct data representation with minimal wrapping.
...
**Gwei (namespace)** - Flexible, accepts any input:
```typescript
import * as Gwei from '@tevm/voltaire/Gwei'

Gwei.toWei("50")        // Converts string to Gwei, then to Wei
Gwei.from(50n)          // Auto-converts any numeric input
```

**BrandedGwei** - Type-safe, requires BrandedGwei:
```typescript
import * as BrandedGwei from '@tevm/voltaire/BrandedGwei'

const gwei = BrandedGwei.from(50n)  // Create BrandedGwei
BrandedGwei.toWei(gwei)             // Must pass BrandedGwei type
```

Key differences:
- **Gwei namespace**: Auto-converts inputs, wrapper functions
- **BrandedGwei**: Enforces type at compile time, direct data operations
- **Performance**: BrandedGwei avoids wrapper overhead
- **Use case**: Gwei for convenience, BrandedGwei for type safety
...
const gwei1 = BrandedGwei.from(50n)      // 50 Gwei gas price
const gwei2 = BrandedGwei.from(21)       // 21 Gwei
const gwei3 = BrandedGwei.from("100")    // String input
const gwei4 = BrandedGwei.from("0x32")   // Hex input (50 Gwei)
```
...
const gwei2 = BrandedGwei.from(21n)
...
const gasPriceGwei = BrandedGwei.from(50n)
...
console.log(`Total: ${BrandedWei.from(totalCost)} Wei`)
```
...
const baseFee = BrandedGwei.from(30n)
const priorityFee = BrandedGwei.from(2n)
...
const maxFeeGwei = BrandedGwei.from(maxFeeU256)  // 32n Gwei
...
const largeGwei = BrandedGwei.from(1_000_000_000n)  // 1 ETH
...
// RPC returns Wei
const gasPriceWei = BrandedWei.from(50_000_000_000n)
...
// Gas price levels
const LOW_GAS = BrandedGwei.from(10n)      // 10 Gwei - cheap
const NORMAL_GAS = BrandedGwei.from(30n)   // 30 Gwei - normal
const HIGH_GAS = BrandedGwei.from(100n)    // 100 Gwei - fast
const URGENT_GAS = BrandedGwei.from(200n)  // 200 Gwei - urgent

// Priority fees (EIP-1559)
const LOW_PRIORITY = BrandedGwei.from(1n)   // 1 Gwei tip
const MED_PRIORITY = BrandedGwei.from(2n)   // 2 Gwei tip
const HIGH_PRIORITY = BrandedGwei.from(5n)  // 5 Gwei tip
```
...
- [Gwei](/primitives/denomination/gwei) - Namespace API with auto-conversion
