---
title: Gasconstants Fundamentals
description: Learn EVM gas cost model, categories, and calculation patterns
---


<Info>
**Conceptual Guide** - For API reference and method documentation, see [GasConstants API](/primitives/gasconstants/index).
</Info>
Gas is the unit that measures computational work on the Ethereum Virtual Machine. Every operation consumes gas, preventing DoS attacks and incentivizing efficient code. This guide teaches gas fundamentals using Tevm.
...
```typescript
import * as GasConstants from 'tevm/GasConstants';
...
const transferCalldata = new Uint8Array([
  ...
  ...Bytes32(),    // address padded (12 zero + 20 non-zero)
  ...Bytes32()     // uint256 amount (likely has zeros)
]);
...
function estimateERC20Transfer(): bigint {
  ...
  const address = Bytes32(); // 12 zero + 20 non-zero = assume 20 non-zero
  const amount = Bytes32();  // Assume 28 zero + 4 non-zero
