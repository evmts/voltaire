...
```typescript
function hashCode(key: BrandedStorageKey): number
```
...
```zig
pub fn hashCode(key: StorageKey) u64
```
...
console.log(State.StorageKey.hashCode(key1) === State.StorageKey.hashCode(key2)); // true

// Different keys (usually) produce different hashes
console.log(State.StorageKey.hashCode(key1) === State.StorageKey.hashCode(key3)); // false
```
...
// Same key produces same hash
const hash1 = State.hashCode(key1);
const hash2 = State.hashCode(key2);
const hash3 = State.hashCode(key3);
