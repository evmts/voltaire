...
```typescript
function toString(key: BrandedStorageKey): string
```
...
```zig
pub fn toString(buffer: []u8, key: StorageKey) ![]const u8
```
...
const key = State.StorageKey{ .address = contract_addr, .slot = 0 };

var buffer: [128]u8 = undefined;
const key_str = try State.toString(&buffer, key);
