...
<Info>
**Conceptual Guide** - For API reference and method documentation, see [State API](/primitives/state).
</Info>
...
// EOA account
const eoa_address = try Address.from("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e");
const eoa_account = .{
  .balance = 1000000000000000000,  // 1 ETH in wei
  .nonce = 42,                      // Transaction count
  .storage_root = try Hash.from("0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421"),
  .code_hash = try Hash.from("0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470"),
};

// Contract account
const contract_address = try Address.from("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48");
const contract_account = .{
  .balance = 0,
  .nonce = 1,
  .storage_root = try Hash.from("0x..."),
  .code_hash = try Hash.from("0x..."),
};
```
</Tab>
</Tabs>
...
// USDC contract storage
const usdc_address = try Address.from("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48");

// Storage slot 0: total supply
const slot0 = State.StorageKey{
  .address = usdc_address,
  .slot = 0,
};

// User balance slot (computed via keccak256)
const user_address = try Address.from("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e");
const balance_slot = computeMappingSlot(user_address, 1);
const user_balance_key = State.StorageKey{
  .address = usdc_address,
  .slot = balance_slot,
};
```
...
const beforeTransfer = Hash.from("0xaaa...");
// ... execute transfer transaction ...
const afterTransfer = Hash.from("0xbbb...");
...
// Block state root
const block_state_root = try Hash.from("0x123abc...");

// State root changes with any state modification
const before_transfer = try Hash.from("0xaaa...");
// ... execute transfer ...
const after_transfer = try Hash.from("0xbbb...");

const changed = !Hash.equals(before_transfer, after_transfer); // true
```
</Tab>
</Tabs>
...
const contract = try Address.from("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48");

// Create storage keys
const key1 = State.StorageKey{ .address = contract, .slot = 0 };
const key2 = State.StorageKey{ .address = contract, .slot = 1 };

// Compare keys
const equal = State.StorageKey.equals(key1, key2); // false

// Serialize (conceptual - Zig uses different approach)
var buffer: [128]u8 = undefined;
const key_str = try State.StorageKey.toString(&buffer, key1);
...
// State transition
const sender = try Address.from("0xAlice...");
const receiver = try Address.from("0xBob...");
const amount = 1_000_000_000_000_000_000; // 1 ETH

// Before state
var state_before = StateMap.init(allocator);
defer state_before.deinit();

try state_before.put(sender, .{
...
// Apply transaction (modifies state in place)
try applyTransaction(&state_before, .{
  .from = sender,
  .to = receiver,
  .value = amount,
...
// State root changes
const old_root = try computeStateRoot(state_before);
const new_root = try computeStateRoot(state_after);
const changed = !Hash.equals(old_root, new_root); // true
```
</Tab>
</Tabs>
...
const contractAddr = Address.from("0xContract...");
...
fn verifyAccountProof(
  ...
  // 1. RLP encode account
  const account_rlp = try Rlp.encode(allocator, .{
    account_data.nonce,
    account_data.balance,
    account_data.storage_root,
    account_data.code_hash,
  });
  defer allocator.free(account_rlp);

  var current_hash = try Keccak256.hash(account_rlp);

  // 2. Walk up trie
  for (proof) |proof_node| {
    const combined = try std.mem.concat(allocator, u8, &[_][]const u8{
      proof_node,
      &current_hash,
    });
    defer allocator.free(combined);

    current_hash = try Keccak256.hash(combined);
  }

  // 3. Verify against state root
  return Hash.equals(state_root, current_hash);
}
```
</Tab>
</Tabs>
...
async function verifyBalance(
  ...
  const stateRoot = Hash.from(header.result.stateRoot);
