...
Source: [constants.ts:10-11](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint256/constants.ts#L10-L11)
...
// Range checking
function inRange(value: BrandedUint256): boolean {
...
Source: [constants.ts:14-16](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint256/constants.ts#L14-L16)
...
Source: [constants.ts:18-20](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint256/constants.ts#L18-L20)
...
Source: [constants.ts:22-24](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint256/constants.ts#L22-L24)
...
Source: [constants.ts:4-6](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint256/constants.ts#L4-L6)
...
function validateUint256(value: bigint): BrandedUint256 {
  if (value < Uint.MIN) {
...
interface Transaction {
  ...
  nonce: BrandedUint256
  gasLimit: BrandedUint256
}
...
const defaultTx: Transaction = {
  value: Uint.ZERO,
  nonce: Uint.ZERO,
  gasLimit: Uint(21000n)
}

// Optional values with defaults
function transfer(
  amount: BrandedUint256 = Uint.ZERO,
  gasPrice: BrandedUint256 = Uint.ONE
) {
  // ...
}
```
...
const mask8bit = Uint(0xffn)          // 8-bit mask
const mask16bit = Uint(0xffffn)       // 16-bit mask
const mask32bit = Uint(0xffffffffn)   // 32-bit mask

// Or create dynamically
function createMask(bits: number): BrandedUint256 {
...
function wouldOverflow(a: BrandedUint256, b: BrandedUint256): boolean {
  return a.greaterThan(Uint.MAX.minus(b))
}

// Safe addition
function safeAdd(a: BrandedUint256, b: BrandedUint256): BrandedUint256 | null {
...
// Half of MAX
const halfMax = Uint.MAX.dividedBy(Uint(2n))
// Or equivalently
...
const WAD = Uint(10n ** 18n)  // 18 decimals (like ether)
const RAY = Uint(10n ** 27n)  // 27 decimals (high precision)

function wadMul(a: BrandedUint256, b: BrandedUint256): BrandedUint256 {
  return a.times(b).dividedBy(WAD)
}

function wadDiv(a: BrandedUint256, b: BrandedUint256): BrandedUint256 {
  return a.times(WAD).dividedBy(b)
}

// Example: 2.5 * 1.5 = 3.75
const a = Uint(25n * 10n ** 17n)  // 2.5 WAD
const b = Uint(15n * 10n ** 17n)  // 1.5 WAD
...
function testBoundaries(fn: (value: BrandedUint256) => BrandedUint256) {
  // Test with zero
  ...
  console.log("Small:", fn(Uint(100n)))

  // Test with large value
  console.log("Large:", fn(Uint.MAX.dividedBy(Uint(2n))))
...
const UINT8_MAX = Uint((2n ** 8n) - 1n)    // 255
const UINT16_MAX = Uint((2n ** 16n) - 1n)  // 65535
const UINT32_MAX = Uint((2n ** 32n) - 1n)  // 4294967295
const UINT64_MAX = Uint((2n ** 64n) - 1n)  // 18446744073709551615
const UINT128_MAX = Uint((2n ** 128n) - 1n) // 2^128 - 1
const UINT256_MAX = Uint.MAX                     // 2^256 - 1

// Common Ethereum values
const GWEI = Uint(10n ** 9n)      // 1 gwei = 10^9 wei
const ETHER = Uint(10n ** 18n)    // 1 ether = 10^18 wei

// Gas limits
const MIN_GAS = Uint(21000n)      // Minimum transaction gas
const BLOCK_GAS_LIMIT = Uint(30_000_000n)  // Typical block gas limit
```
