...
Transaction hashing currently available in TypeScript only. FFI overhead makes native calls inefficient for transaction operations.

Use TypeScript API for transaction hashing and signing operations.
...
**Returns:** `BrandedHash` - 32-byte keccak256 signing hash
...
Transaction signing hash computation currently available in TypeScript only. FFI overhead makes native calls inefficient for transaction operations.
...
- [Hash](/primitives/hash) - 32-byte hash primitive
