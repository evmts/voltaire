...
```typescript
function getChainId(tx: Any): bigint | undefined
```

**Parameters:**
- `tx: Any` - Any transaction type

**Returns:** `bigint | undefined` - Chain ID if present, undefined for pre-EIP-155 legacy transactions
...
const chainId = getChainId(eip1559)
...
const legacyChainId = getChainId(legacy)
console.log(legacyChainId) // 1n
...
const noChainId = getChainId(preEip155)
console.log(noChainId) // undefined
```

**Source:** [getChainId.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Transaction/getChainId.ts)
