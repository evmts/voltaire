...
const eip1559: Transaction.EIP1559 = {
  ...
  gasLimit: 21000n,
  to: Address.fromHex('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e'),
...
// "Legacy tx to 0x..., value: 0.5 ETH, nonce: 0"

// Contract creation (to = null)
const creation: Transaction.EIP1559 = {
  type: Transaction.Type.EIP1559,
  chainId: 1n,
  nonce: 3n,
  maxPriorityFeePerGas: 2000000000n,
...
console.log(format(creation))
// "EIP-1559 contract creation, data: 1234 bytes, nonce: 3"
```

**Source:** [format.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Transaction/format.ts)
...
function customFormat(tx: Transaction.Any): string {
  ...
  const data = tx.data.length > 0 ?
    ` | Data: ${Hex.fromBytes(tx.data).slice(0, 10)}...` : ''
