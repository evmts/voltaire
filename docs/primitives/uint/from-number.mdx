...
## `Uint.fromNumber(value: number): BrandedUint`

Create Uint256 from number. Must be non-negative integer.
...
Uint.fromNumber(3.14)      // Error: not integer
Uint.fromNumber(NaN)       // Error: invalid
Uint.fromNumber(Infinity)  // Error: invalid
```

**Defined in:** [primitives/Uint/BrandedUint/fromNumber.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Uint/BrandedUint/fromNumber.ts)

</Tab>
<Tab title="Namespace API (Tree-shakeable)">

## `fromNumber(value: number): BrandedUint`
...
const a = try Uint.fromInt(100);
const b = try Uint.fromInt(@as(u64, 255));
```
...
```typescript
function getElement(index: number): BrandedUint {
...
```typescript
function safeFromNumber(n: number): BrandedUint {
...
- [fromBigInt](/primitives/uint/from-bigint) - From BigInt (for large values)
