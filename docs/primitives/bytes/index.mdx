...
Generic fixed-size byte types that form the foundation of Voltaire's type system. Bytes represents N bytes with compile-time type safety through TypeScript branding and hardcoded length properties.

<Info>
**Type System Foundation** - Bytes is the base pattern for all fixed-size types in Voltaire. Address (Bytes20), Keccak256Hash (Bytes32), and all other fixed-size primitives extend this pattern.
</Info>
