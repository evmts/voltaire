...
// Merkle node
const node = Bytes32.fromHex('0x' + 'ab'.repeat(32));
...
const bytes = Bytes32.fromBytes(hash);

// Merkle tree nodes
const left = Bytes32.fromHex('0x...');
const right = Bytes32.fromHex('0x...');
```
...
const slot: BrandedBytes32 = Bytes32.fromNumber(0);

// Convert between them
const hashAsBytes = Bytes32.fromBytes(hash);
...
```typescript
const bytes32 = Bytes32.from(42);
const bytes16 = Bytes16.from('0x' + '12'.repeat(16));
...
```typescript
// From hex
const b1 = Bytes32.from('0x' + '12'.repeat(32));

// From bytes
const b2 = Bytes32.from(new Uint8Array(32));

// From number (padded)
const b3 = Bytes32.from(42);

// From bigint (padded)
const b4 = Bytes32.from(123456789012345678901234567890n);
```
...
// Without 0x prefix
const bytes2 = Bytes32.fromHex('12'.repeat(32));

// Throws on wrong length
Bytes32.fromHex('0x1234'); // Error: must be 64 hex chars
```
...
const bytes = Bytes32.fromBytes(arr);

// Throws on wrong size
Bytes32.fromBytes(new Uint8Array(31)); // Error
```
...
```typescript
const slot = Bytes32.fromNumber(0);
const value = Bytes32.fromNumber(42);
...
```typescript
const large = Bytes32.fromBigint(123456789012345678901234567890n);
...
```typescript
const bytes = Bytes32.fromNumber(255);
...
const large = Bytes32.fromHex('0x' + 'ff'.repeat(32));
...
const storageSlot = Bytes32.fromHex('0x' + '00'.repeat(12) + 'aa'.repeat(20));
...
```typescript
try {
  Bytes32.fromBytes(new Uint8Array(31));
} catch (error) {
...
```typescript
// Slot 0
const slot0 = Bytes32.fromNumber(0);

// Slot for address mapping
function mappingSlot(key: Address, slot: number): Bytes32 {
  const data = concat([key, Bytes32.fromNumber(slot)]);
  const hash = Hash.keccak256(data);
  return Bytes32.fromBytes(hash);
}
```
...
```typescript
// Merkle tree node
const leftNode = Bytes32.fromHex('0x...');
const rightNode = Bytes32.fromHex('0x...');

// Compute parent
const parent = Hash.keccak256(concat([leftNode, rightNode]));
const parentNode = Bytes32.fromBytes(parent);
```
...
```typescript
// Block hash
const blockHash = Bytes32.fromHex('0x...');

// Transaction hash
const txHash = Bytes32.fromHex('0x...');

// State root
const stateRoot = Bytes32.fromHex('0x...');
```
