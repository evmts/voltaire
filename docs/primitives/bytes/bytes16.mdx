...
// Create from hex
const bytes = Bytes16.from('0x' + '12'.repeat(16));
...
Bytes16.from(new Uint8Array(15)); // Error: must be 16 bytes
```
...
**UUIDs (128-bit)**
```typescript
const uuid = Bytes16.fromHex('550e8400e29b41d4a716446655440000');
```

**Medium Hashes**
```typescript
// MD5, RIPEMD-128, etc.
const hash = Bytes16.fromHex('0x...');
```

**Fixed-Size Keys**
```typescript
const key = Bytes16.fromHex('0x' + 'ab'.repeat(16));
```
...
```typescript
// From hex
const b1 = Bytes16.from('0x' + '12'.repeat(16));

// From bytes
const b2 = Bytes16.from(new Uint8Array(16));
```
...
// Without 0x prefix
const bytes2 = Bytes16.fromHex('ab'.repeat(16));

// Throws on wrong length
Bytes16.fromHex('0x1234'); // Error: must be 32 hex chars
```
...
const bytes = Bytes16.fromBytes(arr);

// Throws on wrong size
Bytes16.fromBytes(new Uint8Array(15)); // Error
```
...
```typescript
const a = Bytes16.fromHex('0x' + 'aa'.repeat(16));
const b = Bytes16.fromHex('0x' + 'aa'.repeat(16));
...
```typescript
const a = Bytes16.fromHex('0x' + '01'.repeat(16));
const b = Bytes16.fromHex('0x' + '02'.repeat(16));
...
```typescript
const original = Bytes16.from('0x' + '12'.repeat(16));
...
const nonZero = Bytes16.fromHex('0x' + '00'.repeat(15) + '01');
...
```typescript
try {
  Bytes16.fromBytes(new Uint8Array(15));
} catch (error) {
