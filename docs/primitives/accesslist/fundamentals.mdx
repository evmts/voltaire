---
title: Accesslist Fundamentals
description: Learn EIP-2930 access lists, gas mechanics, and when to use pre-declared storage access
sidebarTitle: AccessList
---

<Info>
**Conceptual Guide** - For API reference and method documentation, see [AccessList API](/primitives/accesslist/index).
</Info>
...
function getBalanceSlot(user: Address): Hash {
  ...
  const padding = Hex.from('0x000000000000000000000000'); // 12 bytes padding
  const slotBytes = Uint.toBytes(Uint.fromBigint(slot, 32));

  const encoded = Hex.toBytes(
    Hex.concat(
      Hex.fromBytes(userBytes),
      padding,
      Hex.fromBytes(slotBytes)
    )
  );
