...
## `eventLog.matchesTopics(topics: readonly (BrandedHash | BrandedHash[] | null)[]): boolean`

Check if log topics match filter. `null` entries match any topic, arrays match any of the hashes (OR logic).

**Parameters:**
- `topics: readonly (BrandedHash | BrandedHash[] | null)[]` - Topic filter array
  - `null` - Matches any topic at that position
  - `BrandedHash` - Matches that specific hash
  - `BrandedHash[]` - Matches any hash in array (OR logic)

**Returns:** `boolean` - True if all non-null filter topics match
...
const matches1 = log.matchesTopics([
  TRANSFER_SIG,  // topic0 must match
  null,          // topic1 matches any
  userHash,      // topic2 must match
]);
...
const matches2 = log.matchesTopics([
  TRANSFER_SIG,
  null,
  [userHash, anotherHash, thirdHash], // topic2 matches any of these
]);
console.log(matches2); // true
```

**Defined in:** [primitives/EventLog/BrandedEventLog/matchesTopics.js](https://github.com/evmts/voltaire/blob/main/src/primitives/EventLog/BrandedEventLog/matchesTopics.js)
...
const log = EventLog.from({
  address: Address.fromHex('0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2'),
  topics: [TRANSFER_SIG, Hash.fromHex('0x000...from'), userHash],
  data: Hex.from('0x'),
});
...
const TRANSFER_SIG = Hash.fromHex('0xddf252ad...');
const userAddress = Address.fromHex('0xa1b2c3d4...');
const userHash = Hash.from(
  Hex.concat(Hex.from('0x000000000000000000000000'), Address.toHex(userAddress))
);
...
const TRANSFER_SIG = Hash.fromHex('0xddf252ad...');
const APPROVAL_SIG = Hash.fromHex('0x8c5be1e5...');
const SWAP_SIG = Hash.fromHex('0xd78ad95f...');
...
const groupHashes = [
  Hash.fromHex('0x000...addr1'),
  Hash.fromHex('0x000...addr2'),
  Hash.fromHex('0x000...addr3'),
];
...
const SWAP_SIG = Hash.fromHex('0xd78ad95f...');
const routerHash = Hash.fromHex('0x000...router');
...
const TRANSFER_SIG = Hash.fromHex('0xddf252ad...');
const user1Hash = Hash.fromHex('0x000...user1');
const user2Hash = Hash.fromHex('0x000...user2');
const user3Hash = Hash.fromHex('0x000...user3');
...
const matches1 = log.matchesTopics([Hash.fromHex('0x...')]);
