...
## `EventLog.sortLogs(logs: readonly BrandedEventLog[]): BrandedEventLog[]`

Sort logs by block number (ascending) then log index (ascending). Returns new sorted array.

**Parameters:**
- `logs: readonly BrandedEventLog[]` - Array of event logs to sort

**Returns:** `BrandedEventLog[]` - New array sorted chronologically
...
const logs = [
  EventLog({
    ...
    topics: [Hash.fromHex('0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef')],
    data: Hex.from('0x'),
...
const logs = [
  EventLog.from({ /* block 2, index 5 */ }),
  EventLog.from({ /* block 1, index 10 */ }),
  EventLog.from({ /* block 2, index 2 */ }),
];

const sorted = EventLog.sortLogs(logs);

// Sorted by block, then by index within block
```
