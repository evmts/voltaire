...
## `EventLog.filterLogs(logs: readonly BrandedEventLog[], filter: Filter): BrandedEventLog[]`

Filter array of logs by complete filter criteria. Returns new array with matching logs.

**Parameters:**
- `logs: readonly BrandedEventLog[]` - Array of event logs to filter
- `filter: Filter` - Filter criteria (address, topics, block range, block hash)

**Returns:** `BrandedEventLog[]` - New array containing only matching logs
...
**Defined in:** [primitives/EventLog/BrandedEventLog/filterLogs.js](https://github.com/evmts/voltaire/blob/main/src/primitives/EventLog/BrandedEventLog/filterLogs.js)
