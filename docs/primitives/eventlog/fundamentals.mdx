...
sidebarTitle: EventLog
---


<Info>
**Conceptual Guide** - For API reference and method documentation, see [EventLog API](/primitives/eventlog).
</Info>
...
const fromAddress = Address.fromHex('0xa1b2c3d4e5f67890abcdef1234567890abcdef12');
const toAddress = Address.fromHex('0xfedcba0987654321fedcba0987654321fedcba09');

// Create topic hashes (addresses are right-aligned in 32-byte topics)
const fromHash = Hash.fromBytes(
  new Uint8Array([...new Uint8Array(12).fill(0), ...fromAddress])
);
const toHash = Hash.fromBytes(
  new Uint8Array([...new Uint8Array(12).fill(0), ...toAddress])
);
...
const EventLog = @import("primitives").EventLog;
...
pub fn main() !void {
    ...
    // ERC-20 Transfer event signature
    const transfer_sig = try Hash.fromHex(allocator, "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef");
    defer transfer_sig.deinit(allocator);

    // Create log (simplified - actual implementation varies)
    var log = EventLog{
        .address = try Address.fromHex(allocator, "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2"),
        .topics = &[_]Hash{transfer_sig, from_hash, to_hash},
        .data = try Hex.toBytes(allocator, "0x0000000000000000000000000000000000000000000000000de0b6b3a7640000"),
    ...
    // Access signature
    const signature = log.getTopic0();
    std.debug.print("Signature: {s}\n", .{try Hash.toHex(allocator, signature)});

    // Access indexed parameters
    const indexed = log.getIndexedTopics();
    std.debug.print("From: {s}\n", .{try Hash.toHex(allocator, indexed[0])});
    std.debug.print("To: {s}\n", .{try Hash.toHex(allocator, indexed[1])});
}
```
...
const daiAddress = Address.fromHex('0x6B175474E89094C44Da98b954EedeAC495271d0F');
...
const userHash = Hash.fromHex('0x000000000000000000000000a1b2c3d4...');
...
const anonymousLog = EventLog({
  ...
  data: new Uint8Array([]), // No non-indexed data
});
...
const userAddress = Address.fromHex('0xa1b2c3d4e5f67890abcdef1234567890abcdef12');
const userHash = Hash.fromBytes(
