...
function isSupportedChain(chainId: BrandedChain): boolean {
  return SUPPORTED_CHAINS.some(id =>
    Chain.equals(chainId, Chain.from(id))
  );
}
...
function getProviderConfig(chainId: BrandedChain): ProviderConfig {
  const configs: Record<number, ProviderConfig> = {
    1: {
      ...
      chainId: Chain.from(1),
      name: "Ethereum Mainnet"
    },
    137: {
      rpcUrl: "https://polygon.llamarpc.com",
      chainId: Chain.from(137),
      name: "Polygon"
    },
    // ... more chains
  };
...
async function createProvider(chainId: BrandedChain): Promise<Provider> {
  ...
  if (!Chain.equals(Chain.from(actualChainId), chainId)) {
    throw new Error(
...
function getGasSettings(chainId: BrandedChain): {
  ...
  if (eip1559Chains.some(id => Chain.equals(chainId, Chain.from(id)))) {
    return {
...
// Test chains
const TEST_CHAINS = {
  mainnet: Chain.from(1),
  sepolia: Chain.from(11155111),
  hardhat: Chain.from(31337),
  ganache: Chain.from(1337)
};
