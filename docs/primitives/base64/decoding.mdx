...
### `base64.decode(allocator: Allocator, encoded: []const u8) ![]u8`

Decodes standard base64 string to bytes. Accepts strings encoded with RFC 4648 alphabet (A-Z, a-z, 0-9, +, /) with padding.

**Parameters:**
- `allocator: Allocator` - Memory allocator for decoded data
- `encoded: []const u8` - Base64 string to decode

**Returns:** `![]u8` - Decoded binary data. Caller owns memory.
...
// Decode binary data
const b64Hash = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
const hash = try base64.decode(allocator, b64Hash);
defer allocator.free(hash);
```

**Defined in:** [primitives/base64.zig:28](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L28)
...
### `base64.decodeUrlSafe(allocator: Allocator, encoded: []const u8) ![]u8`

Decodes URL-safe base64 string to bytes. Accepts strings encoded with URL-safe alphabet (A-Z, a-z, 0-9, -, _) without padding.

**Parameters:**
- `allocator: Allocator` - Memory allocator for decoded data
- `encoded: []const u8` - URL-safe base64 string (without padding)
...
// Decode URL-safe base64 (no padding)
const urlSafe = "SGVsbG8";
const decoded = try base64.decodeUrlSafe(allocator, urlSafe);
defer allocator.free(decoded);
// decoded = [72, 101, 108, 108, 111] ("Hello")

// Works with URL-safe characters (-, _)
const urlSafeData = "AB-_CD";
const data = try base64.decodeUrlSafe(allocator, urlSafeData);
defer allocator.free(data);
```

**Defined in:** [primitives/base64.zig:65](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L65)
