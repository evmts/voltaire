...
### `base64.calcEncodedSize(data_len: usize) usize`

Calculates size of base64-encoded output given input data length. Useful for pre-allocating buffers before encoding.

**Parameters:**
- `data_len: usize` - Length of data to encode in bytes

**Returns:** `usize` - Size of base64 output in characters
...
const encoded = try base64.encode(std.testing.allocator, &data);
defer std.testing.allocator.free(encoded);
// encoded.len == encoded_size
```

**Defined in:** [primitives/base64.zig:37](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L37)
...
Calculates maximum size of decoded output given base64 string length. Returns error if calculation fails.

**Parameters:**
- `encoded_len: usize` - Length of base64 string in characters

**Returns:** `!usize` - Maximum size of decoded output in bytes
...
// Calculate decoded size (note: may error)
const encoded_len: usize = 20;
const max_size = try base64.calcDecodedSize(encoded_len);

// Typical usage would directly use decoder
const encoded = "SGVsbG8sIHdvcmxkIQ==";
const decoded = try base64.decode(std.testing.allocator, encoded);
defer std.testing.allocator.free(decoded);
```

**Note:** In Zig, prefer using `decode` directly rather than pre-calculating size, as the decoder handles allocation internally.

**Defined in:** [primitives/base64.zig:42](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L42)
