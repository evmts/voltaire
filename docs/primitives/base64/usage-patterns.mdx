...
const hexData = Hex.from("0x48656c6c6f");  // "Hello"
...
console.log(Array.from(decoded));  // [1, 2, 3, 4, 5]

// Decode to hex
const hex = Hex.fromBytes(decoded);
...
function safeDecode(input: string): Uint8Array | null {
  ...
  try {
    return Base64.decode(Base64.from(input));
  } catch (err) {
...
function parsePayload(payload: APIPayload): {
  ...
  return {
    data: Base64.decode(Base64.from(payload.data)),
    signature: Base64.decode(Base64.from(payload.signature))
  };
}
```
...
async function loadBinaryData(
  ...
  return Base64.decode(Base64.from(encoded));
}
```
...
function loadConfig(config: Config): {
  ...
  return {
    privateKey: Base64.decode(Base64.from(config.privateKey)),
    salt: Base64.decode(Base64.from(config.salt))
  };
}
```
