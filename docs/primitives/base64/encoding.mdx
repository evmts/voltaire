...
const hash = new Uint8Array(32) // 32-byte hash
...
### `base64.encode(allocator: Allocator, data: []const u8) ![]u8`

Encodes bytes to standard base64 string using RFC 4648 alphabet (A-Z, a-z, 0-9, +, /) with padding.

**Parameters:**
- `allocator: Allocator` - Memory allocator for encoded string
- `data: []const u8` - Binary data to encode

**Returns:** `![]u8` - Standard base64-encoded string with padding. Caller owns memory.
...
const encoded2 = try base64.encode(allocator, &binary);
defer allocator.free(encoded2);
```

**Defined in:** [primitives/base64.zig:17](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L17)
...
const tokenBytes = new Uint8Array(16) // 16-byte random token
...
### `base64.encodeUrlSafe(allocator: Allocator, data: []const u8) ![]u8`

Encodes bytes to URL-safe base64 string. Uses URL-safe alphabet (A-Z, a-z, 0-9, -, _) without padding.

**Parameters:**
- `allocator: Allocator` - Memory allocator for encoded string
- `data: []const u8` - Binary data to encode

**Returns:** `![]u8` - URL-safe base64 string without padding. Caller owns memory.
...
// No +, /, or = characters - safe for URLs and filenames
```

**Defined in:** [primitives/base64.zig:54](https://github.com/evmts/voltaire/blob/main/src/primitives/base64.zig#L54)
...
All encoding methods available as tree-shakeable imports:
...
const urlSafeStr = encodeStringUrlSafe("Hello")
```

**Tree-shaking benefit:** Import only the encoding variant you need. If you only use standard encoding, URL-safe encoding code is excluded from bundle.
