...
Defined in: [primitives/Address/BrandedAddress/BrandedAddress.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/BrandedAddress/BrandedAddress.ts)
...
// Create hex string variants from BrandedAddress (Uint8Array)
...
These functions convert from `BrandedAddress` (Uint8Array) to hex string variants (`ChecksumAddress`, `LowercaseAddress`, `UppercaseAddress`).
...
### BrandedAddress and Address Class
...
### BrandedAddress and Hex Variants

BrandedAddress (Uint8Array) and hex string variants are separate types with conversion functions:

```typescript
import type { BrandedAddress } from '@tevm/voltaire/Address'
import type { ChecksumAddress, LowercaseAddress } from '@tevm/voltaire/Address'

// BrandedAddress = Uint8Array & { [brand]: "Address" }
const addr: BrandedAddress = Address.from("0x742d35Cc...")
...
const backToBytes: BrandedAddress = Address.from(checksumHex) // hex string → Uint8Array
```

**Type relationship:**
- **BrandedAddress:** 20-byte Uint8Array (runtime data)
- **HexAddress variants:** Hex.Sized<20> strings (display/serialization)
