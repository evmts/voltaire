...
## `deduplicateAddresses(addresses: BrandedAddress[]): BrandedAddress[]`

Remove duplicate addresses from array, preserving first occurrence of each unique address.

**Parameters:**
- `addresses: BrandedAddress[]` - Array of addresses to deduplicate

**Returns:** `BrandedAddress[]` - New array with duplicates removed
...
const addr3 = Address.fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e") // Duplicate of addr1

const unique = Address.deduplicateAddresses([addr1, addr2, addr3])
// Returns [addr1, addr2] - preserves first occurrence
```

**Defined in:** [primitives/Address/BrandedAddress/deduplicateAddresses.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/BrandedAddress/deduplicateAddresses.js)
...
Effect.ts does not currently provide a static `deduplicateAddresses` method. Use the BrandedAddress implementation directly:
...
// Convert to BrandedAddress for deduplication
const branded = [addr1.address, addr2.address, addr3.address]
const unique = BrandedAddress.deduplicateAddresses(branded)

// Convert back to Effect.ts Address if needed
const effectAddresses = unique.map(addr => new Address({ value: addr }))
```
...
Zig does not provide array deduplication utilities. Implement manually using equality comparison:

```zig
const std = @import("std");
const Address = @import("primitives").Address;

pub fn deduplicateAddresses(
    ...
    var seen = std.ArrayList(Address){};
    defer seen.deinit(allocator);

    for (addresses) |addr| {
        var found = false;
        for (seen.items) |seen_addr| {
            if (std.mem.eql(u8, &addr.bytes, &seen_addr.bytes)) {
                found = true;
                break;
            }
        }
...
const addr1a = Address.fromHex("0x0000000000000000000000000000000000000001")
const addr2 = Address.fromHex("0x0000000000000000000000000000000000000002")
const addr1b = Address.fromHex("0x0000000000000000000000000000000000000001")
...
const allowlist = new Allowlist([
  ...
  Address.fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e"), // Duplicate
])
...
function getUniqueTokenHolders(logs: EventLog[]): BrandedAddress[] {
  ...
  for (const log of logs) {
    ...
    if (log.topics.length >= 3) {
      ...
      const from = Address.fromAbiEncoded(log.topics[1]!)
      holders.push(from)

      // topics[2] is 'to' address
      const to = Address.fromAbiEncoded(log.topics[2]!)
      holders.push(to)
    }
  }
...
**Algorithm complexity:** O(n²) time, O(n) space - uses nested loops for comparison.
