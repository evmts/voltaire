...
function requireChecksummed(addr: string): BrandedAddress {
  if (!Address.isValidChecksum(addr)) {
    throw new Error(
      `Address checksum invalid. Expected: ${Address.toChecksummed(Address.from(addr))}`
    );
  }
...
function normalizeAddress(addr: string): string {
  const parsed = Address.from(addr);
...
const KNOWN_ADDRESSES = {
  zero: Address.zero(),
  weth: Address.from("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"),
...
const TEST_ADDRESSES = {
  alice: Address.from("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e"),
  bob: Address.from("0x1234567890123456789012345678901234567890"),
  carol: Address.from("0xabcdefabcdefabcdefabcdefabcdefabcdefabcd")
};
...
function generateTestAddresses(count: number): BrandedAddress[] {
  return Array.from({ length: count }, (_, i) => testAddress(i));
}
