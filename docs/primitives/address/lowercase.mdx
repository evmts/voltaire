...
The `Lowercase` type is a **branded string** extending `Hex.Sized<20>`, not a `Uint8Array` like `BrandedAddress`.
...
```typescript
import * as LowercaseAddress from '@tevm/voltaire/Address/LowercaseAddress'
import type { BrandedAddress, Lowercase } from '@tevm/voltaire/Address'
