...
**Type difference:**
- `toLowercase()` returns branded `Lowercase` type extending `Hex.Sized<20>` with `__lowercase: true` property
- `toHex()` returns generic hex string
- Both are strings at runtime - type tags exist only at compile time
...
const addr = Address("0xAbCdEf1234567890")
...
```typescript
import { Address } from 'tevm'
import type { Lowercase } from 'tevm/Address'
...
// Type error - addr.toHex() returns HexType, not Lowercase
