...
<Tip>
View the complete executable example at [`playground/src/examples/primitives/address/abi-encoded.ts`](https://github.com/evmts/voltaire/blob/main/playground/src/examples/primitives/address/abi-encoded.ts).
</Tip>
...
The `fromAbiEncoded` function throws typed errors for invalid input:

```typescript
import { Address, InvalidAddressLengthError } from '@tevm/voltaire'

// Wrong length - throws InvalidAddressLengthError
try {
  const tooShort = new Uint8Array(20)
  Address.fromAbiEncoded(tooShort)
} catch (e) {
  if (e instanceof InvalidAddressLengthError) {
    console.error(e.name)     // "InvalidAddressLengthError"
    console.error(e.code)     // "INVALID_ABI_ENCODED_LENGTH"
    console.error(e.value)    // 20
    console.error(e.expected) // "32 bytes"
  }
}
...
try {
  ...
  if (e instanceof InvalidAddressLengthError) {
    console.error(e.message) // "ABI-encoded Address must be exactly 32 bytes, got 33"
  }
}
```
...
| Error | When Thrown |
|-------|-------------|
| `InvalidAddressLengthError` | Input is not exactly 32 bytes |
