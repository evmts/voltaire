...
**Defined in:** [primitives/Address/address.zig:45](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/address.zig#L45)
...
Static method that creates zero address using Effect.ts Schema API.

**Returns:** `Address` - Effect.ts Schema class instance wrapping zero address
...
const program = Effect.gen(function* () {
  const addr = yield* Effect.sync(() => Address.zero())

  // Check if zero
  const isZero = yield* Effect.sync(() => addr.isZero())
...
console.log(result) // "0x0000000000000000000000000000000000000000"
```
...
// Schema validation
const validateAddress = Schema.decodeUnknownSync(Schema.instanceOf(Address))

const zero = Address.zero()
const validated = validateAddress(zero) // Type-safe validation
```

**Defined in:** [primitives/Address/effect.ts:100](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/effect.ts#L100)
