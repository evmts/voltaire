...
const restored = encoded.map(b64 => Address(b64))
```
...
const fromAddr = Address(decoded.from)
const toAddr = Address(decoded.to)
```
...
const max = Address(new Uint8Array(20).fill(0xFF))
