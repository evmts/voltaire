...
## `address.compare(other: Address | BrandedAddress): number`

Lexicographic comparison with automatic schema validation.

**Parameters:**
- `other: Address | BrandedAddress` - Address to compare with (accepts both Effect and branded types)
...
// Direct comparison
console.log(addr1.compare(addr2)) // -1

// Effect composition
const program = Effect.gen(function* () {
  const a1 = yield* Effect.sync(() => Address.from(10n))
  const a2 = yield* Effect.sync(() => Address.from(20n))
  const result = yield* Effect.sync(() => a1.compare(a2))
  return result
})
...
The C API currently provides `primitives_address_equals()` but not a three-way comparison. Implement lexicographic comparison manually:
...
**See also:** [primitives_address_equals()](/primitives/address/equals#c) for equality comparison
