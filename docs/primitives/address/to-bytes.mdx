...
Convert Address schema instance to plain Uint8Array. Returns a copy of the underlying bytes.

**Returns:** `Uint8Array` - 20-byte array (unbranded)
...
const addr = Address.fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e")
const bytes = addr.address // Access BrandedAddress directly

// Convert to plain Uint8Array (copy)
const plainBytes = new Uint8Array(addr.address)
console.log(plainBytes.length) // 20
console.log(plainBytes[0])     // 0x74
```
...
const bytes: Uint8Array = addr.address

// Encode/decode with Schema.Uint8ArrayFromSelf
const BytesSchema = Schema.Uint8ArrayFromSelf
const encoded = Schema.encodeSync(BytesSchema)(bytes)
```

**Effect Schema Features:**
- Access underlying BrandedAddress via `.address` property
- Schema validates 20-byte length
- Type-safe encoding/decoding
- Integration with Effect's type system

**Defined in:** [primitives/Address/effect.ts:37](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/effect.ts#L37)
