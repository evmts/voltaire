...
Computes Keccak-256 hash of hex-encoded data. Decodes hex to bytes then hashes.
...
## `keccak256Hex(hex: string): BrandedHash`
...
// Hash ABI-encoded data
const abiEncoded = "0x000000000000000000000000742d35cc6634c0532925a3b844bc9e7595f51e3e"
const hash2 = Hash.keccak256Hex(abiEncoded)
```
...
## Manual Hex Decoding + Hash

Zig requires manual hex decoding:
...
// Decode hex string
const hex = "1234";
const bytes = try std.fmt.allocPrint(allocator, "{}", .{hex});
defer allocator.free(bytes);

var decoded = try allocator.alloc(u8, hex.len / 2);
defer allocator.free(decoded);
_ = try std.fmt.hexToBytes(decoded, hex);

// Hash
const hash = keccak(decoded);
```
