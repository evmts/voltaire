...
- **Zero-overhead** [branded type](/getting-started/branded-types) wrapping `Uint8Array` (32 bytes)
...
Runtime-validated 32-byte `Uint8Array`. Uses [symbol branding](/getting-started/branded-types) to prevent accidental mixing with other Uint8Arrays.

Defined in: [primitives/Hash/BrandedHash/BrandedHash.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Hash/BrandedHash/BrandedHash.ts)
...
<Note>
Use `Hash()` constructor instead of `Hash.from()` for Class API. `Hash.from()` remains as an alias.
</Note>
...
const branded: BrandedHash.BrandedHash = BrandedHash("0x...")
