...
<Info>
**Conceptual Guide** - For API reference and method documentation, see [Hash API](/primitives/hash/index).
</Info>
...
const publicKey = Bytes64(); // ... actual key bytes

// Hash and take last 20 bytes
const hash = Hash.keccak256(publicKey);
const address = hash.slice(12, 32); // Last 20 bytes

console.log(`Address: 0x${Array(address).map(b => b.toString(16).padStart(2, '0')).join('')}`);
```
