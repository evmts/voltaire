...
Validates hex string is valid hash format. Checks length and hex characters.

**Parameters:**
- `hex: string` - Hex string to validate (with or without `0x` prefix)

**Returns:** `boolean` - True if valid hash hex format (64 hex chars)
...
Hash.isValidHex("not hex") // false (invalid chars)
Hash.isValidHex("") // false (empty)
...
**Defined in:** [primitives/Hash/BrandedHash/isValidHex.js:19](https://github.com/evmts/voltaire/blob/main/src/primitives/Hash/BrandedHash/isValidHex.js#L19)
...
**Parameters:**
- `hex: string` - Hex string to validate
...
Zig requires manual hex validation:
...
fn isValidHashHex(hex: []const u8) bool {
    // Strip 0x prefix if present
    const clean = if (std.mem.startsWith(u8, hex, "0x"))
        hex[2..]
    else
        hex;

    // Must be exactly 64 hex chars
    if (clean.len != 64) return false;
...
if (Hash.isValidHex(input)) {
  const hash = Hash.fromHex(input) // Validates again
}
