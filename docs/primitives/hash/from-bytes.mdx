...
const hash = Hash(new Uint8Array(buffer))

// Or from offset
const bigBuffer = new ArrayBuffer(64)
const hash2 = Hash(new Uint8Array(bigBuffer, 0, 32))
```
...
const hash = Hash(new Uint8Array(u32Array.buffer))
```
...
```typescript
// All valid (32 bytes)
Hash(new Uint8Array(32)) // All zeros
Hash(new Uint8Array([0xff, ...new Array(31).fill(0)])) // Any values

// Invalid (wrong length)
Hash(new Uint8Array(31)) // Error: wrong length
Hash(new Uint8Array(33)) // Error: wrong length
Hash(new Uint8Array(0)) // Error: wrong length
```
...
const genesisHash = Hash(GENESIS_HASH)
```
