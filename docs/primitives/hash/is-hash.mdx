...
## `Hash.isHash(value: unknown): value is BrandedHash`

Type guard to check if value is a valid Hash. Validates type and length.

**Parameters:**
- `value: unknown` - Value to check

**Returns:** `boolean` - True if value is Hash type (Uint8Array with 32 bytes)

**Type guard:** Narrows type to `BrandedHash` when true
...
if (Hash.isHash(value)) {
  ...
  console.log(value.length) // 32
}

// Check before using
const maybeHash = getUserInput()
if (Hash.isHash(maybeHash)) {
  processHash(maybeHash)
} else {
  console.error("Invalid hash")
}
```

**Defined in:** [primitives/Hash/BrandedHash/isHash.js:19](https://github.com/evmts/voltaire/blob/main/src/primitives/Hash/BrandedHash/isHash.js#L19)

</Tab>
<Tab title="Namespace API">

## `isHash(value: unknown): value is BrandedHash`
...
**Returns:** `boolean` - True if value is Hash type
...
fn processHash(hash: [32]u8) void {
    // Type is enforced at compile time
    // No runtime check needed
}

const hash: [32]u8 = .{0} ** 32;
processHash(hash); // Compiles

const wrong: [31]u8 = .{0} ** 31;
// processHash(wrong); // Compile error
```
