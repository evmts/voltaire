...
Returns copy of hash as raw bytes. Creates new Uint8Array - modifications won't affect original.
...
// Safe mutation (doesn't affect hash)
bytes[0] = 0xff
console.log(hash[0]) // Original unchanged
```
...
## `toBytes(hash: BrandedHash): Uint8Array`
...
Zig `[32]u8` arrays are already bytes - no conversion needed:
...
No allocation - arrays are stack values.
...
- Type conversion (Hash → Uint8Array)
