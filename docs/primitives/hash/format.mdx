...
## `hash.format(prefixLength?: number, suffixLength?: number): string`

Returns shortened display format with ellipsis. Default shows first 6 + last 4 hex chars.
...
hash.format()
// "0x1c8aff...eac8" (default: 6+4)

hash.format(8, 6)
// "0x1c8aff95...6deac8" (custom: 8+6)

hash.format(10, 8)
// "0x1c8aff9506...a36deac8" (custom: 10+8)
```
...
## `format(hash: BrandedHash, prefixLength?: number, suffixLength?: number): string`

Returns shortened display format with ellipsis. Default shows first 6 + last 4 hex chars.

**Parameters:**
- `hash: BrandedHash` - Hash to format
- `prefixLength?: number` - Characters after `0x` to show (default: 6)
- `suffixLength?: number` - Characters at end to show (default: 4)
...
Zig requires manual string formatting:
...
fn formatHash(
    ...
    return try std.fmt.allocPrint(
        allocator,
        "0x{s}...{s}",
        .{hex[0..prefix_len], hex[hex.len - suffix_len..]}
    );
}
```
...
console.timeEnd('format') // ~200μs = ~20ns each
```
