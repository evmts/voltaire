...
## `Hash.assert(value: unknown, message?: string): asserts value is BrandedHash`

Asserts value is a valid Hash. Throws error if validation fails.

**Parameters:**
- `value: unknown` - Value to assert
- `message?: string` - Optional custom error message

**Returns:** `void` (type assertion)

**Throws:** `Error` - If value is not a Hash

**Type assertion:** Narrows type to `BrandedHash` after call
...
Hash.assert(txHash)
Hash.assert(blockHash)
// Both must be valid or function throws
```

**Defined in:** [primitives/Hash/BrandedHash/assert.js:18](https://github.com/evmts/voltaire/blob/main/src/primitives/Hash/BrandedHash/assert.js#L18)

</Tab>
<Tab title="Namespace API">

## `assert(value: unknown, message?: string): asserts value is BrandedHash`
...
Zig uses compile-time assertions and types:
...
fn assertHash(value: []const u8) ![32]u8 {
    if (value.len != 32) {
