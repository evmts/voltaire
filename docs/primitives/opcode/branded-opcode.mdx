...
// From number
const add = BrandedOpcode(0x01)
const push1 = BrandedOpcode(0x60)
const sstore = BrandedOpcode(0x55)
...
const opcode = BrandedOpcode(0x60)
...
for (const instr of instructions) {
  ...
  if (instr.immediate) {
    console.log(`  Data: ${Hex(instr.immediate)}`)
  }
}
...
function disassemble(bytecode: Uint8Array): string {
  ...
  for (const instr of instructions) {
    ...
    if (instr.immediate) {
      const data = Hex(instr.immediate)
...
```typescript
const opcode: BrandedOpcode = Opcode(0x01)
