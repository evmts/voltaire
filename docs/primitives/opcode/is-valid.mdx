...
// Invalid opcodes
Opcode.isValid(0x0C)  // false (undefined between SIGNEXTEND and LT)
Opcode.isValid(0x0D)  // false
Opcode.isValid(0x0E)  // false
Opcode.isValid(0x0F)  // false
Opcode.isValid(0x21)  // false (undefined after KECCAK256)
...
pub fn main() void {
    ...
    const invalid = Opcode.isValid(0x0C);   // false

    std.debug.print("0x60 valid: {}\n", .{valid});
    std.debug.print("0x0C valid: {}\n", .{invalid});
}
```
</Tab>
</Tabs>
