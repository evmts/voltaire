...
## `function.getSelector(): Hex`

Get the 4-byte function selector by hashing the function signature with keccak256 and taking the first 4 bytes.

**Returns:** `Hex` - 4-byte function selector with `0x` prefix
...
**Defined in:** [primitives/Abi/function/BrandedFunction/getSelector.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Abi/function/BrandedFunction/getSelector.js)

</Tab>
<Tab title="Namespace API (Tree-shakeable)">

## `getSelector(fn: BrandedFunction): Hex`
...
```typescript
import { getSelector } from '@tevm/voltaire/Abi/function'

const selector = getSelector(transferFn)
```
...
Function selector calculation in Zig is planned for a future release. Use TypeScript API for now.
