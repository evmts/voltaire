...
Get the canonical function signature string in the format `functionName(type1,type2,...)`.

**Returns:** `string` - Canonical function signature (no spaces, no parameter names)
...
**Defined in:** [primitives/Abi/function/BrandedFunction/getSignature.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Abi/function/BrandedFunction/getSignature.js)

</Tab>
<Tab title="Namespace API (Tree-shakeable)">

## `getSignature(fn: BrandedFunction): string`
...
```typescript
import { getSignature } from '@tevm/voltaire/Abi/function'

const signature = getSignature(transferFn)
```
...
Function signature generation in Zig is planned for a future release. Use TypeScript API for now.
