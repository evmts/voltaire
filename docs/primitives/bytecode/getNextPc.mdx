...
## `bytecode.getNextPc(currentPc: number): number | undefined`

Calculate the next program counter position after the instruction at `currentPc`.

Automatically handles variable-width PUSH instructions by skipping immediate data bytes.

**Parameters:**
- `currentPc: number` - Current program counter position

**Returns:** `number | undefined` - Next PC, or undefined if at end of bytecode
...
// PUSH1 0x01 (PC 0-1), PUSH1 0x02 (PC 2-3), ADD (PC 4), JUMPDEST (PC 5), STOP (PC 6)

console.log(code.getNextPc(0));  // 2 (skip PUSH1 + 1 byte data)
console.log(code.getNextPc(2));  // 4 (skip PUSH1 + 1 byte data)
console.log(code.getNextPc(4));  // 5 (ADD is 1 byte)
console.log(code.getNextPc(5));  // 6 (JUMPDEST is 1 byte)
console.log(code.getNextPc(6));  // undefined (at end)
```

**Defined in:** [BrandedBytecode/getNextPc.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/BrandedBytecode/getNextPc.js)

</Tab>
<Tab title="Namespace API">

## `getNextPc(code: BrandedBytecode, currentPc: number): number | undefined`
...
**Returns:** `number | undefined` - Next PC, or undefined if at end
...
const code = Bytecode.fromHex("0x60016002015b00");
const nextPc = Bytecode.getNextPc(code, 0);

console.log(nextPc); // 2
```

**Tree-shakeable import:**

```typescript
import { getNextPc } from '@tevm/voltaire/BrandedBytecode';

const nextPc = getNextPc(code, 0);
```
...
## `getNextPc(bytecode: Bytecode, current_pc: usize) ?usize`

Calculate the next program counter position after the instruction at `current_pc`.

**Parameters:**
- `bytecode: Bytecode` - Bytecode to analyze
- `current_pc: usize` - Current program counter position

**Returns:** `?usize` - Next PC, or null if at end of bytecode
...
const code = try primitives.Bytecode.fromHex(allocator, "0x60016002015b00");
defer code.deinit(allocator);

if (code.getNextPc(0)) |next_pc| {
    std.debug.print("Next PC: {}\n", .{next_pc}); // 2
} else {
    std.debug.print("At end of bytecode\n", .{});
}
```
