...
## `bytecode.getBlock(pc: number): BasicBlock | undefined`

Get the basic block that contains the given program counter position.

Returns the block metadata including gas cost, stack effects, and control flow information.

**Parameters:**
- `pc: number` - Program counter (byte offset) to look up

**Returns:** `BasicBlock | undefined` - Block containing the PC, or undefined if not found
...
// Find block containing PC 5
const block = code.getBlock(5);
if (block) {
...
**Defined in:** [BrandedBytecode/getBlock.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/BrandedBytecode/getBlock.js)
...
- `pc: number` - Program counter to look up
...
const code = Bytecode.fromHex("0x60016002015b80016003015b00");
const block = Bytecode.getBlock(code, 5);

if (block) {
  console.log(`Found in block ${block.index}`);
}
```
...
```typescript
import { getBlock } from '@tevm/voltaire/BrandedBytecode';

const block = getBlock(code, 5);
```
...
TypeScript implementation only. Control flow graph analysis with block detection is complex and not yet ported to Zig.

```zig
// TypeScript implementation only
// CFG analysis not yet available in Zig
```
