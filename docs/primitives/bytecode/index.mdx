...
EVM bytecode is the low-level machine code that smart contracts compile to. Each byte is either an [opcode](/primitives/opcode) (instruction) or constant data.

## Overview

Bytecode is represented as a [branded](/concepts/branded-types) [`Uint8Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) following the [data-first pattern](/concepts/data-first) used throughout Voltaire.

```typescript
export type BrandedBytecode = Uint8Array & { readonly __tag: "Bytecode" };
```

The EVM reads bytecode sequentially, executing each instruction. Voltaire provides comprehensive tools for analyzing bytecode structure, parsing instructions, validating jump destinations, and extracting metadata.

<Accordion title="What is EVM Bytecode? (New to bytecode?)">
