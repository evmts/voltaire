...
## `bytecode.analyzeStack(options?: StackAnalysisOptions): StackAnalysis`

Validate stack depth constraints and track stack effects throughout bytecode execution.

Returns comprehensive analysis identifying underflows, overflows, and block-level stack requirements.

**Parameters:**
- `options?: StackAnalysisOptions` - Analysis configuration

**Returns:** `StackAnalysis` - Complete stack validation results
...
if (!analysis.valid) {
  ...
  analysis.issues.forEach(issue => {
    console.error(`${issue.type} at PC ${issue.pc}: ${issue.message}`);
  });
} else {
  console.log(`Valid! Max stack depth: ${analysis.maxDepth}`);
}
```

**Defined in:** [BrandedBytecode/analyzeStack.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/BrandedBytecode/analyzeStack.js)

</Tab>
<Tab title="Namespace API">

## `analyzeStack(code: BrandedBytecode, options?: StackAnalysisOptions): StackAnalysis`
...
const code = Bytecode.fromHex("0x6001600201");
const analysis = Bytecode.analyzeStack(code);
...
```typescript
import { analyzeStack } from '@tevm/voltaire/BrandedBytecode';

const analysis = analyzeStack(code);
```
...
## `analyzeStack()` - Zig Implementation

TypeScript implementation only - Zig version planned.

<Tip>
Use the TypeScript API for stack validation and depth analysis. For Zig, manually track stack effects using Opcode metadata or use `scan()` to iterate and calculate stack changes.
</Tip>
...
```typescript
const code = Bytecode.fromHex("0x01");
...
```typescript
const code = Bytecode.fromHex("0x60016002600360045760065b5b00");
...
const fragment = Bytecode.fromHex("0x910190016300");
...
```typescript
const code = Bytecode.fromHex("0x60016002016080016090");
...
```typescript
const code = Bytecode.fromHex("0x8F");
...
```typescript
const code = Bytecode.fromHex("0x6001600460065760025b01");
