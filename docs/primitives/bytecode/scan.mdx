...
## `bytecode.scan(options?: ScanOptions): Generator<Instruction>`

Iterate through bytecode instructions lazily, automatically handling PUSH data and fusion patterns.

Returns generator yielding `Instruction` objects with detailed metadata about each operation.

**Parameters:**
- `options?: ScanOptions` - Configuration for iteration behavior

**Returns:** `Generator<Instruction>` - Iterator yielding instruction objects
...
**Defined in:** [BrandedBytecode/scan.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/BrandedBytecode/scan.js)

</Tab>
<Tab title="Namespace API">

## `scan(code: BrandedBytecode, options?: ScanOptions): Generator<Instruction>`
...
```typescript
import { scan } from '@tevm/voltaire/BrandedBytecode';

const iterator = scan(code);
...
## `Bytecode.scan(self: *const Bytecode, start_pc: u32, end_pc: u32): Scanner`

Create a scanner iterator to traverse bytecode instructions. Automatically skips PUSH immediate data and handles variable-length instructions.

**Parameters:**
- `self: *const Bytecode` - Bytecode reference
- `start_pc: u32` - Starting program counter
- `end_pc: u32` - Ending program counter (exclusive)

**Returns:** `Scanner` - Iterator struct yielding `Instruction` objects
...
while (scanner.next()) |inst| {
  std.debug.print("PC {}: opcode 0x{x:0>2}, size {}\n", .{ inst.pc, inst.opcode, inst.size });
  if (inst.push_value) |val| {
    std.debug.print("  PUSH value: 0x{x}\n", .{val});
  }
}
```
...
- `next() -> ?Instruction` - Get next instruction, advancing PC. Returns null at end.

**Instruction Fields:**

- `pc: u32` - Program counter (byte offset)
- `opcode: u8` - Raw opcode byte
- `size: u8` - Instruction size (1 for non-PUSH, 1+N for PUSHN)
- `push_value: ?u256` - Immediate value for PUSH instructions (null for others)

**Defined in:** [primitives/Bytecode/bytecode.zig:299](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/bytecode.zig#L299)
