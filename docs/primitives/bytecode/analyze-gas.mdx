...
## `bytecode.analyzeGas(options?: GasAnalysisOptions): GasAnalysis`

Comprehensive static gas analysis with instruction-level, block-level, and path-level breakdown.

Returns detailed gas cost estimates for sequential execution and alternative code paths.

**Parameters:**
- `options?: GasAnalysisOptions` - Analysis configuration

**Returns:** `GasAnalysis` - Complete gas cost breakdown
...
console.log(`Instructions: ${analysis.byInstruction.length}`);
analysis.expensive.forEach(inst => {
  console.log(`Expensive: ${inst.opcode} (${inst.gas} gas) at PC ${inst.pc}`);
});
```

**Defined in:** [BrandedBytecode/analyzeGas.js](https://github.com/evmts/voltaire/blob/main/src/primitives/Bytecode/BrandedBytecode/analyzeGas.js)

</Tab>
<Tab title="Namespace API">

## `analyzeGas(code: BrandedBytecode, options?: GasAnalysisOptions): GasAnalysis`
...
console.log(`Total: ${analysis.total} gas`);
```

**Tree-shakeable import:**

```typescript
import { analyzeGas } from '@tevm/voltaire/BrandedBytecode';

const analysis = analyzeGas(code);
```
...
## `Bytecode.analyzeGasTotal(): u64`

Calculate total static gas cost for all bytecode.

Sums gas costs of all opcodes, correctly handling PUSH instruction immediates.

**Returns:** `u64` - Total gas cost
...
pub fn main() !void {
  ...
  // PUSH1 1 (3 gas), PUSH1 2 (3 gas), ADD (3 gas) = 9 gas total
  const code = "60016002015b00";

  var bytecode = try Bytecode.init(allocator, code);
  defer bytecode.deinit();

  const total_gas = bytecode.analyzeGasTotal();
  std.debug.print("Total gas: {}\n", .{total_gas});
}
```

<Tip>
For detailed instruction-level and path-level analysis, use the TypeScript API. Zig provides basic static gas cost calculation.
</Tip>
