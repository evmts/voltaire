...
description: "Methods for comparing hardfork versions chronologically"
---
...
Check if current hardfork is before target version (less than).
...
```typescript
Hardfork.equals(a: BrandedHardfork, b: BrandedHardfork): boolean
a.equals(b: BrandedHardfork): boolean
```
...
MERGE.equals(fromString("paris"));  // true (alias)
MERGE.equals(fromString("london")); // false
```
