...
## `Rlp.validate(data: Uint8Array): boolean`

Validates if data is valid RLP encoding. Returns true if valid, false otherwise.

**Parameters:**
- `data: Uint8Array` - Data to validate
...
// Invalid RLP (incomplete)
const invalid = Rlp.validate(new Uint8Array([0x83, 1]))
// => false (says 3 bytes but only 1 provided)

// Valid single byte
const byte = Rlp.validate(new Uint8Array([0x7f]))
// => true

// Valid list
const list = Rlp.validate(new Uint8Array([0xc2, 0x01, 0x02]))
// => true

// Invalid non-canonical
const nonCanonical = Rlp.validate(new Uint8Array([0x81, 0x7f]))
// => false (single byte < 0x80 should not be prefixed)
```

**Defined in:** [primitives/Rlp/BrandedRlp/validate.js:18](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/validate.js#L18)
...
```typescript
import { validate } from '@tevm/voltaire/BrandedRlp'

const valid = validate(new Uint8Array([0x83, 1, 2, 3]))
// => true

const invalid = validate(new Uint8Array([0x83, 1]))
// => false
```
...
## `rlp.validate(bytes: []const u8) bool`

Validate if bytes are valid RLP encoding.

**Parameters:**
- `bytes: []const u8` - Bytes to validate
...
const valid = [_]u8{ 0x83, 1, 2, 3 };
const isValid = rlp.validate(&valid);
// isValid = true

const invalid = [_]u8{ 0x83, 1 };
const isValid = rlp.validate(&invalid);
// isValid = false
```

**Defined in:** [primitives/rlp/validate.zig](https://github.com/evmts/voltaire/blob/main/src/primitives/rlp/validate.zig)
...
// Safe for any input
const result1 = Rlp.validate(new Uint8Array([]))
// => false (no throw)
