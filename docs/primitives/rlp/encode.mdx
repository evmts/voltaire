...
## `Rlp.encode(data: Encodable): Uint8Array`

Universal encoder that dispatches based on input type. Accepts bytes, lists, or RLP data structures.

**Parameters:**
- `data: Encodable` - Data to encode (Uint8Array, BrandedRlp, or array)
...
**Defined in:** [primitives/Rlp/BrandedRlp/encode.js:50](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/encode.js#L50)

</Tab>
<Tab title="Namespace API">

## `encode(data: Encodable): Uint8Array`
...
// Encode nested
const nested = [bytes, [bytes, bytes]]
...
## `rlp.encode(allocator: Allocator, data: anytype) ![]u8`

Encode data to RLP format. Automatically dispatches based on data type.

**Parameters:**
- `allocator: Allocator` - Memory allocator for encoded bytes
- `data: anytype` - Data to encode (slice, array, or struct)
...
**Memory:** Caller must free returned slice with same allocator.
