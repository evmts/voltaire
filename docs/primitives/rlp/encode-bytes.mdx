...
Encodes a byte array according to RLP string rules. Use when you know the input is bytes (not a list).
...
**Defined in:** [primitives/Rlp/BrandedRlp/encodeBytes.js:32](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/encodeBytes.js#L32)
...
```typescript
import { encodeBytes } from '@tevm/voltaire/BrandedRlp'
...
## `rlp.encodeBytes(allocator: Allocator, bytes: []const u8) ![]u8`

Encode byte slice according to RLP string rules.

**Parameters:**
- `allocator: Allocator` - Memory allocator for encoded bytes
- `bytes: []const u8` - Byte slice to encode
...
const encoded = try rlp.encodeBytes(allocator, &bytes);
...
Use `encodeBytes` instead of generic `encode` when:

1. **Known type** - You know the input is bytes (not a list)
2. **Type-specific** - Better tree-shaking (smaller bundle)
