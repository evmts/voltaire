---
title: Rlp.decode
description: Universal decoder for parsing RLP-encoded bytes back to data structures
---

<Tabs>
<Tab title="Class API">

## `Rlp.decode(bytes: Uint8Array, stream?: boolean): Decoded`

Decodes RLP-encoded bytes into data structures with comprehensive validation.
...
**Returns:** `Decoded` - Object with `data` (decoded RLP structure) and `remainder` (leftover bytes)
...
**Defined in:** [primitives/Rlp/BrandedRlp/decode.js:202](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/decode.js#L202)

</Tab>
<Tab title="Namespace API">

## `decode(bytes: Uint8Array, stream?: boolean): Decoded`

Decodes RLP-encoded bytes into data structures with comprehensive validation.

**Parameters:**
- `bytes: Uint8Array` - RLP-encoded data to decode
- `stream?: boolean` - If true, allows extra data after decoded value

**Returns:** `Decoded` - Object with `data` and `remainder`
...
## `rlp.decode(allocator: Allocator, bytes: []const u8) !Decoded`

Decode RLP-encoded bytes with validation.

**Parameters:**
- `allocator: Allocator` - Memory allocator for decoded data
- `bytes: []const u8` - RLP-encoded bytes

**Returns:** `Decoded` - Decoded data (caller owns memory)
...
const encoded = [_]u8{ 0x83, 1, 2, 3 };
const decoded = try rlp.decode(allocator, &encoded);
defer decoded.deinit(allocator);

// Access decoded data
if (decoded.data == .bytes) {
    std.debug.print("Bytes: {any}\n", .{decoded.data.bytes});
}
```

**Memory:** Caller must call `deinit()` on returned value.
