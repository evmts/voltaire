...
## `Rlp.getEncodedLength(data: Encodable): number`

Get the total byte length of RLP-encoded data without actually encoding. Useful for pre-allocating buffers.

**Parameters:**
- `data: Encodable` - Data to measure (Uint8Array, BrandedRlp, or array)
...
const length = Rlp.getEncodedLength(list)
// => 4 (0xc2 prefix + 2 single bytes)

// Single byte < 0x80
const single = new Uint8Array([0x7f])
const length = Rlp.getEncodedLength(single)
// => 1 (no prefix)

// Long string
const long = new Uint8Array(60).fill(0x42)
const length = Rlp.getEncodedLength(long)
// => 62 (0xb8 + 60 + 60 bytes)

// Instance method
const rlpData = new Rlp(new Uint8Array([1, 2, 3]))
const length = rlpData.getEncodedLength()
// => 4
```

**Defined in:** [primitives/Rlp/BrandedRlp/getEncodedLength.js:18](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/getEncodedLength.js#L18)

</Tab>
<Tab title="Namespace API">

## `getEncodedLength(data: Encodable): number`

Get the total byte length of RLP-encoded data without actually encoding.

**Parameters:**
- `data: Encodable` - Data to measure
...
const bytes = new Uint8Array([1, 2, 3])
const length = getEncodedLength(bytes)
// => 4 (0x83 prefix + 3 bytes)
```
...
## `rlp.getEncodedLength(data: anytype) usize`

Calculate RLP-encoded size without encoding.

**Parameters:**
- `data: anytype` - Data to measure

**Returns:** `usize` - Length in bytes
