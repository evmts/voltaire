...
## `Rlp.encodeList(items: Encodable[]): Uint8Array`

Encodes a list of RLP-encodable items. Use when you know the input is a list.
...
**Defined in:** [primitives/Rlp/BrandedRlp/encodeList.js:32](https://github.com/evmts/voltaire/blob/main/src/primitives/Rlp/BrandedRlp/encodeList.js#L32)
...
## `rlp.encodeList(allocator: Allocator, items: []const []const u8) ![]u8`

Encode array of byte slices as RLP list.

**Parameters:**
- `allocator: Allocator` - Memory allocator for encoded bytes
- `items: []const []const u8` - Array of byte slices

**Returns:** `[]u8` - RLP-encoded list (caller owns memory)
...
const proof = [
  ...
  new Uint8Array(32).fill(0x04)   // Node 4
]
