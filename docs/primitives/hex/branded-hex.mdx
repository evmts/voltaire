...
- **Zero-overhead** [branded type](/concepts/branded-types) wrapping hex strings with `0x` prefix
...
Runtime-validated hex string with `0x` prefix. [Brand](/concepts/branded-types) prevents accidental mixing with plain strings. `Sized<N>` adds compile-time size tracking.
