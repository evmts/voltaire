...
// From number
const numHex = Hex.fromNumber(255);  // "0xff"

// From bigint
const bigintHex = Hex.fromBigint(1000000n);  // "0xf4240"

// From boolean
const boolHex = Hex.fromBoolean(true);  // "0x01"

// From string
const strHex = Hex.fromString("Hello");  // "0x48656c6c6f"

// From bytes
const bytes = new Uint8Array([1, 2, 3]);
const bytesHex = Hex.fromBytes(bytes);  // "0x010203"
```
...
function toHex(input: string | number | bigint | boolean | Uint8Array): BrandedHex {
  if (typeof input === 'string') {
    ...
    return Hex.fromNumber(input);
  } else if (typeof input === 'bigint') {
    return Hex.fromBigint(input);
  } else if (typeof input === 'boolean') {
    return Hex.fromBoolean(input);
  } else {
    return Hex.fromBytes(input);
  }
}
```
...
function isAddressHex(hex: string): boolean {
  return Hex.isHex(hex) && Hex.isSized(Hex.from(hex), 20);
}

// Validate hash size (32 bytes)
function isHashHex(hex: string): boolean {
  return Hex.isHex(hex) && Hex.isSized(Hex.from(hex), 32);
}
```
...
function encodeFunctionCall(
  ...
  const selectorHex = Hex.from(selector);
...
const transferCall = encodeFunctionCall(
  ...
  Hex.pad(Hex.fromBytes(recipientAddress), 32),
  Hex.pad(Hex.fromBigint(1000n), 32)
);
```
...
const packed = encodePacked(
  Hex.fromBytes(address),      // 20 bytes
  Hex.fromNumber(255)           // 1 byte
);  // 21 bytes total
```
...
const TEST_HEX = {
  ...
  one: Hex.from("0x01"),
  max8: Hex.from("0xff"),
  max16: Hex.from("0xffff"),
  max32: Hex.from("0x" + "ff".repeat(32))
};
```
