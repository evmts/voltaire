...
const isValid = BrandedAuthorization.isValid(auth)

// Serialize for transaction
const encoded = BrandedAuthorization.encode(auth)

// Deserialize from transaction
const decoded = BrandedAuthorization.decode(data)
```
