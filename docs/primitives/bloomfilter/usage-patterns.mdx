...
class EventWatcher {
  ...
  async processBlock(blockNum: number): Promise<Log[]> {
    ...
    // Quick bloom check
    const hasWatchedAddress = Array.from(this.watching).some(addr =>
      filter.contains(Address.from(addr))
    )

    const hasWatchedEvent = Array.from(this.eventTopics.values()).some(topic =>
    ...
    return logs.filter(log =>
      ...
      log.topics.some(t => {
        ...
        return Array.from(this.eventTopics.values()).some(watched =>
...
class EventAggregator {
  ...
  async scanRange(
    ...
    for (let blockNum = startBlock; blockNum <= endBlock; blockNum++) {
      ...
      const candidates = Array.from(this.contracts.entries()).filter(
...
class BloomValidator {
  async validateBlock(blockNum: number): Promise<{
    ...
    for (const log of logs) {
      // Add address
      rebuiltBloom.add(Address.from(log.address))
