...
Defined in: [BrandedBloomFilter/contains.js:3-30](https://github.com/evmts/voltaire/blob/main/src/primitives/BloomFilter/BrandedBloomFilter/contains.js#L3-L30)

</Tab>
<Tab title="Zig">

### `BloomFilter.contains(self: *const BloomFilter, item: []const u8) bool`

Checks if item might be in the set. Returns `false` if definitely not present, `true` if possibly present.

**Parameters:**
- `self: *const BloomFilter` - Pointer to bloom filter
- `item: []const u8` - Item to check (arbitrary byte slice)
...
if (filter.contains("foo")) {
    // Item might be present (true - definitely added)
}

if (!filter.contains("bar")) {
    // Item definitely not present (true - never added)
}
```

**Defined in:** [primitives/BloomFilter/bloom_filter.zig:36](https://github.com/evmts/voltaire/blob/main/src/primitives/BloomFilter/bloom_filter.zig#L36)
...
Defined in: [BrandedBloomFilter/isEmpty.js:3-20](https://github.com/evmts/voltaire/blob/main/src/primitives/BloomFilter/BrandedBloomFilter/isEmpty.js#L3-L20)
