...
### `BloomFilter.init(allocator: Allocator, m: usize, k: usize) !BloomFilter`

Initialize new empty bloom filter with specified bit count and hash functions.

**Parameters:**
- `allocator: Allocator` - Memory allocator for bit array
- `m: usize` - Number of bits in the filter
- `k: usize` - Number of hash functions

**Returns:** `!BloomFilter` - Empty bloom filter (all bits zero). Caller owns memory.
...
// Filter size in bytes: (m + 7) / 8
// 1000 bits = 125 bytes
```

**Defined in:** [primitives/BloomFilter/bloom_filter.zig:11](https://github.com/evmts/voltaire/blob/main/src/primitives/BloomFilter/bloom_filter.zig#L11)
...
const restored: BrandedBloomFilter = fromHex(hexString, 2048, 3)
```

See [BrandedBloomFilter](/primitives/bloomfilter/branded-bloomfilter) for details on tree-shakeable API.
