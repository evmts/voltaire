...
const filter = BloomFilter(logsBloom, 2048, 3)
...
const restored = BloomFilter(hex, 2048, 3)
...
```typescript
import { BloomFilter, BITS, DEFAULT_HASH_COUNT, SIZE } from 'tevm'
...
async function loadBlockFilter(block: BlockHeader) {
  ...
  const addressBytes = Address("0x...").bytes
...
function saveAndRestore() {
  ...
  const restored = BloomFilter(saved.hex, saved.m, saved.k)
