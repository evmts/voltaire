...
const b = Uint128.fromHex("0xff");
const c = Uint128.fromNumber(255);
...
const d = Uint128.from("1000000000000000000");
```
...
const b = Uint128.fromHex("ffffffffffffffffffffffffffffffff");
```
...
const wrapped = Uint128.plus(max, Uint128.from(1n));  // 0n
```
...
const wrapped = Uint128.minus(zero, Uint128.from(1n));  // MAX
```
...
```typescript
const isPower = Uint128.isPowerOf2(Uint128.from(16n));   // true
const notPower = Uint128.isPowerOf2(Uint128.from(15n));  // false
```
...
let cumulativeGas = Uint128.from(0n);

for (const block of blocks) {
  const blockGas = Uint128.from(block.gasUsed);
...
const tokenAmounts = [
  Uint128.from(1000000000000000000n),  // 1 token
  Uint128.from(2500000000000000000n),  // 2.5 tokens
  Uint128.from(500000000000000000n),   // 0.5 tokens
];
...
const principal = Uint128.from(1000000n);
const rate = Uint128.from(105n);  // 5% as 105/100
const base = Uint128.from(100n);

// Interest after 10 periods
const multiplier = Uint128.toPower(rate, Uint128.from(10n));
const amount = Uint128.times(principal, multiplier);
const result = Uint128.dividedBy(amount, Uint128.toPower(base, Uint128.from(10n)));
```
...
const FLAG_A = Uint128.from(1n << 0n);
const FLAG_B = Uint128.from(1n << 1n);
const FLAG_C = Uint128.from(1n << 127n);
...
// Convert first
const converted = Uint128.from(Uint64.toBigInt(b));
