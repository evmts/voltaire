...
## Dispatch-Based Execution: The Core Innovation

### Traditional Interpreter vs Dispatch System

```
Traditional EVM (MinimalEvm)              Guillotine (Dispatch-Based)
============================              ============================

Bytecode: [0x60, 0x05, 0x01]             Bytecode: [0x60, 0x05, 0x01]
          PUSH1  5    ADD                          PUSH1  5    ADD
              ↓                                         ↓
    ┌─────────────────────┐                   [Preprocessing Phase]
    │  while (pc < len) { │                            ↓
    │    switch(code[pc]){│              ┌──────────────────────────┐
    │      0x60: push()   │              │   Dispatch Schedule:     │
    │      0x01: add()    │              │   [0] first_block_gas    │
    │      ...256 cases   │              │   [1] &push_handler      │
    │    }                │              │   [2] push_inline {5}    │
    │    pc++             │              │   [3] &add_handler       │
    │  }                  │              └──────────────────────────┘
    └─────────────────────┘                            ↓
              ↓                              cursor[0].handler(frame)
     [Runtime Validation]                              ↓
     [Per-Op Gas Check]                       [Tail Call Chain]
     [Bytecode Reads]                         [No Switch/PC]
                                              [Pre-validated]
```

### How Jump Operations Differ

```
JUMP in Traditional Interpreter          JUMP in Dispatch System
================================          ========================

Stack: [dest_pc]                        Stack: [dest_pc]
       ↓                                        ↓
1. Pop destination                      1. Pop destination
2. Validate dest < bytecode.len         2. Binary search jump table
3. Check bytecode[dest] == JUMPDEST     3. Get pre-validated dispatch ptr
4. If valid: pc = dest                  4. Tail call to destination
5. If invalid: revert

Per-Jump Cost:                          Per-Jump Cost:
- Runtime validation                    - Binary search only
- Bytecode access                       - Pre-validated
- PC update                            - Direct function call
```

### Static Jump Optimization

```
Bytecode Pattern: PUSH1 10, JUMP        Traditional: 3 operations
                                        Dispatch: 1 synthetic opcode!

Traditional Execution:                  Dispatch Execution:
======================                  ==================
1. PUSH1 handler                       jump_to_static_location:
2. Read immediate (10)                   ↓
3. Push to stack                       cursor[1].jump_static.dispatch
4. JUMP handler                          ↓
5. Pop from stack                      Direct tail call!
6. Validate destination                (No stack operations)
7. Update PC                          (No validation)
```
...
#### Complete Bytecode to Execution Flow
...
```
    ...
    │         Dispatch Schedule               │
    │  ┌──────────────────────────────┐      │
    │  │ [0] first_block_gas {15}     │←─────┼─ Gas for basic block
    │  │ [1] &push_handler             │      │
    │  │ [2] push_inline {value: 5}    │←─────┼─ Inline metadata
    │  │ [3] &add_handler              │      │
    │  │ [4] &jump_handler             │      │
    │  │ [5] &jumpdest_handler         │      │
    │  │ [6] jump_dest {gas: 3}        │←─────┼─ Next block metadata
    │  │ [7] &stop_handler             │      │
    │  └──────────────────────────────┘      │
    └────────────────────────────────────────┘
                         ↓
    ...
    │                                         │
    │  cursor = schedule.ptr                 │
    │  cursor[0].handler(frame, cursor)      │
    │       ↓ (tail call)                    │
    │  cursor[3].handler(frame, cursor+3)    │
    │       ↓ (tail call)                    │
    │  cursor[4].handler(frame, cursor+4)    │
    │       ↓ (tail call)                    │
    │  ... continues via tail calls          │
    └────────────────────────────────────────┘
```
...
#### Opcode Fusion Examples

```
Common Pattern Detection:
========================

PUSH + ADD Pattern:               Fused Synthetic Opcode:
[0x60, 0x05, 0x01]               PUSH_ADD_INLINE
 PUSH1  5    ADD                 Single operation!
     3 ops                            1 op

PUSH + JUMP Pattern:             Fused Synthetic Opcode:
[0x60, 0x10, 0x56]              JUMP_TO_STATIC_LOCATION
 PUSH1  16   JUMP                Pre-resolved destination!
     3 ops                            1 op

Function Selector Pattern:        Fused Synthetic Opcode:
[0x63, ...., 0x14,               FUNCTION_DISPATCH
 0x60, ...., 0x57]               4-byte selector check
 PUSH4 EQ PUSH JUMPI             with static jump!
     4 ops                            1 op
```
