...
```zig
/// LOG2 opcode (0xa2) - Emit log with 2 indexed topics
pub fn log2(frame: *FrameType) FrameType.EvmError!void {
    ...
    // Gas: 375 base + 375 per topic (2 topics = 750) + 8 per data byte
    const log_cost = 375 + 750 + @as(u64, length_u32) * 8;
    ...
    // Create and append log with 2 topics
    var topics_buf = try evm.arena.allocator().alloc(u256, 2);
    topics_buf[0] = topic0;
    topics_buf[1] = topic1;
