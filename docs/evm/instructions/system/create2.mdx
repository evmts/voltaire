...
// Example usage
const creator = Address.from("0x742d35Cc6634C0532925a3b844Bc454e4438f44e");
const salt = 0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdefn;
const initCode = new Uint8Array([/* bytecode */]);
...
pub fn create2(frame: *FrameType) FrameType.EvmError!void {
    ...
    var gas_cost: u64 = 32000;  // Base
    gas_cost += @as(u64, words) * 2;   // Init code (EIP-3860)
    gas_cost += @as(u64, words) * 6;   // Keccak256
    ...
    if (length > 0) {
        ...
        var j: u32 = 0;
        while (j < len) : (j += 1) {
            code[j] = frame.readMemory(off + j);
        }
        init_code = code;
    }

    // Compute deterministic address
    const init_code_hash = crypto.keccak256(init_code);
    var preimage: [85]u8 = undefined;
    preimage[0] = 0xff;
    @memcpy(preimage[1..21], &frame.address.bytes);

    // Convert salt to bytes
    var i: usize = 0;
    while (i < 32) : (i += 1) {
        preimage[21 + i] = @truncate(salt >> @intCast((31 - i) * 8));
    }

    @memcpy(preimage[53..85], &init_code_hash);

    const address_hash = crypto.keccak256(&preimage);
    var addr_bytes: [20]u8 = undefined;
    @memcpy(&addr_bytes, address_hash[12..32]);
