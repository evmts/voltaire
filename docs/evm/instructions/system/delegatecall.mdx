...
const frame = createFrame({
  ...
  caller: Address.from("0x5678..."),
  value: 1_000_000_000_000_000_000n  // 1 ETH from original call
});

// Call library function
const libraryAddress = Address.from("0xLibrary...");
...
pub fn delegatecall(frame: *FrameType) FrameType.EvmError!void {
    ...
    // Execute delegatecall (preserve context!)
    const result = try evm.inner_delegatecall(
        code_address,
        frame.address,  // Storage context
        frame.caller,   // msg.sender PRESERVED
        frame.value,    // msg.value PRESERVED
        calldata,
        available_gas
    );
