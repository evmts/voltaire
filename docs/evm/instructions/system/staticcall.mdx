...
pub fn staticcall(frame: *FrameType) FrameType.EvmError!void {
    ...
    var gas_cost: u64 = GasConstants.CallGas;  // 700
    ...
    const gas_after_charge = if (remaining_gas >= gas_cost)
        ...
        0;
    const max_gas = gas_after_charge - (gas_after_charge / 64);
    ...
    // Execute static call (is_static = true)
    const result = try evm.inner_staticcall(
        call_address,
        calldata,
        available_gas
    );
