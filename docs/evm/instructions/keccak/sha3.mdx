...
pub fn sha3(frame: *FrameType) FrameType.EvmError!void {
    ...
    // Charge memory expansion and update memory size
    ...
    const aligned_size = ((end_addr + 31) / 32) * 32;
    if (@as(u32, @intCast(aligned_size)) > frame.memory_size) {
        frame.memory_size = @intCast(aligned_size);
    }

    // Read data from memory
    const data = try frame.allocator.alloc(u8, size_u32);
    for (0..size_u32) |i| {
        data[i] = frame.readMemory(offset_u32 + @as(u32, @intCast(i)));
    }
