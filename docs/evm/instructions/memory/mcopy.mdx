...
pub fn mcopy(frame: *FrameType) FrameType.EvmError!void {
    ...
    // If len == 0, no memory expansion occurs
    const mem_cost = if (len == 0)
        ...
        const end_dest: u64 = dest_u64 +| len_u64;
    ...
    const copy_cost: u64 = if (len <= std.math.maxInt(u32))
        ...
        std.math.maxInt(u64);  // Huge value triggers OutOfGas

    const total_gas = GasConstants.GasFastestStep +| mem_cost +| copy_cost;
    try frame.consumeGas(total_gas);

    // Fast path: zero length - gas charged but no copy
    if (len == 0) {
        frame.pc += 1;
        return;
    }
    ...
    // Expand memory to cover both ranges
