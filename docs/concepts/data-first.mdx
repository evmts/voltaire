...
**Data comes first.** We default to the most simple JavaScript structure with runtime validation for all types. Ethereum primitives are represented using the most idiomatic data type for each language that closely matches what the data actually is:
