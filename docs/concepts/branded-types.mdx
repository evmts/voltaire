...
const addr: BrandedAddress = fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e");
...
function process(input: string | Uint8Array | number | bigint) {
  ...
  // Now you have a validated BrandedAddress
}
```
...
type BrandedAddress = Uint8Array & { readonly [brand]: "Address" };
type BrandedHash = Uint8Array & { readonly [brand]: "Hash" };
```
...
**If you have a `BrandedAddress`, it's valid.** No need for runtime validation libraries like Zod.
...
| Type | Class instance | BrandedAddress | BrandedAddress |
...
// After - type safe
import type { BrandedAddress } from '@tevm/voltaire/Address';
function transfer(to: BrandedAddress, amount: bigint) { }
const hash = keccak256(data);
...
// ✓ Good - explicit types
function verify(addr: BrandedAddress, hash: BrandedHash) {
