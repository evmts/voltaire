...
**Defined in:** [src/primitives.h](https://github.com/evmts/tevm/blob/main/src/primitives.h)
...
function getUniqueParticipants(txs: Transaction[]): AddressType[] {
...
```typescript
import * as Address from 'tevm/Address'
import type { EventLog } from 'tevm'

function getUniqueTokenHolders(logs: EventLog[]): AddressType[] {
  const holders: AddressType[] = []
...
function deduplicateWithMap(
  ...
  const seen = new Map<string, AddressType>()
