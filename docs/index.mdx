    ...
    <script dangerouslySetInnerHTML={{__html: `
      ...
      function setupCodeExecution(editor) {
        ...
        function createWorker() {
          const workerCode = 'let Tevm = null;' +
            ...
            '  if (loadTevm && !Tevm) {' +
            '    try {' +
            '      self.postMessage({ type: "log", args: ["Loading Tevm APIs..."] });' +
            '      const module = await import(new URL("/lib/tevm.js", location.origin).href);' +
            '      Tevm = module;' +
            '      Object.assign(globalThis, module);' +
            '      self.postMessage({ type: "log", args: ["Tevm loaded! Available: Address, Hex, Keccak256, etc."] });' +
            '    } catch (err) {' +
            '      self.postMessage({ type: "error", args: ["Failed to load Tevm: " + err.message] });' +
            '    }' +
            '  }' +
            '  try {' +
            '    const fn = new Function("console", "Tevm", code);' +
            '    fn(console, Tevm);' +
