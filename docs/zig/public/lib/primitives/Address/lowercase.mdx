...
The `Lowercase` type is a **branded string** extending `Hex.Sized<20>`, not a `Uint8Array` like `AddressType`.
...
Defined in: [primitives/Address/AddressType/LowercaseAddress.js:20](https://github.com/evmts/voltaire/blob/main/src/primitives/Address/AddressType/LowercaseAddress.js#L20)
...
function saveAccount(address: Address, balance: bigint, nonce: number) {
  const record: AccountRecord = {
    address: LowercaseAddress(address),  // Canonical format
    balance,
    nonce
  }
...
function isSameAddress(a: Address, b: Address): boolean {
  return LowercaseAddress(a) === LowercaseAddress(b)
}
...
<Note>
For `AddressType` instances, use `Address.equals()` instead - more efficient than string comparison.
</Note>
...
function setBalance(address: Address, balance: bigint) {
  balances.set(LowercaseAddress(address), balance)
}

function getBalance(address: Address): bigint {
  return balances.get(LowercaseAddress(address)) ?? 0n
}

// Works regardless of input case
const addr1 = Address("0x742D35CC...")
const addr2 = Address("0x742d35cc...")
...
function serializeTransaction(from: Address, to: Address, value: bigint): string {
  const tx: Transaction = {
    from: LowercaseAddress(from),
    to: LowercaseAddress(to),
...
const lowercase = LowercaseAddress(addr)
...
```zig
import * as LowercaseAddress from '@tevm/voltaire/Address/LowercaseAddress'
import type { AddressType, Lowercase } from '@tevm/voltaire/Address'

class CachedAddress {
  private _lowercaseCache?: Lowercase

  constructor(private address: AddressType) {}

  toLowercase(): Lowercase {
    if (!this._lowercaseCache) {
      this._lowercaseCache = LowercaseAddress(this.address)
    }
...
```zig
import type { Lowercase } from '@tevm/voltaire/Address/LowercaseAddress'
...
const addr = Address("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")
const lowercase = LowercaseAddress(addr)
...
const lower1 = LowercaseAddress(addr)
const addr2 = Address(lower1)
const lower2 = LowercaseAddress(addr2)
...
- [toLowercase](/primitives/address/to-lowercase) - Convert AddressType to lowercase string
- [toChecksummed](/primitives/address/to-checksummed) - Convert to EIP-55 checksummed hex
- [AddressType](/primitives/address/branded-address) - Tree-shakeable address API
