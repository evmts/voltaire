...
description: Learn the core patterns and architecture of Tevm
---
...
Tevm is a modern Ethereum library built with performance, type safety, and tree-shaking as first-class priorities. This guide will teach you the core concepts and patterns that make Tevm different.
...
Let's build a simple example that demonstrates Tevm's core capabilities: hashing, signing, and address validation.
...
<Tabs>
  <Tab title="Node.js/Bun">
    ```typescript
    import { Keccak256, Secp256k1, Address } from "tevm";

    // Hash a message
    const message = "Welcome to Tevm";
...
You've just hashed data, signed a message, and recovered an Ethereum address using Tevm.

## Core Architecture

Tevm uses a **data-first architecture** with two complementary APIs:
...
At the core of Tevm are **branded primitives** — native JavaScript types (Uint8Array, bigint, string) with TypeScript brands for type safety.
...
type Keccak256Hash = Uint8Array & { readonly [brand]: "Keccak256Hash" };
type Uint = bigint & { readonly [brand]: "Uint256" };

// These prevent accidents like:
const address: Address = someHash; // ❌ Type error
const hash: Keccak256Hash = someAddress;     // ❌ Type error
```
...
For familiar OOP ergonomics, Tevm provides class wrappers:
...
The Class API provides all you need for most use cases with familiar OOP patterns.

## Method Naming Convention

Tevm follows consistent naming patterns across all types:
...
const deployer = Address("0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2");
...
```typescript
import { Keccak256, Hex } from "tevm";
...
Keccak256.equals(hash, hash2); // true
```
...
```typescript
import { Transaction, Hex } from "tevm";
...
// Recover sender
const sender = Transaction(tx);
```

## Type Safety

Tevm provides opt-in type safety through branded types:

```typescript
import type { AddressType } from "tevm/Address";
import type { Keccak256Hash } from "tevm/Keccak256";

function processAddress(addr: AddressType) {
  // TypeScript ensures addr is a valid Address
}

function processHash(hash: Keccak256Hash) {
  // TypeScript ensures hash is a Keccak256 output
}
...
Tevm is optimized for performance:
...
```typescript
import { Transaction, Address, Hex } from "tevm";
...
const txHex = Hex.from(serialized);
...
console.log("Transaction hash:", Keccak256.toHex(txHash));
```
...
// ✅ Correct
import { Address } from "tevm";
import { AddressType } from "tevm/Address";
...
If you get type errors like "Type 'Uint8Array' is not assignable to type 'AddressType'", you need to construct the branded type properly:

```typescript
// ❌ Won't work - raw Uint8Array doesn't have the brand
const addr: AddressType = new Uint8Array(20);

// ✅ Correct
const addr = Address("0x...");
// or
const addr = AddressType("0x...");
```
...
<Tabs>
  ...
  <Tab title="Signing">
    ...
    // Tevm
    import { Keccak256, Secp256k1 } from "tevm";
    const hash = Keccak256.hash("hello");
...
[Read guide →](/getting-started/branded-types)
