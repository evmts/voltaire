---
title: SIWE Tests
description: Test examples for Sign-In with Ethereum (SIWE) message creation and verification.
---

## createSiweMessage formats an EIP-4361 message

Creates a properly formatted SIWE message from parameters.

```typescript
import { Address } from 'voltaire/Address'
import { createSiweMessage } from 'voltaire-effect/auth/siwe'

const address = Address.fromHex("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")
...
## parseSiweMessage parses a real SIWE message

Parses a SIWE message string into its component fields.

```typescript
import { Effect } from 'effect'
import { parseSiweMessage } from 'voltaire-effect/auth/siwe'
...
## verifySiweMessage verifies a valid SIWE message and signature

Verifies that a signature matches the message and expected address.

```typescript
import { Effect } from 'effect'
import { Address } from 'voltaire/Address'
import type { HashType } from 'voltaire/Hash'
import * as Secp256k1 from 'voltaire-effect/crypto/Secp256k1'
import { hashMessage } from 'voltaire-effect/crypto/Signature'
import { createSiweMessage, verifySiweMessage } from 'voltaire-effect/auth/siwe'
...
const createSignedMessage = () =>
  Effect.gen(function* () {
    ...
    const signature = yield* Secp256k1.sign(hash as unknown as HashType, privateKey)
...
## verifySiweMessage fails verification when nonce mismatches

Verification fails when the expected nonce doesn't match.

```typescript
import { Effect, Exit } from 'effect'
import { verifySiweMessage, VerifyError } from 'voltaire-effect/auth/siwe'
import { CryptoLive } from 'voltaire-effect/crypto'
