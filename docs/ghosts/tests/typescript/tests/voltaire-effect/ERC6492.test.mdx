---
title: ERC6492 Tests
description: Test examples for ERC-6492 signature wrapping/unwrapping for counterfactual contracts.
---

## Wraps and unwraps signatures

Demonstrates wrapping a signature with factory deployment data and unwrapping it.
...
import { unwrapSignature, wrapSignature } from "voltaire-effect/crypto/ERC6492";
...
it.effect("wraps and unwraps signatures", () =>
  Effect.gen(function* () {
    ...
    expect(Hex.toBytes(unwrapped.factoryData)).toEqual(Hex.toBytes(factoryData));
    expect(Hex.toBytes(unwrapped.signature)).toEqual(Hex.toBytes(innerSignature));
  }),
);
```

## Returns null for non-wrapped signatures

Shows that regular signatures are not mistakenly unwrapped.

```typescript
import * as Effect from "effect/Effect";
import { unwrapSignature } from "voltaire-effect/crypto/ERC6492";
...
## Verifies using deployless validator

Demonstrates signature verification using a deployless contract call.

```typescript
import { encodeParameters } from "@tevm/voltaire/Abi";
import * as Hex from "@tevm/voltaire/Hex";
import * as Effect from "effect/Effect";
import { verifySignature } from "voltaire-effect/crypto/ERC6492";
...
it.effect("verifies using deployless validator", () =>
  Effect.gen(function* () {
    ...
    const encodedParams = Hex.fromBytes(
      encodeParameters(VALIDATOR_PARAMS as any, [signer, messageHash, innerSignature] as any),
    );
...
## Surfaces revert reason from validator

Shows how validation errors are surfaced with revert data.
...
import { VerifyError } from "voltaire-effect/crypto/Signature";
import { verifySignature } from "voltaire-effect/crypto/ERC6492";
...
it.effect("surfaces revert reason from validator", () =>
  Effect.gen(function* () {
    ...
    const provider = {
      call: () => Effect.succeed(`0x08c379a0${revertData.slice(2)}` as `0x${string}`),
    } as any;
