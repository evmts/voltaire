---
title: ReceiptSchema Tests
description: Test examples for transaction Receipt schema showing post-Byzantium status, pre-Byzantium root, and EIP-4844 blob fields.
---

## Parses post-Byzantium receipt with status

Demonstrates parsing a modern receipt with status field.
...
## Parses pre-Byzantium receipt with root

Demonstrates parsing a legacy receipt with state root.
...
## Rejects receipt with both status and root

Demonstrates mutual exclusivity of status and root fields.
...
## Parses EIP-4844 blob gas fields

Demonstrates parsing receipts with blob transaction fields.
...
Demonstrates that all EVM transaction types are supported.
...
Demonstrates that required fields like effectiveGasPrice and type are enforced.
