---
title: '[TS/JS] src/wasm/fork-provider.wasm.test.ts'
source: 'src/wasm/fork-provider.wasm.test.ts'
---

> Auto-generated from test file: src/wasm/fork-provider.wasm.test.ts
...
describe("ForkProvider (WASM)", () => {
	...
	const hasForkWasm =
		existsSync(stateManagerPath) && existsSync(blockchainPath);

	it.skipIf(!hasForkWasm)(
		"reads forked state and blocks via injected EIP-1193 provider",
		async () => {
