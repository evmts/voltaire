...
const txHash = Hash.from('0xabc123...');
...
const blockHash = Hash.from('0xdef456...');
...
const trace = await provider.debug_traceCall({
  from: Address.from('0x742d35...'),
  to: Address.from('0xcontract...'),
  data: Hex.from('0xa9059cbb...')  // transfer(address,uint256)
}, 'latest', {
  tracer: 'callTracer'
});
