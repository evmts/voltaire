...
await provider.eth_getBalance(address, Quantity(18000000n));
```
...
const gasEstimate = await provider.eth_estimateGas({
  from: Address('0x...'),
  to: Address('0x...'),
  value: Quantity(1000000000000000000n),  // 1 ETH in wei
  data: Hex('0x...')
});
```
...
await provider.eth_getBalance(Address('0x742d35...'), 'latest');
