...
const txHash = await provider.eth_sendTransaction({
  ...
  to: Address.from('0x...'),
  value: Quantity.from(1000000000000000000n),
  data: Hex.from('0x...')
});
// Response<Hash>
```
...
const proof = await provider.eth_getProof(
  address,
  [Quantity.from(0), Quantity.from(1)],
  'latest'
);
// Response<Proof>
```
...
const result = await provider.eth_call({
  from: Address.from('0x...'),
  to: Address.from('0x...'),
  data: Hex.from('0x70a08231...')  // balanceOf(address)
}, 'latest');
// Response<Hex>
```
...
const logs = await provider.eth_getLogs({
  ...
  topics: [Hash.from('0x...')]  // Event signature
});
// Response<Log[]>
```
...
if (!codeRes.error && codeRes.result !== '0x') {
  ...
  const result = await provider.eth_call({
    to: contractAddress,
    data: Hex.from('0x18160ddd')  // totalSupply()
  }, 'latest');
}
```
