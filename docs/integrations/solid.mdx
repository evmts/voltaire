...
function AddressInput() {
  ...
  createEffect(async () => {
    ...
    try {
      ...
      setBalance(Uint.toEther(result))
    } finally {
      setLoading(false)
    }
  })
...
async function fetchBalances(addresses: string[]) {
  return Promise.all(
    addresses.map(async (addr) => {
      ...
      return { address: addr, balance: Uint.toEther(balance) }
    })
  )
}
  ...
  const actions = {
    async fetchBalance(address: string) {
      ...
      try {
        const addr = Address.from(address)
        const balance = await provider.eth.getBalance({ address: addr })
        setState('balances', address, Uint.toEther(balance))
      } finally {
