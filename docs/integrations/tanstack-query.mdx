...
```bash
bun add @tanstack/react-query tevm
```
...
import * as Uint from 'tevm/primitives/Uint'
import { createJsonRpcProvider } from 'tevm/jsonrpc'
  ...
  return useQueries({
    queries: addresses.map(address => ({
      ...
      queryFn: async () => {
        const addr = Address.from(address)
        const balance = await provider.eth.getBalance({ address: addr })
        return { address, balance: Uint.toEther(balance) }
      },
    })),
  })
}
  ...
  return useMutation({
    ...
    onMutate: async ({ to, value }) => {
      ...
      queryClient.setQueryData(['balance', to], (old: string) => {
        const oldBigInt = Uint.fromEther(old)
        const newBalance = oldBigInt + value
        return Uint.toEther(newBalance)
      })
