...
import { Denomination } from "voltaire-effect"
...
function Balance({ address }: { address: `0x${string}` }) {
  ...
  return Result.match(result, {
    ...
    onSuccess: (balance) => (
      <div>{Denomination.formatEther(balance)} ETH</div>
    ),
    onFailure: (cause) => (
...
const formatted = Result.map(result, (balance) =>
  Denomination.formatEther(balance)
)
```
...
function BalanceWithRefresh({ address }: { address: `0x${string}` }) {
  ...
  return (
    <div>
      {Result.match(result, {
        onInitial: () => "Loading...",
        onSuccess: (b) => `${Denomination.formatEther(b)} ETH`,
        onFailure: () => "Error",
      })}
...
function BalanceSuspense({ address }: { address: `0x${string}` }) {
  ...
  return <div>{Denomination.formatEther(balance)} ETH</div>
}
    ...
    <main>
      ...
      <section>
        ...
        {Result.match(balanceResult, {
          onInitial: () => <p>Loading...</p>,
          onSuccess: (balance) => (
            <p>{Denomination.formatEther(balance)} ETH</p>
          ),
