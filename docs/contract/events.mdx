...
- Starts with 500 blocks per request
...
type EventStreamResult = {
  ...
  metadata: {
    currentBlock: bigint;   // Latest block height
...
Create EventStream without a Contract:
...
```typescript
import { BlockRangeTooLargeError, EventStreamAbortedError } from '@voltaire/contract';
