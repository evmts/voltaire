...
Adapters enable conversion between Tevm's Provider interface and other provider formats.
...
Convert Tevm's Provider to a standard EIP-1193 provider.

### Eip1193Provider.from

Wrap a Tevm Provider to expose an EIP-1193 compatible interface:

```typescript
import { Eip1193Provider } from 'tevm/provider/adapters';
import type { Provider } from 'tevm/provider';

// Your Tevm provider
const provider: Provider = /* ... */;

// Convert to EIP-1193
const eip1193 = Eip1193Provider(provider);
...
// provider.eth_getBalance(Address('0x742d35...'), 'latest')
```
...
const eip1193 = Eip1193Provider(tevmProvider);
...
```typescript
import { HttpHandler } from 'tevm/provider/adapters';
import type { Provider } from 'tevm/provider';

const provider: Provider = /* ... */;

// Create HTTP handler
const handler = HttpHandler(provider);
...
```typescript
import type { Provider } from 'tevm/provider';
import type * as Address from 'tevm/primitives/Address';
import type * as Hash from 'tevm/primitives/Hash';
...
export class WebSocketProvider implements Provider {
  ...
  async eth_blockNumber(): Promise<Response<Quantity>> {
    ...
    return response.error
      ...
      : { result: Quantity(response.result) };
  }
...
export class MockProvider implements Provider {
  ...
  async eth_blockNumber(): Promise<Response<Quantity>> {
    return { result: Quantity(this.blockNumber) };
  }
  ...
  // Test helper methods
  setBalance(address: Address.AddressType, balance: bigint) {
