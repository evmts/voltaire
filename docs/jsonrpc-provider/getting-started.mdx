...
Install Tevm and start making type-safe JSON-RPC requests to Ethereum nodes.
...
Tevm provides the `Provider` interface but doesn't include transport implementations. Create providers using adapters or third-party libraries.
...
```typescript
import type { Provider, Response } from 'tevm/provider';
...
const transferSignature = Keccak256.from(
...
const gasEstimate = await provider.request(request);
...
- [Keccak256](/crypto/keccak256) - Hash primitive documentation
