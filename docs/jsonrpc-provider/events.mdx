...
const targetAddress = Address('0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb0');
...
for await (const log of provider.events.logs({
  ...
  const from = Address('0x' + log.topics[1].slice(26));
  const to = Address('0x' + log.topics[2].slice(26));
...
async function monitorContractEvents(
  contractAddress: Address.AddressType,
  eventSignature: Keccak256.Keccak256Hash
) {
...
| Feature | EIP-1193 | Tevm Provider |
|---------|----------|-------------------|
