...
<Note>
The WASM API is **100% identical** to the TypeScript API. You can swap implementations without changing your code.
</Note>

```typescript
import { Keccak256, Secp256k1, Hex } from '@tevm/voltaire/wasm'

// Crypto operations - same API as TypeScript
const hash = Keccak256.hash(data)
const recovered = Secp256k1.recoverAddress(hash, signature)

// Primitives - same API as TypeScript
...
<Note>
The Native API is **100% identical** to the TypeScript API. You can swap implementations without changing your code.
</Note>

```typescript
import { Keccak256, isBun, isNode } from '@tevm/voltaire/native'
...
const selector = await Keccak256.selector('transfer(address,uint256)')
