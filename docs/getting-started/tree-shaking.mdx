...
Tevm takes a **data-first approach** to Ethereum primitives. Core types are validated [branded types](/getting-started/branded-types) that separate data from operations, enabling maximum tree-shakeability.
...
Tevm provides a tree-shakeable API architecture for TypeScript built on data-first principles.

## How Classes Avoid Coupling

Tevm's Class API uses `Object.setPrototypeOf()` to add methods without encapsulating data:

```typescript
// Factory function creates instance
export function Address(value) {
  const result = AddressType(value);
...
Address.prototype.toChecksummed = function() {
  return AddressType.toChecksummed(this);
};

// Static method delegates to functional implementation
Address.toChecksummed = AddressType.toChecksummed;
```
...
import { fromHex, toChecksummed, isZero } from 'tevm/Address';
...
<Tip>
Importing from `tevm/Address` instead of `tevm` enables tree-shaking. For example, avoiding `toChecksummed()` excludes the entire keccak256 implementation (~50kb).
</Tip>
...
```typescript
// Import specific functions (tree-shakeable)
import { fromHex, toChecksummed, calculateCreateAddress } from 'tevm/Address';
