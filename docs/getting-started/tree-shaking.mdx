...
import { fromHex, toHex, equals } from '@voltaire/primitives/Address';

const addr = fromHex('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e');
const hex = toHex(addr);
```

Compared to the ergonomic Class API (default in docs):

```typescript
// Class API - more intuitive, less tree-shakeable
...
<Tip>
Importing from `@voltaire/primitives/Address` instead of `@voltaire/primitives` enables better tree-shaking. Avoiding methods like `toChecksummed()` excludes the keccak256 implementation (~50kb).
</Tip>
...
import { ToChecksummed } from '@voltaire/primitives/Address';
