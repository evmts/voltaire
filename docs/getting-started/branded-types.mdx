...
Voltaire uses `Uint8Array` as the underlying type instead of strings because:
- **Eliminates casing bugs** - No issues with lowercase vs checksummed vs uppercase addresses
- **Better performance** - Native byte operations without encoding/decoding overhead
- **Simplest representation** - Follows Voltaire's philosophy of using the simplest JavaScript data structure that correctly represents the type at all times
</Note>
