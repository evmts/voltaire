...
Add compile-time-only brand field using Symbols. A `BrandedAddress` is a `Uint8Array` with a compile-time-only Symbol tag:

```typescript
type BrandedAddress = Uint8Array & { readonly [Symbol.for("type")]: "Address" }
type BrandedBytecode = Uint8Array & { readonly [Symbol.for("type")]: "Bytecode" }
...
All primitives use Symbol-based branding with importable symbols:
...
The `brand` symbol can be imported from `@tevm/voltaire/brand`:

```typescript
import { brand } from '@tevm/voltaire/brand';

// The brand symbol is:
brand  // Symbol.for("type") - Base type identifier for all branded types
```

## Hierarchical Type System

Voltaire uses boolean flags to build type hierarchies. Base types are extended with additional properties:

```typescript
import { brand } from '@tevm/voltaire/brand';

// Base types
type Bytes20 = Uint8Array & { readonly [brand]: "Bytes20" }
type Hex = string & { readonly [brand]: "Hex" }

// Extended types add boolean flags
type Address = Bytes20 & { readonly checksum: true }
type ChecksumAddress = Hex & {
  readonly [brand]: "Hex"
...
This enables fine-grained type safety:

```typescript
function requireChecksummed(addr: ChecksumAddress) { /* ... */ }
function requireLowercase(hex: LowercaseHex) { /* ... */ }

const addr = Address.toChecksummed(address); // ChecksumAddress
requireChecksummed(addr); // ✓ Valid

const lower = Address.toLowercase(address); // LowercaseHex
requireChecksummed(lower); // ✗ Error: Missing checksum flag
requireLowercase(lower); // ✓ Valid
```
