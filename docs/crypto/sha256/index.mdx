...
const publicKey = Hex.from('0x04' + '1234...'); // 65-byte uncompressed secp256k1 public key
...
function merkleRoot(leaves: Uint8Array[]): Uint8Array {
  ...
  while (hashes.length > 1) {
    ...
    for (let i = 0; i < hashes.length; i += 2) {
      ...
      const combined = Hex.from('0x' +
        Array.from(left).map(b => b.toString(16).padStart(2, '0')).join('') +
        Array.from(right).map(b => b.toString(16).padStart(2, '0')).join('')
      );
