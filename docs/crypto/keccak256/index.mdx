...
Tevm provides Keccak256 as a **namespace with multiple implementation variants**:
...
All variants share the same data-first API and return `Keccak256Hash` type.
...
Tevm provides four Keccak256 implementations, each optimized for different use cases:
...
```typescript
import { Keccak256Accel } from 'tevm/native/crypto/keccak256-accel';
...
### `Keccak256.hash(data: Uint8Array): Keccak256Hash`
...
### `Keccak256.hashString(str: string): Keccak256Hash`
...
### `Keccak256.hashHex(hex: string): Keccak256Hash`
...
### `Keccak256.hashMultiple(chunks: readonly Uint8Array[]): Keccak256Hash`
...
### `Keccak256.topic(signature: string): Keccak256Hash`
...
type Keccak256Hash = Uint8Array & { readonly __tag: "Hash" };
...
### 1. Pure TypeScript (Default)
Direct wrapper around @noble/hashes battle-tested implementation:

```typescript
import { keccak_256 } from "@noble/hashes/sha3.js";

export function hash(data: Uint8Array): Keccak256Hash {
...
await Keccak256Wasm.init(); // Load primitives.wasm
const hash = Keccak256Wasm.hash(data);
```
...
**Characteristics:**
- x86-64: AVX2/SSE2 vector instructions
- ARM: NEON vector instructions
- Rust keccak-asm crate (portable and asm features)
- ~500-1000 MB/s throughput
- Requires native FFI bindings

**Source:** `src/crypto/keccak_asm.zig` + `Cargo.toml` (keccak-asm)

### 4. Hardware Acceleration (SIMD/AVX2)
SIMD-accelerated implementation using AVX2 vector operations (x86-64):
...
- Uses AVX2 vector instructions for parallel state transformation
- Optimized bit rotation using BMI2 instructions
- Vectorized XOR operations for theta step
- ~800+ MB/s throughput on x86-64 with AVX2
- Auto-fallback to standard library on unsupported platforms
