...
All Keccak256 implementations share the same API. Return type is `BrandedHash` (32-byte `Uint8Array` with type branding).
...
All Keccak256 functions return `BrandedHash`, a branded `Uint8Array` type:

```typescript
import type { BrandedHash } from '@tevm/voltaire/primitives/Hash';

// BrandedHash is a Uint8Array with type branding for compile-time safety
type BrandedHash = Uint8Array & { readonly __tag: "Hash" };

// All hash functions return this type
const hash: BrandedHash = Keccak256.hash(data);
const topic: BrandedHash = Keccak256.topic("Transfer(address,address,uint256)");

// Zero runtime overhead - just TypeScript compile-time checking
console.log(hash instanceof Uint8Array); // true
```

See [Hash Primitive](/primitives/hash) for full type documentation.
