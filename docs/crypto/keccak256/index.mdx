...
Voltaire provides multiple Keccak256 implementations optimized for different environments:

### Default (Pure TypeScript)
Uses @noble/hashes for broad compatibility and zero native dependencies.

```typescript
import { Keccak256 } from '@tevm/voltaire/crypto/Keccak256';

const hash = Keccak256.hash(data); // Pure JS, ~100-200 MB/s
```

### WASM (Unified)
Compiled from Zig via unified loader. Size-optimized ReleaseSmall build.

```typescript
import { Keccak256Wasm } from '@tevm/voltaire/crypto/keccak256.wasm';

await Keccak256Wasm.init();
const hash = Keccak256Wasm.hash(data); // ~200-400 MB/s
```

### Standalone WASM
Minimal 3KB WASM module with only Keccak256.

```typescript
import { Keccak256Standalone } from '@tevm/voltaire/crypto/keccak256.standalone';

await Keccak256Standalone.init();
const hash = Keccak256Standalone.hash(data); // ~200-400 MB/s, tiny bundle
```

### Native/Hardware Accelerated
Assembly-optimized implementation via keccak-asm Rust crate for x86-64 and ARM.

```typescript
// When using native FFI bindings (Node.js, Bun, Deno)
import { Keccak256 } from '@tevm/voltaire/native/crypto/keccak256';

const hash = Keccak256.hash(data); // ~500-1000 MB/s
```

See [Implementation Guide](/crypto/keccak256/implementations) for detailed comparison and selection criteria.
...
```typescript
// Choose implementation at import time
import { Keccak256 } from '@tevm/voltaire/crypto/Keccak256';       // default
import { Keccak256Wasm } from '@tevm/voltaire/crypto/keccak256.wasm'; // WASM
...
function processData(algorithm: typeof Keccak256, data: Uint8Array) {
  return algorithm.hash(data);
}

// Select based on environment
const impl = typeof WebAssembly !== 'undefined' ? Keccak256Wasm : Keccak256;
const result = processData(impl, data);
```
