...
async function getKeccak256Impl() {
  ...
  if (typeof process !== 'undefined' && process.versions?.node) {
    try {
      const { Keccak256 } = await import('@tevm/voltaire/native/crypto/keccak256');
      return Keccak256;
    } catch {
  ...
  // Fallback to pure JS
  const { Keccak256 } = await import('@tevm/voltaire/crypto/Keccak256');
  return Keccak256;
}
...
const Keccak256 = await import('@tevm/voltaire/crypto/keccak256.standalone')
