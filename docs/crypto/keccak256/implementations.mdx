...
Voltaire provides four Keccak256 implementations optimized for different deployment scenarios. All share the same data-first API, enabling transparent algorithm swapping.
...
**Use when:**
- Using multiple Voltaire primitives/crypto functions
...
const Keccak256Impl = await (async () => {
  if (typeof process !== 'undefined' && process.versions?.node) {
    // Node.js - try native
    try {
      return await import('@tevm/voltaire/native/crypto/keccak256');
    } catch {
      // Fallback to WASM
  ...
  if (typeof WebAssembly !== 'undefined') {
    ...
    const { Keccak256Standalone } = await import('@tevm/voltaire/crypto/keccak256.standalone');
    await Keccak256Standalone.init();
    return { Keccak256: Keccak256Standalone };
  }

  // Fallback to pure JS
  return await import('@tevm/voltaire/crypto/Keccak256');
})();
