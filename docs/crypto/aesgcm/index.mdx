...
const salt = Hex.from('0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d');
...
const rawKey = Hex.from('0xf8e9a72d4c3b1a6e7d5f2c8b9e1a4d7f3c6b9e2a5d8f1c4b7e0a3d6f9c2b5e8a');
...
async function encryptWithPassword(plaintext, password) {
  ...
  import * as Hex from '@tevm/voltaire/primitives/Hex';
  const salt = Hex.from('0x3e4d5c6b7a8910293847566574839201');
...
// Load and use
const storedKeyBytes = Uint8Array.from(
...
async function storeEncryptedKey(masterKey, userPassword) {
  const salt = Hex.from('0x7f8e9d0c1b2a3948576e8d9c0b1a2938');
...
const salt = Hex.from('0xa7b6c5d4e3f2a1b0c9d8e7f6a5b4c3d2');
```
...
async function encryptFile(fileData, password) {
  const salt = Hex.from('0x9e8d7c6b5a4938271605948372615049');
  const key = await AesGcm.deriveKey(password, salt, 600000, 256);
  const nonce = AesGcm.generateNonce();
