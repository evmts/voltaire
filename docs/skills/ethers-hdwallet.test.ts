...
describe("Ethers HD Wallet Playbook", () => {
	...
	it.skipIf(!hasNativeHDWallet)("should work with HDWallet derivation", async () => {
		const { Bip39 } = await import("../../src/crypto/index.js");
	...
	it.skipIf(!hasNativeHDWallet)("should derive Ethereum accounts with BIP-44 path", async () => {
		const { Bip39 } = await import("../../src/crypto/index.js");
		const { HDWallet } = await import("../../src/native/index.js");
		...
		const privateKey0 = HDWallet.getPrivateKey(account0);
		const privateKey1 = HDWallet.getPrivateKey(account1);

		expect(privateKey0).toBeInstanceOf(Uint8Array);
		expect(privateKey1).toBeInstanceOf(Uint8Array);
		expect(privateKey0).not.toEqual(privateKey1);
	});
