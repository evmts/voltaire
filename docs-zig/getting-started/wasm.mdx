...
| **Native** | `@tevm/voltaire/native` | Node.js, Bun only | Maximum performance via FFI |
...
const hash = Keccak256.hash(Hex.fromString('hello'))
...
Direct bindings to Zig via Bun FFI or Node-API. Maximum performance.
...
<Warning>
Native FFI requires the compiled `.dylib` (macOS), `.so` (Linux), or `.dll` (Windows) to be available. Run `zig build build-ts-native` to compile.
</Warning>
