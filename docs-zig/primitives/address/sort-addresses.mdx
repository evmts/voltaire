...
<Tip>
View the complete executable example at [`playground/src/examples/primitives/address/sort-dedupe.ts`](https://github.com/roninjin10/voltaire/blob/main/playground/src/examples/primitives/address/sort-dedupe.ts).
</Tip>
...
**Note:** Tevm provides optimized `deduplicateAddresses` function. Above shown for educational purposes.
...
```zig
import { Address } from 'tevm'
import type { EventLog } from 'tevm/EventLog'
