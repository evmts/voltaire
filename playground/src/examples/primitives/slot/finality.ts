...
for (const epoch of epochs) {
	const checkpointSlot = Slot(getCheckpoint(epoch));
}

// Normal finality (2 epochs)
const normalHead = Slot(8000000n);
...
const delayedHead = Slot(8100000n);
...
// Critical delay
const criticalHead = Slot(8200000n);
...
const currentCheckpoint = Slot(8300000n);
...
const weakSubjectivitySlot = Slot(
	weakSubjectivityEpoch * BigInt(SLOTS_PER_EPOCH),
);
const reorgHead = Slot(8400000n);
const reorgHeadEpoch = Slot.toEpoch(reorgHead);
const safeSlot = Slot((reorgHeadEpoch - 1n) * BigInt(SLOTS_PER_EPOCH)); // 1 epoch back
const finalizedReorgSlot = Slot(
	(reorgHeadEpoch - 2n) * BigInt(SLOTS_PER_EPOCH),
); // 2 epochs back
const votingEpoch = 9000n;
const votingSlot = Slot(votingEpoch * BigInt(SLOTS_PER_EPOCH));
