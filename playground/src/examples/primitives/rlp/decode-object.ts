...
const schema: (keyof Transaction)[] = ['nonce', 'gasPrice', 'gasLimit', 'to', 'value', 'data'];
...
const simpleEncoded = Rlp.encodeObject({ id: new Uint8Array([0x01]), value: new Uint8Array([0x42]) }, ['id', 'value']);
const simpleDecoded = Rlp.decodeObject<SimpleData>(simpleEncoded, ['id', 'value']);
...
const nestedDecoded = Rlp.decodeObject<NestedData>(nestedEncoded, ['header', 'items']);
...
console.log('Round-trip field1 matches:', Hex.toHex(testDecoded.field1) === Hex.toHex(original.field1));
console.log('Round-trip field2 matches:', Hex.toHex(testDecoded.field2) === Hex.toHex(original.field2));
