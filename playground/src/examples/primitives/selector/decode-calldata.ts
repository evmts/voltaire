...
const transferSel = Selector(transferCalldata.slice(0, 10)); // "0x" + 8 hex chars
...
const approveSel = Selector(approveCalldata.slice(0, 10));
...
const swapSel = Selector(swapCalldata.slice(0, 10));
...
const nftSel = Selector(safeTransferCalldata.slice(0, 10));
...
function identifyFunction(calldata: string): string {
	...
	const sel = Selector(calldata.slice(0, 10));
...
try {
	...
	Selector(shortCalldata);
} catch (e) {}

try {
	const emptyCalldata = "0x";
	Selector(emptyCalldata);
} catch (e) {}
