...
export class Executor {
  ...
  private extractImports(code: string): Array<{ name: string; path: string; isNamed: boolean; names?: string[] }> {
    const imports: Array<{ name: string; path: string; isNamed: boolean; names?: string[] }> = [];

    // Match: import * as Name from 'path'
    const namespaceRegex = /import\s+\*\s+as\s+(\w+)\s+from\s+['"]([^'"]+)['"]\s*;?/g;
    let match;
  ...
  private transformImports(code: string, imports: Array<{ name: string; path: string; isNamed: boolean; names?: string[] }>): string {
