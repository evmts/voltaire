...
The block module provides Effect-wrapped utilities for fetching blockchain blocks. These are standalone functions that work with `TransportService`, useful for one-off block fetches without needing streaming helpers.

For continuous block streaming with reorg detection, use [`makeBlockStream`](/services/block-stream).
...
- [Block Streaming](/services/block-stream) — Continuous block streaming with reorg detection
