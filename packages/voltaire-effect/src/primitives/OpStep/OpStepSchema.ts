 ...
 * @since 0.0.1
 */
export const OpStepSchema: S.Schema<OpStepType, OpStepInput> =
	S.transformOrFail(
		S.Struct({
			pc: S.Number,
			op: S.Number,
		...
		OpStepTypeSchema,
		{
			...
			decode: (value, _options, ast) => {
				try {
					return ParseResult.succeed(
						OpStep.from(value as Parameters<typeof OpStep.from>[0]),
					);
				} catch (e) {
			...
			encode: (opStep) =>
				ParseResult.succeed({
					...
					depth: opStep.depth,
					...(opStep.stack !== undefined && {
						stack: opStep.stack as readonly bigint[],
					}),
					...(opStep.memory !== undefined && { memory: opStep.memory }),
					...(opStep.storage !== undefined && {
						storage: opStep.storage as Record<string, bigint>,
					}),
