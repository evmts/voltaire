import type { TransactionStatus } from "@tevm/voltaire";
 ...
 * @since 0.0.1
 */
export type TransactionStatusType = ReturnType<
...
const TransactionStatusTypeSchema = S.declare<TransactionStatusType>(
	(u): u is TransactionStatusType => {
		...
		return (
			"type" in u &&
			(u.type === "pending" || u.type === "success" || u.type === "failed")
		);
	},
 ...
 * @since 0.0.1
 */
export const Schema = S.transformOrFail(S.Any, TransactionStatusTypeSchema, {
	strict: true,
	decode: (value, _options, ast) => {
		...
		return ParseResult.fail(
			new ParseResult.Type(ast, value, `Expected TransactionStatus object`),
		);
	},
