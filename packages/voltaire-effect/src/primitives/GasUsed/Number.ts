/**
 * @fileoverview Schema for GasUsed from number.
 * @module GasUsed/Number
 * @since 0.1.0
 */

import { GasUsed } from "@tevm/voltaire";
import * as ParseResult from "effect/ParseResult";
import * as S from "effect/Schema";
import type { GasUsedType } from "./BigInt.js";

const GasUsedTypeSchema = S.declare<GasUsedType>(
	(u): u is GasUsedType => typeof u === "bigint" && u >= 0n,
	{ identifier: "GasUsed" },
);

/**
 * Schema for GasUsed from number.
 *
 * @example
 * ```typescript
 * import * as GasUsed from 'voltaire-effect/primitives/GasUsed'
 * import * as S from 'effect/Schema'
 *
 * const used = S.decodeSync(GasUsed.Number)(21000)
 * ```
 *
 * @since 0.1.0
 */
export const Number: S.Schema<GasUsedType, number> = S.transformOrFail(
	S.Number,
	GasUsedTypeSchema,
	{
		strict: true,
		decode: (value, _options, ast) => {
			try {
				return ParseResult.succeed(GasUsed.from(value));
			} catch (e) {
				return ParseResult.fail(
					new ParseResult.Type(ast, value, (e as Error).message),
				);
			}
		},
		encode: (g) => ParseResult.succeed(globalThis.Number(g)),
	},
).annotations({ identifier: "GasUsed.Number" });
