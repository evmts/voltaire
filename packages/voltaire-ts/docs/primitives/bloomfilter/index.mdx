...
```typescript
import { BloomFilter, BITS, DEFAULT_HASH_COUNT } from 'tevm';
...
const blockFilter = BloomFilter(block.logsBloom, BITS, DEFAULT_HASH_COUNT);
...
const filter1 = BloomFilter(block1.logsBloom, BITS, DEFAULT_HASH_COUNT);
const filter2 = BloomFilter(block2.logsBloom, BITS, DEFAULT_HASH_COUNT);
...
const restored = BloomFilter(hex, BITS, DEFAULT_HASH_COUNT);
```
...
import { create, add, contains, toHex } from 'tevm/BrandedBloomFilter';
...
- [BloomFilter (Effect)](https://voltaire-effect.tevm.sh/primitives/bloomfilter) - Effect.ts integration with Schema validation
...
- [Keccak256](/crypto/keccak256) - Keccak256 hashing for log topics
