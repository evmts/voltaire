...
describe.skipIf(!hasNativeKzg)("docs/crypto/kzg/proofs.mdx - KZG Proofs", async () => {
	...
	describe.skipIf(!hasCkzg)("Compute Proof - Factory API", () => {
		...
		it("should compute proof using factory pattern", async () => {
			...
			try {
				// c-kzg v4.x requires loadTrustedSetup(precompute, filePath)
				// precompute=0 means no precomputation, filePath undefined uses default
				// @ts-expect-error - loadTrustedSetup signature varies by version
				ckzg.loadTrustedSetup(0);
			} catch {
				// Already loaded or setup failed - continue anyway
			}
