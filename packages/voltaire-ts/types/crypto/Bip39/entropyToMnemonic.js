...
export function entropyToMnemonic(entropy, wl = wordlist) {
    ...
    if (!validLengths.includes(entropy.length)) {
        throw new InvalidEntropyError(`Entropy must be 16, 20, 24, 28, or 32 bytes, got ${entropy.length}`, {
            code: -32602,
    ...
    try {
        return /** @type {import('./MnemonicType.js').Mnemonic} */ (_entropyToMnemonic(entropy, wl));
    }
    catch (error) {
