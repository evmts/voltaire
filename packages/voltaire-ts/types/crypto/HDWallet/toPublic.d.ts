/**
 * Create public-only version of extended key (neutered key).
 *
 * Removes private key material, keeping only public key and chain code.
 *
 * @see https://voltaire.tevm.sh/crypto for crypto documentation
 * @since 0.0.0
 * @param {import('./ExtendedKeyType.js').BrandedExtendedKey} key - Extended key with or without private key
 * @returns {import('./ExtendedKeyType.js').BrandedExtendedKey} Public-only extended key (cannot derive hardened children)
 * @throws {HDWalletError} If key does not have public key
 * @example
 * ```javascript
 * import * as HDWallet from './crypto/HDWallet/index.js';
 * const pubOnlyKey = HDWallet.toPublic(key);
 * // Can share publicly without exposing private key
 * ```
 */
export function toPublic(key: import("./ExtendedKeyType.js").BrandedExtendedKey): import("./ExtendedKeyType.js").BrandedExtendedKey;
//# sourceMappingURL=toPublic.d.ts.map