...
import { InvalidPrivateKeyError, InvalidPublicKeyError, Secp256k1Error, } from "./errors.js";
/**
...
export function ecdh(privateKey, publicKey) {
    if (privateKey.length !== PRIVATE_KEY_SIZE) {
        throw new InvalidPrivateKeyError(`Private key must be ${PRIVATE_KEY_SIZE} bytes, got ${privateKey.length}`, {
    ...
    if (publicKey.length !== PUBLIC_KEY_SIZE) {
        throw new InvalidPublicKeyError(`Public key must be ${PUBLIC_KEY_SIZE} bytes, got ${publicKey.length}`, {
