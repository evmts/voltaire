...
export function verify(signature, messageHash, publicKey) {
    ...
    if (publicKey.length !== PUBLIC_KEY_SIZE) {
        throw new InvalidSignatureError(`Public key must be ${PUBLIC_KEY_SIZE} bytes, got ${publicKey.length}`, {
    ...
    if (signature.r.length !== SIGNATURE_COMPONENT_SIZE) {
        throw new InvalidSignatureError(`Signature r must be ${SIGNATURE_COMPONENT_SIZE} bytes, got ${signature.r.length}`, {
            code: "INVALID_SIGNATURE_R_LENGTH",
            docsPath: "/crypto/secp256k1/verify#error-handling",
        });
    }
    if (signature.s.length !== SIGNATURE_COMPONENT_SIZE) {
        throw new InvalidSignatureError(`Signature s must be ${SIGNATURE_COMPONENT_SIZE} bytes, got ${signature.s.length}`, {
