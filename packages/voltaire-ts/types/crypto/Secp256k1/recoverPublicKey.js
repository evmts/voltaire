...
export function recoverPublicKey(signature, messageHash) {
    ...
    if (signature.r.length !== SIGNATURE_COMPONENT_SIZE) {
        throw new InvalidSignatureError(`Signature r must be ${SIGNATURE_COMPONENT_SIZE} bytes`, {
    ...
    if (signature.s.length !== SIGNATURE_COMPONENT_SIZE) {
        throw new InvalidSignatureError(`Signature s must be ${SIGNATURE_COMPONENT_SIZE} bytes`, {
    ...
    try {
        ...
        return /** @type {import('./Secp256k1PublicKeyType.js').Secp256k1PublicKeyType} */ (uncompressed.slice(1));
    }
    catch (error) {
