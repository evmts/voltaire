...
export function fromBytes(bytes) {
    if (bytes.length !== 64) {
        throw new InvalidPublicKeyError(`Invalid public key: expected 64 bytes, got ${bytes.length}`, {
    ...
    if (!isValidPublicKey(bytes)) {
        throw new InvalidPublicKeyError("Invalid public key: not a valid point on the secp256k1 curve", {
            code: "SECP256K1_INVALID_CURVE_POINT",
            context: { publicKeyLength: bytes.length },
            docsPath: "/crypto/secp256k1/public-key#error-handling",
        });
    }
    return /** @type {import('../Secp256k1PublicKeyType.js').Secp256k1PublicKeyType} */ (bytes);
}
