...
import { InvalidPublicKeyError, InvalidSecretKeyError, X25519Error, } from "./errors.js";
/**
...
export function scalarmult(secretKey, publicKey) {
    if (secretKey.length !== SECRET_KEY_SIZE) {
        throw new InvalidSecretKeyError(`Secret key must be ${SECRET_KEY_SIZE} bytes, got ${secretKey.length}`);
    }
