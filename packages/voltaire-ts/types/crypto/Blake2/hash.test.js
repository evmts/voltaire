...
describe("Blake2 hash function", () => {
    ...
    describe("output length validation", () => {
        it("should throw error for invalid output length (too small)", () => {
            expect(() => hash(new Uint8Array([]), 0)).toThrow("Invalid output length: 0. Must be between 1 and 64 bytes.");
            expect(() => hash(new Uint8Array([]), -1)).toThrow("Invalid output length: -1. Must be between 1 and 64 bytes.");
        });
        it("should throw error for invalid output length (too large)", () => {
            expect(() => hash(new Uint8Array([]), 65)).toThrow("Invalid output length: 65. Must be between 1 and 64 bytes.");
            expect(() => hash(new Uint8Array([]), 100)).toThrow("Invalid output length: 100. Must be between 1 and 64 bytes.");
        });
