...
export function EncodeValue({ keccak256, hashStruct }) {
    ...
    return function encodeValue(type, value, types) {
        ...
        if (arrayMatch) {
            ...
            const arr = /** @type {import('./EIP712Type.js').MessageValue[]} */ (value);
            /** @type {Uint8Array[]} */
        ...
        if (type.startsWith("uint") || type.startsWith("int")) {
            ...
            if (typeof value === "bigint") {
                ...
                throw new Eip712EncodingError(`Cannot encode value of type ${typeof value} as ${type}`, {
                    code: -32602,
            ...
            if (bits < 8 || bits > 256 || bits % 8 !== 0) {
                throw new Eip712EncodingError(`Invalid ${isUnsigned ? "uint" : "int"} size: ${bits}`, {
                    code: -32602,
                    context: { type, bits },
                    docsPath: "/crypto/eip712/encode-value#error-handling",
                });
            ...
            if (isUnsigned) {
                ...
                if (num < 0n || num > max) {
                    throw new Eip712EncodingError(`Value ${num} out of range for ${type} (0 to ${max})`, {
                        code: -32602,
                ...
                if (num < min || num > max) {
                    throw new Eip712EncodingError(`Value ${num} out of range for ${type} (${min} to ${max})`, {
                        code: -32602,
        ...
        if (type === "address") {
            const addr = 
            /** @type {import('../../primitives/Address/AddressType.js').AddressType} */ (value);
        ...
        if (type.startsWith("bytes")) {
            ...
            if (match?.[1]) {
                ...
                if (bytes.length !== size) {
                    throw new Eip712EncodingError(`${type} must be ${size} bytes, got ${bytes.length}`, {
                        code: -32602,
