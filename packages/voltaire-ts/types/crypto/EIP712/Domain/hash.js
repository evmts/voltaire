 ...
 * @returns {boolean}
 */
const isByteLike = (value) => value instanceof Uint8Array ||
    typeof value === "string" ||
 ...
 * @returns {boolean}
 */
const isChainIdType = (value) => typeof value === "bigint" ||
    typeof value === "number" ||
...
const validateName = (value) => {
    if (typeof value !== "string") {
        throwInvalidDomain(`Invalid domain field: 'name' must be a string, got ${typeof value}`, {
            field: "name",
            value,
            expectedType: "string",
        });
...
const validateVersion = (value) => {
    if (typeof value !== "string") {
        throwInvalidDomain(`Invalid domain field: 'version' must be a string, got ${typeof value}`, {
            field: "version",
            value,
            expectedType: "string",
        });
...
const validateChainId = (value) => {
    if (!isChainIdType(value)) {
        throwInvalidDomain(`Invalid domain field: 'chainId' must be a bigint, number, or Uint8Array, got ${typeof value}`, {
            field: "chainId",
            value,
...
const validateVerifyingContract = (value) => {
    if (!isByteLike(value)) {
        throwInvalidDomain(`Invalid domain field: 'verifyingContract' must be an address (Uint8Array or hex string), got ${typeof value}`, {
    ...
    if (value instanceof Uint8Array && value.length !== 20) {
        throwInvalidDomain(`Invalid domain field: 'verifyingContract' must be 20 bytes, got ${value.length} bytes`, {
            field: "verifyingContract",
            value,
            expectedLength: 20,
...
const validateSalt = (value) => {
    if (!isByteLike(value)) {
        throwInvalidDomain(`Invalid domain field: 'salt' must be bytes32 (Uint8Array or hex string), got ${typeof value}`, {
            field: "salt",
            value,
    ...
    if (value instanceof Uint8Array && value.length !== 32) {
        throwInvalidDomain(`Invalid domain field: 'salt' must be 32 bytes, got ${value.length} bytes`, {
            field: "salt",
            value,
            expectedLength: 32,
            actualLength: value.length,
...
export function Hash({ hashStruct }) {
    return function hash(domain) {
        ...
        /** @type {import('../EIP712Type.js').Message} */ (filteredDomain), domainTypes);
    };
}
