...
export class KzgNotInitializedError extends KzgError {
    _tag = "KzgNotInitializedError";
    constructor(message = "KZG trusted setup not initialized", options) {
        super(message, {
            code: options?.code ?? -32000,
            context: options?.context,
            docsPath: options?.docsPath || "/crypto/kzg/load-trusted-setup#error-handling",
            cause: options?.cause,
        });
...
export class KzgVerificationError extends KzgError {
    _tag = "KzgVerificationError";
    constructor(message, options) {
        super(message, {
            code: options?.code ?? -32000,
            context: options?.context,
            docsPath: options?.docsPath || "/crypto/kzg/verify-blob-kzg-proof#error-handling",
            cause: options?.cause,
        });
