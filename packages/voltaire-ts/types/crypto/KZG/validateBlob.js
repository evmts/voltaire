...
export function validateBlob(blob) {
    ...
    if (blob.length !== BYTES_PER_BLOB) {
        throw new KzgInvalidBlobError(`Blob must be ${BYTES_PER_BLOB} bytes, got ${blob.length}`, {
            code: -32602,
    ...
    for (let i = 0; i < FIELD_ELEMENTS_PER_BLOB; i++) {
        ...
        if (blob[offset] !== 0) {
            throw new KzgInvalidBlobError(`Invalid field element at index ${i}: top byte must be 0`, {
                code: -32602,
