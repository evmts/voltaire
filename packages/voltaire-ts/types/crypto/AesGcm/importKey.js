...
export async function importKey(keyMaterial) {
    if (keyMaterial.length !== AES128_KEY_SIZE &&
        keyMaterial.length !== AES256_KEY_SIZE) {
        throw new InvalidKeyError(`Key must be ${AES128_KEY_SIZE} or ${AES256_KEY_SIZE} bytes, got ${keyMaterial.length}`, {
            code: -32602,
            context: {
    ...
    try {
        ...
        /** @type {*} */ (keyMaterial), { name: "AES-GCM" }, true, ["encrypt", "decrypt"]);
    }
    catch (error) {
