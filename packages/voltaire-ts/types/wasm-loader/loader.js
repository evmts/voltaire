...
const nodeCryptoModule = null;
let wasmInstance = null;
let wasmMemory = null;
let wasmExports = null;
let memoryOffset = 0; // Simple bump allocator
let loadedWasmPath = null; // Track loaded WASM file
/**
 ...
 * ```
 */
export async function loadWasm(wasmPath, forceReload = false) {
    const pathStr = wasmPath instanceof URL
        ? wasmPath.href
    ...
    const wasi = {
        ...
        fd_seek: () => 0,
        fd_close: () => 0,
        fd_read: (_fd, _iovs, _iovs_len, nread) => {
            if (!wasmMemory)
                return -1;
            const mem = new DataView(wasmMemory.buffer);
        ...
        fd_pread: (_fd, _iovs, _iovs_len, _offset, nread) => {
            if (!wasmMemory)
                return -1;
            const mem = new DataView(wasmMemory.buffer);
            mem.setUint32(nread, 0, true);
        ...
        fd_pwrite: (_fd, _iovs, _iovs_len, _offset, nwritten) => {
            if (!wasmMemory)
                return -1;
            const mem = new DataView(wasmMemory.buffer);
    ...
    const importObject = {
        env: {
            ...
            bip39_mnemonic_from_bytes: () => 0,
            bip39_mnemonic_to_seed: () => 0,
            bip39_mnemonic_to_seed512: () => 0,
            bip39_mnemonic_validate: () => 0,
            bip39_validate_mnemonic: () => 0,
            bip32_key_from_seed_alloc: () => 0,
            bip32_key_from_parent_path_alloc: () => 0,
            bip32_key_free: () => { },
            wally_free_string: () => { },
            // HD Wallet stubs (not implemented in WASM)
            hdwallet_derive_child: () => 0,
            hdwallet_derive_path: () => 0,
            // KZG (c-kzg-4844) stubs for WASM builds that expect env imports
            load_trusted_setup: () => -1,
            load_trusted_setup_file: () => -1,
            free_trusted_setup: () => { },
            blob_to_kzg_commitment: () => -1,
            compute_kzg_proof: () => -1,
            compute_blob_kzg_proof: () => -1,
            verify_kzg_proof: () => -1,
            verify_blob_kzg_proof: () => -1,
...
function malloc(size) {
    ...
    if (!wasmMemory || !wasmMemory.buffer) {
        throw new Error("WASM memory not initialized. Ensure loadWasm() is called before using primitives.");
    }
    // Align to 8 bytes
    ...
    if (pagesNeeded > currentPages) {
        ...
        try {
            ...
            throw new Error(`Out of memory: failed to grow WASM memory from ${currentPages} to ${pagesNeeded} pages (tried to grow by ${pagesToGrow} pages). Current offset: ${memoryOffset}, requested size: ${size}`);
        }
    }
    return ptr;
}
 ...
 */
function readFixedString(ptr, length) {
    if (!wasmMemory) {
        throw new Error("WASM memory not initialized");
    }
    // CRITICAL: Get fresh buffer reference - wasmMemory.buffer may change after grow()
...
export function calculateCreateAddress(sender, nonce) {
    ...
    try {
        ...
        const result = exports.primitives_calculate_create_address(senderPtr, BigInt(nonce), outPtr);
        checkResult(result);
        return readBytes(outPtr, 20);
    }
    finally {
...
export function calculateCreate2Address(sender, salt, initCode) {
    ...
    try {
        ...
        const result = exports.primitives_calculate_create2_address(senderPtr, saltPtr, codePtr, initCode.length, outPtr);
        checkResult(result);
        return readBytes(outPtr, 20);
    }
    finally {
...
export function eip191HashMessage(message) {
    ...
    try {
        ...
        const result = exports.primitives_eip191_hash_message(msgPtr, message.length, outPtr);
        checkResult(result);
        return readBytes(outPtr, 32);
    }
    finally {
 ...
 */
export function blake2Hash(data, outputLength) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const dataPtr = malloc(data.length);
...
export function solidityKeccak256(packedData) {
    ...
    try {
        ...
        const result = exports.primitives_solidity_keccak256(dataPtr, packedData.length, outPtr);
        checkResult(result);
        return readBytes(outPtr, 32);
    }
    finally {
...
export function soliditySha256(packedData) {
    ...
    try {
        ...
        const outPtr = malloc(32);
        writeBytes(packedData, dataPtr);
        const result = exports.primitives_solidity_sha256(dataPtr, packedData.length, outPtr);
        checkResult(result);
        return readBytes(outPtr, 32);
    }
    finally {
...
export function rlpEncodeBytes(data) {
    ...
    try {
        ...
        writeBytes(data, dataPtr);
        const resultLen = exports.primitives_rlp_encode_bytes(dataPtr, data.length, outPtr, data.length + 10);
        if (resultLen < 0) {
...
export function rlpToHex(rlpData) {
    ...
    try {
        ...
        const resultLen = exports.primitives_rlp_to_hex(dataPtr, rlpData.length, outPtr, rlpData.length * 2 + 2);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
...
export function bytecodeAnalyzeJumpdests(code) {
    ...
    try {
        ...
        const count = exports.primitives_bytecode_analyze_jumpdests(codePtr, code.length, outPtr, code.length);
        if (count < 0) {
            checkResult(count);
        }
        // Read u32 array
...
export function bytecodeIsBoundary(code, position) {
    ...
    try {
        ...
        const codePtr = malloc(code.length);
        writeBytes(code, codePtr);
        return (exports.primitives_bytecode_is_boundary(codePtr, code.length, position) !== 0);
    }
    finally {
        memoryOffset = savedOffset;
    }
...
export function bytecodeIsValidJumpdest(code, position) {
    ...
    try {
        ...
        const codePtr = malloc(code.length);
        writeBytes(code, codePtr);
        return (exports.primitives_bytecode_is_valid_jumpdest(codePtr, code.length, position) !== 0);
    }
    finally {
        memoryOffset = savedOffset;
    }
 ...
 */
export function bytecodeGetNextPc(code, currentPc) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const codePtr = malloc(code.length);
        writeBytes(code, codePtr);
        const nextPc = exports.primitives_bytecode_get_next_pc(codePtr, code.length, currentPc);
 ...
 */
export function bytecodeScan(code, startPc, endPc) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const codePtr = malloc(code.length);
        ...
        const count = exports.primitives_bytecode_scan(codePtr, code.length, startPc, endPc, outPtr, outLenPtr);
        if (count < 0) {
            checkResult(count);
        }
 ...
 */
export function bytecodeDetectFusions(code) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const codePtr = malloc(code.length);
        ...
        const lenView = new DataView(wasmMemory.buffer);
        lenView.setBigUint64(outLenPtr, BigInt(code.length * 8), true);
        const count = exports.primitives_bytecode_detect_fusions(codePtr, code.length, outPtr, outLenPtr);
        if (count < 0) {
            checkResult(count);
        }
...
export function hexToBytes(hex) {
    ...
    try {
        ...
        const resultLen = exports.primitives_hex_to_bytes(hexPtr, outPtr, hex.length);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
        return readBytes(outPtr, resultLen);
...
export function bytesToHex(data) {
    ...
    try {
        ...
        writeBytes(data, dataPtr);
        const resultLen = exports.primitives_bytes_to_hex(dataPtr, data.length, outPtr, data.length * 2 + 3);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
        return readFixedString(outPtr, resultLen);
...
export function compressPublicKey(uncompressed) {
    ...
    try {
        ...
        const result = exports.primitives_compress_public_key(uncompressedPtr, outPtr);
        checkResult(result);
 ...
 * ```
 */
export function secp256k1Sign(messageHash, privateKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const hashPtr = malloc(32);
 ...
 */
export function secp256k1Verify(messageHash, r, s, publicKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const hashPtr = malloc(32);
...
export function secp256k1RecoverPubkey(messageHash, r, s, v) {
    ...
    try {
        ...
        writeBytes(r, rPtr);
        writeBytes(s, sPtr);
        const result = exports.primitives_secp256k1_recover_pubkey(hashPtr, rPtr, sPtr, v, outPtr);
        checkResult(result);
        return readBytes(outPtr, 64);
    }
    finally {
...
export function secp256k1RecoverAddress(messageHash, r, s, v) {
    ...
    try {
        ...
        writeBytes(r, rPtr);
        writeBytes(s, sPtr);
        const result = exports.primitives_secp256k1_recover_address(hashPtr, rPtr, sPtr, v, outPtr);
        checkResult(result);
        return readBytes(outPtr, 20);
    }
    finally {
...
export function secp256k1PubkeyFromPrivate(privateKey) {
    ...
    try {
        ...
        const outPtr = malloc(64);
        writeBytes(privateKey, keyPtr);
        const result = exports.primitives_secp256k1_pubkey_from_private(keyPtr, outPtr);
        checkResult(result);
        return readBytes(outPtr, 64);
    }
    finally {
...
export function signatureIsCanonical(r, s) {
    ...
    if (rBig === 0n || rBig >= SECP256K1_N)
        return false;
    if (sBig === 0n || sBig > HALF_N)
        return false;
    return true;
}
/**
...
export function signatureParse(sigData) {
    ...
    try {
        ...
        const result = exports.primitives_signature_parse(sigPtr, sigData.length, rPtr, sPtr, vPtr);
        checkResult(result);
...
export function signatureSerialize(r, s, v, includeV) {
    ...
    try {
        ...
        writeBytes(r, rPtr);
        writeBytes(s, sPtr);
        const resultLen = exports.primitives_signature_serialize(rPtr, sPtr, v, includeV ? 1 : 0, outPtr);
        return readBytes(outPtr, resultLen);
    }
    finally {
        memoryOffset = savedOffset;
 ...
 * ```
 */
export function abiComputeSelector(signature) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 */
export function abiEncodeParameters(types, values) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const resultLen = exports.primitives_abi_encode_parameters(typesPtr, valuesPtr, outPtr, estimatedSize);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
        return readBytes(outPtr, resultLen);
 ...
 */
export function abiDecodeParameters(data, types) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const outPtr = malloc(estimatedSize);
        const resultLen = exports.primitives_abi_decode_parameters(dataPtr, data.length, typesPtr, outPtr, estimatedSize);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
 ...
 */
export function abiEncodeFunctionData(signature, types, values) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const sigPtr = writeString(signature);
        ...
        const outPtr = malloc(estimatedSize);
        const resultLen = exports.primitives_abi_encode_function_data(sigPtr, typesPtr, valuesPtr, outPtr, estimatedSize);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
        return readBytes(outPtr, resultLen);
 ...
 */
export function abiDecodeFunctionData(data, types) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const dataPtr = malloc(data.length);
        ...
        const estimatedSize = types.length * 128 + 1024;
        const outPtr = malloc(estimatedSize);
        const resultLen = exports.primitives_abi_decode_function_data(dataPtr, data.length, typesPtr, selectorPtr, outPtr, estimatedSize);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
...
export function abiEncodePacked(types, values) {
    ...
    try {
        ...
        const outPtr = malloc(estimatedSize);
        const resultLen = exports.primitives_abi_encode_packed(typesPtr, valuesPtr, outPtr, estimatedSize);
        if (resultLen < 0) {
            checkResult(resultLen);
        }
        return readBytes(outPtr, resultLen);
 ...
 */
export function abiEstimateGas(data) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const dataPtr = malloc(data.length);
 ...
 * @returns Gas cost
 */
export function accessListGasCost(accessList) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const jsonStr = JSON.stringify(accessList.map((item) => ({
 ...
 * @returns Gas savings
 */
export function accessListGasSavings(accessList) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const jsonStr = JSON.stringify(accessList.map((item) => ({
 ...
 */
export function accessListIncludesAddress(accessList, address) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const jsonStr = JSON.stringify(accessList.map((item) => ({
        ...
        const addrPtr = malloc(20);
        writeBytes(address, addrPtr);
        const result = exports.primitives_access_list_includes_address(jsonPtr, addrPtr);
        return result !== 0;
    }
    finally {
        memoryOffset = savedOffset;
 ...
 */
export function accessListIncludesStorageKey(accessList, address, storageKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const jsonStr = JSON.stringify(accessList.map((item) => ({
        ...
        const result = exports.primitives_access_list_includes_storage_key(jsonPtr, addrPtr, keyPtr);
        return result !== 0;
    }
    finally {
        memoryOffset = savedOffset;
 ...
 */
export function authorizationSigningHash(chainId, address, nonce) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const addrPtr = malloc(20);
        const outPtr = malloc(32);
        writeBytes(address, addrPtr);
        const result = exports.primitives_authorization_signing_hash(chainId, addrPtr, nonce, outPtr);
        checkResult(result);
        return readBytes(outPtr, 32);
    }
    finally {
 ...
 * @returns Gas cost
 */
export function authorizationGasCost(authCount, emptyAccounts) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 */
export function blobFromData(data) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const dataPtr = malloc(data.length);
        ...
        writeBytes(data, dataPtr);
        const result = exports.primitives_blob_from_data(dataPtr, data.length, blobPtr);
        checkResult(result);
 ...
 */
export function blobIsValid(blobLen) {
    const exports = getExports();
 ...
 */
export function blobCalculateGas(blobCount) {
    const exports = getExports();
 ...
 */
export function blobEstimateCount(dataSize) {
    const exports = getExports();
 ...
 */
export function blobCalculateGasPrice(excessBlobGas) {
    const exports = getExports();
 ...
 */
export function blobCalculateExcessGas(parentExcess, parentUsed) {
    const exports = getExports();
 ...
 */
export function eventLogMatchesAddress(logAddress, filterAddresses) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        // Write log address
        ...
        const result = exports.primitives_eventlog_matches_address(logAddrPtr, filterPtr, filterAddresses.length);
        return result !== 0;
    }
    finally {
        memoryOffset = savedOffset;
 ...
 * @returns true if matches
 */
export function eventLogMatchesTopic(logTopic, filterTopic) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const result = exports.primitives_eventlog_matches_topic(logTopicPtr, filterTopicPtr, nullTopic);
        return result !== 0;
    }
    finally {
        memoryOffset = savedOffset;
 ...
 * @returns true if matches
 */
export function eventLogMatchesTopics(logTopics, filterTopics) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const result = exports.primitives_eventlog_matches_topics(logTopicsPtr, logTopics.length, filterTopicsPtr, filterNullsPtr, filterTopics.length);
        return result !== 0;
    }
    finally {
        memoryOffset = savedOffset;
 ...
 */
export function x25519DerivePublicKey(secretKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 */
export function x25519Scalarmult(secretKey, publicKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const secretPtr = malloc(32);
 ...
 */
export function x25519KeypairFromSeed(seed) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 * @returns 32-byte public key
 */
export function ed25519DerivePublicKey(secretKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const secretPtr = malloc(64);
 ...
 */
export function p256Sign(messageHash, privateKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const hashPtr = malloc(32);
 ...
 * @returns True if signature is valid
 */
export function p256Verify(messageHash, signature, publicKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const hashPtr = malloc(32);
 ...
 * @returns Uncompressed public key (64 bytes)
 */
export function p256DerivePublicKey(privateKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        const keyPtr = malloc(32);
 ...
 */
export function p256Ecdh(privateKey, publicKey) {
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 */
export function kzgIsInitialized() {
 ...
 */
export function kzgLoadTrustedSetup() {
 ...
 */
export function kzgFreeTrustedSetup() {
 ...
 */
export function kzgBlobToCommitment(blob) {
    if (!kzgInitialized) {
        throw new Error("KZG trusted setup not initialized. Call kzgLoadTrustedSetup() first.");
    }
    if (blob.length !== KZG_BLOB_SIZE) {
        throw new Error(`Invalid blob length: expected ${KZG_BLOB_SIZE}, got ${blob.length}`);
    }
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 */
export function kzgComputeProof(blob, z) {
    if (!kzgInitialized) {
        throw new Error("KZG trusted setup not initialized. Call kzgLoadTrustedSetup() first.");
    }
    if (blob.length !== KZG_BLOB_SIZE) {
    ...
    if (z.length !== KZG_FIELD_ELEMENT_SIZE) {
        throw new Error(`Invalid z length: expected ${KZG_FIELD_ELEMENT_SIZE}, got ${z.length}`);
    }
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
 ...
 * @throws {Error} If trusted setup not loaded or inputs are invalid
 */
export function kzgComputeBlobProof(blob, commitment) {
    if (!kzgInitialized) {
        throw new Error("KZG trusted setup not initialized. Call kzgLoadTrustedSetup() first.");
    }
    if (blob.length !== KZG_BLOB_SIZE) {
    ...
    if (commitment.length !== KZG_COMMITMENT_SIZE) {
        throw new Error(`Invalid commitment length: expected ${KZG_COMMITMENT_SIZE}, got ${commitment.length}`);
    }
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const result = exports.kzg_compute_blob_proof(blobPtr, commitmentPtr, proofPtr);
        if (result !== 0) {
 ...
 * @throws {Error} If trusted setup not loaded or inputs are invalid
 */
export function kzgVerifyProof(commitment, z, y, proof) {
    if (!kzgInitialized) {
        throw new Error("KZG trusted setup not initialized. Call kzgLoadTrustedSetup() first.");
    }
    if (commitment.length !== KZG_COMMITMENT_SIZE) {
    ...
    if (y.length !== KZG_FIELD_ELEMENT_SIZE) {
        throw new Error(`Invalid y length: expected ${KZG_FIELD_ELEMENT_SIZE}, got ${y.length}`);
    }
    if (proof.length !== KZG_PROOF_SIZE) {
        throw new Error(`Invalid proof length: expected ${KZG_PROOF_SIZE}, got ${proof.length}`);
    }
    const savedOffset = memoryOffset;
    try {
        const exports = getExports();
        ...
        const result = exports.kzg_verify_proof(commitmentPtr, zPtr, yPtr, proofPtr);
 ...
 * @throws {Error} If trusted setup not loaded or inputs are invalid
 */
export function kzgVerifyBlobProof(blob, commitment, proof) {
    if (!kzgInitialized) {
        throw new Error("KZG trusted setup not initialized. Call kzgLoadTrustedSetup() first.");
    }
    if (blob.length !== KZG_BLOB_SIZE) {
    ...
    try {
        ...
        writeBytes(commitment, commitmentPtr);
        writeBytes(proof, proofPtr);
        const result = exports.kzg_verify_blob_proof(blobPtr, commitmentPtr, proofPtr);
        // Returns 1 for valid, 0 for invalid, negative for error
        if (result < 0) {
