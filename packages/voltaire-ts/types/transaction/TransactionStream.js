...
export function TransactionStream(options) {
    ...
    async function* watchPending(opts = {}) {
        ...
        while (!signal?.aborted) {
            try {
                ...
                const pendingTxs = /** @type {any[]} */ (await provider
    ...
    async function* watchConfirmed(opts = {}) {
        const { filter, pollingInterval = DEFAULT_POLLING_INTERVAL, fromBlock, confirmations = 1, signal, } = opts;
                ...
                while (lastBlock <= confirmedBlock && !signal?.aborted) {
                    ...
                    for (const tx of transactions) {
                        ...
                        const confirmed = {
                            ...parsed,
                            blockHash: 
                            /** @type {import('../primitives/BlockHash/BlockHashType.js').BlockHashType} */ (hexToBytes(blockData.hash)),
                            ...
                            receipt: /** @type {any} */ ({
                                ...
                                transactionIndex: Number.parseInt(receiptData.transactionIndex, 16),
    ...
    async function* track(txHash, opts = {}) {
        const { pollingInterval = DEFAULT_POLLING_INTERVAL, timeout = DEFAULT_TIMEOUT, confirmations = 1, signal, } = opts;
                        ...
                        if (confirms >= BigInt(confirmations)) {
                            ...
                            if (receipt) {
                                ...
                                const confirmed = {
                                    ...parsed,
                                    blockHash: 
                                    /** @type {import('../primitives/BlockHash/BlockHashType.js').BlockHashType} */ (hexToBytes(txData.blockHash)),
                                    blockNumber,
                                    transactionIndex: Number.parseInt(txData.transactionIndex, 16),
                                    receipt: /** @type {any} */ ({
                                        transactionHash: hexToBytes(receiptData.transactionHash),
                                        transactionIndex: Number.parseInt(receiptData.transactionIndex, 16),
                                        blockHash: hexToBytes(receiptData.blockHash),
