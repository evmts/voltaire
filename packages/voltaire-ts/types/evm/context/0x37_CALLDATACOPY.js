 ...
 */
export function calldatacopy(frame) {
    const destOffsetResult = popStack(frame);
    if (destOffsetResult.error)
        return destOffsetResult.error;
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (srcAddResult.error)
            return srcAddResult.error;
        const srcIdx = srcAddResult.value;
        const byte = srcIdx < frame.calldata.length
            ? /** @type {number} */ (frame.calldata[srcIdx])
            : 0;
        const destAddResult = addU32(destOff, i);
        if (destAddResult.error)
            return destAddResult.error;
