import { LogBase, LogData, LogTopic, } from "../../primitives/GasConstants/constants.js";
...
export function handler_0xa3_LOG3(frame) {
    ...
    if (topic0Result.error)
        return topic0Result.error;
    const topic0 = topic0Result.value;
    const topic1Result = popStack(frame);
    if (topic1Result.error)
        return topic1Result.error;
    const topic1 = topic1Result.value;
    const topic2Result = popStack(frame);
    if (topic2Result.error)
        return topic2Result.error;
    ...
    // Calculate gas cost: 375 + 3*375 + 8*dataLength + memory expansion
    const logCost = LogBase + 3n * LogTopic + LogData * BigInt(len);
    ...
    // Create log entry with 3 topics
    const log = {
