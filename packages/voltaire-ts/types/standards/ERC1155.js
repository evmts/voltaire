 ...
 */
export function encodeBalanceOf(account, id) {
 ...
 */
export function encodeSafeTransferFrom(from, to, id, amount, data = new Uint8Array(0)) {
 ...
 */
export function encodeIsApprovedForAll(account, operator) {
    // Convert addresses (20 bytes each) to hex and pad to 32 bytes
    const accountHex = Array.from(account, (b) => b.toString(16).padStart(2, "0"))
        .join("")
        .padStart(64, "0");
