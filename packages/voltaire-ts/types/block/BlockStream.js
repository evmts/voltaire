...
import { BlockStreamAbortedError, UnrecoverableReorgError, } from "../stream/errors.js";
...
export function BlockStream(options) {
    ...
    function fetchBlock(blockNumber, include, retryOptions, signal) {
        return fetchBlockImpl(provider, blockNumber, include, fetchBlockReceipts, retryOptions, signal);
    }
    /**
    ...
    function fetchBlockByHash(blockHash, include, retryOptions, signal) {
        return fetchBlockByHashImpl(provider, blockHash, include, fetchBlockReceipts, retryOptions, signal);
    }
    /**
     ...
     * @returns {Promise<any[]>}
     */
    async function fetchBlocksInRange(fromBlock, toBlock, include, retryOptions, signal) {
        const blocks = [];
    ...
    async function* backfill(backfillOptions) {
        ...
        while (currentFrom <= toBlock) {
            ...
            try {
                const blocks = await fetchBlocksInRange(currentFrom, currentTo, include, retry, signal);
    ...
    async function* watch(watchOptions) {
        ...
        const pollingInterval = watchOptions?.pollingInterval ?? DEFAULT_POLLING_INTERVAL;
        const maxQueuedBlocks = watchOptions?.maxQueuedBlocks ?? DEFAULT_MAX_QUEUED_BLOCKS;
        ...
        async function findCommonAncestor(newBlock) {
            ...
            while (true) {
                ...
                const matchIdx = unfinalizedBlocks.findIndex((b) => Hex.fromBytes(b.hash) ===
                    Hex.fromBytes(currentNewBlock.header?.parentHash ?? currentNewBlock.parentHash));
                if (matchIdx !== -1) {
                    // Found common ancestor
                    const commonAncestor = /** @type {LightBlock} */ (unfinalizedBlocks[matchIdx]);
                ...
                if (unfinalizedBlocks.length === 0) {
                    throw new UnrecoverableReorgError(BigInt(newChainBlocks.length), BigInt(removed.length + unfinalizedBlocks.length));
                }
                ...
                const parentHashHex = typeof parentHash === "string"
                    ? parentHash
                ...
                currentNewBlock = await fetchBlockByHash(parentHashHex, include, retry, signal);
                newChainBlocks.push(currentNewBlock);
                // Safety check - don't go too far back
                if (newChainBlocks.length > maxQueuedBlocks) {
                    throw new UnrecoverableReorgError(BigInt(newChainBlocks.length), BigInt(maxQueuedBlocks));
                }
            }
        }
        /**
        ...
        async function* reconcileBlock(block) {
            ...
            const blockNumber = typeof block.header?.number === "bigint"
                ? block.header.number
            ...
            if (blockNumber <= latestBlock.number) {
                const { commonAncestor, removed, newChainBlocks } = await findCommonAncestor(block);
            ...
            if (blockNumber > latestBlock.number + 1n) {
                const missingBlocks = await fetchBlocksInRange(latestBlock.number + 1n, blockNumber - 1n, include, retry, signal);
            ...
            const parentHashHex = typeof blockParentHash === "string"
                ? blockParentHash
        ...
        while (true) {
            ...
            try {
                ...
                if (currentBlockNumber > latestTracked.number) {
                    ...
                    const latestBlock = await fetchBlock(currentBlockNumber, include, retry, signal);
