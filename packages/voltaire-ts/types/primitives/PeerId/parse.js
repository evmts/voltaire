...
export function parse() {
    ...
    if (!this.startsWith("enode://")) {
        throw new InvalidFormatError("Peer ID must start with 'enode://' to be parsed as enode URL", {
            value: this,
    ...
    if (!/^[0-9a-fA-F]{128}$/.test(publicKey)) {
        throw new InvalidFormatError("Enode public key must be 128 hex characters", {
            value: publicKey,
    ...
    const ipPortPart = queryIndex === -1 ? addressPart : addressPart.slice(0, queryIndex);
    ...
    if (queryPart) {
        ...
        if (discportStr) {
            ...
            if (Number.isNaN(discoveryPort) ||
                discoveryPort < 0 ||
                discoveryPort > 65535) {
                throw new InvalidFormatError("Invalid discovery port number in enode URL", {
                    value: discoveryPort,
                    expected: "Port between 0 and 65535",
                    code: -32602,
                    docsPath: "/primitives/peer-id/parse#error-handling",
