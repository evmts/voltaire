...
export function VerifyBatch({ verifyBlobKzgProofBatch }) {
    ...
    return function verifyBatch(blobs, commitments, proofs) {
        ...
        if (blobs.length > MAX_PER_TRANSACTION) {
            throw new InvalidLengthError(`Too many blobs: ${blobs.length} (max ${MAX_PER_TRANSACTION})`, {
                value: blobs.length,
        ...
        for (let i = 0; i < commitments.length; i++) {
            ...
            if (commitment.length !== 48) {
                throw new InvalidLengthError(`Invalid commitment size at index ${i}: ${commitment.length}`, {
