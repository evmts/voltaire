...
function insertNode(node, stem, idx, v, depth) {
    ...
    if (depth > 300) {
        throw new InvalidTreeStateError("Maximum tree depth exceeded - possible infinite recursion", {
            value: depth,
            expected: "<= 300",
    ...
    switch (node.type) {
        case "empty": {
            ...
            values[idx] = v;
            return /** @type {import('./BinaryTreeType.js').StemNode} */ ({
                type: "stem",
                stem,
                values,
            });
        ...
        case "internal": {
            ...
            throw new InvalidTreeStateError("Cannot insert into tree with internal node at this depth - tree structure limitation", {
                value: "internal",
...
function splitStems(existing, newStem, newIdx, newVal, depth) {
    ...
    if (existingBit === newBit) {
        ...
        const childHash = hashNode(
        /** @type {import('./BinaryTreeType.js').Node} */ (child));
        if (existingBit === 0) {
            return /** @type {import('./BinaryTreeType.js').InternalNode} */ ({
                type: "internal",
                left: childHash,
