...
export function fromHex(hex, m, k) {
    ...
    if (cleanHex.length !== expectedHexLen) {
        throw new InvalidBloomFilterLengthError(`Expected ${expectedHexLen} hex chars, got ${cleanHex.length}`, {
    ...
    return /** @type {import('./BloomFilterType.js').BloomFilterType} */ (bytes);
}
