    ...
    switch (type) {
        case "uint8": {
            ...
            if (n < 0 || n > UINT8_MAX || !Number.isInteger(n)) {
                throw new RangeError(`Value ${value} out of range for uint8 (0-${UINT8_MAX})`);
            }
        ...
        case "uint16": {
            ...
            if (n < 0 || n > UINT16_MAX || !Number.isInteger(n)) {
                throw new RangeError(`Value ${value} out of range for uint16 (0-${UINT16_MAX})`);
            }
        ...
        case "uint32": {
            ...
            if (n < 0 || n > UINT32_MAX || !Number.isInteger(n)) {
                throw new RangeError(`Value ${value} out of range for uint32 (0-${UINT32_MAX})`);
            }
...
export function decodeBasic(bytes, type) {
    switch (type) {
        case "uint8": {
            if (bytes.length !== 1)
                throw new Error("Invalid length for uint8");
            return /** @type {number} */ (bytes[0]);
        }
        case "uint16": {
            if (bytes.length !== 2)
                throw new Error("Invalid length for uint16");
            const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
            return view.getUint16(0, true); // little-endian
        }
        case "uint32": {
            if (bytes.length !== 4)
                throw new Error("Invalid length for uint32");
            const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
            return view.getUint32(0, true); // little-endian
        }
        case "uint64": {
            if (bytes.length !== 8)
                throw new Error("Invalid length for uint64");
            const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
            return view.getBigUint64(0, true); // little-endian
        }
        case "uint256": {
            if (bytes.length !== 32)
                throw new Error("Invalid length for uint256");
            let result = 0n;
            for (let i = 31; i >= 0; i--) {
        ...
        case "bool": {
            if (bytes.length !== 1)
                throw new Error("Invalid length for bool");
            const val = bytes[0];
            if (val === 0)
                return false;
            if (val === 1)
