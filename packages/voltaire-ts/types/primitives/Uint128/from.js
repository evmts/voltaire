...
export function from(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isInteger(value)) {
            throw new Uint128NotIntegerError(`Uint128 value must be an integer: ${value}`, { value });
        }
        bigintValue = BigInt(value);
    }
    else {
    ...
    if (bigintValue < 0n) {
        throw new Uint128NegativeError(`Uint128 value cannot be negative: ${bigintValue}`, { value: bigintValue });
    }
    if (bigintValue > MAX) {
        throw new Uint128OverflowError(`Uint128 value exceeds maximum: ${bigintValue}`, { value: bigintValue });
    }
