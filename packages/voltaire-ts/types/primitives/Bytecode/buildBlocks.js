...
export function buildBlocks(instructions, boundaries, _jumpDests) {
    ...
    for (let i = 0; i < instructions.length; i++) {
        ...
        if (boundaries.has(inst.position) && currentBlockInsts.length > 0) {
            ...
            const block = finalizeBlock(blockIndex++, currentBlockStart, currentBlockInsts, instructions, i - 1);
            blocks.push(block);
            // Start new block
        ...
        if (isTerminatorOpcode(inst.opcode)) {
            const block = finalizeBlock(blockIndex++, currentBlockStart, currentBlockInsts, instructions, i);
            blocks.push(block);
            currentBlockStart = -1;
            currentBlockInsts = [];
        }
    ...
    if (currentBlockInsts.length > 0) {
        const block = finalizeBlock(blockIndex++, currentBlockStart, currentBlockInsts, instructions, instructions.length - 1);
        blocks.push(block);
    }
    return blocks;
}
...
function finalizeBlock(index, startPc, blockInsts, allInsts, lastInstIndex) {
    ...
    const { gasEstimate, minStack, maxStack, stackEffect } = analyzeBlockMetrics(blockInsts);
    // Determine terminator
    const { terminator, target } = analyzeTerminator(blockInsts, allInsts, lastInstIndex);
    return {
        index,
        startPc,
        endPc,
