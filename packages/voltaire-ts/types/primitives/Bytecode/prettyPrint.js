...
export function prettyPrint(bytecode, options = {}) {
    const { colors = true, showGas = true, showStack = true, showBlocks = false, showJumpArrows = false, showFusions = false, lineNumbers = true, showSummary = true, maxWidth = 80, compact = false, } = options;
    void showJumpArrows;
    void showFusions;
    ...
    for (const inst of scan(bytecode, {
        ...
        if (showStack && inst.stackEffect) {
            const effect = formatStackEffect(inst.stackEffect.pop, inst.stackEffect.push);
            parts.push(effect);
        }
