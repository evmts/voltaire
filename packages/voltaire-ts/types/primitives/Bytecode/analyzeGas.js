...
export function analyzeGas(bytecode, options = {}) {
    const { analyzePaths = false, maxPaths = 100, includeDynamic = false, context, } = options;
    void includeDynamic;
    void context;
    // Parse instructions
    ...
    /** @type {import('./BytecodeType.js').GasAnalysis} */
    const result = {
        total: totalGas,
        byInstruction,
        byBlock,
 ...
 *
 * @param {import('./BytecodeType.js').Instruction[]} instructions
 * @param {import('./BytecodeType.js').InstructionGas[]} byInstruction
 * @param {bigint} totalGas
 * @returns {import('./BytecodeType.js').BlockGas[]}
 */
...
function analyzeExecutionPaths(instructions, byInstruction, _maxPaths) {
    ...
    const totalGas = byInstruction.length > 0
