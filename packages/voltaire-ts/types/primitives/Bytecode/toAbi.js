...
export function toAbi(bytecode) {
    ...
    return /** @type {import('./BytecodeType.js').BrandedAbi} */ (Object.freeze(abi));
}
/**
...
function analyzeFunctionEntry(instructions, selectorIndex) {
    ...
    for (let i = selectorIndex; i < Math.min(selectorIndex + 100, instructions.length); i++) {
        const inst = instructions[i];
        if (!inst)
            continue;
