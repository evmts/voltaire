...
export function unpack(packedUserOp) {
    ...
    for (let i = 0; i < 16; i++) {
        verificationGasLimit =
            (verificationGasLimit << 8n) |
                BigInt(/** @type {number} */ (accountGasLimits[i]));
    }
    let callGasLimit = 0n;
    for (let i = 16; i < 32; i++) {
        callGasLimit =
    ...
    for (let i = 0; i < 16; i++) {
        maxPriorityFeePerGas =
            (maxPriorityFeePerGas << 8n) | BigInt(/** @type {number} */ (gasFees[i]));
    }
    let maxFeePerGas = 0n;
    for (let i = 16; i < 32; i++) {
        maxFeePerGas =
            (maxFeePerGas << 8n) | BigInt(/** @type {number} */ (gasFees[i]));
    }
