 ...
 * ```
 */
export function computeStealthPrivateKey(spendingPrivateKey, hashedSharedSecret) {
    if (spendingPrivateKey.length !== PRIVATE_KEY_SIZE) {
        throw new StealthAddressError(`Spending private key must be ${PRIVATE_KEY_SIZE} bytes, got ${spendingPrivateKey.length}`, {
    ...
    if (hashedSharedSecret.length !== PRIVATE_KEY_SIZE) {
        throw new StealthAddressError(`Hashed shared secret must be ${PRIVATE_KEY_SIZE} bytes, got ${hashedSharedSecret.length}`, {
    ...
    try {
        ...
        throw new StealthAddressError(`Stealth private key computation failed: ${error}`, {
