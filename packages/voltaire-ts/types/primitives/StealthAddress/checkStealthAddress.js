 ...
 * ```
 */
export function checkStealthAddress(viewingPrivateKey, ephemeralPublicKey, viewTag, spendingPublicKey, stealthAddress) {
    if (viewingPrivateKey.length !== PRIVATE_KEY_SIZE) {
        throw new StealthAddressError(`Viewing private key must be ${PRIVATE_KEY_SIZE} bytes, got ${viewingPrivateKey.length}`, {
    ...
    if (ephemeralPublicKey.length !== COMPRESSED_PUBLIC_KEY_SIZE) {
        throw new StealthAddressError(`Ephemeral public key must be ${COMPRESSED_PUBLIC_KEY_SIZE} bytes, got ${ephemeralPublicKey.length}`, {
    ...
    if (spendingPublicKey.length !== COMPRESSED_PUBLIC_KEY_SIZE) {
        throw new StealthAddressError(`Spending public key must be ${COMPRESSED_PUBLIC_KEY_SIZE} bytes, got ${spendingPublicKey.length}`, {
    ...
    try {
        ...
        /** @type {*} */ (spendingPubKeyUncompressed), stealthPoint);
        ...
        const computedStealthAddress = BrandedAddress.fromPublicKey(xBigInt, yBigInt);
        // 9. Check if addresses match
        const addressesMatch = BrandedAddress.equals(computedStealthAddress, stealthAddress);
        if (!addressesMatch) {
            return { isForRecipient: false };
        }
