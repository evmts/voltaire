...
export function decompressPublicKey(compressed) {
    if (compressed.length !== COMPRESSED_PUBLIC_KEY_SIZE) {
        throw new InvalidPublicKeyError(`Compressed public key must be ${COMPRESSED_PUBLIC_KEY_SIZE} bytes, got ${compressed.length}`, {
    ...
    try {
        ...
        throw new InvalidPublicKeyError(`Public key decompression failed: ${error}`, {
