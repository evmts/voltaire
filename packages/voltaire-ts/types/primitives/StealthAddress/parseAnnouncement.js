...
export function parseAnnouncement(announcement) {
    ...
    if (announcement.length !== expectedLength) {
        throw new InvalidAnnouncementError(`Announcement must be ${expectedLength} bytes, got ${announcement.length}`, {
            code: "INVALID_ANNOUNCEMENT_LENGTH",
            context: { actualLength: announcement.length, expectedLength },
        });
    }
    const ephemeralPublicKey = 
    /** @type {import('./StealthAddressType.js').EphemeralPublicKey} */ (announcement.slice(0, COMPRESSED_PUBLIC_KEY_SIZE));
    const viewTag = /** @type {import('./StealthAddressType.js').ViewTag} */ (announcement[COMPRESSED_PUBLIC_KEY_SIZE]);
