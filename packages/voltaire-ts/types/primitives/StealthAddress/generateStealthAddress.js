...
export function generateStealthAddress(metaAddress, ephemeralPrivateKey) {
    if (ephemeralPrivateKey.length !== PRIVATE_KEY_SIZE) {
        throw new StealthAddressGenerationError(`Ephemeral private key must be ${PRIVATE_KEY_SIZE} bytes, got ${ephemeralPrivateKey.length}`, {
    ...
    try {
        ...
        /** @type {import('./StealthAddressType.js').EphemeralPublicKey} */ (compressPublicKey(ephemeralPublicKeyUncompressed));
        ...
        throw new StealthAddressGenerationError(`Stealth address generation failed: ${error}`, {
