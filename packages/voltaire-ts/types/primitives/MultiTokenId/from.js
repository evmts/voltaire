...
export function from(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isInteger(value)) {
            throw new InvalidMultiTokenIdError(`MultiTokenId value must be an integer: ${value}`, { value });
        }
        bigintValue = BigInt(value);
    }
    else {
    ...
    if (bigintValue < 0n) {
        throw new InvalidMultiTokenIdError(`MultiTokenId value cannot be negative: ${bigintValue}`, { value: bigintValue });
    }
    if (bigintValue > MAX) {
        throw new InvalidMultiTokenIdError(`MultiTokenId value exceeds maximum: ${bigintValue}`, { value: bigintValue, context: { max: MAX } });
    }
    return /** @type {import('./MultiTokenIdType.js').MultiTokenIdType} */ (bigintValue);
}
