...
export function encodeValue(type, value, types, crypto) {
    ...
    if (arrayMatch) {
        ...
        const encodedElements = value.map((elem) => encodeValue(baseType, elem, types, crypto));
...
function encodeAtomicValue(type, value) {
    ...
    if (uintMatch) {
        ...
        if (num < 0n || num > max) {
            throw new InvalidEIP712ValueError(`Value ${num} out of range for ${type} (0 to ${max})`, {
                value: num,
                expected: `0 to ${max}`,
                type,
            });
    ...
    if (intMatch) {
        ...
        if (num < min || num > max) {
            throw new InvalidEIP712ValueError(`Value ${num} out of range for ${type} (${min} to ${max})`, {
                value: num,
                expected: `${min} to ${max}`,
                type,
            });
