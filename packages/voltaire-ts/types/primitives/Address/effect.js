...
import { IsValid as IsValidChecksumFactory } from "./ChecksumAddress.js";
import * as BrandedAddressImpl from "./internal-index.js";
// Create isValidChecksum function for Schema validation (synchronous)
const isValidChecksum = IsValidChecksumFactory({ keccak256 });
import { InvalidAddressLengthError, InvalidChecksumError, InvalidHexFormatError, InvalidHexStringError, InvalidPrivateKeyError, InvalidValueError, } from "./effect-errors.js";
 ...
 */
export const AddressBrand = Brand.refined((bytes) => bytes instanceof Uint8Array && bytes.length === 20, (bytes) => Brand.error(`Expected 20-byte Uint8Array, got ${bytes instanceof Uint8Array ? `${bytes.length} bytes` : typeof bytes}`));
/**
 * Effect Brand constructor (nominal - validation happens separately via keccak)
 */
export const ChecksumAddressBrand = Brand.nominal();
/**
 * Schema for Address from various input types
 * Uses Effect Brand validation for type safety
 */
export class AddressSchema extends Schema.Class("Address")({
     ...
     */
    static fromPrivateKey(value) {
        return Effect.gen(function* () {
            // Validate private key length
            if (value.length !== 32) {
                return yield* Effect.fail(new InvalidAddressLengthError({
                    value,
                    actualLength: value.length,
                    expectedLength: 32,
                }));
            ...
            const publicKey = yield* secp.derivePublicKey(value).pipe(Effect.mapError((e) => {
     ...
     */
    equals(other) {
        const otherAddr = other instanceof AddressSchema
            ? other.address
            : other;
     ...
     */
    calculateCreateAddress(nonce) {
        const self = this;
        return Effect.gen(function* () {
            // Validate nonce
            if (nonce < 0n) {
                return yield* Effect.fail(new InvalidValueError({
                    value: nonce,
     ...
     * @returns Effect that uses Keccak256Service
     */
    calculateCreate2Address(salt, initCode) {
        const self = this;
        return Effect.gen(function* () {
            // Validate salt
            if (salt.length !== 32) {
...
export class ChecksumAddress extends Schema.Class("ChecksumAddress")({
    value: Schema.String.pipe(Schema.filter((str) => {
        ...
        return Effect.gen(function* () {
            ...
            if (typeof value === "string" && value.startsWith("0x")) {
                ...
                if (hasUpperCase && hasLowerCase && value !== checksummed) {
                    return yield* Effect.fail(new InvalidChecksumError({
                        address: value,
    ...
    static isValid(str) {
        ...
        return Effect.gen(function* () {
            ...
            for (let i = 0; i < hex.length; i++) {
                ...
                const hashNibble = Number.parseInt(hashHex[i], 16);
                if (hashNibble >= 8) {
                    if (actualChar !== char.toUpperCase())
                        return false;
                }
                else {
                    if (actualChar !== char.toLowerCase())
                        return false;
                }
            }
            return true;
 ...
 */
export const AddressFromHex = Schema.transform(Schema.String, Schema.instanceOf(AddressSchema), {
    decode: (_hex) => {
        // This is problematic because Schema transforms are synchronous
        // but our method returns Effect now
        // We'll need to use a different approach
        throw new Error("AddressFromHex schema cannot be used directly with Effect-based methods. " +
 ...
 * Note: These schemas need to be used within an Effect context with services provided
 */
export const AddressFromChecksummed = Schema.transform(Schema.String, Schema.instanceOf(ChecksumAddress), {
    decode: (_str) => {
        // This is problematic because Schema transforms are synchronous
        // but our method returns Effect now
        throw new Error("AddressFromChecksummed schema cannot be used directly with Effect-based methods. " +
 ...
 * Note: These schemas need to be used within an Effect context with services provided
 */
export const AddressFromUnknown = Schema.transform(Schema.Union(Schema.Number, Schema.BigIntFromSelf, Schema.String, Schema.Uint8ArrayFromSelf), Schema.instanceOf(AddressSchema), {
    decode: (_value) => {
        // This is problematic because Schema transforms are synchronous
        // but our method returns Effect now
        throw new Error("AddressFromUnknown schema cannot be used directly with Effect-based methods. " +
