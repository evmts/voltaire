 ...
 */
export const Keccak256ServiceLive = Layer.succeed(Keccak256Service, Keccak256Service.of({
    hash: (data) => Effect.try({
 ...
 */
export const Secp256k1ServiceLive = Layer.succeed(Secp256k1Service, Secp256k1Service.of({
    derivePublicKey: (privateKey) => Effect.try({
    ...
    getPublicKey: (privateKey) => Effect.try({
        try: () => {
 ...
 */
export const RlpEncoderServiceLive = Layer.succeed(RlpEncoderService, RlpEncoderService.of({
    encode: (data) => Effect.try({
        try: () => rlpEncodeNative(data),
        catch: (error) => new RlpEncodingError({
            data,
            message: error instanceof Error ? error.message : String(error),
            cause: error,
        }),
 ...
 */
export const AddressServicesLive = Layer.mergeAll(Keccak256ServiceLive, Secp256k1ServiceLive, RlpEncoderServiceLive);
/**
...
Layer.succeed(Keccak256Service, Keccak256Service.of({
    hash: (_data) => Effect.succeed(new Uint8Array(32).fill(0xaa)),
})), 
// Mock Secp256k1 that returns predictable keys
Layer.succeed(Secp256k1Service, Secp256k1Service.of({
    derivePublicKey: (_privateKey) => Effect.succeed(new Uint8Array(64).fill(0xbb)),
    getPublicKey: (_privateKey) => Effect.succeed({ x: 0xccccccccn, y: 0xdddddddddn }),
})), 
// Mock RLP encoder
Layer.succeed(RlpEncoderService, RlpEncoderService.of({
