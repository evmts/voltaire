...
export function from(value) {
    ...
    if (value.transactions.length === 0) {
        throw new InvalidBundleError("Bundle must contain at least one transaction", {
            value,
        });
    }
    ...
    const transactions = value.transactions.map((tx, idx) => {
        ...
        if (typeof tx === "string") {
            ...
            if (hex.length % 2 !== 0) {
                throw new InvalidBundleError(`Transaction ${idx} has invalid hex length`, { value: tx });
            }
        ...
        throw new InvalidBundleError(`Transaction ${idx} must be Uint8Array or hex string`, { value: tx });
    });
    const bundle = {
        transactions,
    };
    ...
    if (value.revertingTxHashes !== undefined) {
        if (!Array.isArray(value.revertingTxHashes)) {
            throw new InvalidBundleError("Bundle.revertingTxHashes must be an array", { value });
        }
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: complex logic required
        bundle.revertingTxHashes = value.revertingTxHashes.map((hash, idx) => {
            if (hash instanceof Uint8Array) {
                if (hash.length !== 32) {
                    throw new InvalidBundleError(`Reverting hash ${idx} must be 32 bytes`, { value: hash });
                }
                return hash;
            }
            if (typeof hash === "string") {
                ...
                if (hex.length !== 64) {
                    throw new InvalidBundleError(`Reverting hash ${idx} must be 32 bytes`, {
                        value: hash,
                    });
                }
            ...
            throw new InvalidBundleError(`Reverting hash ${idx} must be Uint8Array or hex string`, { value: hash });
        });
    }
    return bundle;
}
