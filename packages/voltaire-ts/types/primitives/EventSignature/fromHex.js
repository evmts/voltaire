...
export function fromHex(hex) {
    ...
    if (bytes.length !== 32) {
        throw new Error(`EventSignature hex must be exactly 32 bytes (64 chars), got ${bytes.length} bytes`);
    }
    return /** @type {import('./EventSignatureType.js').EventSignatureType} */ (bytes);
}
