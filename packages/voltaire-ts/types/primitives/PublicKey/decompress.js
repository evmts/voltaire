...
export function decompress(compressed) {
    ...
    if (compressed.length !== 33) {
        throw new Error(`Invalid compressed public key length: expected 33 bytes, got ${compressed.length}`);
    }
    // Validate prefix
    const prefix = /** @type {number} */ (compressed[0]);
    if (prefix !== 0x02 && prefix !== 0x03) {
        throw new Error(`Invalid compressed public key prefix: expected 0x02 or 0x03, got 0x${prefix.toString(16).padStart(2, "0")}`);
    }
