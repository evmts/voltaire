...
export function from(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isInteger(value)) {
            throw new Uint64NotIntegerError(`Uint64 value must be an integer: ${value}`, { value });
        }
        bigintValue = BigInt(value);
    }
    else {
    ...
    if (bigintValue < 0n) {
        throw new Uint64NegativeError(`Uint64 value cannot be negative: ${bigintValue}`, { value: bigintValue });
    }
    if (bigintValue > MAX) {
        throw new Uint64OverflowError(`Uint64 value exceeds maximum: ${bigintValue}`, { value: bigintValue });
    }
