...
describe("toBytes", () => {
    ...
    describe("edge cases", () => {
        it("handles hash with mixed values", () => {
            const hash = fromHex("0x00ff0aa0123456789abcdef0123456789abcdef0123456789abcdef012345678");
            const bytes = toBytes(hash);
        ...
        it("handles sequential byte pattern", () => {
            const hash = fromHex("0x0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20");
            const bytes = toBytes(hash);
            for (let i = 0; i < 32; i++) {
