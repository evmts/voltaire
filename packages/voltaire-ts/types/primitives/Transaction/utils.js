 ...
 * @internal
 */
export function encodeBigintCompact(value) {
    if (value === 0n) {
 ...
 * @internal
 */
export function decodeBigint(bytes) {
    if (bytes.length === 0) {
        return 0n;
    }
    let result = 0n;
 ...
 */
export function recoverAddress(signature, messageHash) {
 ...
 */
export function encodeAccessList(accessList) {
 ...
 */
export function decodeAccessList(data) {
 ...
 * Validation happens at the boundary when creating branded types.
 */
export function encodeAuthorizationList(authList) {
 ...
 */
export function decodeAuthorizationList(data) {
    return data.map((item) => {
        ...
        if (!chainIdData ||
            ...
            throw new InvalidFormatError("Invalid authorization data", {
                code: -32602,
                value: item,
                expected: "Valid authorization data with correct field types and lengths",
                docsPath: "/primitives/transaction/utils#error-handling",
            });
        }
        return {
