...
export function deserialize(data) {
    ...
    if (fields.length !== 9) {
        throw new DecodingError(`Invalid legacy transaction: expected 9 fields, got ${fields.length}`, {
            code: -32602,
    ...
    for (let i = 0; i < fields.length; i++) {
        if (fields[i]?.type !== "bytes") {
            throw new DecodingError(`Invalid legacy transaction: field ${i} must be bytes`, {
                code: -32602,
    ...
    const dataBytes = /** @type {{ type: "bytes"; value: Uint8Array }} */ (fields[5]).value;
