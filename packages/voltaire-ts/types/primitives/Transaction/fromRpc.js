import { InvalidLengthError, InvalidTransactionTypeError, } from "../errors/index.js";
...
function hexToAddress(hex) {
    ...
    if (bytes.length !== 20) {
        throw new InvalidLengthError(`Invalid address length: ${bytes.length}, expected 20`, {
            code: -32602,
            value: bytes,
            expected: "20 bytes",
...
function hexToHash(hex) {
    const bytes = fromHex(hex);
    if (bytes.length !== 32) {
        throw new InvalidLengthError(`Invalid hash length: ${bytes.length}, expected 32`, {
            code: -32602,
            value: bytes,
            expected: "32 bytes",
            context: { actualLength: bytes.length },
