...
export function fromHex(hex) {
    ...
    if (normalized.length !== 64) {
        throw new InvalidBlockHashFormatError(`BlockHash hex must be 64 characters (32 bytes), got ${normalized.length}`, {
            value: hex,
            expected: "64 hex characters",
        });
    }
    if (!/^[0-9a-f]{64}$/.test(normalized)) {
        throw new InvalidBlockHashFormatError("BlockHash hex contains invalid characters", {
            value: hex,
