...
export function from(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isInteger(value)) {
            throw new InvalidTokenBalanceError(`TokenBalance value must be an integer: ${value}`, { value });
        }
        bigintValue = BigInt(value);
    }
    else {
    ...
    if (bigintValue < 0n) {
        throw new InvalidTokenBalanceError(`TokenBalance value cannot be negative: ${bigintValue}`, { value: bigintValue });
    }
    if (bigintValue > MAX) {
        throw new InvalidTokenBalanceError(`TokenBalance value exceeds maximum: ${bigintValue}`, { value: bigintValue, context: { max: MAX } });
    }
    return /** @type {import('./TokenBalanceType.js').TokenBalanceType} */ (bigintValue);
}
