...
export function parse(raw) {
    ...
    for (const part of parts) {
        ...
        const start = fields[0] !== "" && fields[0] !== undefined
            ? Number.parseInt(fields[0], 10)
            : prevStart;
        const length = fields[1] !== "" && fields[1] !== undefined
            ? Number.parseInt(fields[1], 10)
            : prevLength;
        const fileIndex = fields[2] !== "" && fields[2] !== undefined
            ? Number.parseInt(fields[2], 10)
            : prevFileIndex;
        const jump = fields[3] !== "" && fields[3] !== undefined
            ? /** @type {"i" | "o" | "-"} */ (fields[3])
            : prevJump;
        /** @type {number | undefined} */
        const modifierDepth = fields[4] !== "" && fields[4] !== undefined
