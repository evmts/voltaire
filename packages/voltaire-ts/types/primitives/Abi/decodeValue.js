...
function safeToNumber(value, context) {
    if (value > BigInt(Number.MAX_SAFE_INTEGER)) {
        throw new AbiDecodingError(`${context} exceeds maximum safe integer (got ${value}, max ${Number.MAX_SAFE_INTEGER})`);
    }
    return Number(value);
}
/**
...
export function decodeValue(type, data, offset, components) {
    ...
    if (type === "tuple" && components) {
        ...
        if (isDynamic) {
            ...
            /** @type {any} */ (components), data.slice(dataOffset));
            return { value, newOffset: offset + 32 };
        }
        // Static tuple - decode inline
        const value = decodeParameters(
        /** @type {any} */ (components), data.slice(offset));
    ...
    if (type.endsWith("[]")) {
        ...
        const value = decodeParameters(
        /** @type {any} */ (elementParams), data.slice(dataOffset + 32));
        return { value, newOffset: offset + 32 };
    }
    ...
    if (fixedArrayMatch?.[1] && fixedArrayMatch[2]) {
        ...
        if (isDynamicType(elementType, components)) {
            ...
            const value = decodeParameters(
            /** @type {any} */ (elementParams), data.slice(dataOffset));
            return { value, newOffset: offset + 32 };
        }
        const value = decodeParameters(
        /** @type {any} */ (elementParams), data.slice(offset));
