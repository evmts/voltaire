...
export function getFunctionBySelector(abi, selector) {
    const selectorBytes = typeof selector === "string" ? Hex.toBytes(selector) : selector;
    if (selectorBytes.length !== 4) {
        throw new AbiItemNotFoundError(`Invalid selector length: expected 4 bytes, got ${selectorBytes.length}`, {
    ...
    /** @type {unknown} */ (abi)).find((/** @type {import('./AbiType.js').Item} */ item) => {
        ...
        // Compare bytes
        for (let i = 0; i < 4; i++) {
            if (selectorBytes[i] !== itemSelector[i])
                return false;
        }
        return true;
    });
    if (!item || item.type !== "function") {
        throw new AbiItemNotFoundError(`Function with selector ${Hex.fromBytes(selectorBytes)} not found in ABI`, {
