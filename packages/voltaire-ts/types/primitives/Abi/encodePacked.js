...
export function encodePacked(types, values) {
    if (types.length !== values.length) {
        throw new AbiParameterMismatchError(`Type/value count mismatch: ${types.length} types, ${values.length} values`, {
            value: values.length,
...
function encodePackedValue(type, value, isArrayElement = false) {
    ...
    if (fixedArrayMatch) {
        ...
        if (array.length !== length) {
            throw new AbiEncodingError(`Invalid ${type} length: expected ${length}, got ${array.length}`, {
                context: {
                    type,
    ...
    if (type.startsWith("int")) {
        ...
        const unsigned = bigintValue < 0n ? (1n << BigInt(signBits)) + bigintValue : bigintValue;
