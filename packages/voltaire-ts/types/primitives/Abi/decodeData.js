...
export function decodeData(data) {
    ...
    const item = this.find((item) => {
        if (item.type !== "function")
            return false;
        const itemSelector = AbiFunction.getSelector(item);
        // Compare bytes
        for (let i = 0; i < 4; i++) {
            if (selector[i] !== itemSelector[i])
                return false;
        }
        return true;
    });
    if (!item || item.type !== "function") {
        throw new AbiItemNotFoundError(`Function with selector ${Hex.fromBytes(selector)} not found in ABI`, {
            value: Hex.fromBytes(selector),
            expected: "valid function selector in ABI",
            context: { selector: Hex.fromBytes(selector), abi: this },
        });
    }
    const args = AbiFunction.decodeParams(item, data);
    return {
