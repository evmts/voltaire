...
describe("format", () => {
    describe("function formatting", () => {
        ...
        it("includes parameter names when present", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function transfer(address recipient, uint256 value)");
        });
    });
    describe("state mutability", () => {
        ...
        it("includes pure state mutability", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function calculate(uint256) returns (uint256) pure");
        });
    ...
    describe("complex types", () => {
        it("formats function with array parameters", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function batchTransfer(address[] recipients, uint256[] amounts)");
        });
        ...
        it("formats function with fixed bytes parameter", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function verify(bytes32 hash) returns (bool) pure");
        });
    ...
    describe("multiple outputs", () => {
        it("formats function with multiple return values", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function getData() returns (uint256, string, bool) view");
        });
    ...
    describe("edge cases", () => {
        ...
        it("handles mixed named and unnamed parameters", () => {
            ...
            const formatted = format(func);
            expect(formatted).toBe("function mixed(uint256 first, address, bool last) pure");
        });
    });
    describe("real-world examples", () => {
        ...
        it("formats ERC20 Approval event", () => {
            ...
            const formatted = format(event);
            expect(formatted).toBe("event Approval(address owner, address spender, uint256 value)");
        });
    });
});
