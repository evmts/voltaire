...
import { UintNegativeError, UintNotIntegerError, UintOverflowError, } from "./errors.js";
/**
...
export function from(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isInteger(value)) {
            throw new UintNotIntegerError(`Uint256 value must be an integer: ${value}`, { value });
        }
        bigintValue = BigInt(value);
    }
    else {
    ...
    if (bigintValue < 0n) {
        throw new UintNegativeError(`Uint256 value cannot be negative: ${bigintValue}`, { value: bigintValue });
    }
    if (bigintValue > MAX) {
