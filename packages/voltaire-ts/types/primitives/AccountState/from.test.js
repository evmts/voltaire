...
describe("AccountState.from", () => {
    describe("constructor tests", () => {
        ...
        it("creates contract account with custom storage root", () => {
            const customRoot = "0x1234567890123456789012345678901234567890123456789012345678901234";
            const state = from({
                nonce: nonceFrom(1n),
                balance: weiFrom(0n),
                storageRoot: StateRoot.from(customRoot),
                codeHash: Hash("0xabcdef1234567890123456789012345678901234567890123456789012345678"),
            });
            expect(state.storageRoot).toBeDefined();
            expect(state.codeHash).toBeDefined();
        });
    ...
    describe("immutability tests", () => {
        ...
        it("cannot modify storageRoot", () => {
            ...
            expect(() => {
                /** @type {*} */ (state).storageRoot = StateRoot.from("0x1234567890123456789012345678901234567890123456789012345678901234");
            }).toThrow();
        });
        it("cannot modify codeHash", () => {
            ...
            expect(() => {
                /** @type {*} */ (state).codeHash = Hash("0x1234567890123456789012345678901234567890123456789012345678901234");
            }).toThrow();
        });
    });
    describe("error cases", () => {
        ...
        it("throws on null input", () => {
            expect(() => from(/** @type {*} */ (null))).toThrow("AccountState must be an object with nonce, balance, storageRoot, and codeHash");
        });
        it("throws on undefined input", () => {
            expect(() => from(/** @type {*} */ (undefined))).toThrow("AccountState must be an object with nonce, balance, storageRoot, and codeHash");
        });
        it("throws on string input", () => {
            expect(() => from(/** @type {*} */ ("not an object"))).toThrow("AccountState must be an object with nonce, balance, storageRoot, and codeHash");
        });
        it("throws on number input", () => {
            expect(() => from(/** @type {*} */ (42))).toThrow("AccountState must be an object with nonce, balance, storageRoot, and codeHash");
        });
