...
describe("AccountState.equals", () => {
    describe("equality tests", () => {
        ...
        it("returns true for identical contract states", () => {
            ...
            const customCode = "0xabcdef1234567890123456789012345678901234567890123456789012345678";
            const state1 = from({
                nonce: nonceFrom(1n),
    ...
    describe("inequality tests - storageRoot", () => {
        it("returns false for different storage roots", () => {
            ...
            const state2 = from({
                nonce: nonceFrom(0n),
                balance: weiFrom(0n),
                storageRoot: StateRoot.from("0x1234567890123456789012345678901234567890123456789012345678901234"),
                codeHash: Hash(EMPTY_CODE_HASH),
            });
            expect(equals(state1, state2)).toBe(false);
        });
        it("returns false when storage roots differ by one byte", () => {
            ...
            const state2 = from({
                nonce: nonceFrom(0n),
                balance: weiFrom(0n),
                storageRoot: StateRoot.from("0x1234567890123456789012345678901234567890123456789012345678901235"),
                codeHash: Hash(EMPTY_CODE_HASH),
            });
            expect(equals(state1, state2)).toBe(false);
        });
    ...
    describe("inequality tests - codeHash", () => {
        ...
        it("returns false when code hashes differ by one byte", () => {
            ...
            const state2 = from({
                ...
                balance: weiFrom(0n),
                storageRoot: StateRoot.from(EMPTY_TRIE_HASH),
                codeHash: Hash("0x1234567890123456789012345678901234567890123456789012345678901235"),
            });
            expect(equals(state1, state2)).toBe(false);
        });
    });
