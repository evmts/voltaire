...
export function assertValid(receipt) {
    ...
    if (hasStatus && hasRoot) {
        throw new InvalidReceiptError("Receipt cannot have both status and root (Byzantium fork incompatibility)", {
            expected: "either status (post-Byzantium) or root (pre-Byzantium), not both",
        });
    }
    if (!hasStatus && !hasRoot) {
        throw new InvalidReceiptError("Receipt must have either status (post-Byzantium) or root (pre-Byzantium)", { expected: "status or root field" });
    }
}
