...
export function fromNumber(value, size) {
    ...
    if (value > Number.MAX_SAFE_INTEGER) {
        throw new UnsafeIntegerError(`Number exceeds MAX_SAFE_INTEGER (${Number.MAX_SAFE_INTEGER}). Use Bytes.fromBigInt() for larger values.`, { value });
    }
    ...
    if (minBytes > targetSize) {
        throw new SizeExceededError(`Number ${value} requires ${minBytes} bytes but size is ${targetSize}.`, {
            value,
