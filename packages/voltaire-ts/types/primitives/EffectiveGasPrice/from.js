...
export function from(value) {
    ...
    if (typeof value === "bigint") {
        ...
        if (!Number.isInteger(value)) {
            throw new InvalidFormatError(`EffectiveGasPrice must be an integer, got ${value}`, {
                value,
                expected: "Integer",
                code: -32602,
        ...
        if (value.startsWith("0x")) {
            ...
            throw new InvalidFormatError(`EffectiveGasPrice string must be hex with 0x prefix, got ${value}`, {
                value,
        ...
        throw new InvalidFormatError(`EffectiveGasPrice must be bigint, number, or hex string, got ${typeof value}`, {
            value,
    ...
    if (result < 0n) {
        throw new InvalidFormatError(`EffectiveGasPrice cannot be negative, got ${result}`, {
            value: result,
