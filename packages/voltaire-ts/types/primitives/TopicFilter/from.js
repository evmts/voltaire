...
export function from(topics) {
    ...
    if (topics.length > 4) {
        throw new InvalidTopicFilterError("TopicFilter cannot have more than 4 entries", {
            value: topics,
    ...
    for (let i = 0; i < topics.length; i++) {
        ...
        if (Array.isArray(entry)) {
            ...
            if (entry.length === 0) {
                throw new InvalidTopicFilterError(`Topic entry at index ${i} cannot be an empty array`, {
                    value: entry,
            ...
            for (const hash of entry) {
                if (!(hash instanceof Uint8Array) || hash.length !== 32) {
                    throw new InvalidTopicFilterError(`Topic entry at index ${i} must contain valid Hash values`, {
                        value: hash,
            ...
            throw new InvalidTopicFilterError(`Topic entry at index ${i} must be Hash, Hash[], or null`, {
                value: entry,
    ...
    return /** @type {import('./TopicFilterType.js').TopicFilterType} */ (
    /** @type {unknown} */ (topics));
}
