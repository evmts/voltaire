...
export function VerifySignature({ keccak256, recoverPublicKey, addressFromPublicKey, }) {
    ...
    return async function verifySignature(provider, address, hash, signature) {
        try {
            ...
            const addressHex = typeof address === "string" ? address : Address.toHex(address);
            ...
            if (typeof code === "string" &&
                ...
                if (signature instanceof Uint8Array) {
                    ...
                    if (signature.length !== 65) {
                        throw new InvalidSignatureFormatError(`Invalid signature length: expected 65 bytes, got ${signature.length}`);
                    }
                    sigComponents = {
                ...
                const expectedAddress = typeof address === "string" ? Address(address) : address;
            ...
            if (error instanceof Error &&
                ...
                throw new ContractSignatureError(`Network error during signature verification: ${error.message}`, { cause: error });
            }
