 ...
 * ```
 */
export async function isValidSignature(provider, contractAddress, hash, signature) {
    try {
        ...
        throw new ContractCallError(`Failed to call isValidSignature: ${error instanceof Error ? error.message : String(error)}`, { contractAddress, hash, signature, cause: error });
    }
}
