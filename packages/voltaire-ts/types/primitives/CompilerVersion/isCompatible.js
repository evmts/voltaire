...
export function isCompatible(version, range) {
    ...
    if (range.startsWith("^")) {
        const rangeVersion = 
        /** @type {import('./CompilerVersionType.js').CompilerVersionType} */ (range.slice(1));
    ...
    if (range.startsWith(">=")) {
        const rangeVersion = 
        /** @type {import('./CompilerVersionType.js').CompilerVersionType} */ (range.slice(2).trim());
        return compare(version, rangeVersion) >= 0;
    }
    // Handle > operator
    if (range.startsWith(">") && !range.startsWith(">=")) {
        const rangeVersion = 
        /** @type {import('./CompilerVersionType.js').CompilerVersionType} */ (range.slice(1).trim());
        return compare(version, rangeVersion) > 0;
    }
    // Handle <= operator
    if (range.startsWith("<=")) {
    ...
    // Exact match
    const rangeVersion = 
    /** @type {import('./CompilerVersionType.js').CompilerVersionType} */ (range);
    return compare(version, rangeVersion) === 0;
}
