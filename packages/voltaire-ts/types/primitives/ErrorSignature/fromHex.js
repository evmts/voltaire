...
export function fromHex(hex) {
    ...
    if (bytes.length !== 4) {
        throw new Error(`ErrorSignature hex must be exactly 4 bytes (8 chars), got ${bytes.length} bytes`);
    }
    return /** @type {import('./ErrorSignatureType.js').ErrorSignatureType} */ (bytes);
}
