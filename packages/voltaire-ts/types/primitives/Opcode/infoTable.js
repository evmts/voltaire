 ...
 * @returns {Map<import('./OpcodeType.js').OpcodeType, import('./OpcodeType.js').Info>}
 * @internal
 */
function createInfoTable() {
    /** @type {Map<import('./OpcodeType.js').OpcodeType, import('./OpcodeType.js').Info>} */
    const table = new Map();
    /**
     * @param {import('./OpcodeType.js').OpcodeType} op
    ...
    for (let i = 0; i < 32; i++) {
        add(
        /** @type {import('./OpcodeType.js').OpcodeType} */ (0x60 + i), GAS_FASTEST_STEP, 0, 1, `PUSH${i + 1}`);
    }
    // 0x80-0x8f: DUP1-DUP16
    for (let i = 0; i < 16; i++) {
        add(
        /** @type {import('./OpcodeType.js').OpcodeType} */ (0x80 + i), GAS_FASTEST_STEP, i + 1, i + 2, `DUP${i + 1}`);
    }
    // 0x90-0x9f: SWAP1-SWAP16
    for (let i = 0; i < 16; i++) {
        add(
        /** @type {import('./OpcodeType.js').OpcodeType} */ (0x90 + i), GAS_FASTEST_STEP, i + 2, i + 2, `SWAP${i + 1}`);
    }
    // 0xa0-0xa4: LOG0-LOG4
    for (let i = 0; i <= 4; i++) {
        add(
        /** @type {import('./OpcodeType.js').OpcodeType} */ (0xa0 + i), LOG_GAS + i * LOG_TOPIC_GAS, 2 + i, 0, `LOG${i}`);
    }
