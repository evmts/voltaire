...
export function from(value) {
    if (value instanceof Uint8Array) {
        if (value.length !== SIZE) {
            throw new InvalidBundleHashError(`BundleHash must be ${SIZE} bytes, got ${value.length}`, { value });
        }
    ...
    if (typeof value === "string") {
        ...
        if (hex.length !== SIZE * 2) {
            throw new InvalidBundleHashError(`BundleHash hex string must be ${SIZE * 2} characters, got ${hex.length}`, { value });
        }
    ...
    throw new InvalidBundleHashError("BundleHash must be hex string or Uint8Array", { value });
}
