...
export function getS(signature) {
    if (signature.algorithm !== "secp256k1" && signature.algorithm !== "p256") {
        throw new InvalidAlgorithmError(`getS only supported for ECDSA signatures, got ${signature.algorithm}`, {
            value: signature.algorithm,
            expected: "secp256k1 or p256",
            docsPath: "/primitives/signature/get-s#error-handling",
        });
    }
    return /** @type {import('../Hash/index.js').HashType} */ (signature.slice(COMPONENT_SIZE, COMPONENT_SIZE * 2));
}
