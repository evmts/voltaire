import { COMPONENT_SIZE, ECDSA_SIZE, ECDSA_WITH_V_SIZE, ED25519_SIZE, } from "./constants.js";
...
export function fromCompact(bytes, algorithmOrV) {
    const algorithm = typeof algorithmOrV === "string" ? algorithmOrV : "secp256k1";
    const explicitV = typeof algorithmOrV === "number" ? algorithmOrV : undefined;
    if (algorithm === "ed25519") {
        if (bytes.length !== ED25519_SIZE) {
            throw new InvalidSignatureLengthError(`Ed25519 signature must be ${ED25519_SIZE} bytes, got ${bytes.length}`, {
                value: bytes.length,
    ...
    if (bytes.length === ECDSA_SIZE) {
        ...
        const yParity = explicitV !== undefined
            ? explicitV
    ...
    throw new InvalidSignatureLengthError(`Invalid signature length for ${algorithm}: ${bytes.length} bytes`, {
        value: bytes.length,
        expected: algorithm === "secp256k1"
