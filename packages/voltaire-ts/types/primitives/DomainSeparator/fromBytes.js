...
export function fromBytes(bytes) {
    if (bytes.length !== 32) {
        throw new InvalidDomainSeparatorLengthError(`DomainSeparator must be 32 bytes, got ${bytes.length}`, { value: bytes, expected: "32 bytes" });
    }
    return /** @type {import('./DomainSeparatorType.js').DomainSeparatorType} */ (bytes);
}
