...
```typescript
import { BloomFilter, Address, Hex } from 'tevm'
...
class EventIndexer {
  ...
  async indexBlock(blockNum: number) {
    ...
    // Store block bloom
    const bloom = BloomFilter(block.logsBloom, 2048, 3)
...
class UniswapMonitor {
  ...
  async monitorPair(
    ...
    const addr = Address(pairAddress)
    const swaps: Log[] = []

    for (let block = startBlock; block <= endBlock; block++) {
      const blockData = await provider.getBlock(block)
      const filter = BloomFilter(blockData.logsBloom, 2048, 3)
