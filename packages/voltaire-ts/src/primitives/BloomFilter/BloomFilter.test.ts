...
describe("BloomFilter - Basic Operations", () => {
	describe("create", () => {
		...
		it("throws InvalidBloomFilterParameterError with correct name for zero m", () => {
			try {
				BloomFilter.create(0, 3);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("InvalidBloomFilterParameterError");
			}
		});
	...
	describe("merge", () => {
		...
		it("throws InvalidBloomFilterParameterError with correct name for mismatched m", () => {
			const f1 = BloomFilter.create(2048, 3);
			const f2 = BloomFilter.create(4096, 3);
