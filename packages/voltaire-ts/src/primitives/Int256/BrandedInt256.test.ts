...
describe("Int256 - EVM SDIV (Signed Division)", () => {
	...
	it("SDIV: MIN / -1 throws IntegerOverflowError (EVM undefined behavior)", () => {
		// In EVM, MIN / -1 overflows because -MIN doesn't fit in Int256
		try {
			Int256.dividedBy(Int256.MIN, -1n);
			expect.fail("Should throw");
		} catch (e) {
			expect((e as Error).name).toBe("IntegerOverflowError");
		}
	...
	it("SDIV: division by zero throws InvalidRangeError", () => {
		try {
			Int256.dividedBy(10n, 0n);
			expect.fail("Should throw");
		} catch (e) {
...
describe("Int256 - EVM SMOD (Signed Modulo)", () => {
	...
	it("SMOD: division by zero throws InvalidRangeError", () => {
		try {
			Int256.modulo(10n, 0n);
			expect.fail("Should throw");
		} catch (e) {
			expect((e as Error).name).toBe("InvalidRangeError");
			expect((e as Error).message).toContain("Division by zero");
