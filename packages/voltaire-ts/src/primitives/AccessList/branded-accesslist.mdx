...
<Card title="Try it Live" icon="play" href="https://playground.tevm.sh?example=primitives/access-list.ts">
...
- `storageKeys`: Readonly array of 32-byte HashType

**Generic Parameters:**
- `TAddress`: Address type (defaults to AddressType)
- `TStorageKeys`: Storage keys type (defaults to readonly HashType[])
...
const safe: BrandedAccessList = AccessList(unbranded);
```
...
namespace BrandedAccessList {
  ...
  export function withAddress(list: BrandedAccessList, addr: AddressType): BrandedAccessList;
  // ...
}
