...
- `InvalidFormatError` - If data is empty
- `InvalidTransactionTypeError` - If first byte doesn't match any known type
...
// Error on empty data
import { InvalidFormatError, InvalidTransactionTypeError } from 'tevm/errors'
...
try {
  ...
  if (e instanceof InvalidTransactionTypeError) {
    console.error(e.name)     // "InvalidTransactionTypeError"
...
switch (firstByte) {
  ...
  default: throw new InvalidTransactionTypeError(`Unknown transaction type: 0x${firstByte.toString(16)}`)
}
```
...
```typescript
import { detectType, deserialize } from 'tevm/Transaction'
