...
describe("Gas.createCost (convenience form)", () => {
	it("calculates cost using this: pattern", () => {
		const result = Gas.createCost.call({
			initcodeSize: 1000n,
			deployedSize: 500n,
		});
		expect(result.base).toBe(32000n);
...
describe("Gas.Precompile.calculateEcPairingCost", () => {
	...
	it("calculates cost for single pair in Byzantium", () => {
		expect(Gas.Precompile.calculateEcPairingCost(1n, "byzantium")).toBe(
			180000n,
		); // 100000 + 1*80000
	});

	it("calculates cost for multiple pairs in Byzantium", () => {
		expect(Gas.Precompile.calculateEcPairingCost(3n, "byzantium")).toBe(
			340000n,
		); // 100000 + 3*80000
	});
...
describe("Gas.Precompile.ecPairingCost (convenience form)", () => {
	it("calculates cost using this: pattern", () => {
		const result = Gas.Precompile.ecPairingCost.call({
			pairCount: 2n,
			hardfork: "istanbul",
		});
