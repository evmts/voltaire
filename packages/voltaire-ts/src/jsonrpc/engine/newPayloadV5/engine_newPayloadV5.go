...
const Method = "engine_newPayloadV5"

// Params represents the parameters for engine_newPayloadV5
type Params struct {
	// Execution payload object V4
	Execution payload types.Quantity `json:"-"`
	Expected blob versioned hashes types.Quantity `json:"-"`
	// 32 byte hex value
	Parent beacon block root types.Hash `json:"-"`
...
func (p *Params) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(arr[2], &p.Parent beacon block root); err != nil {
		return fmt.Errorf("parameter 2 (Parent beacon block root): %w", err)
	}
...
// Result represents the result for engine_newPayloadV5
//
