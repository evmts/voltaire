...
describe("Bip39", () => {
	...
	describe("assertValidMnemonic", () => {
		...
		it("throws InvalidMnemonicError with correct name", () => {
			const mnemonic =
				"abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon";
			try {
	...
	describe("entropyToMnemonic", () => {
		...
		it("throws InvalidEntropyError with correct name", () => {
			const entropy = new Uint8Array(17);
			try {
				Bip39.entropyToMnemonic(entropy);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("InvalidEntropyError");
			}
		});
	...
	describe("Unicode normalization", () => {
		...
		it("handles combining characters in passphrase", async () => {
			const mnemonic =
				"abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
			const passphrase = "e\u0301"; // é as combining chars
			const seed = await Bip39.mnemonicToSeed(mnemonic, passphrase);
		...
		it("handles emoji normalization", async () => {
			const mnemonic =
				"abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
			const passphrase = "👨‍👩‍👧‍👦"; // Family emoji with ZWJ
			const seed = await Bip39.mnemonicToSeed(mnemonic, passphrase);
