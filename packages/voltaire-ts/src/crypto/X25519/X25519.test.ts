...
describe("X25519", () => {
	...
	describe("derivePublicKey", () => {
		...
		it("throws InvalidSecretKeyError with correct name for invalid size", () => {
			const invalidSecretKey = new Uint8Array(16);

			try {
				X25519.derivePublicKey(invalidSecretKey);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("InvalidSecretKeyError");
			}
		});
	...
	describe("scalarmult (ECDH)", () => {
		...
		it("produces symmetric shared secret (commutative)", () => {
			...
			const shared1to2 = X25519.scalarmult(
				keypair1.secretKey,
				keypair2.publicKey,
			);
			const shared2to1 = X25519.scalarmult(
				keypair2.secretKey,
				keypair1.publicKey,
			);
		...
		it("produces different shared secrets for different keypairs", () => {
			...
			const shared1to3 = X25519.scalarmult(
				keypair1.secretKey,
				keypair3.publicKey,
			);
		...
		it("throws on invalid secret key size", () => {
			...
			expect(() =>
				X25519.scalarmult(invalidSecretKey, keypair.publicKey),
			).toThrow();
		});

		it("throws InvalidSecretKeyError with correct name for invalid secret key", () => {
			const invalidSecretKey = new Uint8Array(16);
			const seed = new Uint8Array(32).fill(1);
			const keypair = X25519.keypairFromSeed(seed);
		...
		it("throws on invalid public key size", () => {
			...
			expect(() =>
				X25519.scalarmult(keypair.secretKey, invalidPublicKey),
			).toThrow();
		});

		it("throws InvalidPublicKeyError with correct name for invalid public key", () => {
			const seed = new Uint8Array(32).fill(1);
			const keypair = X25519.keypairFromSeed(seed);
			const invalidPublicKey = new Uint8Array(16);
		...
		it("matches @noble/curves implementation", () => {
			...
			const shared = X25519.scalarmult(keypair1.secretKey, keypair2.publicKey);
			const nobleShared = x25519.getSharedSecret(
				keypair1.secretKey,
				keypair2.publicKey,
			);
	...
	describe("Security Edge Cases", () => {
		...
		it("produces different shared secrets with different partners", () => {
			...
			const aliceBobShared = X25519.scalarmult(
				aliceKeypair.secretKey,
				bobKeypair.publicKey,
			);
			const aliceCharlieShared = X25519.scalarmult(
				aliceKeypair.secretKey,
	...
	describe("Iterative Key Agreement", () => {
		it("performs multi-party key agreement", () => {
			...
			const bobCharlieShared = X25519.scalarmult(
				bob.secretKey,
				charlie.publicKey,
			);
