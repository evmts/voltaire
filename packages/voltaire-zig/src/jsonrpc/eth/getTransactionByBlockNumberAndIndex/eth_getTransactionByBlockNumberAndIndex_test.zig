...
const eth_getTransactionByBlockNumberAndIndex = @import("eth_getTransactionByBlockNumberAndIndex.zig");

// Tests for eth_getTransactionByBlockNumberAndIndex Result type
...
test "Result type accepts TransactionResponse" {
    ...
    const result = eth_getTransactionByBlockNumberAndIndex.Result{
        .value = tx,
    };
...
test "jsonStringify serializes TransactionResponse correctly" {
    ...
    try testing.expect(std.mem.indexOf(u8, json, "\"blockNumber\":\"0x1442e\"") != null);
...
test "Result includes block number from query" {
    const allocator = testing.allocator;
