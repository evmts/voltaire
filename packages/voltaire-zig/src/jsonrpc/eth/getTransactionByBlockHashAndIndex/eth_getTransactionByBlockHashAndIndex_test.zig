...
const eth_getTransactionByBlockHashAndIndex = @import("eth_getTransactionByBlockHashAndIndex.zig");

// Tests for eth_getTransactionByBlockHashAndIndex Result type

test "Result type accepts TransactionResponse" {
    const metadata = types.TransactionResponse.Metadata{
        .block_hash = .{ .bytes = [_]u8{0x55} ** 32 },
        .block_number = 2,
    ...
    const result = eth_getTransactionByBlockHashAndIndex.Result{
        .value = tx,
    };
...
test "jsonStringify serializes TransactionResponse correctly" {
    ...
    try testing.expect(std.mem.indexOf(u8, json, "\"transactionIndex\":\"0x3\"") != null);
}
...
test "Result includes block metadata from query" {
    const allocator = testing.allocator;

    const block_hash = [_]u8{0xbf} ** 32;
    const metadata = types.TransactionResponse.Metadata{
        .block_hash = .{ .bytes = block_hash },
        .block_number = 0x1234,
        .block_timestamp = 0x5678,
        .transaction_index = 2,
        .from = .{ .bytes = [_]u8{0x74} ** 20 },
        .hash = .{ .bytes = [_]u8{0xab} ** 32 },
    };
    ...
    const json = buf.items;
    try testing.expect(std.mem.indexOf(u8, json, "\"blockHash\"") != null);
    try testing.expect(std.mem.indexOf(u8, json, "\"blockNumber\":\"0x1234\"") != null);
    try testing.expect(std.mem.indexOf(u8, json, "\"transactionIndex\":\"0x2\"") != null);
}
