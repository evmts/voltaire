...
export function parseMessage(message: string): SiweMessage {
  ...
  const statement = lines[2] && !lines[2].startsWith("URI:") ? lines[2] : undefined;

  let currentLine = statement ? 3 : 2;
  ...
  if (!result.uri || !result.version || result.chainId === undefined || !result.nonce || !result.issuedAt) {
...
export function validateMessage(message: SiweMessage): boolean {
  if (!message.domain || !message.address || !message.uri || !message.version || message.chainId === undefined || !message.nonce || !message.issuedAt) {
    return false;
  }
...
export function formatMessage(message: SiweMessage): string {
  ...
  parts.push(`${message.domain} wants you to sign in with your Ethereum account:`);
