...
Use these built-in utilities for provider implementation:
...
| Utility | Provider Use Case |
|---------|-------------------|
| `retryWithBackoff(fn, opts)` | Retry failed RPC requests with backoff |
| `poll(fn, opts)` | Poll for subscription updates, block changes |
| `pollForReceipt(hash, getReceipt)` | Wait for transaction confirmation |
| `RateLimiter({ maxRequests, interval })` | Respect RPC endpoint rate limits |
| `BatchQueue({ maxBatchSize, maxWaitTime, processBatch })` | JSON-RPC batch requests |
| `withTimeout(promise, { ms })` | Request timeouts |
| `createDeferred()` | Pending request tracking |

**Example - Request with retry + timeout**:
```typescript
async request(args, options) {
  return retryWithBackoff(
    () => withTimeout(this.send(args), { ms: options?.timeout ?? 30000 }),
