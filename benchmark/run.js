...
const voltaireJsGzip = gzipSync(
	readFileSync("dist/voltaire-keccak256.min.js"),
).length;
const wasmGzip = existsSync(wasmPath)
	? gzipSync(readFileSync(wasmPath)).length
	: 0;

console.log("              minified      gzipped");
console.log(
	`viem          ${formatSize(viemSize).padEnd(12)} ${formatSize(viemGzip)}`,
);
console.log(
	`voltaire JS   ${formatSize(voltaireJsSize).padEnd(12)} ${formatSize(voltaireJsGzip)}`,
);
console.log(
	`  +.wasm      ${formatSize(wasmSize).padEnd(12)} ${formatSize(wasmGzip)}`,
);
const totalSize = voltaireJsSize + wasmSize;
const totalGzip = voltaireJsGzip + wasmGzip;
console.log(
	`  =total      ${formatSize(totalSize).padEnd(12)} ${formatSize(totalGzip)}`,
);
...
if (diffGzip > 0) {
	console.log(
		`gzipped: voltaire is ${formatSize(diffGzip)} larger (+${pctGzip}%)`,
	);
} else {
	console.log(
		`gzipped: voltaire is ${formatSize(-diffGzip)} smaller (${-pctGzip}%)`,
	);
}
