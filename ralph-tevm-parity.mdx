...
# TEVM Parity Implementation - Milestone 1: Forked Read Node

**Goal**: Implement read-only fork functionality (state reads + block queries)

**Plan Reference**: `/Users/williamcory/voltaire/TEVM_PARITY_PLAN.md`
...
Read TEVM_PARITY_PLAN.md to understand:
...
Create a detailed todo list with all implementation tasks.
...
Use existing Voltaire patterns: Zig implementation + TS wrapper, colocated files, data-first branded types.
...
Create commit:
```
feat(milestone-1): implement forked read node

Implements Milestone 1 of TEVM Parity Plan:
- State manager with fork backend and dual-cache strategy
- Blockchain with fork block fetching
- JSON-RPC handlers for state reads (7 methods)
- Integration tests with Mainnet fork

Acceptance criteria met:
✅ eth_getBalance/getCode/getStorageAt work in fork mode
✅ eth_blockNumber returns fork head
✅ eth_getBlockByNumber fetches remote blocks

Files created:
- src/state-manager/: StateCache, ForkBackend, JournaledState, StateManager
- src/blockchain/: BlockStore, ForkBlockCache, Blockchain
- src/jsonrpc/: JsonRpcServer + 7 eth handlers
...
- Run `zig build && zig build test` frequently (TDD)
</Claude>
