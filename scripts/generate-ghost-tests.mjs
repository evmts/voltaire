...
async function handleTSJSTests() {
  ...
  for (const file of files) {
    ...
    await writeMdxFile(target, { title: `'${title}'`, source: `'${file}'` }, body)
    count++
  }
  return count
}
...
async function handleSwiftTests() {
  ...
  for (const file of files) {
    ...
    const target = path.join(languageRoot, file.replace(/^swift\//, '') + '.mdx')
    await writeMdxFile(target, { title: `'${title}'`, source: `'${file}'` }, body)
    count++
  }
  return count
...
async function main() {
  ...
  console.log('Generating ghost docs from tests...')
  ...
  console.log(`Ghost docs generated: TS/JS=${tsjs}, Swift=${swift}, Zig=${zig} in ${dur}s`)
}
