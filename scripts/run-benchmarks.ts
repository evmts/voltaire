...
async function runBenchmark(benchFile: string): Promise<BenchmarkOutput> {
	try {
		...
		const result =
			await $`bun run vitest bench ${benchFile} --run --reporter=verbose`.text();
 ...
 */
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: benchmark output parsing requires branching
 ...
 */
function generateBenchmarksMarkdown(
