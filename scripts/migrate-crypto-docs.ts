...
function convertStarlightToMintlify(content: string): string {
  ...
  content = content.replace(/^import\s+\{[^}]+\}\s+from\s+['"]@astrojs\/starlight\/components['"];?\s*$/gm, '');

  // Convert <Tabs>/<TabItem> to Mintlify <Tabs>/<Tab>
  content = content.replace(/<Tabs>\s*/g, '<Tabs>\n');
  content = content.replace(/<\/Tabs>/g, '</Tabs>');
  content = content.replace(/<TabItem\s+label=["']([^"']+)["']>/g, '<Tab title="$1">');
  content = content.replace(/<\/TabItem>/g, '</Tab>');

  // Convert <Aside> to Mintlify callouts
  // Handle Aside with both type and title
  content = content.replace(/<Aside\s+type=["'](\w+)["']\s+title=["']([^"']+)["']>/g, (match, type, title) => {
    const calloutType = type === 'caution' ? 'Warning' : type === 'tip' ? 'Tip' : type === 'note' ? 'Note' : type === 'danger' ? 'Warning' : 'Note';
  ...
  content = content.replace(/<\/Aside>/g, (match) => {
    ...
    const asideTag = content.substring(lastAside, content.indexOf('>', lastAside) + 1);

    if (asideTag.includes('Warning') || asideTag.includes('warning') || asideTag.includes('caution') || asideTag.includes('danger')) {
      return '</Warning>';
