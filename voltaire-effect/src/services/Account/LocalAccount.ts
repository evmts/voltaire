...
import { Hex, Address, Rlp, Secp256k1, TypedData, BrandedAddress, BrandedHex, BrandedSignature, BrandedHash } from '@tevm/voltaire'
...
function deriveAddressFromPublicKey(publicKey: Uint8Array, keccakHash: Uint8Array): AddressType {
...
function serializeUnsignedTransaction(tx: UnsignedTransaction): Uint8Array {
  const items: (Uint8Array | readonly Uint8Array[])[] = [
    ...
    tx.gasPrice !== undefined ? bigintToRlpBytes(tx.gasPrice) : new Uint8Array(0),
    tx.gasLimit !== undefined ? bigintToRlpBytes(tx.gasLimit) : new Uint8Array(0),
...
function encodeTypedDataHash(typedData: TypedDataType, keccak256: (data: Uint8Array) => Effect.Effect<Uint8Array>): Effect.Effect<Uint8Array> {
  return Effect.gen(function* () {
    const domainSeparator = yield* hashDomain(typedData.domain, keccak256)
    const structHash = yield* hashStruct(typedData.primaryType, typedData.message as Record<string, unknown>, typedData.types, keccak256)
...
function hashDomain(domain: TypedDataType['domain'], keccak256: (data: Uint8Array) => Effect.Effect<Uint8Array>): Effect.Effect<Uint8Array> {
  return Effect.gen(function* () {
    ...
    const encodedValues = yield* encodeData(types, Object.fromEntries(types.map((t, i) => [t.name, values[i]])), keccak256)
...
function encodeValue(
  ...
  return Effect.gen(function* () {
    ...
    if (type === 'address') {
      const bytes = new Uint8Array(32)
      const addrBytes = typeof value === 'string' ? Hex.toBytes(value as HexType) : Address.toBytes(value as AddressType)
    ...
    if (type.startsWith('bytes')) {
      ...
      const bytes = new Uint8Array(32)
      const src = value instanceof Uint8Array ? value : Hex.toBytes(value as HexType)
...
export const LocalAccount = (privateKeyHex: HexType) => Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const privateKeyBytes = Hex.toBytes(privateKeyHex) as unknown as PrivateKeyType
    const publicKey = Secp256k1.derivePublicKey(privateKeyBytes as unknown as Parameters<typeof Secp256k1.derivePublicKey>[0])
    ...
    return AccountService.of({
      ...
      signMessage: (message: HexType) =>
        Effect.gen(function* () {
          ...
          const prefix = new TextEncoder().encode(`${EIP191_PREFIX}${messageBytes.length}`)
