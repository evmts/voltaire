 ...
 */
export const WebSocketTransport = (options: WebSocketTransportConfig | string): Layer.Layer<TransportService, TransportError> => {
  const config = typeof options === 'string'
    ? { url: options, timeout: 30000 }
    : { url: options.url, protocols: options.protocols, timeout: options.timeout ?? 30000 }
  ...
  return Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      const pendingRef = yield* Ref.make<Map<number, Deferred.Deferred<JsonRpcResponse<unknown>, never>>>(new Map())
      ...
      ws.onerror = () => {
        Effect.runSync(
          Deferred.fail(
            connectDeferred,
            new TransportError({ code: -32603, message: 'WebSocket connection failed' })
          )
        )
      }
      ...
      ws.onclose = () => {
        Effect.runSync(
          Effect.gen(function* () {
            const pending = yield* Ref.get(pendingRef)
            const error = new TransportError({ code: -32603, message: 'WebSocket closed' })
            for (const [id, deferred] of pending) {
              yield* Deferred.succeed(deferred, { jsonrpc: '2.0', id, error: { code: error.code, message: error.message } })
            }
        ...
        request: <T>(method: string, params: unknown[] = []) =>
          Effect.gen(function* () {
            if (ws.readyState !== WebSocket.OPEN) {
              return yield* Effect.fail(
                new TransportError({ code: -32603, message: 'WebSocket not connected' })
              )
            }
            ...
            const pending = yield* Ref.get(pendingRef)
            pending.set(id, deferred as Deferred.Deferred<JsonRpcResponse<unknown>, never>)
            yield* Ref.set(pendingRef, pending)
            ...
            const response = yield* Deferred.await(deferred).pipe(
              ...
              Effect.catchTag('TimeoutException', () =>
                Effect.gen(function* () {
                  ...
                  return yield* Effect.fail(
                    new TransportError({ code: -32603, message: `Request timeout after ${config.timeout}ms` })
                  )
                })
              )
            )
