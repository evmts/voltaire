...
describe("BlockStreamService", () => {
	...
	describe("BlockStream layer", () => {
		...
		it("watch with include transactions option", async () => {
			const mockTransport: TransportShape = {
				request: <T>(method: string, _params?: unknown[]): Effect.Effect<T, never> => {
					if (method === "eth_blockNumber") {
						return Effect.succeed("0x100" as T);
		...
		it("watch with include receipts option", async () => {
			const mockTransport: TransportShape = {
				request: <T>(method: string, _params?: unknown[]): Effect.Effect<T, never> => {
					if (method === "eth_blockNumber") {
						return Effect.succeed("0x100" as T);
		...
		it("backfill with include transactions option", async () => {
			const mockTransport: TransportShape = {
				request: <T>(method: string, _params?: unknown[]): Effect.Effect<T, never> => {
					if (method === "eth_blockNumber") {
						return Effect.succeed("0x100" as T);
	...
	describe("error handling", () => {
		it("wraps transport errors as BlockStreamError", async () => {
			const mockTransport: TransportShape = {
	...
	describe("stream lifecycle", () => {
		...
		it("backfill with large block range", async () => {
			const mockTransport: TransportShape = {
				request: <T>(method: string, _params?: unknown[]): Effect.Effect<T, never> => {
					if (method === "eth_blockNumber") {
