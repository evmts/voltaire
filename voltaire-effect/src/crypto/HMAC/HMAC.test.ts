...
describe("HMACService", () => {
	...
	describe("Key size variations", () => {
		...
		it.effect("handles 32-byte key (block size for SHA256)", () =>
			Effect.gen(function* () {
				const hmac = yield* HMACService;
				const result = yield* hmac.sha256(
		...
		it.effect(
			"SHA512 handles 128-byte key (block size for SHA512 internal)",
			() =>
				Effect.gen(function* () {
					const hmac = yield* HMACService;
					const result = yield* hmac.sha512(
						new Uint8Array(128).fill(0xee),
						message,
					);
					expect(result.length).toBe(64);
				}).pipe(Effect.provide(HMACLive)),
	...
	describe("Message size variations", () => {
		...
		it.effect("handles 64KB message", () =>
			Effect.gen(function* () {
				...
				const result = yield* hmac.sha256(
					key,
					new Uint8Array(65536).fill(0x22),
				);
				expect(result.length).toBe(32);
			}).pipe(Effect.provide(HMACLive)),
		);
