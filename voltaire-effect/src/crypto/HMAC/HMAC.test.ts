...
import { HMACService, HMACLive, HMACTest, hmacSha256, hmacSha512 } from './index.js'
  ...
  describe('HMACLive', () => {
    ...
    it('produces different outputs for different keys', async () => {
      const program = Effect.gen(function* () {
        ...
        const mac2 = yield* hmac.sha256(new Uint8Array([9, 8, 7, 6, 5, 4, 3, 2]), message)
        return { mac1, mac2 }
      })
      ...
      expect(Buffer.from(mac1).toString('hex')).not.toBe(Buffer.from(mac2).toString('hex'))
    })
    ...
    it('produces different outputs for different messages', async () => {
      const program = Effect.gen(function* () {
        const hmac = yield* HMACService
        const mac1 = yield* hmac.sha256(key, message)
        const mac2 = yield* hmac.sha256(key, new Uint8Array([119, 111, 114, 108, 100])) // "world"
        return { mac1, mac2 }
      })
    ...
    it('produces consistent output for same input', async () => {
      ...
      expect(Buffer.from(mac1).toString('hex')).toBe(Buffer.from(mac2).toString('hex'))
    })
  })
