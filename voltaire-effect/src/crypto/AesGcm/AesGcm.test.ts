...
describe('AesGcm', () => {
  ...
  describe('encrypt/decrypt roundtrip', () => {
    it('encrypts and decrypts correctly', async () => {
      ...
      const ciphertext = await Effect.runPromise(AesGcm.encrypt(key, plaintext, nonce))
      const decrypted = await Effect.runPromise(AesGcm.decrypt(key, ciphertext, nonce))
    ...
    it('encrypts with AAD', async () => {
      ...
      const ciphertext = await Effect.runPromise(AesGcm.encrypt(key, plaintext, nonce, aad))
      const decrypted = await Effect.runPromise(AesGcm.decrypt(key, ciphertext, nonce, aad))
