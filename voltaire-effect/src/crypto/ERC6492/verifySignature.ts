/**
 * @fileoverview Verify ERC-6492 signatures via deployless UniversalSigValidator.
 * @module ERC6492/verifySignature
 * @since 0.2.14
 */
...
const UNIVERSAL_VALIDATOR_BYTECODE =
...
const decodeRevertReason = (data: `0x${string}`): string | null => {
	...
	try {
		const decoded = decodeParameters(
			...
			Hex.toBytes(`0x${data.slice(10)}` as `0x${string}`),
 ...
 *
 * @param params - Signature verification inputs
...
export const verifySignature = (params: {
	...
	Effect.gen(function* () {
		...
		const messageBytes = yield* Effect.try({
			try: () => Hex.toBytes(messageHex),
			catch: (e) =>
				new VerifyError({
					message: `Invalid message hash: ${e instanceof Error ? e.message : String(e)}`,
					cause: e,
				}),
		});
		...
		if (messageBytes.length !== 32) {
			return yield* Effect.fail(
				new VerifyError({
					message: `Invalid message hash length: expected 32 bytes, got ${messageBytes.length}`,
				}),
			);
		}
		...
		if (revertReason) {
			return yield* Effect.fail(
				new VerifyError({
					message: `Signature verification reverted: ${revertReason}`,
				}),
			);
		}

		if (callHex === "0x01" || callHex === "0x1") {
			return true;
		}
		if (callHex === "0x" || callHex === "0x00" || callHex === "0x0") {
			return false;
		}

		return yield* Effect.fail(
			new VerifyError({
				message: `Unexpected result from UniversalSigValidator: ${callHex}`,
			}),
		);
	});
