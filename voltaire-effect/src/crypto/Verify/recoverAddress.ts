...
import { AddressDerivationError, SignatureRecoveryError } from "./errors.js";
 ...
 * @example
 * ```typescript
 * import { recoverAddress, KeccakLive, Secp256k1Live } from 'voltaire-effect/crypto/Verify'
 * import * as Effect from 'effect/Effect'
 ...
 * ```
 *
 * @throws {SignatureRecoveryError} When public key recovery fails
...
export const recoverAddress = (params: {
	...
	AddressType,
	SignatureRecoveryError | AddressDerivationError,
	...
	Effect.gen(function* () {
		...
		const normalizedSig = yield* Effect.try({
			try: () => normalizeSignature(params.signature),
				...
				SignatureRecoveryError.of(`Invalid signature format: ${e}`, {
					cause: e,
				}),
		});
