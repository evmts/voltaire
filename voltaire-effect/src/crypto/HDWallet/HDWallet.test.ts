...
import { derive, generateMnemonic, fromSeed, mnemonicToSeed, getPrivateKey, getPublicKey } from './derive.js'
...
describe('HDWalletService', () => {
  describe('HDWalletTest', () => {
    ...
    it('mnemonicToSeed returns 64-byte seed', async () => {
      const program = Effect.gen(function* () {
        const hdwallet = yield* HDWalletService
        return yield* hdwallet.mnemonicToSeed(['abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'abandon', 'about'])
      })
