...
export const EIP712Live = Layer.succeed(EIP712Service, {
  ...
  signTypedData: (typedData, privateKey) => Effect.sync(() => EIP712.signTypedData(typedData, privateKey)),
  verifyTypedData: (signature, typedData, address) => Effect.sync(() => EIP712.verifyTypedData(signature, typedData, address)),
  recoverAddress: (signature, typedData) => Effect.sync(() => EIP712.recoverAddress(signature, typedData)),
  hashDomain: (domain) => Effect.sync(() => EIP712.Domain.hash(domain)),
  hashStruct: (primaryType, data, types) => Effect.sync(() => EIP712.hashStruct(primaryType, data, types))
})

const mockHash = new Uint8Array(32) as HashType
const mockAddress = new Uint8Array(20) as AddressType
const mockSignature: Signature = { r: new Uint8Array(32), s: new Uint8Array(32), v: 27 }
