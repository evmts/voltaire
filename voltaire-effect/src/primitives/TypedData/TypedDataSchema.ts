 ...
 * 
 * Transforms from JSON-compatible input to TypedData output
 * 
 * @example
 * ```typescript
 * import * as Schema from 'effect/Schema'
 * import { TypedDataSchema } from './TypedDataSchema.js'
 * 
 * const typedData = Schema.decodeSync(TypedDataSchema)({
 *   types: {
 *     EIP712Domain: [
 ...
 * ```
 */
export const TypedDataSchema = S.transformOrFail(
      ...
      try {
        const result = TypedData.from({
          ...
          domain: {
            ...
            chainId: input.domain.chainId !== undefined ? toChainId(input.domain.chainId) : undefined,
...
).annotations({ identifier: 'TypedDataSchema' })
