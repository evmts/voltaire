 ...
 * @since 0.0.1
 */
export const ContractResultSchema: S.Schema<ContractResultType, ContractResultInput, never> = S.transformOrFail(
  S.Struct({
    isSuccess: S.Boolean,
  ...
  ContractResultTypeSchema,
  {
    ...
    decode: (input, _options, ast) => {
      try {
        return ParseResult.succeed(ContractResult.from(input.isSuccess, input.data))
      } catch (e) {
    ...
    encode: (result) => {
      if (result.success) {
        ...
        return ParseResult.succeed({
          isSuccess: false,
          data: typeof reason === 'string' ? reason : reason as unknown as Uint8Array,
        })
      }
    }
  }
