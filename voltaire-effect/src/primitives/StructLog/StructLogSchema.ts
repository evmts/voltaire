 ...
 * @since 0.0.1
 */
export const StructLogSchema: S.Schema<StructLogType, {
  pc: number
  op: string
  gas: bigint
  gasCost: bigint
  ...
  StructLogTypeSchema,
  {
    ...
    decode: (value, _options, ast) => {
      try {
        return ParseResult.succeed(StructLog.from(value as any) as StructLogType)
      } catch (e) {
        return ParseResult.fail(new ParseResult.Type(ast, value, (e as Error).message))
      }
    },
    encode: (log) => ParseResult.succeed({
      pc: log.pc,
      op: log.op,
      gas: log.gas,
      gasCost: log.gasCost,
