 ...
 * @since 0.0.1
 */
export const TransactionHashSchema: S.Schema<TransactionHashType, string> =
	S.transformOrFail(S.String, TransactionHashTypeSchema, {
		strict: true,
		...
		encode: (hash, _options, ast) => {
			try {
				...
				return ParseResult.fail(
					new ParseResult.Type(ast, hash, (e as Error).message),
				);
			}
		},
 ...
 * @since 0.0.1
 */
export const TransactionHashFromBytesSchema: S.Schema<
	TransactionHashType,
	Uint8Array
> = S.transformOrFail(S.Uint8ArrayFromSelf, TransactionHashTypeSchema, {
	...
	decode: (bytes, _options, ast) => {
		try {
			return ParseResult.succeed(
				TransactionHash.from(bytes) as unknown as TransactionHashType,
			);
		} catch (e) {
			return ParseResult.fail(
