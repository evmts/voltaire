...
describe("decodeEventLog", () => {
	describe("decoding with known topics", () => {
		it.effect("decodes Transfer event log with known topics", () =>
			Effect.gen(function* () {
		...
		it.effect("decodes Approval event log with known topics", () =>
			Effect.gen(function* () {
				const value = 5000n;
				const data = `0x${value.toString(16).padStart(64, "0")}` as HexType;
				const topics = [
		...
		it.effect("decodes zero value correctly", () =>
			Effect.gen(function* () {
		...
		it.effect("decodes max uint256 correctly", () =>
			Effect.gen(function* () {
