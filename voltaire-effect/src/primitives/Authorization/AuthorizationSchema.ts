...
export type AuthorizationInput = {
  /** Chain ID as string, number, or bigint */
  chainId: string | number | bigint
  /** Contract address to authorize as hex string */
  address: string
  /** Account nonce as string, number, or bigint */
  nonce: string | number | bigint
  /** Signature y-parity (0 or 1) */
  yParity: number
  /** Signature r value as hex string */
  r: string
  /** Signature s value as hex string */
  s: string
}
    ...
    decode: (input, _options, ast) => {
      try {
        const auth: AuthorizationType = {
          ...
          address: hexToBytes(input.address, 20) as AuthorizationType['address'],
...
).annotations({ identifier: 'AuthorizationSchema' })
