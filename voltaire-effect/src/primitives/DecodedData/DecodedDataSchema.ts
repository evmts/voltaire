 ...
 * @since 0.0.1
 */
export const DecodedDataSchema: S.Schema<DecodedDataType, DecodedDataInput> = S.transformOrFail(
  S.Struct({
    values: S.Unknown,
  ...
  DecodedDataTypeSchema,
  {
    ...
    decode: (input, _options, ast) => {
      try {
        return ParseResult.succeed(DecodedData.from(input.values, input.types))
      } catch (e) {
        return ParseResult.fail(new ParseResult.Type(ast, input, (e as Error).message))
      }
    },
    encode: (data) => ParseResult.succeed({
      values: data.values,
      types: data.types,
    })
  }
