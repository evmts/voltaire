...
describe("arithmetic operations", () => {
	...
	describe("minus", () => {
		it("subtracts two values", () => {
			const result = Uint.minus(a, b);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(70n);
		});

		it("handles zero", () => {
			const result = Uint.minus(a, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(100n);
		});

		it("wraps on underflow", () => {
			const result = Uint.minus(zero, one);
	...
	describe("times", () => {
		it("multiplies two values", () => {
			const result = Uint.times(a, b);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(3000n);
		});

		it("handles zero", () => {
			const result = Uint.times(a, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});

		it("handles one", () => {
			const result = Uint.times(a, one);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(100n);
		});

		it("wraps on overflow", () => {
			const result = Uint.times(max, S.decodeSync(Uint.BigInt)(2n));
...
describe("comparison operations", () => {
	...
	describe("lessThan", () => {
		...
		it("returns false when a >= b", () => {
			...
			expect(Uint.lessThan(a, c)).toBe(false);
		});
	});
	...
	describe("greaterThan", () => {
		...
		it("returns false when a <= b", () => {
			expect(Uint.greaterThan(b, a)).toBe(false);
			expect(Uint.greaterThan(a, c)).toBe(false);
		});
	});
	...
	describe("lessThanOrEqual", () => {
		it("returns true when a <= b", () => {
			...
			expect(Uint.lessThanOrEqual(a, c)).toBe(true);
		});
	...
	describe("greaterThanOrEqual", () => {
		...
		it("returns false when a < b", () => {
			expect(Uint.greaterThanOrEqual(b, a)).toBe(false);
		});
	});
});
...
describe("bitwise operations", () => {
	...
	describe("bitwiseAnd", () => {
		...
		it("AND with zero is zero", () => {
			const result = Uint.bitwiseAnd(a, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
	});
	...
	describe("bitwiseOr", () => {
		...
		it("OR with zero is identity", () => {
			const result = Uint.bitwiseOr(a, zero);
	...
	describe("bitwiseNot", () => {
		it("performs NOT operation", () => {
			const result = Uint.bitwiseNot(zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(MAX_UINT256);
		});

		it("NOT of max is zero", () => {
			const result = Uint.bitwiseNot(max);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
	});
	...
	describe("shiftRight", () => {
		it("shifts right", () => {
			...
			const bits = S.decodeSync(Uint.BigInt)(8n);
			const result = Uint.shiftRight(val, bits);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(1n);
		});

		it("shift by zero is identity", () => {
			const result = Uint.shiftRight(a, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0b1100n);
		});
	});
});
...
describe("utility functions", () => {
	...
	describe("isZero", () => {
		it("returns true for zero", () => {
			expect(Uint.isZero(zero)).toBe(true);
		});

		it("returns false for non-zero", () => {
			expect(Uint.isZero(one)).toBe(false);
		});
	});

	describe("isPowerOf2", () => {
		it("returns true for powers of 2", () => {
			expect(Uint.isPowerOf2(one)).toBe(true);
			expect(Uint.isPowerOf2(two)).toBe(true);
			expect(Uint.isPowerOf2(sixteen)).toBe(true);
		});

		it("returns false for non-powers of 2", () => {
			expect(Uint.isPowerOf2(zero)).toBe(false);
			expect(Uint.isPowerOf2(fifteen)).toBe(false);
		});
	});
	...
	describe("bitLength", () => {
		it("returns correct bit length", () => {
			...
			expect(Uint.bitLength(fifteen)).toBe(4);
			expect(Uint.bitLength(sixteen)).toBe(5);
		});
	});
	...
	describe("clone", () => {
		it("creates a copy with same value", () => {
			...
			const cloned = Uint.clone(original);
...
describe("edge cases", () => {
	describe("zero", () => {
		it("zero + zero = zero", () => {
			const zero = S.decodeSync(Uint.BigInt)(0n);
			const result = Uint.plus(zero, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
		...
		it("zero * anything = zero", () => {
			const zero = S.decodeSync(Uint.BigInt)(0n);
			const max = S.decodeSync(Uint.BigInt)(MAX_UINT256);
			const result = Uint.times(zero, max);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
	});
	...
	describe("max uint256", () => {
		it("max + max wraps correctly", () => {
			const max = S.decodeSync(Uint.BigInt)(MAX_UINT256);
			const result = Uint.plus(max, max);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(MAX_UINT256 - 1n);
		});

		it("max AND max = max", () => {
			const max = S.decodeSync(Uint.BigInt)(MAX_UINT256);
			const result = Uint.bitwiseAnd(max, max);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(MAX_UINT256);
		});

		it("max XOR max = zero", () => {
			const max = S.decodeSync(Uint.BigInt)(MAX_UINT256);
			const result = Uint.bitwiseXor(max, max);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
	});
	...
	describe("shift edge cases", () => {
		...
		it("shift right by 256 or more results in zero", () => {
			const max = S.decodeSync(Uint.BigInt)(MAX_UINT256);
			const bits = S.decodeSync(Uint.BigInt)(256n);
			const result = Uint.shiftRight(max, bits);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});
	});
});
...
describe("additional function tests", () => {
	...
	describe("toPower", () => {
		it("computes 2^10", () => {
			const result = Uint.toPower(two, ten);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(1024n);
		});

		it("any number to power 0 is 1", () => {
			const zero = S.decodeSync(Uint.BigInt)(0n);
			const result = Uint.toPower(ten, zero);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(1n);
		});

		it("any number to power 1 is itself", () => {
			const result = Uint.toPower(ten, one);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(10n);
		});

		it("0 to any positive power is 0", () => {
			const zero = S.decodeSync(Uint.BigInt)(0n);
			const result = Uint.toPower(zero, ten);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(0n);
		});

		it("1 to any power is 1", () => {
			const result = Uint.toPower(one, ten);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(1n);
		});
	});
	...
	describe("notEquals", () => {
		it("returns true for different values", () => {
			expect(Uint.notEquals(one, two)).toBe(true);
		});

		it("returns false for equal values", () => {
			const another = S.decodeSync(Uint.BigInt)(1n);
			expect(Uint.notEquals(one, another)).toBe(false);
		});
	});
});
...
describe("additional edge cases", () => {
	describe("boundary values", () => {
		...
		it("adding 1 to max-1 gives max", () => {
			const almostMax = S.decodeSync(Uint.BigInt)(MAX_UINT256 - 1n);
			const one = S.decodeSync(Uint.BigInt)(1n);
			const result = Uint.plus(almostMax, one);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(MAX_UINT256);
		});
	});
	...
	describe("bitLength edge cases", () => {
		...
		it("bitLength of 2^255 is 256", () => {
			...
			expect(Uint.bitLength(val)).toBe(256);
		});
	});
	...
	describe("lcm edge cases", () => {
		it("lcm of 1 and any number is that number", () => {
			const one = S.decodeSync(Uint.BigInt)(1n);
			const val = S.decodeSync(Uint.BigInt)(42n);
			const result = Uint.lcm(one, val);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(42n);
		});

		it("lcm of same value is that value", () => {
			const val = S.decodeSync(Uint.BigInt)(42n);
			const result = Uint.lcm(val, val);
			expect(S.encodeSync(Uint.BigInt)(result)).toBe(42n);
		});
	});
