...
const getTokenBalance = (token: string, user: string) => Effect.gen(function* () {
  ...
  const { balance, decimals } = yield* Effect.all({
    balance: contract.read.balanceOf(user),
    decimals: contract.read.decimals()
  })
...
const makeSignerLayer = (privateKey: Uint8Array) =>
  Layer.mergeAll(Signer.Live, CryptoLayer, ProviderLayer).pipe(
...
}).pipe(Effect.provide(makeSignerLayer(privateKey)))
```
...
const waitForTx = (txHash: string, confirmations = 1, timeout = "120 seconds") =>
  waitForTransactionReceipt(txHash, { confirmations }).pipe(
    withTimeout(timeout),
...
const makeProviderLayer = (url: string) => Provider.pipe(
...
const withFallback = <A>(operation: Effect.Effect<A, any, any>) =>
  providers.reduce(
    (acc, url, i) => i === 0
      ? operation.pipe(Effect.provide(makeProviderLayer(url)))
      : acc.pipe(
          Effect.orElse(() => operation.pipe(Effect.provide(makeProviderLayer(url))))
        ),
