...
const program: Effect.Effect<bigint, ProviderError, ProviderService>
...
// Effect.Effect<bigint, ProviderError, never>
...
const FailingProvider = Layer.succeed(ProviderService, {
  getBlockNumber: () => Effect.fail(new ProviderError('RPC timeout')),
  getBalance: () => Effect.fail(new ProviderError('Rate limited'))
})
