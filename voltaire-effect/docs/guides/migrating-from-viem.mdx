...
voltaire-effect provides key advantages over viem and ethers:
...
const program = Effect.gen(function* () {
  const provider = yield* ProviderService
  return yield* provider.getBalance(address)
})
...
const program = Effect.gen(function* () {
  const provider = yield* ProviderService
  return yield* provider.readContract({
...
const watchTransfers = Effect.gen(function* () {
  const provider = yield* ProviderService
  const stream = yield* provider.watchEvent({
    address: contractAddress,
...
import { Provider, HttpTransport } from 'voltaire-effect/services'
import { Secp256k1Live, KeccakLive } from 'voltaire-effect/crypto'
