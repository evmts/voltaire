...
ProviderService is a **convenience composition** of focused services:
`BlocksService`, `ProviderAccountService` (node account ops and signing), `TransactionService`,
`SimulationService`, `EventsService`, `NetworkService`, and `StreamingService`.
You can depend on any of those tags directly if you want narrower dependencies.
...
const program = Effect.gen(function* () {
  ...
  const balance = yield* provider.getBalance('0x1234...').pipe(
    withTimeout('5 seconds')
  )
  
  // Custom retry schedule for critical calls
  const block = yield* provider.getBlock({ blockTag: 'latest' }).pipe(
...
```typescript
import { BlocksService, Blocks } from 'voltaire-effect'

const program = Effect.gen(function* () {
  const blocks = yield* BlocksService
  return yield* blocks.getBlockNumber()
}).pipe(
  Effect.provide(Blocks),
  Effect.provide(Provider),
...
const receipts = yield* provider.getBlockReceipts?.({ blockTag: 'latest' })
const uncleCount = yield* provider.getUncleCount?.({ blockTag: 'latest' })
```
...
const txByIndex = yield* provider.getTransactionByBlockHashAndIndex?.('0x...', 0)
const txByNumber = yield* provider.getTransactionByBlockNumberAndIndex?.('latest', 0)
...
```typescript
const txHash = yield* provider.sendTransaction?.({
  from: '0x1234...',
  to: '0xabcd...',
  value: 1_000_000_000_000_000_000n
})

const sig = yield* provider.sign?.('0x1234...', '0x...')
const signed = yield* provider.signTransaction?.({
  from: '0x1234...',
  to: '0xabcd...',
  value: 1_000_000_000_000_000_000n
})
...
ProviderService includes access to the raw simulation RPCs when supported by your node:

```typescript
const simV1 = yield* provider.simulateV1?.({
  blockStateCalls: [
    { calls: [{ to: tokenAddress, data: '0x...' }] }
  ]
})

const simV2 = yield* provider.simulateV2?.({ calls: [{ to: tokenAddress }] })
```
...
```typescript
const stream = provider.watchBlocks()
const subId = yield* provider.subscribe?.('newHeads')
const unsub = yield* provider.unsubscribe?.(subId as `0x${string}`)
```
...
```typescript
const syncing = yield* provider.getSyncing?.()
const accounts = yield* provider.getAccounts?.()
const coinbase = yield* provider.getCoinbase?.()
const version = yield* provider.netVersion?.()
```

## Mining & Protocol (optional)

```typescript
const protocol = yield* provider.getProtocolVersion?.()
const mining = yield* provider.getMining?.()
const hashrate = yield* provider.getHashrate?.()
const work = yield* provider.getWork?.()

const submitted = yield* provider.submitWork?.(
  '0x0000000000000001',
  '0xpowHash',
  '0xmixDigest'
)
const submitHashrate = yield* provider.submitHashrate?.('0xhashrate', '0xid')
```
...
Each method has a method-specific error alias (for example `GetBlockNumberError`), which unions `TransportError` with relevant provider errors.
...
```typescript
import { OpStackActionsService, OpStackActions } from 'voltaire-effect'
...
- [Engine API Service](/services/engine-api) — engine_* RPC methods for consensus integration
- [Signer Service](/services/signer) — Transaction signing and sending
- [Multicall](/services/multicall) — Batch multiple calls
