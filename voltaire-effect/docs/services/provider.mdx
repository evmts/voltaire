...
type ProviderShape = {
  ...
  readonly call: (tx: CallRequest, blockTag?: BlockTag) => Effect.Effect<HexType | `0x${string}`, CallError>
  readonly estimateGas: (tx: CallRequest) => Effect.Effect<bigint, EstimateGasError>
