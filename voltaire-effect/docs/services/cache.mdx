...
```typescript
import { Effect } from 'effect'
import { ProviderService, withoutCache } from 'voltaire-effect'

const program = Effect.gen(function* () {
  const provider = yield* ProviderService
  
  // Force fresh fetch, bypassing any request cache
  const blockNum = yield* provider.getBlockNumber().pipe(withoutCache)
  return blockNum
})
```
...
import { Provider, MemoryCache, HttpTransport } from 'voltaire-effect'
