...
import { RpcBatchService, RpcBatch, EthBlockNumber, EthGetBalance, HttpTransport } from 'voltaire-effect/services'
...
const program = Effect.gen(function* () {
  ...
  const [blockNumber, balance1, balance2] = yield* Effect.all([
    ...
    batch.request(new EthGetBalance({ address: '0x1234...', blockTag: 'latest' })),
    batch.request(new EthGetBalance({ address: '0x5678...', blockTag: 'latest' })),
...
const program = Effect.gen(function* () {
  ...
  const results = yield* Effect.all([
    batch.request(new EthGetBalance({ address: validAddr, blockTag: 'latest' })),
    batch.request(new EthCall({ to: invalidContract, data: '0x...', blockTag: 'latest' })),
  ], { concurrency: 'unbounded', mode: 'either' })

  // results[0] might succeed while results[1] fails
...
import { RpcBatch, HttpTransport } from 'voltaire-effect/services'
