...
import { RateLimiterService, DefaultRateLimiter, TransportService } from 'voltaire-effect/services'
...
```typescript
import { RateLimitError } from 'voltaire-effect/services'
...
```typescript
import { NoopRateLimiter } from 'voltaire-effect/services'
...
import { Provider, DefaultRateLimiter, HttpTransport } from 'voltaire-effect/services'
  ...
  return yield* Effect.all(
    blockNumbers.map((n) =>
      rateLimiter.withRateLimit(
        ...
        provider.getBlock({ blockTag: `0x${n.toString(16)}` })
      )
    ),
