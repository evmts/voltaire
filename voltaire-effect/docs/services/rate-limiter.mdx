...
import { ProviderService, withTimeout, withRetrySchedule, withTracing } from 'voltaire-effect'

const program = Effect.gen(function* () {
  const provider = yield* ProviderService
  
  // 5 second timeout with custom retry
  const balance = yield* provider.getBalance(addr).pipe(
  ...
  const debugBlock = yield* provider.getBlock({ blockTag: 'latest' }).pipe(
    withTracing(true)
  )
