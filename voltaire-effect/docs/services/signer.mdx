...
type TransactionRequest = {
  ...
  readonly nonce?: bigint              // auto-filled from getTransactionCount
...
3. **If neither provided** → Legacy with auto-fetched gasPrice

When using EIP-1559, if `maxFeePerGas` is not provided, it's calculated as:
`baseFee + maxPriorityFeePerGas` (no 2x multiplier currently)

### Concurrency Warning

**⚠️ Signer is NOT safe for concurrent sends without NonceManager.**

Each `sendTransaction` call fetches the current nonce. If you send multiple transactions concurrently, they may get the same nonce and all but one will fail.

For concurrent sends, either:
...
type SignerShape = {
  ...
  readonly sendTransaction: (tx: TransactionRequest) => Effect.Effect<HexType, SignerError>
  readonly sendRawTransaction: (signedTx: HexType) => Effect.Effect<HexType, SignerError>
...
// HexType is a branded Uint8Array - the return from sendTransaction is the tx hash
type HexType = Uint8Array & { readonly __tag: "Hex" }
```
