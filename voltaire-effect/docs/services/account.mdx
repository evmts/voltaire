...
<Info>
AccountService is for local or wallet signing. For node account RPCs like `eth_accounts`, `eth_coinbase`, or `eth_sign*`, use `ProviderAccountService` (or `ProviderService`).
</Info>
