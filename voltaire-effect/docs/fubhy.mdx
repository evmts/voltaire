...
## 5. Provider Service (Large Composed Service)

### Composition via Type Intersection

<Note>
**INTERNAL**: ProviderService is a facade composed from sub-service shapes.
</Note>
...
### Live Implementation Pattern

```typescript
// INTERNAL: src/services/Provider/Provider.ts
      ...
      return {
        getBlockNumber: () =>
          request<string>("eth_blockNumber").pipe(
            Effect.flatMap((hex) => parseHexToBigInt({ method: "eth_blockNumber", response: hex })),
          ),
        
        getBalance: (address: AddressInput, blockTag: BlockTag = "latest") =>
          request<string>("eth_getBalance", [toAddressHex(address), blockTag]).pipe(
            Effect.flatMap((hex) => parseHexToBigInt({ method: "eth_getBalance", response: hex })),
          ),
...
**Question**: Should we split into multiple smaller services instead of one large ProviderService?
...
1. **Schema.declare**: Right for Uint8Array branded types, or use `Schema.transform`?
2. **Context.Tag pattern**: `class extends Context.Tag(...)` vs `Context.GenericTag`?
3. **Layer composition**: `Layer.mergeAll` for 15+ services?
4. **TaggedError constructors**: Custom constructors vs factory functions?
5. **Large services**: One ProviderService with 50+ methods vs smaller focused services?
6. **Runtime.runPromise**: Correct for bridging to async iterators?
7. **FiberRef.unsafeMake**: Module-level for default configs?
8. **Effect.try**: vs `Effect.sync` + `Effect.catchAll`?
9. **@effect/platform**: Should we use HttpClient everywhere or allow fetch fallback?
10. **RequestResolver**: Is our batching pattern idiomatic?
11. **acquireRelease**: Right pattern for secure key zeroing?
...
- **ProviderService** - High-level provider interface (composed from sub-services)
- **BlocksService** - Block fetching/queries
- **TransactionService** - Transaction submission/tracking
- **NetworkService** - Network/chain info
- **EventsService** - Event log queries
- **StreamingService** - Subscription streaming
- **AccountService** (Provider) - Account balance/nonce queries
- **SimulationService** - Transaction simulation
...
| **Provider** | ProviderService composed from BlocksShape, AccountShape, TransactionShape, etc. |
