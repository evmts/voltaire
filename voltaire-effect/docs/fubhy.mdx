...
export const getBlockNumber = () =>
  Effect.gen(function* () {
    ...
    const hex = yield* transport.request<string>("eth_blockNumber")
    return yield* parseHexToBigInt({ method: "eth_blockNumber", response: hex })
  })
...
export const getBalance = (address: AddressInput, blockTag: BlockTag = "latest") =>
  Effect.gen(function* () {
    const transport = yield* TransportService
    const hex = yield* transport.request<string>("eth_getBalance", [toAddressHex(address), blockTag])
    return yield* parseHexToBigInt({ method: "eth_getBalance", response: hex })
  })
...
**Note**: We use free functions instead of a ProviderService facade. Each function depends on TransportService internally.
