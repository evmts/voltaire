...
function getBalance(address: AddressType): Effect<bigint, RpcError | NetworkError>
```

Now TypeScript tells you: this can fail with `RpcError` or `NetworkError`. Handle them or propagate them—the compiler tracks it.
...
import { Provider, MainnetLayer } from 'voltaire-effect/services/Provider'

const program = Effect.gen(function* () {
  // Decode with typed ParseError
  const addr = yield* S.decode(Address.Hex)(input)

  // RPC with typed RpcError | NetworkError
  const provider = yield* Provider
  const balance = yield* provider.getBalance(addr)

  return { addr, balance }
})
// Effect<{addr, balance}, ParseError | RpcError | NetworkError, Provider>
```
...
program.pipe(
  Effect.catchTags({
    ...
    RpcError: (e) => retryWithBackoff(program),
    NetworkError: (e) => useFallbackProvider(program),
  })
)
```
...
// Effect<Receipt, ParseError | SignerError | RpcError, Signer>
```
