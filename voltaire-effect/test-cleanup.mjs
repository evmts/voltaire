...
async function testWithPrivateKey() {
	...
	const program = Effect.gen(function* () {
		...
		yield* withPrivateKey(master, (key) =>
			Effect.sync(() => {
				capturedKey = key;
				console.log(
					"Inside withPrivateKey, key has data:",
					key.some((b) => b !== 0),
				);
			}),
		);
	...
	console.log(
		"After withPrivateKey, key is zeroed:",
...
async function testWithSeed() {
	...
	const program = Effect.gen(function* () {
		yield* withSeed(["abandon", "abandon"], (seed) =>
			Effect.sync(() => {
				capturedSeed = seed;
				console.log(
					"Inside withSeed, seed has data:",
					seed.some((b) => b !== 0),
				);
			}),
		);
	...
	await Effect.runPromise(program);
	console.log(
		"After withSeed, seed is zeroed:",
		capturedSeed.every((b) => b === 0),
	);
