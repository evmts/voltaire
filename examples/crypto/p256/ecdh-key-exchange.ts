...
console.log(`  Private: ${Hex.fromBytes(alicePrivate).slice(0, 40)}... (secret)`);
...
const secretsMatch = Hex.fromBytes(sharedSecretAlice) === Hex.fromBytes(sharedSecretBob);
...
console.log(`Session 1 secret: ${Hex.fromBytes(session1Secret).slice(0, 40)}...`);
...
console.log(`Session 2 secret: ${Hex.fromBytes(session2Secret).slice(0, 40)}...`);

const sessionSecretsMatch = Hex.fromBytes(session1Secret) === Hex.fromBytes(session2Secret);
...
console.log(`   - Client public: ${Hex.fromBytes(clientPublic).slice(0, 40)}...`);
...
console.log(`   - Server public: ${Hex.fromBytes(serverPublic).slice(0, 40)}...`);

console.log('\n3. Both parties compute shared secret:');
const tlsSharedSecret = P256.ecdh(clientPrivate, serverPublic);
console.log(`   - Shared secret: ${Hex.fromBytes(tlsSharedSecret).slice(0, 40)}...`);

console.log('\n4. Derive session keys using HKDF:');
const handshakeSecret = SHA256.hash(tlsSharedSecret);
console.log(`   - Handshake secret: ${Hex.fromBytes(handshakeSecret).slice(0, 40)}...`);
...
console.log(`Alice-Bob secret:   ${Hex.fromBytes(aliceBobSecret).slice(0, 40)}...`);

// Alice-Carol shared secret
const aliceCarolSecret = P256.ecdh(alicePrivate, carolPublic);
console.log(`Alice-Carol secret: ${Hex.fromBytes(aliceCarolSecret).slice(0, 40)}...`);

// Bob-Carol shared secret
const bobCarolSecret = P256.ecdh(bobPrivate, carolPublic);
console.log(`Bob-Carol secret:   ${Hex.fromBytes(bobCarolSecret).slice(0, 40)}...`);
