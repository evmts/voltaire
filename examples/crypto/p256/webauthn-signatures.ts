...
console.log(
	`RP ID hash (32 bytes): ${Hex.fromBytes(rpIdHash).slice(0, 20)}...`,
);
console.log(
	`Flags (1 byte): 0x${flags[0].toString(16).padStart(2, "0")} (UP=1, UV=1)`,
);
console.log(
	`Sign count (4 bytes): ${new DataView(signCount.buffer).getUint32(0)}`,
);
...
console.log(
	`Passkey signature: ${Hex.fromBytes(passkeySignature.r).slice(0, 20)}...`,
);
...
console.log(
	`X: ${Hex.fromBytes(authenticatorPublicKey.slice(0, 32)).slice(0, 40)}...`,
);
console.log(
	`Y: ${Hex.fromBytes(authenticatorPublicKey.slice(32, 64)).slice(0, 40)}...`,
);
