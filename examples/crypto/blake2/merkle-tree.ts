...
console.log(`   SHA-256: ${proof.length} × 32 bytes = ${proof.length * 32} bytes`);
console.log(`   Blake2-256: ${proof.length} × 32 bytes = ${proof.length * 32} bytes (same)`);
console.log(`   Blake2-128: ${proof.length} × 16 bytes = ${proof.length * 16} bytes (50% smaller!)`);
...
const newLeafHash = Blake2.hash(
  new TextEncoder().encode(updatedData[1]),
  32
);
