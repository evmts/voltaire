...
console.log('   r:', Buffer.from(validSignature.r).toString('hex').slice(0, 16) + '...');
console.log('   s:', Buffer.from(validSignature.s).toString('hex').slice(0, 16) + '...');
...
console.log('   s <= n/2:', sValue <= halfOrder ? '✓ Yes (low-s)' : '✗ No (high-s)');
...
validKey[31] = 42;
console.log('   Valid key (42):', Secp256k1.isValidPrivateKey(validKey) ? '✓ Yes' : '✗ No');

// Zero key (invalid)
const zeroKey = new Uint8Array(32);
console.log('   Zero key:', Secp256k1.isValidPrivateKey(zeroKey) ? '✓ Yes' : '✗ No (expected)');

// Key >= n (invalid)
const tooLargeKey = new Uint8Array([
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xfe, 0xba, 0xae, 0xdc, 0xe6, 0xaf, 0x48, 0xa0, 0x3b, 0xbf, 0xd2, 0x5e, 0x8c, 0xd0, 0x36,
	0x41, 0x41, // n (curve order) - invalid
]);
console.log('   Key >= n:', Secp256k1.isValidPrivateKey(tooLargeKey) ? '✓ Yes' : '✗ No (expected)');

// 7. Public key validation
console.log('\n7. Public Key Validation');

// Valid key (derived)
console.log('   Valid derived key:', Secp256k1.isValidPublicKey(publicKey) ? '✓ Yes' : '✗ No');

// Random bytes (likely not on curve)
const randomBytes = new Uint8Array(64);
crypto.getRandomValues(randomBytes);
console.log('   Random bytes:', Secp256k1.isValidPublicKey(randomBytes) ? '✓ Yes' : '✗ No (expected)');

// Wrong length
const wrongLength = new Uint8Array(32);
console.log('   Wrong length (32):', Secp256k1.isValidPublicKey(wrongLength) ? '✓ Yes' : '✗ No (expected)');
