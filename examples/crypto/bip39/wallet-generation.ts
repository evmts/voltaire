...
for (let i = 0; i < words.length; i += 6) {
	...
	console.log(
		`  ${row.map((w, j) => `${(i + j + 1).toString().padStart(2)}. ${w.padEnd(12)}`).join(" ")}`,
	);
}
...
console.log(
	`Match: ${Address.toHex(accounts[0].address) === Address.toHex(restoredAddress)}\n`,
);
...
console.log(
	`Different: ${Address.toHex(addrNoPass) !== Address.toHex(addrWithPass)}`,
);
...
for (let account = 0; account < 3; account++) {
	...
	for (let index = 0; index < 2; index++) {
		...
		console.log(
			`  Address ${index} (m/44'/60'/${account}'/0/${index}): ${Address.toHex(address)}`,
		);
	}
	console.log();
}
