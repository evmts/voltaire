...
console.log(`Winner: ${bobChoice === 'paper' && aliceChoice === 'rock' ? 'Bob' : 'Alice'}\n`);
...
const verifyCommitment = Keccak256.hashMultiple([
  verifyHash,
  timestampBytes,
]);

console.log(`Document matches: ${Hex.fromBytes(verifyHash) === Hex.fromBytes(documentHash)}`);
console.log(`Commitment matches: ${Hex.fromBytes(verifyCommitment) === Hex.fromBytes(timestampedCommitment)}\n`);
...
console.log(`Proof valid: ${Hex.fromBytes(computedRoot) === Hex.fromBytes(root)}\n`);
...
console.log(`  Matches:    ${Hex.fromBytes(output) === Hex.fromBytes(verifyOutput)}\n`);
...
for (let i = chain.length - 2; i >= 0; i--) {
  ...
  console.log(`  Password ${chain.length - 1 - i}: ${Hex.fromBytes(password).slice(0, 12)}... (valid: ${valid})`);
}
...
console.log('Verification:');
console.log(`  Valid: ${Hex.fromBytes(response) === Hex.fromBytes(expectedResponse)}`);
