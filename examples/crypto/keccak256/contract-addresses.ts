...
for (const nonce of [0n, 1n, 5n, 10n, 100n]) {
  ...
  console.log(`Nonce ${nonce.toString().padStart(3)}: ${Address.toHex(contractAddr)}`);
}
...
console.log('-'.repeat(40));
console.log('Formula: keccak256(0xff ++ sender ++ salt ++ keccak256(initCode))[12:]\n');
...
for (const saltValue of [0n, 1n, 42n, 12345n]) {
  ...
  const contractAddr = Keccak256.create2Address(factory, saltBytes, initCodeHash);
  console.log(`Salt ${saltValue.toString().padStart(5)}: ${Address.toHex(contractAddr)}`);
}
...
const create2Addr = Keccak256.create2Address(deployer2, saltZero, initCodeHash2);
...
for (const { name, value } of salts) {
  ...
  const addr = Keccak256.create2Address(factoryAddr, saltBytes, factoryInitCodeHash);
...
const sequential = Keccak256.create2Address(factoryAddr2, salt1, strategyInitCodeHash);
...
const timestampAddr = Keccak256.create2Address(factoryAddr2, salt2, strategyInitCodeHash);
...
const userAddr = Keccak256.create2Address(factoryAddr2, userSalt, strategyInitCodeHash);
...
const uniswapFactory = Address.from('0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f');
...
const pairAddr = Keccak256.create2Address(uniswapFactory, pairSaltBytes, pairInitCodeHash);

console.log(`Factory:        ${Address.toHex(uniswapFactory)}`);
console.log(`Salt:           ${pairSalt}`);
console.log(`Init Code Hash: ${Hex.fromBytes(pairInitCodeHash).slice(0, 20)}...`);
