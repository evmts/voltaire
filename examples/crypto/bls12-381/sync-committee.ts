...
console.log('First validator pubkey (compressed):', Buffer.from(syncCommittee[0].pubKeyBytes).toString('hex').slice(0, 32), '...');
...
const participationBits = Array.from({ length: SYNC_COMMITTEE_SIZE }, () =>
  Math.random() < PARTICIPATION_RATE
);
const participantCount = participationBits.filter(p => p).length;

console.log('Expected participation:', (PARTICIPATION_RATE * 100).toFixed(1), '%');
console.log('Actual participants:', participantCount, '/', SYNC_COMMITTEE_SIZE);
console.log('Participation rate:', (participantCount / SYNC_COMMITTEE_SIZE * 100).toFixed(1), '%');
...
console.log('Bitfield (first 16 bytes):', Buffer.from(participationBytes.subarray(0, 16)).toString('hex'), '...\n');

// 4. Collect Signatures
console.log('4. Collect Signatures from Participants');
console.log('-'.repeat(60));

const participatingValidators = syncCommittee.filter((_, i) => participationBits[i]);
const signatures = participatingValidators.map(v => signMessage(v.privKey, blockRoot));

console.log('Signatures collected:', signatures.length);
console.log('First signature (x):', signatures[0].toAffine().x.toString(16).slice(0, 16), '...');
console.log('Last signature (x):', signatures[signatures.length - 1].toAffine().x.toString(16).slice(0, 16), '...\n');
...
console.log('Individual signatures:', signatures.length, '× 48 bytes =', signatures.length * 48, 'bytes');
console.log('Aggregated signature: 1 × 48 bytes = 48 bytes');
console.log('Compression ratio:', (signatures.length * 48 / 48).toFixed(1) + 'x');
console.log('Aggregation time:', aggTime, 'ms');
console.log('\nAggregated signature (x):', aggregatedSignature.toAffine().x.toString(16).slice(0, 16), '...\n');
...
const totalSize = syncAggregate.syncCommitteeBits.length + syncAggregate.syncCommitteeSignature.length;
...
console.log('Bandwidth savings:', (1 - totalSize / (participantCount * 48)).toFixed(2) + 'x or',
...
console.log('\nvs individual verification:', participantCount, '× ~2ms ≈', participantCount * 2, 'ms');
console.log('Speedup:', Math.round(participantCount * 2 / verifyTime) + 'x faster\n');
...
console.log('Supermajority threshold: 2/3 =', SUPERMAJORITY_THRESHOLD, 'validators');
console.log('Actual participants:', participantCount);
console.log('Has supermajority:', hasSupermajority);
console.log('\nLight client accepts block:', hasSupermajority && valid ? 'YES' : 'NO\n');
...
const periodDuration = EPOCHS_PER_SYNC_COMMITTEE_PERIOD * SLOTS_PER_EPOCH * SECONDS_PER_SLOT;
const hours = periodDuration / 3600;

console.log('Sync committee period:', EPOCHS_PER_SYNC_COMMITTEE_PERIOD, 'epochs');
...
console.log('Bandwidth per day:', (bandwidthPerDay / 1024 / 1024).toFixed(2), 'MB');
console.log('\nvs individual signatures per day:', (syncAggregatePerDay * participantCount * 48 / 1024 / 1024).toFixed(2), 'MB');
console.log('Daily bandwidth savings:', ((syncAggregatePerDay * participantCount * 48 - bandwidthPerDay) / 1024 / 1024).toFixed(2), 'MB\n');
