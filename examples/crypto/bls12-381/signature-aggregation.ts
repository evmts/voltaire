...
validators.forEach(v => {
  console.log(`Validator ${v.index}: privkey=${v.privKey}, pubkey=(${v.pubKey.toAffine().x.c0.toString(16).slice(0, 12)}...)`);
});
console.log();
...
const signatures = validators.map(v => {
  ...
  console.log(`Validator ${v.index} signature: ${sig.toAffine().x.toString(16).slice(0, 12)}...`);
  return sig;
});
console.log();
...
console.log('Bandwidth savings:', ((signatures.length * 48 - 48) / (signatures.length * 48) * 100).toFixed(1), '%');
console.log('\nAggregated signature:', aggregatedSignature.toAffine().x.toString(16).slice(0, 16), '...\n');
...
console.log('Aggregated public key (x.c0):', aggregatedPubKey.toAffine().x.c0.toString(16).slice(0, 16), '...\n');
...
console.log('vs individual verification:', numValidators * 2, 'pairings (~' + numValidators * 2 + 'ms)');
...
console.log('\nIncremental matches full aggregate:', incrementalSig.equals(aggregatedSignature));
...
const participation = new Array(syncCommitteeSize).fill(true).map((_, i) =>
  Math.random() > 0.1 // 90% participation rate
);
const participantCount = participation.filter(p => p).length;

console.log('Sync committee size:', syncCommitteeSize);
console.log('Participants:', participantCount, '(' + (participantCount / syncCommitteeSize * 100).toFixed(1) + '%)');
...
console.log('Compression ratio:', (syncCommitteeSize * 48 / 112).toFixed(1) + 'x\n');
...
console.log('  Attacker chooses: pubkey_attack = pubkey_target - pubkey_honest');
...
function aggregateSignatures(sigs: typeof signatures): typeof aggregatedSignature {
...
function aggregatePublicKeys(pubKeys: typeof validators[0]['pubKey'][]): typeof aggregatedPubKey {
