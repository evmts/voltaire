...
console.log('Honest validator pubkey (x.c0):', honestPubKey.toAffine().x.c0.toString(16).slice(0, 16), '...');
console.log('Victim pubkey (x.c0):', victimPubKey.toAffine().x.c0.toString(16).slice(0, 16), '...');
console.log('Attacker rogue key (x.c0):', roguePubKey.toAffine().x.c0.toString(16).slice(0, 16), '...');

// Aggregate keys
const aggregatedPubKey = honestPubKey.add(roguePubKey);
console.log('\nAggregated pubkey equals victim pubkey:', aggregatedPubKey.equals(victimPubKey));
...
function generatePoP(privateKey: bigint, publicKey: typeof bls12_381.G2.Point.BASE): Uint8Array {
...
const validator1PrivKey = BigInt('0x' + randomBytes(32).toString('hex')) % bls12_381.fields.Fr.ORDER;
...
console.log('  Private key:', validator1PrivKey.toString(16).slice(0, 16), '...');
console.log('  Public key (compressed):', Buffer.from(validator1PubKey.toRawBytes(true)).toString('hex').slice(0, 32), '...');
console.log('  Proof of Possession:', Buffer.from(validator1PoP).toString('hex').slice(0, 32), '...');
...
console.log('Validator 1 PoP verification:', validator1Valid ? 'VALID ✓' : 'INVALID ✗');
...
console.log('Rogue key with valid PoP:', roguePopValid ? 'VALID ✓' : 'INVALID ✗\n');
...
console.log('  Public key:', Buffer.from(deposit.pubkey).toString('hex').slice(0, 32), '...');
console.log('  Withdrawal credentials:', Buffer.from(deposit.withdrawalCredentials).toString('hex').slice(0, 32), '...');
console.log('  Amount:', (deposit.amount / GWEI / 1_000_000_000n).toString(), 'ETH');
console.log('  Signature (PoP):', Buffer.from(deposit.signature).toString('hex').slice(0, 32), '...');
...
console.log('\nDeposit contract PoP verification:', depositValid ? 'ACCEPTED ✓' : 'REJECTED ✗\n');
...
console.log('  Total (1M validators):', (1_000_000 * popSize / 1024 / 1024).toFixed(2), 'MB');
