...
const privateKey1 = BigInt('0x' + randomBytes(32).toString('hex')) % bls12_381.fields.Fr.ORDER;
...
console.log('Public key 1 (x.c0):', publicKey1.toAffine().x.c0.toString(16).slice(0, 16), '...');
console.log('\nPrivate key 2:', privateKey2);
console.log('Public key 2 (x.c0):', publicKey2.toAffine().x.c0.toString(16).slice(0, 16), '...');
...
console.log('Public key 1 hex:', Buffer.from(pubKeyBytes1).toString('hex').slice(0, 32), '...\n');
...
console.log('Hashed to G1 point (x):', messageHash.toAffine().x.toString(16).slice(0, 16), '...');
...
console.log('Signature 1 (x):', signature1.toAffine().x.toString(16).slice(0, 16), '...');
console.log('Signature 2 (x):', signature2.toAffine().x.toString(16).slice(0, 16), '...');
...
console.log('Signature 1 hex:', Buffer.from(sigBytes1).toString('hex').slice(0, 32), '...\n');
...
console.log('Signature 2 (first):', signature2.toAffine().x.toString(16).slice(0, 16), '...');
console.log('Signature 2 (second):', signature2Again.toAffine().x.toString(16).slice(0, 16), '...');
...
console.log('\nSignature A (x):', signature2.toAffine().x.toString(16).slice(0, 16), '...');
console.log('Signature B (x):', signature3.toAffine().x.toString(16).slice(0, 16), '...');
...
console.log('Beacon DST hash:', hashBeacon.toAffine().x.toString(16).slice(0, 16), '...');
console.log('Attestation DST hash:', hashAttest.toAffine().x.toString(16).slice(0, 16), '...');
...
function signMessage(privKey: bigint, msg: Uint8Array): {
...
const { signature: testSig, publicKey: testPubKey } = signMessage(testPrivKey, testMessage);
