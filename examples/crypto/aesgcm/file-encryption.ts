...
console.log(`Overhead: ${encrypted.length - fileContent.length} bytes (salt + nonce + tag)`);
console.log(`Encrypted data: ${Buffer.from(encrypted).toString('hex').slice(0, 60)}...\n`);
...
console.log(`Match: ${Buffer.from(decrypted).toString('hex') === Buffer.from(fileContent).toString('hex')}\n`);
...
const decryptedImage = await decryptFile(encryptedWithMetadata, password, metadata);
...
for (const size of fileSizes) {
  ...
  console.log(`  ${size.toString().padStart(7)} bytes → ${enc.length.toString().padStart(7)} bytes (overhead: ${overhead} bytes)`);
}
console.log();
...
for (const { name, encrypted } of encryptedFiles) {
  ...
  console.log(`  Salt+Nonce: ${Buffer.from(encrypted.slice(0, 28)).toString('hex')}`);
}
...
console.log(`  Bytes  0-15: Salt (${encryptedSample.slice(0, 16).length} bytes)`);
console.log(`  Bytes 16-27: Nonce (${encryptedSample.slice(16, 28).length} bytes)`);
console.log(`  Bytes 28-end: Ciphertext+Tag (${encryptedSample.slice(28).length} bytes)`);
...
console.log(`Data preserved: ${Buffer.from(finalDecrypted).toString('hex') === Buffer.from(sensitiveData).toString('hex')}\n`);
