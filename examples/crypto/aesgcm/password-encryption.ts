...
console.log(
	`  Salt: ${Buffer.from(encrypted.salt).toString("hex")} (${encrypted.salt.length} bytes)`,
);
console.log(
	`  Nonce: ${Buffer.from(encrypted.nonce).toString("hex")} (${encrypted.nonce.length} bytes)`,
);
console.log(
	`  Ciphertext: ${Buffer.from(encrypted.ciphertext).toString("hex").slice(0, 40)}... (${encrypted.ciphertext.length} bytes)`,
);
...
console.log(
	`Password 1 ciphertext: ${Buffer.from(enc1.ciphertext).toString("hex").slice(0, 40)}...`,
);
console.log(
	`Password 2 ciphertext: ${Buffer.from(enc2.ciphertext).toString("hex").slice(0, 40)}...`,
);
console.log(
	`Different: ${Buffer.from(enc1.ciphertext).toString("hex") !== Buffer.from(enc2.ciphertext).toString("hex")}\n`,
);
...
console.log(
	`Different salts: ${Buffer.from(result1.salt).toString("hex") !== Buffer.from(result2.salt).toString("hex")}`,
);
console.log(
	`Different ciphertexts: ${Buffer.from(result1.ciphertext).toString("hex") !== Buffer.from(result2.ciphertext).toString("hex")}\n`,
);
...
console.log(
	`  Ciphertext+Tag: bytes 28-end (${encryptedData.ciphertext.length} bytes)`,
);
...
console.log(
	`Match: ${new TextDecoder().decode(extractedDecrypted) === new TextDecoder().decode(storageData)}\n`,
);
