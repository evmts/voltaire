...
console.log(`Nonce: ${Buffer.from(nonce).toString('hex')} (${nonce.length} bytes)`);

// Encrypt
const ciphertext = await AesGcm.encrypt(plaintext, key256, nonce);
console.log(`Ciphertext length: ${ciphertext.length} bytes (includes 16-byte auth tag)`);
...
for (const msg of messages) {
  ...
  console.log(`Length ${msgBytes.length.toString().padStart(4)}: encrypted to ${msgCt.length} bytes (overhead: ${msgCt.length - msgBytes.length} bytes)`);
}
console.log();
...
const allMatch = Buffer.from(ct1).toString('hex') === Buffer.from(ct2).toString('hex') &&
                 Buffer.from(ct2).toString('hex') === Buffer.from(ct3).toString('hex');

console.log('Same plaintext + key + nonce produces:');
console.log(`Ciphertext 1: ${Buffer.from(ct1).toString('hex').slice(0, 40)}...`);
console.log(`Ciphertext 2: ${Buffer.from(ct2).toString('hex').slice(0, 40)}...`);
console.log(`Ciphertext 3: ${Buffer.from(ct3).toString('hex').slice(0, 40)}...`);
