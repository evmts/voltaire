...
async function encryptMessage(
  ...
  const sharedSecret = X25519.scalarmult(ephemeral.secretKey, recipientPublicKey);

  // Derive encryption key using HKDF (CRITICAL: never use raw shared secret!)
  const derivedKey = hkdf(sha256, sharedSecret, undefined, 'x25519-messaging-v1', 32);
...
async function decryptMessage(
  ...
  const sharedSecret = X25519.scalarmult(recipientSecretKey, encrypted.ephemeralPublicKey);
  ...
  // Decrypt message
  const plaintext = await AesGcm.decrypt(encrypted.ciphertext, aesKey, encrypted.nonce);
...
console.log(`Alice public key: ${Hex.fromBytes(alice.publicKey).slice(0, 24)}...`);
console.log(`Bob public key:   ${Hex.fromBytes(bob.publicKey).slice(0, 24)}...\n`);

const message1 = 'Hello, Bob!';
const encrypted1 = await encryptMessage(message1, alice.secretKey, bob.publicKey);

console.log('Encrypted message:');
console.log(`  Ephemeral key: ${Hex.fromBytes(encrypted1.ephemeralPublicKey).slice(0, 24)}...`);
console.log(`  Nonce: ${Hex.fromBytes(encrypted1.nonce)}`);
console.log(`  Ciphertext: ${Hex.fromBytes(encrypted1.ciphertext).toString().slice(0, 40)}...`);
...
for (let i = 0; i < 3; i++) {
  ...
  console.log(`  Ephemeral key: ${Hex.fromBytes(enc.ephemeralPublicKey).slice(0, 24)}...`);
  console.log(`  Ciphertext: ${Hex.fromBytes(enc.ciphertext).toString().slice(0, 32)}...`);
}
...
const aliceToBob = await encryptMessage(aliceMsg, alice.secretKey, bob.publicKey);
...
const tamperedEncrypted = await encryptMessage(originalMsg, alice.secretKey, bob.publicKey);
...
for (const msg of messages) {
  ...
  console.log(`${msg.length.toString().padStart(4)} bytes → ${enc.ciphertext.length.toString().padStart(4)} bytes (overhead: ${overhead} bytes)`);
}
console.log();
...
const toBob = await encryptMessage(groupMessage, alice.secretKey, bob.publicKey);
const toCarol = await encryptMessage(groupMessage, alice.secretKey, carol.publicKey);
const toDave = await encryptMessage(groupMessage, alice.secretKey, dave.publicKey);

console.log('Encrypted for Bob:');
console.log(`  Ephemeral: ${Hex.fromBytes(toBob.ephemeralPublicKey).slice(0, 24)}...`);
console.log('Encrypted for Carol:');
console.log(`  Ephemeral: ${Hex.fromBytes(toCarol.ephemeralPublicKey).slice(0, 24)}...`);
console.log('Encrypted for Dave:');
console.log(`  Ephemeral: ${Hex.fromBytes(toDave.ephemeralPublicKey).slice(0, 24)}...`);
...
const storedEnc = await encryptMessage(storedMsg, alice.secretKey, bob.publicKey);
...
console.log(`  Ephemeral key: bytes 0-31 (${storedEnc.ephemeralPublicKey.length} bytes)`);
console.log(`  Nonce: bytes 32-43 (${storedEnc.nonce.length} bytes)`);
console.log(`  Ciphertext+Tag: bytes 44-end (${storedEnc.ciphertext.length} bytes)\n`);
...
console.log('✓ Compromised past keys: Cannot decrypt future messages (forward secrecy)');
console.log('✓ Compromised future keys: Cannot decrypt past messages (ephemeral keys)\n');
...
console.log('✓ Add signatures for authentication (X25519 provides only secrecy)');
