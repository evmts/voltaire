...
console.log(
	`Valid secret key (32 bytes): ${X25519.validateSecretKey(validSecretKey) ? "✓" : "✗"}`,
);
console.log(
	`Valid public key (32 bytes): ${X25519.validatePublicKey(validPublicKey) ? "✓" : "✗"}`,
);
...
console.log(
	`Invalid long key (64 bytes): ${X25519.validateSecretKey(invalidLong) ? "✗ Accepted" : "✓ Rejected"}\n`,
);
...
console.log(
	`Keypair 1 public: ${Hex.fromBytes(keypair1.publicKey).slice(0, 32)}...`,
);
console.log(
	`Keypair 2 public: ${Hex.fromBytes(keypair2.publicKey).slice(0, 32)}...`,
);
...
console.log(
	`  Alice: ${Hex.fromBytes(aliceKeypair.publicKey).slice(0, 24)}...`,
);
