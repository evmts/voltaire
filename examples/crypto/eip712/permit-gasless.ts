...
console.log(
	`  Deadline: ${new Date(Number(permit.message.deadline) * 1000).toISOString()}\n`,
);
...
console.log(
	`Match: ${Address.toHex(recoveredOwner).toLowerCase() === Address.toHex(permit.message.owner).toLowerCase()}`,
);
...
console.log(
	`  Deadline: ${new Date(Number(validPermit.message.deadline) * 1000).toISOString()}`,
);
console.log(`  Status: ✓ Valid (future deadline)`);

console.log("\nExpired permit:");
console.log(
	`  Deadline: ${new Date(Number(expiredPermit.message.deadline) * 1000).toISOString()}`,
);
