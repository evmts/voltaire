...
console.log(`Batch verification: ${batchValid ? '✓ All valid' : '✗ Some invalid'}`);
...
const batchInvalid = KZG.verifyBlobKzgProofBatch(blobs, commitments, corruptedProofs);

console.log(`Batch with corrupted proof: ${batchInvalid ? '✓ Valid (BAD!)' : '✗ Invalid (expected)'}`);
...
const maxValid = KZG.verifyBlobKzgProofBatch(maxBlobs, maxCommitments, maxProofs);
