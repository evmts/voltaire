...
console.log('  Max Fee Per Blob Gas:', blobTransaction.maxFeePerBlobGas / 1_000_000_000n, 'gwei');
...
console.log('  Commitments:', blobTransaction.commitments.length, '(96 bytes total)');
...
console.log('  Total on-chain: ~' + (txBodySize + versionedHashesSize), 'bytes');
console.log();
...
console.log('  Total sidecar:', (blobsSize + commitmentsSize + proofsSize), 'bytes');
console.log();
...
const executionGasCost = executionGasUsed * (executionBaseFee + blobTransaction.maxPriorityFeePerGas);
...
const allValid = hash1Valid && hash2Valid && commitment1HashValid &&
                commitment2HashValid && batchValid;
