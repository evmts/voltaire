...
console.log('\nPublic key format:');
console.log(`ssh-ed25519 ${Hex.fromBytes(keypair.publicKey).slice(0, 40)}... user@host`);
...
const keypair2 = Ed25519.keypairFromSeed(crypto.getRandomValues(new Uint8Array(32)));
const keypair3 = Ed25519.keypairFromSeed(crypto.getRandomValues(new Uint8Array(32)));

console.log('User can have multiple Ed25519 keys:');
console.log(`\nKey 1 (GitHub):  ${Hex.fromBytes(keypair.publicKey).slice(0, 40)}...`);
console.log(`Key 2 (Work):    ${Hex.fromBytes(keypair2.publicKey).slice(0, 40)}...`);
console.log(`Key 3 (Servers): ${Hex.fromBytes(keypair3.publicKey).slice(0, 40)}...`);
...
console.log(`Commit: ${new TextDecoder().decode(commitMessage.slice(0, 50))}...`);
console.log(`Signature: ${Hex.fromBytes(commitSig).slice(0, 40)}...`);

const verifyCommit = Ed25519.verify(commitSig, commitMessage, keypair.publicKey);
