...
console.log(
	`G2 Linearity check (7+11)×G2 = (7×G2)+(11×G2): ${g2LinearityHolds}\n`,
);
...
console.log(
	`First 8 bytes (x): ${Array.from(g1Serialized.slice(0, 8))
...
console.log(
	`First 8 bytes (x.c0): ${Array.from(g2Serialized.slice(0, 8))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...`,
);
...
console.log(
	`Distributive property: ${n}×G + ${m}×G = ${n + m}×G: ${BN254.G1.equal(sum, direct)}\n`,
);
