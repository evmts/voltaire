...
console.log(
	`  x (bytes 0-31):  0x${Array.from(g1Bytes.slice(0, 4))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...`,
);
console.log(
	`  y (bytes 32-63): 0x${Array.from(g1Bytes.slice(32, 36))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...\n`,
);
...
console.log(
	`  x.c0 (bytes 0-31):    0x${Array.from(g2Bytes.slice(0, 4))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...`,
);
console.log(
	`  x.c1 (bytes 32-63):   0x${Array.from(g2Bytes.slice(32, 36))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...`,
);
console.log(
	`  y.c0 (bytes 64-95):   0x${Array.from(g2Bytes.slice(64, 68))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...`,
);
console.log(
	`  y.c1 (bytes 96-127):  0x${Array.from(g2Bytes.slice(96, 100))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("")}...\n`,
);
...
try {
	...
	console.log(
		`Invalid point correctly rejected: ${error instanceof Error ? error.message : "validation failed"}\n`,
	);
}
