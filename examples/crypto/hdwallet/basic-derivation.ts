...
console.log(`  Can derive hardened children: ${HDWallet.canDeriveHardened(hardened)}`);
...
console.log(`  Public key: ${watchOnlyPubKey ? Hex.fromBytes(watchOnlyPubKey).slice(0, 32) + '...' : 'null'}`);
console.log(`  Private key: ${HDWallet.getPrivateKey(watchOnlyChild) ? 'Available' : 'Not available (expected)'}\n`);
...
for (const path of validPaths) {
  console.log(`  ${path.padEnd(20)} → ${HDWallet.isValidPath(path) ? '✓' : '✗'}`);
}

console.log('\nInvalid paths:');
for (const path of invalidPaths) {
  console.log(`  ${path.padEnd(20)} → ${HDWallet.isValidPath(path) ? '✗ Accepted (unexpected)' : '✓ Rejected'}`);
}
console.log();
...
console.log(`  "100'" → ${HDWallet.parseIndex("100'")} (hardened, 2^31 + 100)\n`);
