...
console.log('Valid MAC verification:', verifyHmac(authMessage, authKey, validMac));

// Tampered message
const tamperedMessage = new TextEncoder().encode('tampered message');
console.log('Tampered message verification:', verifyHmac(tamperedMessage, authKey, validMac));

// Tampered MAC
const tamperedMac = new Uint8Array(validMac);
tamperedMac[0] ^= 0x01; // Flip one bit
console.log('Tampered MAC verification:', verifyHmac(authMessage, authKey, tamperedMac));
console.log();
...
console.log('Long key (100 bytes):', new TextDecoder().decode(longKey).slice(0, 20) + '...');
...
console.log('Key (20 bytes):', Array.from(rfcKey.slice(0, 10)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '), '...');
console.log('Data:', new TextDecoder().decode(rfcData));
console.log('HMAC:', SHA256.toHex(rfcMac));
console.log('Expected: 0xb0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7');
console.log();
