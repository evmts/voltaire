...
console.log(
	"Results match:",
	SHA256.toHex(oneShotHash) === SHA256.toHex(streamHash),
);
console.log();
...
for (const chunkSize of chunkSizes) {
	...
	console.log(
		`Chunk size ${chunkSize.toString().padStart(3)}:`,
		SHA256.toHex(hash).slice(0, 20) + "...",
	);
}
...
function hashFileWithProgress(fileSize: number, chunkSize: number): Uint8Array {
	...
	while (processed < fileSize) {
		...
		if (processed === fileSize || processed % (chunkSize * 10) === 0) {
			console.log(
				`Progress: ${progress.toFixed(1)}% (${processed}/${fileSize} bytes)`,
			);
		}
	}
...
console.log(
	"Block size:",
	SHA256.BLOCK_SIZE,
	"bytes (internal processing unit)",
);
...
for (const size of blockAlignedChunks) {
	...
	console.log(
		`  ${size.toString().padStart(6)} bytes = ${blocks.toString().padStart(4)} blocks`,
	);
}
console.log();
...
console.log(
	"Inefficient (concatenate): Allocates",
	parts.reduce((s, p) => s + p.length, 0),
	"bytes",
);
