...
if (result3.success) {
	...
	console.log('Matches JavaScript calculation:', output === expected ? '✓ Yes' : '✗ No');
}
...
const testCases = [
	...
	{ base: 2n ** 64n, exp: 2n ** 32n, mod: 2n ** 64n + 1n, desc: 'Large (8+ bytes)' },
];

for (const tc of testCases) {
	const input = createModExpInput(tc.base, tc.exp, tc.mod);
	const result = execute(PrecompileAddress.MODEXP, input, 1000000n, Hardfork.CANCUN);
...
console.log('Input format: baseLen(32) || expLen(32) || modLen(32) || base || exp || mod');
