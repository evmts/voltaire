...
console.log(
	"Signer address:",
	"0x" + Buffer.from(signerAddress).toString("hex"),
);
...
console.log(
	"s value ≤ n/2:",
	s_value <= secp256k1_n_half ? "✓ Yes (EIP-2 compliant)" : "✗ No",
);
...
console.log(
	"Recovered signer:",
	"0x" + Buffer.from(wrongRecovered).toString("hex"),
);
console.log(
	"Signature from expected signer:",
	wrongMatch ? "✓ Yes" : "✗ No (correct)",
);
