...
console.log(`  Anonymity set: Up to 2^${MERKLE_TREE_HEIGHT} = ${2 ** MERKLE_TREE_HEIGHT} deposits\n`);
...
for (let level = 0; level < MERKLE_TREE_HEIGHT; level++) {
  ...
  const sibling = siblingIndex < deposits.length
    ? deposits[siblingIndex]
    : 0n;
...
const vk = {
  ...
  IC: [{ x: 1n, y: 2n }, { x: 3n, y: 4n }], // Per public input
};
...
pairingInput.set(serializeG2(proof.B.x1, proof.B.x2, proof.B.y1, proof.B.y2), 64);

// Pair 2: e(α, β)
pairingInput.set(serializeG1(vk.alpha.x, vk.alpha.y), 192);
pairingInput.set(serializeG2(vk.beta.x1, vk.beta.x2, vk.beta.y1, vk.beta.y2), 256);

// Pair 3: e(-L, γ)
const negL = negateG1(L.x, L.y);
pairingInput.set(serializeG1(negL.x, negL.y), 384);
pairingInput.set(serializeG2(vk.gamma.x1, vk.gamma.x2, vk.gamma.y1, vk.gamma.y2), 448);

// Pair 4: e(C, δ)
pairingInput.set(serializeG1(proof.C.x, proof.C.y), 576);
pairingInput.set(serializeG2(vk.delta.x1, vk.delta.x2, vk.delta.y1, vk.delta.y2), 640);
...
const TOTAL_GAS = PAIRING_GAS + MERKLE_CHECK_GAS + NULLIFIER_CHECK_GAS + STATE_UPDATE_GAS + TRANSFER_GAS;
...
console.log(`  Cost: ${(Number(TOTAL_GAS) * GAS_PRICE_GWEI / 1e9).toFixed(6)} ETH`);
