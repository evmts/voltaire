...
if (pkResult.success) {
	...
	if (mapResult.success) {
		...
		if (signResult.success) {
			...
			console.log(
				`  1. Public key derivation (G1_MUL):     ${pkResult.gasUsed} gas`,
			);
			console.log(
				`  2. Hash to G2 (MAP_FP2_TO_G2):         ${mapResult.gasUsed} gas`,
			);
			console.log(
				`  3. Sign message (G2_MUL):              ${signResult.gasUsed} gas`,
			);
			console.log(
				`  4. Verify signature (PAIRING):         ${verifyResult.gasUsed} gas`,
			);
			...
			console.log(
				`  Total:         ${(mapResult.gasUsed || 0n) + (verifyResult.gasUsed || 0n)} gas\n`,
			);
		}
	}
}
...
console.log(
	`  Size: ${NUM_VALIDATORS} × 96 bytes = ${NUM_VALIDATORS * 96} bytes`,
);
console.log(
	`  Gas: ${NUM_VALIDATORS} × 226k = ${NUM_VALIDATORS * 226000} gas\n`,
);
...
console.log(
	`  Savings: ${((1 - 226000 / (NUM_VALIDATORS * 226000)) * 100).toFixed(1)}%\n`,
);
