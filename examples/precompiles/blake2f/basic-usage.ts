...
if (result1.success) {
  ...
  console.log(`  Hash prefix: 0x${Array.from(hashPrefix).map(b => b.toString(16).padStart(2, '0')).join('')}`);
} else {
...
if (result2.success) {
  ...
  console.log(`  Blake2b("abc"): 0x${Array.from(hash).map(b => b.toString(16).padStart(2, '0')).join('')}`);
} else {
...
for (const rounds of roundTests) {
  ...
  if (result.success) {
    console.log(`  ${rounds.toString().padStart(4)} rounds → ${result.gasUsed} gas`);
  }
}
...
console.log(`Wrong input length (212): ${result4a.success ? 'unexpected success' : '✓ failed as expected'}`);
...
console.log(`Out of gas (50 gas for 100 rounds): ${result4b.success ? 'unexpected success' : '✓ failed as expected'}`);
...
console.log(`Invalid final flag (0x02): ${result4c.success ? 'unexpected success' : '✓ failed as expected'}`);
...
console.log(`Little-endian bytes: ${Array.from(leBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')}`);
