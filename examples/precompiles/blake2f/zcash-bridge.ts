...
const ZCASH_PERSONALIZATION = new Uint8Array([
  ...
  0xc8, 0x00, 0x00, 0x00, // n = 200 (little-endian u32)
  0x09, 0x00, 0x00, 0x00, // k = 9 (little-endian u32)
]);
...
console.log('Header hash:', '0x' + Array.from(headerHash).map(b => b.toString(16).padStart(2, '0')).join(''));
...
for (let i = 0; i < NUM_BLOCKS; i++) {
  ...
  header.set([i & 0xff, (i >> 8) & 0xff, (i >> 16) & 0xff, (i >> 24) & 0xff], 136); // nonce
...
for (let i = 0; i < 3; i++) {
  const hashHex = Array.from(blockHashes[i]).map(b => b.toString(16).padStart(2, '0')).join('');
...
console.log(`Reduction: ${((1 - Number(bridgeGas) / 3000000) * 100).toFixed(1)}%\n`);
