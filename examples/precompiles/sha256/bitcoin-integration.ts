...
if (firstHash.success) {
	console.log('First hash:', '0x' + Buffer.from(firstHash.output).toString('hex'));
	...
	if (blockHash.success) {
		console.log('Block hash (double SHA-256):', '0x' + Buffer.from(blockHash.output).toString('hex'));
		console.log('Total gas:', (firstHash.gasUsed + blockHash.gasUsed).toString());
	}
}
...
console.log('Public key (compressed):', '0x' + Buffer.from(publicKey).toString('hex'));
...
if (sha256Hash.success) {
	console.log('SHA-256(pubkey):', '0x' + Buffer.from(sha256Hash.output).toString('hex'));
	console.log('Next step: RIPEMD-160(SHA-256(pubkey)) - see RIPEMD160 examples');
...
console.log('Node 1 (tx1+tx2):', '0x' + Buffer.from(hash1_second.output).toString('hex').slice(0, 16) + '...');
...
console.log('Node 2 (tx3+tx4):', '0x' + Buffer.from(hash2_second.output).toString('hex').slice(0, 16) + '...');
...
console.log('Merkle root:', '0x' + Buffer.from(merkleRoot.output).toString('hex'));

// Calculate total gas for merkle tree
const totalMerkleGas = (
	hash1_first.gasUsed + hash1_second.gasUsed +
	hash2_first.gasUsed + hash2_second.gasUsed +
	root_first.gasUsed + merkleRoot.gasUsed
);
...
console.log('Difference:', (sha256Gas - keccak256Gas).toString(), 'gas more expensive');
console.log('\nRecommendation: Use Keccak256 for Ethereum-native hashing');
console.log('Use SHA-256 when interoperating with Bitcoin or other SHA-256 systems');

console.log('\n=== Summary ===');
console.log('Bitcoin block hash: 2 SHA-256 calls (~144 gas for 80-byte header)');
