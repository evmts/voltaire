...
if (result.success) {
	...
	console.log('RIPEMD-160 hash (20 bytes):', '0x' + Buffer.from(hash).toString('hex'));
	console.log('Full output (32 bytes):', '0x' + Buffer.from(result.output).toString('hex'));
...
if (emptyResult.success) {
	...
	console.log('Empty string hash:', '0x' + Buffer.from(emptyHash).toString('hex'));
...
for (const size of sizes) {
	...
	console.log(`${size.toString().padStart(4)} bytes: ${gas.toString().padStart(5)} gas (${w} words, ~${perByte.toFixed(2)} gas/byte)`);
}
