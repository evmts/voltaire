...
console.log('KZG commitment: 0x' + Array.from(commitment1.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join('') + '...');

// Compute versioned hash
const vHash1 = versionedHash(commitment1);
console.log('Versioned hash: 0x' + Array.from(vHash1.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join('') + '...\n');
...
console.log(`Verifying ${NUM_BLOBS} blobs (${NUM_BLOBS * BLOB_SIZE / 1024} KB total)\n`);
...
for (let i = 0; i < NUM_BLOBS; i++) {
  ...
  if (result.success) {
    ...
    console.log(`  Blob ${i + 1}: ${valid !== 0 ? '✓ VALID' : '✗ INVALID'} (${result.gasUsed} gas)`);
  } else {
...
console.log(`  Data cost: ${BLOB_DATA_SIZE} bytes × ${CALLDATA_GAS_PER_BYTE} gas/byte`);
...
console.log(`  Blob gas price: ~${blobGasPrice} wei (vs 10-50 gwei regular gas)`);
console.log(`  Effective savings: ~97% reduction\n`);

const savingsPercent = ((calldataGas - blobGas) / calldataGas * 100).toFixed(1);
