...
console.log(
	`  Versioned hash: 0x${Array.from(versionedHash1.slice(0, 4))
...
console.log(
	`  Version byte: 0x${correctHash[0].toString(16).padStart(2, "0")}\n`,
);
...
console.log(
	`Result: ${result4.success ? "unexpected success" : "✗ failed as expected"}`,
);
...
console.log(
	`Wrong length (191 bytes): ${resultLen.success ? "unexpected success" : "✓ failed"}`,
);
...
console.log(
	`Out of gas (40k for 50k): ${resultGas.success ? "unexpected success" : "✓ failed"}`,
);
