...
for (const jump of simpleJumps) {
  console.log(`  Position ${jump.position}: ${jump.valid ? 'valid' : 'invalid'}`);
}
...
const pushBytecode = '0x60ff' + // PUSH1 0xff
  '61abcd' + // PUSH2 0xabcd
  '7f' + '00'.repeat(32) + // PUSH32 (32 zero bytes)
  '5b'; // JUMPDEST
...
console.log('Contract bytecode length:', contractBytecode.length / 2 - 1, 'bytes');
...
while (pc < disasmBytes.length) {
  ...
  if (opcode >= 0x60 && opcode <= 0x7f) {
    ...
    console.log(`  ${pc.toString().padStart(4, '0')}: ${opcodeName} 0x${dataHex}`);
    pc += 1 + pushSize;
  } else {
...
const searchBytecode =
  '0x7f' + '1234567890'.repeat(6) + '123456' + // PUSH32
  '60ff' + // PUSH1 0xff
  '7f' + 'abcdefabcd'.repeat(6) + 'abcdef' + // PUSH32
  '00'; // STOP
