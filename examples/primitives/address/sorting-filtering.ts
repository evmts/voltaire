...
const inRange = allAddresses.filter(addr =>
  !addr.lessThan(min) && !addr.greaterThan(max)
);
...
const unique1 = withDuplicates.filter((addr, i, arr) =>
...
for (const [byte, addrs] of grouped.entries()) {
  console.log(`  0x${byte.toString(16).padStart(2, '0')}: ${addrs.length} addresses`);
...
addToIndex(
  Address.fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e"),
  { label: "Treasury", category: "Protocol" }
);
addToIndex(
  Address.fromHex("0xa0Cf798816D4b9b9866b5330EEa46a18382f251e"),
  { label: "Deployer", category: "Admin" }
);

// Lookup
const lookupAddr = Address.fromHex("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e");
