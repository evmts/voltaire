...
console.log(`Valid checksummed:   ${Address.isValidChecksum(validChecksummed)}`);
...
function safeParseAddress(input: string): Address | null {
  ...
  const hasMixedCase = input !== input.toLowerCase() && input !== input.toUpperCase();
...
try {
  const strictAddr = requireChecksum("0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e");
...
function validateUserInput(input: string): { valid: boolean; address?: Address; error?: string } {
  // Trim whitespace
