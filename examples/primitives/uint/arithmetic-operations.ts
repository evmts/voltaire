...
console.log(`   ${dividend.toString()} / ${divisor.toString()} = ${quotient.toString()} remainder ${remainder.toString()}`);
console.log(`   Verification: ${quotient.toString()} * ${divisor.toString()} + ${remainder.toString()} = ${quotient.times(divisor).plus(remainder).toString()}\n`);
...
console.log(`   MAX + 100 = ${max.plus(Uint.from(100n)).toString()} (wraps to 99)\n`);
...
console.log(`   ZERO - 1 = ${zero.minus(Uint.ONE).toString().slice(0, 50)}... (wraps to MAX)`);
console.log(`   ZERO - 5 = ${zero.minus(Uint.from(5n)).toString().slice(0, 50)}... (wraps to MAX - 4)\n`);
...
console.log(`   Value * Value (wraps): ${large.times(large).toString().slice(0, 50)}...`);
...
function detectAdditionOverflow(num1: typeof Uint.prototype, num2: typeof Uint.prototype): boolean {
...
console.log(`   ${test1.toString()} + ${test2.toString()}: overflow = ${detectAdditionOverflow(test1, test2)}`);
console.log(`   MAX + ${test4.toString()}: overflow = ${detectAdditionOverflow(test3, test4)}\n`);
...
function wadMul(num1: typeof Uint.prototype, num2: typeof Uint.prototype): typeof Uint.prototype {
	return num1.times(num2).dividedBy(WAD);
}

function wadDiv(num1: typeof Uint.prototype, num2: typeof Uint.prototype): typeof Uint.prototype {
...
console.log(`   2.5 * 1.5 = ${(Number(product.toBigInt()) / Number(WAD.toBigInt())).toFixed(2)}`);
...
console.log(`   10 / 4 = ${(Number(division.toBigInt()) / Number(WAD.toBigInt())).toFixed(2)}\n`);
