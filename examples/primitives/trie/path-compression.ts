...
function formatHash(hash: Uint8Array | null): string {
	if (!hash) return 'null';
	return `0x${Array.from(hash).slice(0, 8).map((b) => b.toString(16).padStart(2, '0')).join('')}...`;
}
...
console.log('------------------------------');
{
	...
	trie.put(new Uint8Array([0x01, 0x02, 0x03, 0x04]), encoder.encode('value_at_04'));
	trie.put(new Uint8Array([0x01, 0x02, 0x03, 0x05]), encoder.encode('value_at_05'));
	trie.put(new Uint8Array([0x01, 0x02, 0x03, 0x06]), encoder.encode('value_at_06'));
...
console.log('------------------------------------');
{
	...
	trie.put(new Uint8Array([0x12, 0x34, 0x56, 0x78]), encoder.encode('longer_key'));
	...
	console.log(`  ✓ short:  ${short ? new TextDecoder().decode(short) : 'null'}`);
	console.log(`  ✓ long:   ${long ? new TextDecoder().decode(long) : 'null'}`);
	console.log(`  ✓ longer: ${longer ? new TextDecoder().decode(longer) : 'null'}`);
...
console.log('-----------------------------');
{
	...
	console.log('  Structure: Extension [A,A,B,B] -> Branch (split at nibble C vs 1)');
	...
	console.log('  Structure: Extension [A,A] -> Branch (split at nibble B vs 3)');
