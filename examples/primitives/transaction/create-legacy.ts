...
console.log('  Chain ID:', Transaction.Legacy.getChainId.call(transfer), '(from v value)');
...
console.log('  Gas Limit:', contractCall.gasLimit, '(higher for contract calls)');
console.log();
...
function createChainIdExample(chainId: bigint, chainName: string) {
  ...
  return { chainName, chainId, v, recoveredChainId: Transaction.Legacy.getChainId.call(tx) };
}
...
console.log('  Chain ID:', Transaction.Legacy.getChainId.call(preEIP155), '(null = pre-EIP-155)');
