...
console.log('  Formula: (v - 35) / 2 = (' + legacyTx.v + ' - 35) / 2 = ' + chainId);
console.log();
...
console.log('  Structure: keccak256(0x02 || rlp([chainId, nonce, maxPriorityFee,');
...
function validatePoolTransaction(tx: Transaction.Any): { valid: boolean; error?: string } {
  // Check if signed
  ...
  if (txChainId !== expectedChainId) {
    return { valid: false, error: `Wrong chain: expected ${expectedChainId}, got ${txChainId}` };
  }
