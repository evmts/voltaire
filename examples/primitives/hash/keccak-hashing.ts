...
console.log(
	`Received hash:  ${receivedHash.format()} - ${originalHash.equals(receivedHash) ? "VALID ✓" : "INVALID ✗"}`,
);
console.log(
	`Corrupted hash: ${corruptedHash.format()} - ${originalHash.equals(corruptedHash) ? "VALID ✓" : "INVALID ✗"}\n`,
);
...
console.log(
	`10,000 × 32-byte hashes: ${smallTime.toFixed(2)}ms (${((10000 / smallTime) * 1000).toFixed(0)} hashes/sec)`,
);
