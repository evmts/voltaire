...
// From hex string
const hashFromHex = Hash.fromHex("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
...
console.log("  First 8 bytes:", Array.from(hashBytes.slice(0, 8)).map(b => `0x${b.toString(16).padStart(2, '0')}`).join(' '));
...
const displayHash = Hash.keccak256String("This is a long message that will be hashed");
...
console.log("Original first byte:", `0x${original[0].toString(16).padStart(2, '0')}`);
console.log("Cloned first byte:", `0x${cloned[0].toString(16).padStart(2, '0')}`);
