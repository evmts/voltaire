...
function selectorToHex(selector: Uint8Array): string {
  return "0x" + Array.from(selector).map(b => b.toString(16).padStart(2, '0')).join('');
}

// ERC-20 Functions
const transfer = getFunctionSelector("transfer(address,uint256)");
const approve = getFunctionSelector("approve(address,uint256)");
const transferFrom = getFunctionSelector("transferFrom(address,address,uint256)");
...
console.log("  transferFrom(address,address,uint256):", selectorToHex(transferFrom)); // 0x23b872dd
...
// ERC-721 Functions
const safeTransferFrom = getFunctionSelector("safeTransferFrom(address,address,uint256)");
const safeTransferFromData = getFunctionSelector("safeTransferFrom(address,address,uint256,bytes)");
const ownerOf = getFunctionSelector("ownerOf(uint256)");
const getApproved = getFunctionSelector("getApproved(uint256)");
const setApprovalForAll = getFunctionSelector("setApprovalForAll(address,bool)");
const isApprovedForAll = getFunctionSelector("isApprovedForAll(address,address)");

console.log("ERC-721 Function Selectors:");
console.log("  safeTransferFrom(address,address,uint256):", selectorToHex(safeTransferFrom)); // 0x42842e0e
console.log("  safeTransferFrom(address,address,uint256,bytes):", selectorToHex(safeTransferFromData)); // 0xb88d4fde
console.log("  ownerOf(uint256):", selectorToHex(ownerOf)); // 0x6352211e
console.log("  setApprovalForAll(address,bool):", selectorToHex(setApprovalForAll)); // 0xa22cb465

// ERC-1155 Functions
const safeTransferFrom1155 = getFunctionSelector("safeTransferFrom(address,address,uint256,uint256,bytes)");
const safeBatchTransferFrom = getFunctionSelector("safeBatchTransferFrom(address,address,uint256[],uint256[],bytes)");
const balanceOf1155 = getFunctionSelector("balanceOf(address,uint256)");
const balanceOfBatch = getFunctionSelector("balanceOfBatch(address[],uint256[])");

console.log("\nERC-1155 Function Selectors:");
console.log("  safeTransferFrom(...):", selectorToHex(safeTransferFrom1155)); // 0xf242432a
console.log("  safeBatchTransferFrom(...):", selectorToHex(safeBatchTransferFrom)); // 0x2eb2c2d6
...
const toAddress = "000000000000000000000000742d35Cc6634C0532925a3b844Bc9e7595f51e3e";
...
function checkCollision(signatures: string[]): void {
  ...
  // Find collisions
  const collisions = Array.from(selectors.entries())
