...
function batchContains(filter: typeof BloomFilter.prototype, items: string[]): boolean[] {
  const encoder = new TextEncoder()
...
for (let i = 0; i < queries.length; i++) {
  console.log(`  ${queries[i]}: ${results[i] ? '✓ might exist' : '✗ definitely not present'}`)
}
...
for (let i = 0; i < testAddresses.length; i++) {
  console.log(`  ${testAddresses[i]}: ${watchResults[i] ? 'watched ✓' : 'not watched ✗'}`)
}
...
console.log('Note: bloom filter may have false positives, verify duplicates if needed')
