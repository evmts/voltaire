...
class EventIndex {
  ...
  constructor() {
    ...
    const eventTypes: EventType[] = ['Transfer', 'Approval', 'Swap', 'Mint', 'Burn', 'Sync']
  ...
  getFilterStats(eventType: EventType): { bits: number; k: number; density: number } {
    const filter = this.filters.get(eventType)
...
for (const type of eventTypes) {
  ...
  if (stats.bits > 0) {
    console.log(`  ${type}: ${stats.density.toFixed(2)}% density (${stats.bits} bits, ${stats.k} hash functions)`)
  }
}
