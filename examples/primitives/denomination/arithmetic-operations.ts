...
function deductGasCost(balance: Wei.Type, gasCost: Wei.Type): {
  newBalance: Wei.Type;
  sufficient: boolean;
} {
...
console.log(`   New balance: ${Number(Wei.toU256(result.newBalance)) / 1e18} ETH`);
...
distribution.forEach((share, i) => {
  const percentage = (Number(weights[i]) / Number(weights.reduce((a, b) => a + b))) * 100;
  console.log(`     Share ${i + 1} (${percentage}%): ${Number(Wei.toU256(share)) / 1e18} ETH`);
});
...
function combineAmounts(
  ...
  const total = Wei.toU256(ethWei) + Wei.toU256(gweiWei) + Wei.toU256(weiAmount);
...
const transactions: Transaction[] = [
  { type: 'debit', amount: Wei.from(1_000_000_000_000_000_000n), description: 'Send 1 ETH' },
  { type: 'credit', amount: Wei.from(500_000_000_000_000_000n), description: 'Receive 0.5 ETH' },
  { type: 'debit', amount: Wei.from(50_000_000_000_000n), description: 'Gas fee' },
  { type: 'credit', amount: Wei.from(2_000_000_000_000_000_000n), description: 'Receive 2 ETH' },
];

const balanceHistory = processTransactions(initialBalance, transactions);

console.log(`   Initial balance: ${Number(Wei.toU256(balanceHistory[0])) / 1e18} ETH`);
transactions.forEach((tx, i) => {
  const sign = tx.type === 'credit' ? '+' : '-';
  const amount = Number(Wei.toU256(tx.amount)) / 1e18;
  const newBalance = Number(Wei.toU256(balanceHistory[i + 1])) / 1e18;
  console.log(`   ${sign}${amount} ETH (${tx.description}) → ${newBalance} ETH`);
});
