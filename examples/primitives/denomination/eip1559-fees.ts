...
console.log(`   Estimated (base + priority): ${Wei.toGwei(costs.estimatedCost)} Gwei`);
...
function calculateEffectiveGasPrice(
  ...
  const effectiveU256 = baseAndPriority <= maxFeeU256 ? baseAndPriority : maxFeeU256;
...
for (const scenario of scenarios) {
  ...
  console.log(`     Base: ${scenario.base} Gwei, Priority: ${scenario.priority} Gwei, Max: ${scenario.max} Gwei`);
...
function analyzeTip(
  ...
  const tipPercentage = total > 0n ? Number((Wei.toU256(tipPaid) * 10000n) / total) / 100 : 0;
...
for (const scenario of tipScenarios) {
  ...
  console.log(`   Base: ${scenario.base} Gwei, Priority: ${scenario.priority} Gwei`);
