...
for (const { name, gwei } of gasPrices) {
  ...
  console.log(`     Cost: ${costEther} Ether (~${Number(Wei.toU256(cost)) / 1e18} ETH)`);
}
...
const totalCost = Wei.from(Uint.plus(Wei.toU256(transferValue), Wei.toU256(gasCost)));
...
console.log(`   Can send: ${hasSufficientBalance(balance, value, gas) ? '✓' : '✗'}`);
console.log('');
