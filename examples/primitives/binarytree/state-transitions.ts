...
if (aliceData && bobData) {
  ...
  console.log('   Before:')
  console.log('     Alice: ' + (Number(alice.balance) / 1e18) + ' ETH, nonce ' + alice.nonce)
  console.log('     Bob: ' + (Number(bob.balance) / 1e18) + ' ETH, nonce ' + bob.nonce)
...
if (aliceData2) {
  ...
  state = BinaryTree.insert(state, aliceKey, packAccountState(alice))
  state = BinaryTree.insert(state, contractKey, packAccountState(contractState))
...
if (finalAlice && finalBob && finalContract) {
  ...
  console.log('     Alice: ' + (Number(alice.balance) / 1e18) + ' ETH, nonce ' + alice.nonce)
  console.log('     Bob: ' + (Number(bob.balance) / 1e18) + ' ETH, nonce ' + bob.nonce)
  console.log('     Contract: ' + (Number(contract.balance) / 1e18) + ' ETH, nonce ' + contract.nonce)

  const totalBalance = alice.balance + bob.balance + contract.balance
  console.log('   Total: ' + (Number(totalBalance) / 1e18) + ' ETH')
  console.log('   Conservation: ' + (totalBalance === 3000000000000000000n ? 'OK' : 'FAIL'))
}
console.log('')
