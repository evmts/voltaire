...
console.log("Opcode | Name     | Category      | isPush | getPushSize | isTerminator");
console.log("-------|----------|---------------|--------|-------------|-------------");
...
// Find terminators
const terminators = analysis.instructions.filter((i) => Bytecode.isTerminator(i.opcode));
...
// Find jumps
const jumps = analysis.instructions.filter((i) => i.opcode === 0x56 || i.opcode === 0x57);
...
for (const [opcode, count] of sorted.slice(0, 10)) {
  ...
  const isPush = Bytecode.isPush(opcode);
  const label = isPush ? `PUSH${Bytecode.getPushSize(opcode)}` : `0x${opcode.toString(16).padStart(2, "0")}`;
...
for (const test of testOpcodes) {
  ...
  console.log(`  0x${test.value.toString(16).padStart(2, "0")} (${test.name}):`);
