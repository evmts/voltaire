...
function validateChainFeatures(chainId: number): boolean {
  ...
  try {
    ...
    console.log(`     ✗ RPC: ${error instanceof Error ? error.message : error}`);
    return false;
  }
  ...
  try {
    ...
    console.log(`     ✗ Explorer: ${error instanceof Error ? error.message : error}`);
  }
...
function isChainAllowedInEnvironment(
  ...
  const isTestnet =
    name.includes('testnet') || name.includes('sepolia') || name.includes('goerli');
