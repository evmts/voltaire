...
if (ethereum) {
  console.log(`   Ethereum: ${ethereum.name} (${ethereum.nativeCurrency.symbol})`);
}

const optimism = Chain.byId[10];
if (optimism) {
  console.log(`   Optimism: ${optimism.name} (${optimism.nativeCurrency.symbol})`);
}

const arbitrum = Chain.byId[42161];
if (arbitrum) {
  console.log(`   Arbitrum: ${arbitrum.name} (${arbitrum.nativeCurrency.symbol})\n`);
}
