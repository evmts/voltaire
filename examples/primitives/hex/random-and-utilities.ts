...
const unique = addresses.filter((addr, i, arr) =>
  arr.findIndex(a => Hex.equals(a, addr)) === i
)
...
console.log(`  Sorted by size: ${sorted.map(h => `${Hex.size(h)} bytes`).join(', ')}`)
...
console.log(`  Mock signature: ${mockSignature.slice(0, 16)}... (${Hex.size(mockSignature)} bytes)`)
