...
export class MockRpcClient implements Provider {
	private accounts = new Map<string, MockAccountState>();
	private blocks = new Map<string, MockBlock>();
	private blocksByNumber = new Map<bigint, MockBlock>();
	...
	async request(args: RequestArguments): Promise<unknown> {
		...
		switch (method) {
			...
			case "eth_getBlockByNumber": {
				const blockNum = this.parseBlockTag(p[0] as string);
	...
	private parseBlockTag(tag: string): bigint {
	...
	private formatBlock(block: MockBlock) {
		return {
