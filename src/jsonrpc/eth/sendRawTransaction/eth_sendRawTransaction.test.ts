...
describe("eth_sendRawTransaction", () => {
	...
	describe("Edge Cases", () => {
		...
		test("handles EIP-1559 transaction", () => {
			const signedTx =
				"0x02f873011a8405f5e10085010e87e48082520894743d35cc6634c0532925a3b844bc9e7595f0beb0880de0b6b3a7640000c001a04e6b9b0e1e2c8b5f1e7e9e3f8e7e6e5e4e3e2e1e0e9e8e7e6e5e4e3e2e1e0e9a01e2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2";
			const req = SendRawTransactionRequest(signedTx);
			expect(req.params?.[0]).toBe(signedTx);
		});
		...
		test("handles EIP-4844 blob transaction", () => {
			const signedTx =
				"0x03f9034501820a3c843b9aca00843b9aca07830186a094743d35cc6634c0532925a3b844bc9e7595f0beb08080c0";
			const req = SendRawTransactionRequest(signedTx);
			expect(req.params?.[0]).toBe(signedTx);
		});
	});
