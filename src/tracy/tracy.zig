...
pub const enabled = blk: {
    ...
    if (!builtin.is_test) {
        ...
        if (@hasDecl(options, "enable_tracy")) {
            build_enable = options.enable_tracy;
        }
    }
...
const tracy_stub = struct {
    ...
    pub inline fn ZoneDynamic(name: []const u8, function: []const u8, file: []const u8, line: u32, color: u32) ZoneCtx {
        _ = name;
        _ = function;
        _ = file;
        _ = line;
...
const tracy_full = struct {
    ...
    /// Creates a zone with dynamically allocated source location - allows truly unique zones
    pub inline fn ZoneDynamic(name: []const u8, function: []const u8, file: []const u8, line: u32, color: u32) ZoneCtx {
        // Allocate a new source location with the given name
        const srcloc = c.___tracy_alloc_srcloc_name(
            line,
            file.ptr,
            file.len,
            function.ptr,
        ...
        // Begin zone with the allocated source location
        const zone = if (has_callstack_support)
            c.___tracy_emit_zone_begin_alloc_callstack(srcloc, callstack_depth, 1)
        else
