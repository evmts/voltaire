...
import { type EvmState, formatMemory } from '~/components/evm-debugger/types'
		...
		<Card class="overflow-hidden">
			<CardHeader class="border-b p-3">
				<div class="flex items-center justify-between">
					...
					<div class="text-muted-foreground text-xs">Hexadecimal representation</div>
				</div>
			</CardHeader>
							...
							{(chunk, index) => (
								<div class="group flex justify-between px-4 py-1.5 transition-colors hover:bg-muted/50">
									<div class="flex items-center">
										...
										<Code class="break-all text-sm">0x{chunk}</Code>
									</div>
									<Button
										variant="ghost"
										size="icon"
										onClick={() => handleCopy(chunk, index())}
										class="mt-0.5 h-7 w-7 flex-shrink-0 opacity-0 transition-opacity group-hover:opacity-100"
