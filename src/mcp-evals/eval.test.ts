...
Return the transaction hash in your final message as: ANSWER: 0x...`,
    ...
    it(
      ...
      async () => {
        const { startBlock, endBlock, totalBurned } = EXPECTED_ANSWERS.ethBurnedRange;
...
Return "valid" or "invalid" in your final message as: ANSWER: <valid|invalid>`,
        ...
        expect(result.error).toBeUndefined();
        expect(result.scriptOutput).toBeDefined();
        expect(["valid", "invalid"]).toContain(result.scriptOutput?.toLowerCase());
      },
      { timeout: 4 * 60 * 1000 }
    );
    ...
    it(
      ...
      async () => {
        ...
        const expectedHash = "0x56570de287d73cd1cb6092bb8fdee6173974955fdef345ae579ee9f475ea7432";
...
Return the hash in your final message as: ANSWER: 0x...`,
        ...
        expect(result.error).toBeUndefined();
        expect(result.scriptOutput).toBeDefined();
        expect(result.scriptOutput?.toLowerCase()).toBe(expectedHash.toLowerCase());
      },
      { timeout: 4 * 60 * 1000 }
    );
