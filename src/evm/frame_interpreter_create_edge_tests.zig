...
test "CREATE edge case - maximum init code size (48KB)" {
    ...
    var interpreter = try FrameInterpreterType.init(allocator, bytecode.items, 15_000_000, db_interface);
...
test "CREATE edge case - all gas consumed by subcall" {
    ...
    const FrameInterpreterType = frame_interpreter.FrameInterpreter(.{ .has_database = true });
    var interpreter = try FrameInterpreterType.init(allocator, bytecode.items, 1_000_000, db_interface);
    defer interpreter.deinit(allocator);
