...
pub const JournalingDatabase = struct {
    ...
    pub fn set_account(self: *JournalingDatabase, address: [20]u8, account: Account) DatabaseError!void {
        ...
        if (original_account) |orig| {
            ...
            if (orig.balance != account.balance) {
                self.journal.record_balance_change(self.current_snapshot_id, address, orig.balance) catch {};
            }

            // Journal the original nonce if it's different
            if (orig.nonce != account.nonce) {
                self.journal.record_nonce_change(self.current_snapshot_id, address, orig.nonce) catch {};
            }
        } else {
            // Account didn't exist, journal a "zero" account for revert
            self.journal.record_balance_change(self.current_snapshot_id, address, 0) catch {};

            self.journal.record_nonce_change(self.current_snapshot_id, address, 0) catch {};
        }
    ...
    pub fn delete_account(self: *JournalingDatabase, address: [20]u8) DatabaseError!void {
        ...
        if (self.inner.get_account(address)) |original_account| {
            self.journal.record_balance_change(self.current_snapshot_id, address, original_account.balance) catch {};

            self.journal.record_nonce_change(self.current_snapshot_id, address, original_account.nonce) catch {};
        } else |_| {
    ...
    pub fn set_storage(self: *JournalingDatabase, address: [20]u8, key: u256, value: u256) DatabaseError!void {
        ...
        if (original_value != value) {
            self.journal.record_storage_change(self.current_snapshot_id, address, key, original_value) catch {};
        }
