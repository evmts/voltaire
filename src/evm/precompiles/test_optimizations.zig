...
test "optimized precompiles produce same results as original" {
    ...
    for (test_cases) |tc| {
        ...
        const result_original = precompiles_original.execute_precompile(tc.address, tc.input, &output_original, 100000, chain_rules);

        // Execute with optimized implementation
        const result_optimized = precompiles_optimized.execute_precompile(tc.address, tc.input, &output_optimized, 100000, chain_rules);
        ...
        // Same gas usage
        try testing.expectEqual(result_original.get_gas_used(), result_optimized.get_gas_used());

        // Same output size
        try testing.expectEqual(result_original.get_output_size(), result_optimized.get_output_size());

        // Same output data
        try testing.expectEqualSlices(u8, output_original[0..result_original.get_output_size()], output_optimized[0..result_optimized.get_output_size()]);
    }
}
