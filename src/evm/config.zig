...
pub const EvmConfig = struct {
    ...
    pub fn initWithFeatures(comptime hardfork: Hardfork, comptime features: struct {
    ...
    pub fn getGasCost(comptime self: EvmConfig, comptime op: GasOperation) u64 {
        return switch (op) {
            .sload => if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.BERLIN)) 0 else if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.ISTANBUL)) 800 else if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.TANGERINE_WHISTLE)) 200 else 50,
            .balance => if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.BERLIN)) 0 else if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.ISTANBUL)) 700 else if (@intFromEnum(self.hardfork) >= @intFromEnum(Hardfork.TANGERINE_WHISTLE)) 400 else 20,
            else => op.base_cost(),
        };
    }
