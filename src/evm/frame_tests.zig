...
test "Frame initialization with all parameters" {
    ...
    var frame = try Frame.init(100_000, // gas_remaining
        ...
        memory_db.to_database_interface(), Frame.chainRulesForHardfork(.CANCUN), &self_destruct, &created_contracts, input, allocator, null, // next_frame
    ...
    try testing.expectEqual(true, frame.hot_flags.is_static);
    try testing.expectEqual(@as(u10, 5), frame.hot_flags.depth);
...
test "Frame hardfork detection from chain rules" {
    ...
    for (test_cases) |tc| {
        var frame = try Frame.init(100_000, false, 0, testAddress(0x1000), testAddress(0x2000), 0, &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), tc.rules, null, null, &[_]u8{}, allocator, null, false, false);
        defer frame.deinit();
...
test "Frame gas consumption" {
    ...
    var frame = try Frame.init(1000, // Initial gas
        false, 0, testAddress(0x1000), testAddress(0x2000), 0, &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, null, &[_]u8{}, allocator, null, false, false);
    defer frame.deinit();
...
test "Frame depth tracking" {
    ...
    for (depths) |depth| {
        var frame = try Frame.init(100_000, false, depth, testAddress(0x1000), testAddress(0x2000), 0, &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, null, &[_]u8{}, allocator, null, false, false);
        defer frame.deinit();

        try testing.expectEqual(depth, frame.hot_flags.depth);
    }
}
...
test "Frame static call restrictions" {
    ...
    var static_frame = try Frame.init(100_000, true, // static call
        ...
        &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, null, &[_]u8{}, allocator, null, false, false);
    defer static_frame.deinit();

    try testing.expectEqual(true, static_frame.hot_flags.is_static);
    ...
    try testing.expectEqual(false, normal_frame.hot_flags.is_static);
...
test "Frame block context" {
    ...
    var frame = try Frame.init(100_000, false, 0, testAddress(0x1000), testAddress(0x2000), 0, &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.CANCUN), null, null, &[_]u8{}, allocator, null, false, false);
    defer frame.deinit();
...
test "Frame CREATE context" {
    ...
    var frame = try Frame.init(500_000, false, 0, testAddress(0xFFFF), // Contract being created
        ...
        &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, &created_contracts, &[_]u8{}, allocator, null, true, // is_create_call
        false);
    defer frame.deinit();
...
test "Frame DELEGATECALL context" {
    ...
    var frame = try Frame.init(100_000, false, 1, testAddress(0x6000), // Storage context
        ...
        &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, null, &[_]u8{}, allocator, null, false, true // is_delegate_call
    );
    defer frame.deinit();
...
test "Frame memory and stack access" {
    ...
    var frame = try Frame.init(100_000, false, 0, testAddress(0x1000), testAddress(0x2000), 0, &analysis, &access_list, &journal, host, 0, memory_db.to_database_interface(), Frame.chainRulesForHardfork(.LONDON), null, null, &[_]u8{}, allocator, null, false, false);
    defer frame.deinit();
