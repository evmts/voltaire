...
const txHash = Hash.from(keccak256(rlpEncode(tx)));
...
const sig = Signature.from(signatureBytes);
...
const messageHash = Hash.from(keccak256(prefixedMessage));
...
const expectedAddress = Address.from('0x...');
const sig = Signature.from(signatureBytes);
...
function cachedRecover(sig: BrandedSignature, messageHash: Hash): Address {
  const key = Hex.from(Signature.toBytes(sig)) + Hex.from(messageHash);
