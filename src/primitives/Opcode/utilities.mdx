...
Defined in: [primitives/Opcode/BrandedOpcode/info.js:17](https://github.com/evmts/primitives/blob/main/src/primitives/Opcode/BrandedOpcode/info.js)
...
function extractPushValues(bytecode: Uint8Array): Map<number, bigint> {
  ...
  for (const inst of instructions) {
    if (Opcode.isPush(inst.opcode) && inst.immediate) {
      const value = BigInt('0x' + Array.from(inst.immediate)
...
const metadata = info(opcode)
const name = getName(opcode)
const gas = getGasCost(opcode)
const stackEffect = getStackEffect(opcode)
const bytes = pushBytes(opcode)
const pos = dupPosition(opcode)
