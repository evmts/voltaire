...
export function from(params) {
	...
	if (params.blockhash) {
		if (params.fromBlock !== undefined || params.toBlock !== undefined) {
			throw new InvalidLogFilterError(
				"blockhash cannot be used with fromBlock or toBlock",
				{
					value: params,
		...
		if (
			!(params.blockhash instanceof Uint8Array) ||
	...
	if (params.fromBlock !== undefined) {
		...
		if (
			...
			throw new InvalidLogFilterError(
				"fromBlock must be BlockNumber (bigint) or block tag",
				{
	...
	if (params.toBlock !== undefined) {
		...
		if (
			...
			throw new InvalidLogFilterError(
				"toBlock must be BlockNumber (bigint) or block tag",
				{
		...
		if (Array.isArray(params.address)) {
			for (const addr of params.address) {
				if (!(addr instanceof Uint8Array) || addr.length !== 20) {
					throw new InvalidLogFilterError(
						"address array must contain valid Address values",
						{
							value: addr,
			...
			!(params.address instanceof Uint8Array) ||
