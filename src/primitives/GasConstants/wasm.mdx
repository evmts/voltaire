...
Even the most complex calculation (memory expansion with quadratic formula) completes in <200ns. WASM call overhead alone is 1000-2000ns.
...
4. **Heavy computation** - Operations taking >10μs each

GasConstants provides **building blocks** for these systems, not the systems themselves.

## Zig Implementation

Zig implementation exists at `/Users/williamcory/primitives/src/primitives/GasConstants/gas_constants.zig` for:
- Documentation and reference
- Integration with other WASM-accelerated primitives
- Potential future use in EVM interpreter

But it's not compiled to WASM for direct use because TypeScript is faster for these simple operations.
...
// Still fine: property access is ~5ns
for (const slot of slots) {
