...
pub const SiweMessage = struct {
    ...
    pub fn format(self: *const SiweMessage, allocator: std.mem.Allocator) ![]u8 {
    ...
    pub fn validate(self: *const SiweMessage) !void {
        ...
        if (self.expiration_time) |exp| {
            if (!is_valid_timestamp(exp)) {
                return error.InvalidExpirationTime;
            }
        }
        
        if (self.not_before) |nb| {
            if (!is_valid_timestamp(nb)) {
                return error.InvalidNotBefore;
            }
        }
    }
};
...
pub fn verify_siwe_message(
    ...
    // Hash with EIP-191
    const message_hash = try Hash.eip191_hash_message(formatted, allocator);
    ...
    return Address.equal(recovered_address, message.address);
}

// Test basic SIWE message
...
test "SIWE message validation" {
    ...
    // Empty domain
    message.domain = "";
    try testing.expectError(error.EmptyDomain, message.validate());
    message.domain = "example.com";
    
    // Empty URI
    message.uri = "";
    try testing.expectError(error.EmptyUri, message.validate());
    message.uri = "https://example.com";
    
    // Invalid version
    message.version = "2";
    try testing.expectError(error.InvalidVersion, message.validate());
    message.version = "1";
    
    // Empty nonce
    message.nonce = "";
    try testing.expectError(error.EmptyNonce, message.validate());
    message.nonce = "32891756";
    
    // Invalid timestamp
    message.issued_at = "not-a-timestamp";
    try testing.expectError(error.InvalidIssuedAt, message.validate());
}
...
pub fn parse_siwe_message(allocator: std.mem.Allocator, text: []const u8) !SiweMessage {
    var lines = std.mem.tokenizeScalar(u8, text, '\n');
    
    // Parse header
    const header = lines.next() orelse return error.InvalidFormat;
    const domain_end = std.mem.indexOf(u8, header, " wants you") orelse return error.InvalidFormat;
    ...
    var next_line = lines.next() orelse return error.InvalidFormat;
    
    if (!std.mem.startsWith(u8, next_line, "URI: ")) {
        statement = next_line;
        _ = lines.next(); // Skip empty line
        next_line = lines.next() orelse return error.InvalidFormat;
    }
    
    // Parse required fields
    if (!std.mem.startsWith(u8, next_line, "URI: ")) return error.InvalidFormat;
    const uri = next_line[5..];
    
    const version_line = lines.next() orelse return error.InvalidFormat;
    if (!std.mem.startsWith(u8, version_line, "Version: ")) return error.InvalidFormat;
    const version = version_line[9..];
    
    const chain_line = lines.next() orelse return error.InvalidFormat;
    if (!std.mem.startsWith(u8, chain_line, "Chain ID: ")) return error.InvalidFormat;
    const chain_id = try std.fmt.parseInt(u64, chain_line[10..], 10);
    
    const nonce_line = lines.next() orelse return error.InvalidFormat;
    if (!std.mem.startsWith(u8, nonce_line, "Nonce: ")) return error.InvalidFormat;
    const nonce = nonce_line[7..];
    
    const issued_line = lines.next() orelse return error.InvalidFormat;
    if (!std.mem.startsWith(u8, issued_line, "Issued At: ")) return error.InvalidFormat;
