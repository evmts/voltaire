...
results.push(benchmark("generateNonce - default length (11)", () => Siwe.generateNonce()));

results.push(benchmark("generateNonce - 8 chars", () => Siwe.generateNonce(8)));

results.push(benchmark("generateNonce - 16 chars", () => Siwe.generateNonce(16)));

results.push(benchmark("generateNonce - 32 chars", () => Siwe.generateNonce(32)));
...
console.log("--- Parsing ---");
results.push(benchmark("parse - basic message", () => Siwe.parse(formattedBasic)));

results.push(benchmark("parse - complex message", () => Siwe.parse(formattedComplex)));
...
console.log("--- Standard Form ---");
results.push(
  benchmark("Message.validate - basic", () => Siwe.Message.validate(basicMessage)),
);
...
const expiredMessage = { ...basicMessage, expirationTime: "2021-09-30T15:00:00.000Z" };
results.push(
...
for (const addr of addresses) {
  ...
  results.push(
    benchmark(`format with address 0x${addr[0]?.toString(16).padStart(2, "0")}...`, () =>
...
for (const len of statementLengths) {
  ...
  results.push(
    benchmark(`format - statement length ${len}`, () => Siwe.Message.format(msg)),
  );
}
...
console.log(`  ${sorted[0]?.opsPerSec.toFixed(0)} ops/sec (${sorted[0]?.avgTimeMs.toFixed(4)} ms/op)`);
...
if (typeof Bun !== "undefined") {
  const resultsFile = "/Users/williamcory/primitives/src/primitives/siwe-results.json";
