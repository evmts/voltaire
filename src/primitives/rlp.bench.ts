...
const longList = Array.from({ length: 100 }, (_, i) => new Uint8Array([i % 256]));
...
results.push(
  benchmark("encode medium bytes (32 bytes)", () => Rlp.encode.call(mediumBytes)),
);
results.push(
  benchmark("encode long bytes (200 bytes)", () => Rlp.encode.call(longBytes)),
);
results.push(
  benchmark("encode very long bytes (10KB)", () => Rlp.encode.call(veryLongBytes)),
);
...
results.push(
  benchmark("encode empty list", () => Rlp.encode.call(emptyList)),
);
results.push(
...
results.push(
  benchmark("encode deeply nested list (4+ levels)", () => Rlp.encode.call(deeplyNested)),
);
...
results.push(
  benchmark("decode single byte (< 0x80)", () => Rlp.decode.call(encodedSingleByte)),
);
results.push(
  benchmark("decode short bytes (5 bytes)", () => Rlp.decode.call(encodedShortBytes)),
);
results.push(
  benchmark("decode medium bytes (32 bytes)", () => Rlp.decode.call(encodedMediumBytes)),
);
results.push(
  benchmark("decode long bytes (200 bytes)", () => Rlp.decode.call(encodedLongBytes)),
);
results.push(
  benchmark("decode very long bytes (10KB)", () => Rlp.decode.call(encodedVeryLongBytes)),
);
...
results.push(
  benchmark("decode short list (2 items)", () => Rlp.decode.call(encodedShortList)),
);
results.push(
  benchmark("decode medium list (10 items)", () => Rlp.decode.call(encodedMediumList)),
);
results.push(
  benchmark("decode long list (100 items)", () => Rlp.decode.call(encodedLongList)),
);
results.push(
  benchmark("decode nested list (3 levels)", () => Rlp.decode.call(encodedNestedList)),
);
results.push(
  benchmark("decode deeply nested list (4+ levels)", () => Rlp.decode.call(encodedDeeplyNested)),
);
...
results.push(
  benchmark("getEncodedLength - bytes", () => Rlp.getEncodedLength.call(mediumBytes)),
);
results.push(
  benchmark("getEncodedLength - list", () => Rlp.getEncodedLength.call(mediumList)),
);
...
results.push(
  benchmark("toJSON - bytes", () => Rlp.toJSON.call(data1)),
);
results.push(
  benchmark("toJSON - list", () => Rlp.toJSON.call(listData1)),
);

const jsonData = { type: "bytes", value: [1, 2, 3] };
results.push(
  benchmark("fromJSON - bytes", () => Rlp.fromJSON.call(jsonData)),
);
...
results.push(
  benchmark("Data.fromList", () => Rlp.Data.fromList.call([
    { type: "bytes", value: new Uint8Array([1]) },
    { type: "bytes", value: new Uint8Array([2]) },
  ])),
);
...
results.push(
  benchmark("Data.toList", () => Rlp.Data.toList.call(listData1)),
);
...
results.push(
  benchmark("isData - valid bytes", () => Rlp.isData(data1)),
);
results.push(
  benchmark("isData - valid list", () => Rlp.isData(listData1)),
);
results.push(
  benchmark("isData - invalid", () => Rlp.isData("invalid")),
);
results.push(
  benchmark("isBytesData", () => Rlp.isBytesData(data1)),
);
results.push(
  benchmark("isListData", () => Rlp.isListData(listData1)),
);
...
results.push(
  benchmark("decode simple transaction", () => Rlp.decode.call(encodedSimpleTx)),
);
...
results.push(
  benchmark("encode tx with data (100 bytes)", () => Rlp.encode.call(txWithData)),
);
...
const txWithAccessList = [nonce, gasPrice, gasLimit, to, value, data, accessList];

results.push(
  benchmark("encode tx with access list", () => Rlp.encode.call(txWithAccessList)),
);
...
keyResults.forEach((r) => {
  console.log(`  ${r.name}: ${r.opsPerSec.toFixed(0)} ops/sec (${r.avgTimeMs.toFixed(4)} ms/op)`);
});
...
if (typeof Bun !== "undefined") {
  const resultsFile = "/Users/williamcory/primitives/src/primitives/rlp-results.json";
