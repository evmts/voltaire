...
describe("Authorization.sign", () => {
	it("creates a signed authorization from unsigned", () => {
		...
		privateKey.fill(1); // Fill with non-zero value
...
describe("Authorization.calculateGasCost", () => {
	it("calculates cost for empty list with no empty accounts", () => {
		const authList: AuthorizationType[] = [];
		const cost = Authorization.calculateGasCost(authList, 0);
		expect(cost).toBe(0n);
	});

	it("calculates cost for single authorization with no empty account", () => {
		const authList: BrandedAuthorization[] = [
			{
				chainId: 1n,
				address: addr1,
				nonce: 0n,
	...
	it("calculates cost for single authorization with one empty account", () => {
		...
		const cost = Authorization.calculateGasCost(authList, 1);
		expect(cost).toBe(
	...
	it("calculates cost for multiple authorizations with multiple empty accounts", () => {
		...
		const cost = Authorization.calculateGasCost(authList, 2);
		expect(cost).toBe(
...
describe("Authorization.getGasCost", () => {
	...
	it("returns base cost + empty account cost when account is empty", () => {
		const cost = Authorization.getGasCost(auth, true);
		expect(cost).toBe(
			Authorization.PER_AUTH_BASE_COST + Authorization.PER_EMPTY_ACCOUNT_COST,
		);
...
describe("Authorization.processAll", () => {
	it("returns empty array for empty list", () => {
		const authList: AuthorizationType[] = [];
		const result = Authorization.processAll(authList);
...
describe("Authorization.format", () => {
	it("formats signed authorization", () => {
		...
		const formatted = Authorization.format(auth);
...
describe("Authorization.equals", () => {
	const auth1: BrandedAuthorization = {
		chainId: 1n,
		address: addr1,
		nonce: 0n,
		yParity: 0,
	...
	it("returns true for same authorization instance", () => {
		expect(Authorization.equalsAuth(auth1, auth1)).toBe(true);
	});

	it("returns false for different chainId", () => {
		const auth2 = { ...auth1, chainId: 2n };
		expect(Authorization.equalsAuth(auth1, auth2)).toBe(false);
	});
...
describe("Authorization edge cases", () => {
	...
	it("handles large authorization lists", () => {
		...
		const cost = Authorization.calculateGasCost(authList, 50);
		expect(cost).toBe(
