...
describe("encodeParams", () => {
	...
	it("encodes constructor with multiple parameters", () => {
		...
		const encoded = encodeParams(constructor, ["Token", "TKN", 18n] as any);
		expect(encoded).toBeInstanceOf(Uint8Array);
	...
	it("encodes constructor with array parameter", () => {
		...
		const encoded = encodeParams(constructor, [[1n, 2n, 3n]]);
		expect(encoded).toBeInstanceOf(Uint8Array);
	});
	...
	it("encodes constructor with multiple uint sizes", () => {
		...
		const encoded = encodeParams(constructor, [1n, 2n, 3n] as any);
		expect(encoded).toBeInstanceOf(Uint8Array);
