...
describe("Abi.Parameter encoding/decoding", () => {
	it("encode throws not implemented", () => {
		const param = {
			type: "uint256",
			name: "amount",
		} as const satisfies Abi.Parameter;
		expect(() => Abi.Parameter.encode.call(param, 100n)).toThrow(
			AbiEncodingError,
		);
	});
...
describe("Abi Type Inference", () => {
	it("should infer basic function parameter types", () => {
		...
		type InputTypes = Abi.ParametersToPrimitiveTypes<
			typeof transferFunc.inputs
		>;
		type OutputTypes = Abi.ParametersToPrimitiveTypes<
	...
	it("should infer complex types with arrays", () => {
		...
		type InputTypes = Abi.ParametersToPrimitiveTypes<
			typeof batchTransferFunc.inputs
		>;
	...
	it("should infer error parameter types", () => {
		...
		type ErrorParams = Abi.ParametersToPrimitiveTypes<
			typeof insufficientBalanceError.inputs
		>;
