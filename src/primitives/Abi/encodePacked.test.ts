/**
 * Comprehensive tests for Abi.encodePacked
...
describe("Abi.encodePacked - uint types", () => {
	it("encodes uint8", () => {
		const result = Abi.encodePacked(["uint8"], [42n]);
		expect(result).toMatch(/0x2a/i);
	});

	it("encodes uint8 zero", () => {
		const result = Abi.encodePacked(["uint8"], [0n]);
		expect(result).toMatch(/0x00/i);
	});

	it("encodes uint8 max (255)", () => {
		const result = Abi.encodePacked(["uint8"], [255n]);
		expect(result).toMatch(/0xff/i);
	});

	it("encodes uint16", () => {
		const result = Abi.encodePacked(["uint16"], [0x1234n]);
		expect(result).toMatch(/0x1234/i);
	});

	it("encodes uint32", () => {
		const result = Abi.encodePacked(["uint32"], [0x12345678n]);
		expect(result).toMatch(/0x12345678/i);
	});

	it("encodes uint64", () => {
		const result = Abi.encodePacked(["uint64"], [0x123456789abcdef0n]);
		expect(result).toMatch(/0x123456789abcdef0/i);
	});

	it("encodes uint128", () => {
		const value = 0x123456789abcdef0123456789abcdef0n;
		const result = Abi.encodePacked(["uint128"], [value]);
	...
	it("encodes uint256", () => {
		...
		const result = Abi.encodePacked(["uint256"], [value]);
...
describe("Abi.encodePacked - fixed bytes types", () => {
	it("encodes bytes1 without padding", () => {
		const result = Abi.encodePacked(["bytes1"], ["0x42"]);
		expect(result).toMatch(/0x42/i);
	});

	it("encodes bytes4 without padding", () => {
		const result = Abi.encodePacked(["bytes4"], ["0x12345678"]);
		expect(result).toMatch(/0x12345678/i);
	});

	it("encodes bytes8", () => {
		const result = Abi.encodePacked(["bytes8"], ["0x123456789abcdef0"]);
		expect(result).toMatch(/0x123456789abcdef0/i);
	});

	it("encodes bytes32", () => {
		const value = "0x" + "ff".repeat(32);
		const result = Abi.encodePacked(["bytes32"], [value]);
		expect(result).toMatch(/0xf{64}/i);
	});
...
describe("Abi.encodePacked - string type", () => {
	it("encodes empty string", () => {
		const result = Abi.encodePacked(["string"], [""]);
		expect(result).toMatch(/0x/i);
	});

	it("encodes string as UTF-8 bytes without length", () => {
		const result = Abi.encodePacked(["string"], ["hello"]);
	...
	it("encodes longer string", () => {
		...
		const result = Abi.encodePacked(["string"], [str]);
	...
	it("encodes multiple strings concatenated", () => {
		const result = Abi.encodePacked(["string", "string"], ["hello", "world"]);
		expect(result).toMatch(/0x68656c6c6f776f726c64/i); // "helloworld" in hex
	});
});

describe("Abi.encodePacked - arrays", () => {
	it("encodes empty array", () => {
		const result = Abi.encodePacked(["uint256[]"], [[]]);
		expect(result).toMatch(/0x/i);
	});

	it("encodes uint256[] without length prefix", () => {
		const result = Abi.encodePacked(["uint256[]"], [[1n, 2n, 3n]]);
	...
	it("encodes uint8[] compactly", () => {
		const result = Abi.encodePacked(["uint8[]"], [[0x12n, 0x34n, 0x56n]]);
		expect(result).toMatch(/0x123456/i);
	});
	...
	it("encodes address[] compactly", () => {
		...
		const result = Abi.encodePacked(["address[]"], [addrs]);
		// 40 bytes total, no length prefix
		expect(result.length).toBe(2 + 40 * 2);
	});

	it("encodes bool[] compactly", () => {
		const result = Abi.encodePacked(["bool[]"], [[true, false, true]]);
		expect(result).toMatch(/0x010001/i);
	});
});

describe("Abi.encodePacked - fixed arrays", () => {
	it("encodes uint256[3]", () => {
		const result = Abi.encodePacked(["uint256[3]"], [[1n, 2n, 3n]]);
	...
	it("encodes address[2]", () => {
		...
		const result = Abi.encodePacked(["address[2]"], [addrs]);
		expect(result.length).toBe(2 + 40 * 2);
	});
});
...
describe("Abi.encodePacked - mixed types", () => {
	it("encodes address + uint256 compactly", () => {
		const addr = "0x742d35Cc6634C0532925a3b844Bc9e7595f251e3" as Address;
		const amount = 1000n;
		const result = Abi.encodePacked(["address", "uint256"], [addr, amount]);
	...
	it("encodes string + uint256", () => {
		const result = Abi.encodePacked(["string", "uint256"], ["hello", 42n]);
...
describe("Abi.encodePacked - real-world use cases", () => {
	it("encodes signature message (address + nonce)", () => {
		const addr = "0x742d35Cc6634C0532925a3b844Bc9e7595f251e3" as Address;
		const nonce = 42n;
		const result = Abi.encodePacked(["address", "uint256"], [addr, nonce]);
		expect(result.length).toBe(2 + 52 * 2);
	});
	...
	it("encodes token ID creation (address + uint256)", () => {
		...
		const index = 1n;
		const result = Abi.encodePacked(["address", "uint256"], [creator, index]);
...
describe("Abi.encodePacked - error handling", () => {
	it("throws on type/value count mismatch", () => {
		expect(() => Abi.encodePacked(["uint256", "address"], [42n])).toThrow(
...
describe("Abi.encodePacked - edge cases", () => {
	...
	it("encodes very long array", () => {
		...
		const result = Abi.encodePacked(["uint8[]"], [arr]);
...
describe("Abi.encodePacked vs standard encoding", () => {
	it("packed is more compact than standard for uint8", () => {
		const packed = Abi.encodePacked(["uint8"], [42n]);
	...
	it("packed is more compact for multiple small values", () => {
		...
		const packed = Abi.encodePacked(types, values);
