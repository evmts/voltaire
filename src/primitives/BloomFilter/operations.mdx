...
const filters: BloomFilter[] = [
  BloomFilter(block1.logsBloom, 2048, 3),
  BloomFilter(block2.logsBloom, 2048, 3),
  BloomFilter(block3.logsBloom, 2048, 3)
]
...
class LogAggregator {
  ...
  addLog(log: Log) {
    // Add address
    this.filter.add(Address(log.address))
  ...
  mightContainAddress(address: string): boolean {
    return this.filter.contains(Address(address))
  }
}
```
