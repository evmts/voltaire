...
pub const VersionedHash = Hash.Hash;
...
pub fn commitment_to_versioned_hash(commitment: BlobCommitment) VersionedHash {
    ...
    versioned[0] = BLOB_COMMITMENT_VERSION_KZG;
    @memcpy(versioned[1..32], sha256_hash[1..32]);
    
    return versioned;
}

// Validate versioned hash
pub fn is_valid_versioned_hash(hash: VersionedHash) bool {
...
test "commitment to versioned hash" {
    ...
    try testing.expectEqual(BLOB_COMMITMENT_VERSION_KZG, versioned_hash[0]);
...
test "invalid versioned hash" {
    var hash: VersionedHash = [_]u8{0x00} ** 32;
    hash[0] = 0x02; // Invalid version
    
    try testing.expect(!is_valid_versioned_hash(hash));
}
...
pub const BlobTransaction = struct {
    ...
    pub fn validate(self: *const BlobTransaction) !void {
        ...
        if (self.max_fee_per_blob_gas == 0) {
            return error.ZeroMaxFeePerBlobGas;
        }
    }
...
test "blob transaction validation failures" {
    ...
    try testing.expectError(error.NoBlobs, tx.validate());
    ...
    try testing.expectError(error.TooManyBlobs, tx.validate());
    
    // Invalid versioned hash
    var invalid_hash = [_]u8{0x00} ** 32;
    invalid_hash[0] = 0xFF; // Invalid version
    
    tx = BlobTransaction{
        .max_fee_per_blob_gas = 1000,
        .blob_versioned_hashes = &[_]VersionedHash{invalid_hash},
    };
    try testing.expectError(error.InvalidVersionedHash, tx.validate());
    ...
    try testing.expectError(error.ZeroMaxFeePerBlobGas, tx.validate());
}
...
test "blob sidecar" {
    ...
    const hash = sidecar.versioned_hash();
    try testing.expect(is_valid_versioned_hash(hash));
}
...
test "blob gas economics" {
    ...
    var blob_gas_used = 4 * BLOB_GAS_PER_BLOB;
...
pub fn encode_blob_data(allocator: std.mem.Allocator, data: []const u8) !Blob {
    _ = allocator;
...
pub fn decode_blob_data(allocator: std.mem.Allocator, blob: Blob) ![]u8 {
    // Decode length
    const len = std.mem.bytesToValue(usize, blob[0..8]);
    
    if (len > BYTES_PER_BLOB - 8) {
        return error.InvalidBlobData;
    }
...
test "blob data too large" {
    ...
    try testing.expectError(error.DataTooLarge, result);
}