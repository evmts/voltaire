...
// Direct constructor - preferred pattern
const addr = Address('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e')
addr.toHex()
addr.equals(otherAddr)

// For checksum methods, pass keccak256 to constructor
const addrWithCrypto = Address('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e', {
  keccak256: Keccak256.hash
})
addrWithCrypto.toChecksummed()  // Instance method works
```
...
toHex(addr)
toChecksummed(addr)  // Static function has keccak256 auto-injected
...
// Address() direct call preferred over Address.from()
const addr = Address('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e')
Address.toHex(addr)
Address.toChecksummed(addr)  // Static method has keccak256 auto-injected
```
...
<Note>
**Constructor patterns**: Both `Address('0x...')` and `Address.from('0x...')` work identically. The direct call `Address('0x...')` is preferred for conciseness. Use `Address.fromPrivateKey()`, `Address.fromPublicKey()`, etc. for specialized constructors.
</Note>
...
// Create from hex string - direct call preferred
const addr = Address('0x742d35Cc6634C0532925a3b844Bc9e7595f51e3e');

// Get checksummed representation (static method has keccak256 auto-injected)
...
<Tab title="From Private Key">

```typescript
import { Address } from '@tevm/voltaire/Address';

// Derive address from private key (uses fromPrivateKey internally)
const privateKey = new Uint8Array([...]);  // 32-byte private key
...
const addr2 = Address.fromPublicKey(pubKey);  // 65-byte uncompressed public key
```
...
const initCode = Bytecode('0x6080...');
...
try {
  ...
  // Use static method for checksum (has auto-injected keccak256)
  console.log(`Valid address: ${Address.toChecksummed(addr)}`);
} catch (e) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          const code = \\\`// Address utilities demo\\\\n\\\` +
            ...
            \\\`\\\\n\\\` +
            \\\`// Compare addresses\\\\n\\\` +
            \\\`const addr2 = Address("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045");\\\\n\\\` +
