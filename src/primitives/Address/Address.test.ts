...
describe('Address', () => {
  ...
  describe('fromHex', () => {
    ...
    it('throws on missing 0x prefix', () => {
      expect(() => Address.fromHex('742d35cc6634c0532925a3b844bc9e7595f251e3')).toThrow(Address.InvalidHexFormatError);
    });

    it('throws on invalid length', () => {
      expect(() => Address.fromHex('0x742d35cc')).toThrow(Address.InvalidHexFormatError);
      expect(() => Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3ff')).toThrow(Address.InvalidHexFormatError);
    });

    it('throws on invalid hex characters', () => {
      expect(() => Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251eZ')).toThrow(Address.InvalidHexStringError);
    });
  });
  ...
  describe('fromBytes', () => {
    ...
    it('throws on invalid length', () => {
      expect(() => Address.fromBytes(new Uint8Array(19))).toThrow(Address.InvalidAddressLengthError);
      expect(() => Address.fromBytes(new Uint8Array(21))).toThrow(Address.InvalidAddressLengthError);
      expect(() => Address.fromBytes(new Uint8Array(0))).toThrow(Address.InvalidAddressLengthError);
    });
  });
  ...
  describe('fromNumber', () => {
    ...
    it('throws on negative bigint', () => {
      expect(() => Address.fromNumber(-1n)).toThrow(Address.InvalidValueError);
      expect(() => Address.fromNumber(-1000n)).toThrow(Address.InvalidValueError);
    });

    it('throws on negative number', () => {
      expect(() => Address.fromNumber(-1)).toThrow(Address.InvalidValueError);
      expect(() => Address.fromNumber(-12345)).toThrow(Address.InvalidValueError);
    });
  });
  ...
  describe('fromPublicKey', () => {
    ...
    it('produces deterministic address', () => {
      ...
      expect(Address.equals.call(addr1, addr2)).toBe(true);
    });
  });
  ...
  describe('toHex', () => {
    ...
    it('returns branded Hex type', () => {
      const addr = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      const hex: Hex = Address.toHex.call(addr);
  ...
  describe('isZero', () => {
    it('returns true for zero address', () => {
      const addr = Address.zero();
      expect(Address.isZero.call(addr)).toBe(true);
    });

    it('returns false for non-zero address', () => {
      const addr = Address.fromNumber(1);
      expect(Address.isZero.call(addr)).toBe(false);
    });
  ...
  describe('equals', () => {
    ...
    it('returns false for different addresses', () => {
      ...
      expect(Address.equals.call(addr1, addr2)).toBe(false);
    });

    it('returns true for same instance', () => {
      const addr = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      expect(Address.equals.call(addr, addr)).toBe(true);
    });
  });
  ...
  describe('isValid', () => {
    ...
    it('validates correct hex without 0x prefix', () => {
      expect(Address.isValid('742d35cc6634c0532925a3b844bc9e7595f251e3')).toBe(true);
      expect(Address.isValid('0000000000000000000000000000000000000000')).toBe(true);
    });

    it('rejects invalid formats', () => {
      expect(Address.isValid('0x742d35cc')).toBe(false);
      expect(Address.isValid('0x742d35cc6634c0532925a3b844bc9e7595f251e3ff')).toBe(false);
    ...
    it('rejects invalid characters', () => {
      expect(Address.isValid('0x742d35cc6634c0532925a3b844bc9e7595f251eZ')).toBe(false);
      expect(Address.isValid('0x742d35cc6634c0532925a3b844bc9e7595f251e ')).toBe(false);
    });
  });
  ...
  describe('isValidChecksum', () => {
    it('validates correct checksummed addresses', () => {
      // Verified against ethers.js v6.15.0 and EIP-55 spec
      expect(Address.isValidChecksum('0x742d35Cc6634c0532925a3b844bc9e7595F251E3')).toBe(true);
      expect(Address.isValidChecksum('0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAed')).toBe(true);
    });

    it('rejects incorrect checksums', () => {
      expect(Address.isValidChecksum('0x742d35cc6634C0532925a3b844Bc9e7595f251e3')).toBe(false);
      expect(Address.isValidChecksum('0x742d35CC6634c0532925a3b844bc9e7595f251e3')).toBe(false);
    });
    ...
    it('validates zero address', () => {
      expect(Address.isValidChecksum('0x0000000000000000000000000000000000000000')).toBe(true);
    });
  ...
  describe('is', () => {
    ...
    it('returns false for non-Uint8Array', () => {
      ...
      expect(Address.is('0x742d35cc6634c0532925a3b844bc9e7595f251e3')).toBe(false);
  ...
  describe('calculateCreateAddress', () => {
    it('calculates address for nonce 0 - known vector', () => {
      ...
      const contractAddr = Address.calculateCreateAddress.call(deployer, 0n);
      // Verified against ethers.js v6.15.0 getContractAddress()
      expect(Address.toHex.call(contractAddr)).toBe('0xfa0d70e1d133b2f6cf2777547e8b12810d31b69a');
    });

    it('calculates address for nonce 1 - known vector', () => {
      const deployer = Address.fromHex('0xa0cf798816d4b9b9866b5330eea46a18382f251e');
      const contractAddr = Address.calculateCreateAddress.call(deployer, 1n);
      // Verified against ethers.js v6.15.0 getContractAddress()
      expect(Address.toHex.call(contractAddr)).toBe('0xc244efd33e63b31c1c5e15c17e31040318e68ffa');
    });

    it('calculates address for nonce 2', () => {
      const deployer = Address.fromHex('0xa0cf798816d4b9b9866b5330eea46a18382f251e');
      const contractAddr = Address.calculateCreateAddress.call(deployer, 2n);
      // Verified against ethers.js v6.15.0 getContractAddress()
      expect(Address.toHex.call(contractAddr)).toBe('0xfc17cf067a73c2229a3b463e350143b77a3d8604');
    });

    it('calculates address for nonce 3', () => {
      const deployer = Address.fromHex('0xa0cf798816d4b9b9866b5330eea46a18382f251e');
      const contractAddr = Address.calculateCreateAddress.call(deployer, 3n);
      // Verified against ethers.js v6.15.0 getContractAddress()
      expect(Address.toHex.call(contractAddr)).toBe('0xf0383434be7f79bf1cc399de5720ba73a248025c');
    });

    it('produces deterministic addresses', () => {
      const deployer = Address.fromHex('0x742d35Cc6634C0532925a3b8D39c0E6cfC8C74E4');
      const addr1 = Address.calculateCreateAddress.call(deployer, 42n);
      const addr2 = Address.calculateCreateAddress.call(deployer, 42n);
      expect(Address.equals.call(addr1, addr2)).toBe(true);
    });
    ...
    it('produces different addresses for different deployers', () => {
      const deployer1 = Address.fromHex('0x742d35Cc6634C0532925a3b8D39c0E6cfC8C74E4');
      const deployer2 = Address.fromHex('0x8ba1f109551bD432803012645Ac136c69b95Ee4D');
      const addr1 = Address.calculateCreateAddress.call(deployer1, 0n);
      const addr2 = Address.calculateCreateAddress.call(deployer2, 0n);
      expect(Address.equals.call(addr1, addr2)).toBe(false);
    });
    ...
    it('handles boundary nonce values', () => {
      ...
      for (const nonce of nonces) {
        const addr = Address.calculateCreateAddress.call(deployer, nonce);
        expect(addr.length).toBe(20);
      }
    });
    ...
    it('handles large nonces', () => {
      ...
      const addr = Address.calculateCreateAddress.call(deployer, largeNonce);
      expect(addr.length).toBe(20);
    });

    it('handles zero address deployer', () => {
      const deployer = Address.zero();
      const addr1 = Address.calculateCreateAddress.call(deployer, 0n);
      const addr2 = Address.calculateCreateAddress.call(deployer, 1n);
    ...
    it('handles max address deployer', () => {
      const deployer = Address.fromHex('0xffffffffffffffffffffffffffffffffffffffff');
      const addr = Address.calculateCreateAddress.call(deployer, 0n);
      expect(addr.length).toBe(20);
    });
    ...
    it('produces different addresses for sequential nonces', () => {
      ...
      for (let i = 0; i < addresses.length; i++) {
        for (let j = i + 1; j < addresses.length; j++) {
          expect(Address.equals.call(addresses[i]!, addresses[j]!)).toBe(false);
        }
      }
    });
    ...
    it('throws on negative nonce', () => {
      const deployer = Address.fromHex('0x742d35Cc6634C0532925a3b8D39c0E6cfC8C74E4');
      expect(() => Address.calculateCreateAddress.call(deployer, -1n)).toThrow(Address.InvalidValueError);
    });
  });
  ...
  describe('calculateCreate2Address', () => {
    it('calculates CREATE2 address with zero values', () => {
      ...
      expect(Address.toHex.call(addr)).toBe('0x4d1a2e2bb4f88f0250f26ffff098b0b30b26bf38');
    });

    it('produces deterministic addresses', () => {
      const deployer = Address.fromHex('0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef');
      const salt = new Uint8Array(32);
      salt[0] = 0x12;
      salt[1] = 0x34;
      ...
      const addr1 = Address.calculateCreate2Address.call(deployer, salt, initCode);
    ...
    it('throws on invalid salt length', () => {
      ...
      expect(() => Address.calculateCreate2Address.call(deployer, invalidSalt, initCode)).toThrow();
    });
  });
  ...
  describe('compare', () => {
    it('returns 0 for equal addresses', () => {
      const addr1 = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      const addr2 = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      expect(Address.compare.call(addr1, addr2)).toBe(0);
    });
    ...
    it('returns -1 when first is less than second', () => {
      ...
      expect(Address.compare.call(addr1, addr2)).toBe(-1);
    });
    ...
    it('can be used to sort addresses', () => {
      ...
      addrs.sort((a, b) => Address.compare.call(a, b));
      expect(Address.toU256.call(addrs[0]!)).toBe(1n);
      expect(Address.toU256.call(addrs[1]!)).toBe(2n);
      expect(Address.toU256.call(addrs[2]!)).toBe(3n);
    });
  });
  ...
  describe('lessThan', () => {
    it('returns true when first is less than second', () => {
      ...
      expect(Address.lessThan.call(addr1, addr2)).toBe(true);
    });

    it('returns false when first is equal to second', () => {
      const addr1 = Address.fromNumber(1);
      const addr2 = Address.fromNumber(1);
      expect(Address.lessThan.call(addr1, addr2)).toBe(false);
    });
  ...
  describe('greaterThan', () => {
    it('returns true when first is greater than second', () => {
      const addr1 = Address.fromNumber(2);
      const addr2 = Address.fromNumber(1);
      expect(Address.greaterThan.call(addr1, addr2)).toBe(true);
    });

    it('returns false when first is equal to second', () => {
      const addr1 = Address.fromNumber(1);
      const addr2 = Address.fromNumber(1);
      expect(Address.greaterThan.call(addr1, addr2)).toBe(false);
    });
  ...
  describe('toShortHex', () => {
    ...
    it('formats address with custom lengths', () => {
      const addr = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      const short = Address.toShortHex.call(addr, 8, 6);
    ...
    it('returns full address if lengths exceed 40', () => {
      const addr = Address.fromHex('0x742d35cc6634c0532925a3b844bc9e7595f251e3');
      const short = Address.toShortHex.call(addr, 20, 20);
  ...
  describe('Address.from()', () => {
    ...
    it('throws on negative number', () => {
      expect(() => Address.from(-1)).toThrow(Address.InvalidValueError);
    });

    it('throws on invalid hex string', () => {
      expect(() => Address.from('0x742d35cc')).toThrow(Address.InvalidHexFormatError);
    });

    it('throws on invalid Uint8Array length', () => {
      expect(() => Address.from(new Uint8Array(19))).toThrow(Address.InvalidAddressLengthError);
    });

    it('throws on unsupported type', () => {
      expect(() => Address.from(null as any)).toThrow(Address.InvalidValueError);
      expect(() => Address.from({} as any)).toThrow(Address.InvalidValueError);
    });
  });
  ...
  describe('round-trip conversions', () => {
    it('fromHex -> toHex', () => {
      ...
      const result = Address.toHex.call(addr);
    ...
    it('fromNumber -> toU256', () => {
      const original = 0x742d35Cc6634C0532925a3b844Bc9e7595f251e3n;
      const addr = Address.fromNumber(original);
      const result = Address.toU256.call(addr);
      expect(result).toBe(original);
    });
    ...
    it('fromBytes -> toHex -> fromHex -> equals', () => {
      ...
      const hex = Address.toHex.call(addr1);
