...
describe("Transaction RPC format", () => {
	...
	describe("error handling", () => {
		it("throws InvalidTransactionTypeError for unknown transaction type", () => {
			const rpc = {
				type: "0xff",
				nonce: "0x0",
				gasPrice: "0x1",
			...
			try {
				...
				expect(e).toBeInstanceOf(InvalidTransactionTypeError);
				expect((e as InvalidTransactionTypeError).name).toBe(
					"InvalidTransactionTypeError",
				);
			}
		});

		it("throws InvalidLengthError for invalid address length", () => {
			const rpc = {
				type: "0x0",
				nonce: "0x0",
				gasPrice: "0x1",
				gas: "0x5208",
				to: "0x1234", // Invalid - only 2 bytes
				value: "0x0",
				data: "0x",
				v: "0x1b",
				r: `0x${"11".repeat(32)}`,
			...
			try {
				...
				expect((e as InvalidLengthError).name).toBe("InvalidLengthError");
			}
		});
	});
});
