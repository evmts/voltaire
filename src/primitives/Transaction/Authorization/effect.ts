...
export type AuthorizationBrand = BrandedAuthorization &
	Brand.Brand<"Authorization">;
...
export class AuthorizationSchema extends Schema.Class<AuthorizationSchema>(
	"Authorization",
)({
	...
	yParity: Schema.Number.pipe(
		Schema.filter((n): n is number => n === 0 || n === 1, {
			message: () => "yParity must be 0 or 1",
		}),
	),
	r: Schema.Uint8ArrayFromSelf.pipe(
		Schema.filter(isHash32, { message: () => "r must be 32 bytes" }),
	),
	s: Schema.Uint8ArrayFromSelf.pipe(
		Schema.filter(isHash32, { message: () => "s must be 32 bytes" }),
	),
}) {
	get authorization(): BrandedAuthorization {
		return this as unknown as BrandedAuthorization;
	}
	get branded(): AuthorizationBrand {
		return this.authorization as AuthorizationBrand;
	}
	...
	getSigningHash(): Uint8Array {
		return _getSigningHash(this.authorization) as unknown as Uint8Array;
	}
	verifySignature(): boolean {
		return _verifySignature(this.authorization);
	}
	getAuthorizer(): Uint8Array {
		return _getAuthorizer(this.authorization) as unknown as Uint8Array;
	}
}
