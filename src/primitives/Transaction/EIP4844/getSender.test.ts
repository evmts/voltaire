...
describe("TransactionEIP4844.getSender", () => {
	it("recovers sender from valid signature (yParity 0)", () => {
		...
		const recoveredAddress = TransactionEIP4844.getSender(signedTx);
	...
	it("recovers sender with multiple blob hashes", () => {
		const privateKey = PrivateKey.from(
			"0x0123456789012345678901234567890123456789012345678901234567890123",
		);
		const publicKey = Secp256k1.getPublicKey(privateKey);
	...
	it("throws on invalid signature", () => {
		...
		expect(() => TransactionEIP4844.getSender(tx)).toThrow();
	});
