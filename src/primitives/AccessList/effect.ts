...
const ItemSchema = Schema.Struct({
  ...
  storageKeys: Schema.Array(Schema.Uint8ArrayFromSelf.pipe(Schema.filter(isHash32, { message: () => 'storage key must be 32 bytes' })))
})

export class AccessListSchema extends Schema.Class<AccessListSchema>("AccessList")({
  ...
  get branded(): AccessListBrand { return this.accessList as AccessListBrand }

  static fromBranded(list: AccessListBrand): AccessListSchema {
    return new AccessListSchema({ value: list as unknown as { address: Uint8Array; storageKeys: Uint8Array[] }[] })
  }
  ...
  toBytes(): Uint8Array { return _toBytes(this.accessList) }
  gasCost(): bigint { return _gasCost(this.accessList) }
  gasSavings(): bigint { return _gasSavings(this.accessList) }
  hasSavings(): boolean { return _hasSavings(this.accessList) }
  includesAddress(address: Uint8Array): boolean { return _includesAddress(this.accessList, address as any) }
  includesStorageKey(address: Uint8Array, storageKey: Uint8Array): boolean { return _includesStorageKey(this.accessList, address as any, storageKey as any) }
  keysFor(address: Uint8Array): readonly Uint8Array[] | undefined { return _keysFor(this.accessList, address as any) as any }
  deduplicate(): AccessListSchema { return AccessListSchema.fromBranded(_deduplicate(this.accessList) as any) }
  withAddress(address: Uint8Array): AccessListSchema { return AccessListSchema.fromBranded(_withAddress(this.accessList, address as any) as any) }
  withStorageKey(address: Uint8Array, storageKey: Uint8Array): AccessListSchema { return AccessListSchema.fromBranded(_withStorageKey(this.accessList, address as any, storageKey as any) as any) }
  merge(...others: (AccessListSchema | BrandedAccessList)[]): AccessListSchema {
    ...
    return AccessListSchema.fromBranded(_merge(this.accessList, ...lists) as any)
  }
  assertValid(): void { _assertValid(this.accessList) }
}
