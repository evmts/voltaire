...
const list = AccessList.from(items).deduplicate();
```
...
function buildSwapAccessList(
  ...
  const reserve0Slot = Hash.fromHex('0x08');  // reserves slot
  const reserve1Slot = Hash.fromHex('0x08');
...
async function detectAccessList(
  ...
  const list = AccessList.fromBytes(detected.accessList);
...
function safeConstruction(data: unknown): BrandedAccessList {
  try {
    ...
    if (data instanceof Uint8Array) {
      const list = AccessList.fromBytes(data);
      AccessList.assertValid(list);
      return list;
    }

    if (Array.isArray(data)) {
      const list = AccessList.from(data);
      AccessList.assertValid(list);
      return list;
    }
...
const fixtures = {
  ...
  singleAddress: AccessList.from([
    { address: testAddr1, storageKeys: [] }
  ]),

  withKeys: AccessList.from([
    { address: testAddr1, storageKeys: [testKey1, testKey2] }
  ]),

  multipleAddresses: AccessList.from([
...
- [Gas Optimization](./gas-optimization.mdx) - Gas analysis
- [Manipulation](./manipulation.mdx) - Building lists
