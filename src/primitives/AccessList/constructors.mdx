...
Loose constructor accepting items array or RLP bytes.

```typescript
AccessList.from(value: readonly Item[] | Uint8Array): BrandedAccessList
```
...
**Notes:**
- Same as factory function: `AccessList(value)`
...
**Example:**
```typescript
const bytes = new Uint8Array([...]); // RLP bytes
...
Convenience factory - same as `AccessList.from()`.

**Example:**
```typescript
const list = AccessList([
  { address: token, storageKeys: [slot1, slot2] }
]);
```
...
const clone = AccessList.from(original);
```
...
```typescript
const list = AccessList.from(externalData);
...
- [Manipulation](./manipulation.mdx) - Modifying lists
- [Conversions](./conversions.mdx) - toBytes encoding
