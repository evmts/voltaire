...
describe("StorageKey.is", () => {
	it("returns true for valid storage key", () => {
		const key: StorageKeyType = { address: addr1, slot: 0n };
...
describe("StorageKey.equals", () => {
	it("returns true for identical keys", () => {
		const key1: StorageKeyType = { address: addr1, slot: 42n };
		const key2: StorageKeyType = { address: addr1, slot: 42n };
		expect(StorageKey.equals(key1, key2)).toBe(true);
	});

	it("returns true for same reference", () => {
		const key: StorageKeyType = { address: addr1, slot: 42n };
		expect(StorageKey.equals(key, key)).toBe(true);
	});

	it("returns false for different slots", () => {
		const key1: StorageKeyType = { address: addr1, slot: 42n };
		const key2: StorageKeyType = { address: addr1, slot: 43n };
		expect(StorageKey.equals(key1, key2)).toBe(false);
	});

	it("returns false for different addresses", () => {
		const key1: StorageKeyType = { address: addr1, slot: 42n };
		const key2: StorageKeyType = { address: addr2, slot: 42n };
		expect(StorageKey.equals(key1, key2)).toBe(false);
	});

	it("returns false for different addresses and slots", () => {
		const key1: StorageKeyType = { address: addr1, slot: 42n };
		const key2: StorageKeyType = { address: addr2, slot: 43n };
		expect(StorageKey.equals(key1, key2)).toBe(false);
	});

	it("handles zero values correctly", () => {
		const key1: StorageKeyType = { address: zeroAddr, slot: 0n };
		const key2: StorageKeyType = { address: zeroAddr, slot: 0n };
		expect(StorageKey.equals(key1, key2)).toBe(true);
	});
	...
	it("handles maximum slot values", () => {
		const maxSlot =
			0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffn;
		const key1: StorageKeyType = { address: addr1, slot: maxSlot };
		const key2: StorageKeyType = { address: addr1, slot: maxSlot };
		expect(StorageKey.equals(key1, key2)).toBe(true);
	});
...
describe("StorageKey.fromString", () => {
	it("parses valid string back to key", () => {
		const original: StorageKeyType = { address: addr1, slot: 42n };
	...
	it("round-trips correctly", () => {
		const key: StorageKeyType = { address: addr2, slot: 12345n };
		const str = StorageKey.toString(key);
		const parsed = StorageKey.fromString(str);
		expect(parsed).toBeDefined();
	...
	it("handles large slot values", () => {
		const largeSlot =
			0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffn;
		const key: StorageKeyType = { address: addr1, slot: largeSlot };
		const str = StorageKey.toString(key);
		const parsed = StorageKey.fromString(str);
		expect(parsed).toBeDefined();
...
describe("StorageKey.hashCode", () => {
	...
	it("handles maximum slot values", () => {
		const maxSlot =
			0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffn;
		const key: StorageKeyType = { address: addr1, slot: maxSlot };
		const hash = StorageKey.hashCode(key);
		expect(typeof hash).toBe("number");
	});
...
describe("StorageKey in Map", () => {
	...
	it("handles multiple keys in Map", () => {
		...
		const key2: StorageKeyType = { address: addr2, slot: 1n };
		const key3: StorageKeyType = { address: addr3, slot: 2n };
	...
	it("distinguishes keys with same address but different slots", () => {
		const map = new Map<string, bigint>();
		const key1: StorageKeyType = { address: addr1, slot: 0n };
		const key2: StorageKeyType = { address: addr1, slot: 1n };
...
describe("StorageKey edge cases", () => {
	...
	it("handles address with all 0xFF", () => {
		const key: StorageKeyType = { address: maxAddr, slot: 0n };
		expect(StorageKey.is(key)).toBe(true);
		const str = StorageKey.toString(key);
		const parsed = StorageKey.fromString(str);
		expect(parsed).toBeDefined();
	...
	it("handles slot at boundaries", () => {
		...
		for (const slot of slots) {
			const key: StorageKeyType = { address: addr1, slot };
			const str = StorageKey.toString(key);
			const parsed = StorageKey.fromString(str);
			expect(parsed).toBeDefined();
	...
	it("handles pattern addresses", () => {
		...
		const key: StorageKeyType = { address: patternAddr, slot: 123456789n };
		const str = StorageKey.toString(key);
		const parsed = StorageKey.fromString(str);
		expect(parsed).toBeDefined();
