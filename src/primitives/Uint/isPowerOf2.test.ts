...
describe("Uint256.isPowerOf2", () => {
	describe("true cases", () => {
		...
		it("returns true for 2^128", () => {
			expect(isPowerOf2(from(1n << 128n))).toBe(true);
		});

		it("returns true for 2^255", () => {
			expect(isPowerOf2(from(1n << 255n))).toBe(true);
		});
	});
	...
	describe("edge cases", () => {
		it("returns false for power of 2 minus 1", () => {
			expect(isPowerOf2(from((1n << 128n) - 1n))).toBe(false);
		});
