...
describe("BinaryTree", () => {
	describe("addressToKey", () => {
		...
		test("throws InvalidAddressLengthError with correct name", () => {
			const addr = new Uint8Array(19);
			try {
				BinaryTree.addressToKey(addr);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("InvalidAddressLengthError");
			}
		});
	});
	...
	describe("splitKey", () => {
		...
		test("throws InvalidKeyLengthError with correct name", () => {
			const k = new Uint8Array(31);
			try {
				BinaryTree.splitKey(k);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("InvalidKeyLengthError");
			}
		});
	});
