...
export function hexToBytes(hex: string): Uint8Array {
	try {
		...
		if (message.includes("prefix") || message.includes("0x")) {
			throw new InvalidFormatError(message, { value: hex, cause: e instanceof Error ? e : undefined });
		}
		if (message.includes("odd") || message.includes("length")) {
			throw new OddLengthError(message, { value: hex, cause: e instanceof Error ? e : undefined });
		}
		if (message.includes("character") || message.includes("invalid")) {
			throw new InvalidCharacterError(message, { value: hex, cause: e instanceof Error ? e : undefined });
		}
