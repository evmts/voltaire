...
export function fromHex(hex) {
	...
	if (normalized.length !== 64) {
		throw new InvalidTransactionHashFormatError(
			`TransactionHash hex must be 64 characters (32 bytes), got ${normalized.length}`,
			{
				value: hex,
	...
	if (!/^[0-9a-f]{64}$/.test(normalized)) {
		throw new InvalidTransactionHashFormatError(
			"TransactionHash hex contains invalid characters",
			{
				value: hex,
