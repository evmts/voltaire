---
title: Bytes (N-byte)
description: Foundation of Tevm's type system - generic fixed-size byte types with compile-time length validation
---

# Bytes (N-byte)

Generic fixed-size byte types that form the foundation of Tevm's type system. Bytes represents N bytes with compile-time type safety through TypeScript branding and hardcoded length properties.

<Info>
**Type System Foundation** - Bytes is the base pattern for all fixed-size types in Tevm. Address (Bytes20), Keccak256Hash (Bytes32), and all other fixed-size primitives extend this pattern.
</Info>
...
```typescript
import * as Bytes from 'tevm/primitives/Bytes';
...
```typescript
import * as Bytes from 'tevm/primitives/Bytes/BrandedBytes';
...
```typescript
import { Bytes, Bytes32 } from 'tevm';
...
Tevm provides pre-defined Bytes&lt;N&gt; types:

### Bytes8 (64-bit)
```typescript
import * as Bytes8 from 'tevm/primitives/Bytes/Bytes8';
...
- `fromNumber(n)` - From number (big-endian)
- `fromBigInt(n)` - From bigint (big-endian)
- `random(length)` - Generate cryptographically random bytes
...
- `toNumber(bytes)` - To number (big-endian)
- `toBigInt(bytes)` - To bigint (big-endian)

### Validation
- `isBytes(value)` - Type guard to check if value is Uint8Array
- `assert(value)` - Assert value is bytes (throws if not)

### Padding & Trimming
- `padLeft(bytes, length)` - Left-pad with zeros
- `padRight(bytes, length)` - Right-pad with zeros
- `trimLeft(bytes)` - Remove leading zeros
- `trimRight(bytes)` - Remove trailing zeros
...
const wrong1: Bytes32.Bytes32Type = b16;

// Type error: cannot assign Bytes8 to Bytes32
const wrong2: Bytes32.Bytes32Type = b8;
...
function acceptBytes32(b: Bytes32.Bytes32Type) {
...
- [Keccak256](/crypto/keccak256) - 256-bit Keccak256 hashes (extends Bytes32)
