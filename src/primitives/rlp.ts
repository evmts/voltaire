 ...
 */
function decodeLengthBytes(data: Uint8Array, offset: number, lengthOfLength: number): number {
...
function decodeItem(data: Uint8Array, offset: number): [RlpDecoded, number] {
  ...
  if (prefix <= 0xbf) {
    ...
    // Check for leading zeros in length before decoding
    if (lengthOfLength > 1 && offset + 1 < data.length && data[offset + 1] === 0) {
      throw new LeadingZeros();
    }
