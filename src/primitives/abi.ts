...
function isDynamicType(type: string): boolean {
	return (
		type === AbiType.Bytes || type === AbiType.String || type.endsWith("[]")
	);
}
...
function encodeValue(type: string, value: AbiValue): Uint8Array {
	...
	if (type.endsWith("[]")) {
		...
		const totalLength =
			length.length + encodedElements.reduce((sum, e) => sum + e.length, 0);
...
export function encodeAbiParameters(
	...
	const totalLength =
		staticLength + dynamicParts.reduce((sum, part) => sum + part.length, 0);
	const result = new Uint8Array(totalLength);
	let offset = 0;
...
function decodeValue(
	...
	if (type === AbiType.String) {
		...
		return [
			...
			offset + 32 + Math.ceil(length / 32) * 32,
		];
	}
...
export function encodePacked(
	...
	for (let i = 0; i < params.length; i++) {
		...
		if (type.startsWith("uint") || type.startsWith("int")) {
			...
			const bitSize = sizeMatch ? Number.parseInt(sizeMatch[0]) : 256;
 ...
 */
function decodeBigInt(bytes: Uint8Array, offset = 0): bigint {
	let value = 0n;
	for (let i = 0; i < 32; i++) {
