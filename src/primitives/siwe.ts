 ...
 * All types and operations namespaced under Siwe for intuitive access.
 *
 * @example
 * ```typescript
...
export namespace Siwe {
  ...
  export namespace Message {
     ...
     * ```
     */
    export function format<T extends Siwe.Message>(message: T): string {
      const lines: string[] = [];
     ...
     * ```
     */
    export function validate<T extends Siwe.Message>(
      message: T,
      options?: { now?: Date },
    ): ValidationResult {
      // Domain validation
   ...
   * ```typescript
   * const message: Siwe.Message = { ... };
   * const richMessage = attach(message, { format: Siwe.format });
   ...
   * ```typescript
   * const message: Siwe.Message = { ... };
   * const richMessage = attach(message, { validate: Siwe.validate });
   ...
   * ```typescript
   * const message: Siwe.Message = { ... };
   * const richMessage = attach(message, { getMessageHash: Siwe.getMessageHash });
   ...
   * ```typescript
   * const message: Siwe.Message = { ... };
   * const richMessage = attach(message, { verify: Siwe.verify });
...
/**
 * SIWE Message type alias for convenience
 *
 * Uses TypeScript declaration merging - Siwe is both a namespace and a type.
 */
export type Siwe = Siwe.Message;
