...
describe("Hardfork.isAtLeast", () => {
  it("returns true when current >= target", () => {
    expect(Hardfork.isAtLeast(Hardfork.Id.CANCUN, Hardfork.Id.SHANGHAI)).toBe(true);
    expect(Hardfork.isAtLeast(Hardfork.Id.SHANGHAI, Hardfork.Id.SHANGHAI)).toBe(true);
  });

  it("returns false when current < target", () => {
    expect(Hardfork.isAtLeast(Hardfork.Id.BERLIN, Hardfork.Id.LONDON)).toBe(false);
    expect(Hardfork.isAtLeast(Hardfork.Id.FRONTIER, Hardfork.Id.CANCUN)).toBe(false);
  });
});

describe("Hardfork.isBefore", () => {
  it("returns true when current < target", () => {
    expect(Hardfork.isBefore(Hardfork.Id.BERLIN, Hardfork.Id.LONDON)).toBe(true);
    expect(Hardfork.isBefore(Hardfork.Id.FRONTIER, Hardfork.Id.HOMESTEAD)).toBe(true);
  });

  it("returns false when current >= target", () => {
    expect(Hardfork.isBefore(Hardfork.Id.CANCUN, Hardfork.Id.SHANGHAI)).toBe(false);
    expect(Hardfork.isBefore(Hardfork.Id.LONDON, Hardfork.Id.LONDON)).toBe(false);
  });
});

describe("Hardfork.isAfter", () => {
  it("returns true when current > target", () => {
    expect(Hardfork.isAfter(Hardfork.Id.CANCUN, Hardfork.Id.SHANGHAI)).toBe(true);
    expect(Hardfork.isAfter(Hardfork.Id.PRAGUE, Hardfork.Id.BERLIN)).toBe(true);
  });

  it("returns false when current <= target", () => {
    expect(Hardfork.isAfter(Hardfork.Id.BERLIN, Hardfork.Id.LONDON)).toBe(false);
    expect(Hardfork.isAfter(Hardfork.Id.SHANGHAI, Hardfork.Id.SHANGHAI)).toBe(false);
  });
});
...
describe("Hardfork.isEqual", () => {
  it("returns true when hardforks are equal", () => {
    ...
    expect(Hardfork.isEqual(Hardfork.Id.FRONTIER, Hardfork.Id.FRONTIER)).toBe(true);
  });

  it("returns false when hardforks are not equal", () => {
    expect(Hardfork.isEqual(Hardfork.Id.CANCUN, Hardfork.Id.SHANGHAI)).toBe(false);
    expect(Hardfork.isEqual(Hardfork.Id.BERLIN, Hardfork.Id.LONDON)).toBe(false);
  });
});

describe("Hardfork.compare", () => {
  it("returns negative when a < b", () => {
    expect(Hardfork.compare(Hardfork.Id.BERLIN, Hardfork.Id.LONDON)).toBeLessThan(0);
    expect(Hardfork.compare(Hardfork.Id.FRONTIER, Hardfork.Id.CANCUN)).toBeLessThan(0);
  });

  it("returns zero when a == b", () => {
    expect(Hardfork.compare(Hardfork.Id.CANCUN, Hardfork.Id.CANCUN)).toBe(0);
    expect(Hardfork.compare(Hardfork.Id.SHANGHAI, Hardfork.Id.SHANGHAI)).toBe(0);
  });

  it("returns positive when a > b", () => {
    expect(Hardfork.compare(Hardfork.Id.PRAGUE, Hardfork.Id.SHANGHAI)).toBeGreaterThan(0);
    expect(Hardfork.compare(Hardfork.Id.LONDON, Hardfork.Id.BERLIN)).toBeGreaterThan(0);
  });
});
...
describe("Hardfork.fromString", () => {
  ...
  it("parses alias names", () => {
    ...
    expect(Hardfork.fromString("constantinoplefix")).toBe(Hardfork.Id.PETERSBURG);
  });
...
describe("Hardfork edge cases", () => {
  ...
  it("comparison operations work across full range", () => {
    expect(Hardfork.isAtLeast(Hardfork.Id.OSAKA, Hardfork.Id.FRONTIER)).toBe(true);
    expect(Hardfork.isBefore(Hardfork.Id.FRONTIER, Hardfork.Id.OSAKA)).toBe(true);
  });
});
