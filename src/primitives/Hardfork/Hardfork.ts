 ...
 * All types and functions exported at top-level.
 *
 * @example
 * ```typescript
 ...
 * @example
 * ```typescript
 * const current = Id.CANCUN;
 * if (isAtLeast(current, Id.SHANGHAI)) {
 ...
 * @example
 * ```typescript
 * const current = Id.BERLIN;
 * if (isBefore(current, Id.LONDON)) {
 ...
 * ```typescript
 * const current = Id.CANCUN;
 * if (isAfter(current, Id.SHANGHAI)) {
 ...
 * @example
 * ```typescript
 * compare(Id.BERLIN, Id.LONDON) // negative
 * compare(Id.CANCUN, Id.CANCUN) // 0
 * compare(Id.PRAGUE, Id.SHANGHAI) // positive
 * ```
 */
 ...
 * @example
 * ```typescript
 * const fork = fromString("cancun"); // Id.CANCUN
 * const fork2 = fromString("Paris"); // Id.MERGE
 ...
 * @example
 * ```typescript
 * toString(Id.CANCUN); // "cancun"
 * toString(Id.MERGE); // "merge"
 * ```
 */
 ...
 * @example
 * ```typescript
 * const fork = Id.CANCUN;
 * gte.call(fork, Id.SHANGHAI); // true
 * ```
 */
 ...
 * @example
 * ```typescript
 * const fork = Id.BERLIN;
 * lt.call(fork, Id.LONDON); // true
 * ```
 */
 ...
 * @example
 * ```typescript
 * const fork = Id.SHANGHAI;
 * lte.call(fork, Id.CANCUN); // true
 * ```
 */
 ...
 * @example
 * ```typescript
 * hasEIP1559(Id.LONDON); // true
 * hasEIP1559(Id.BERLIN); // false
 * ```
 */
 ...
 * @example
 * ```typescript
 * const fork = Id.LONDON;
 * supportsEIP1559.call(fork); // true
 * ```
 */
 ...
 * @example
 * ```typescript
 * hasEIP3855(Id.SHANGHAI); // true
 * hasEIP3855(Id.MERGE); // false
 * ```
 */
 ...
 * @example
 * ```typescript
 * hasEIP4844(Id.CANCUN); // true
 * hasEIP4844(Id.SHANGHAI); // false
 * ```
 */
 ...
 * @example
 * ```typescript
 * hasEIP1153(Id.CANCUN); // true
 * hasEIP1153(Id.SHANGHAI); // false
 * ```
 */
 ...
 * ```typescript
 * const fork = Id.CANCUN;
 * supportsTransientStorage.call(fork); // true
 * ```
 */
 ...
 * @example
 * ```typescript
 * isPostMerge(Id.SHANGHAI); // true
 * isPostMerge(Id.LONDON); // false
 * ```
 */
