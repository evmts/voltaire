	...
	describe("error cases", () => {
		test("invalid scheme", () => {
			try {
				...
				expect(e).toBeInstanceOf(InvalidTransactionUrlError);
				expect((e as InvalidTransactionUrlError).name).toBe(
					"InvalidTransactionUrlError",
				);
			}

			try {
				parse("bitcoin:0x1234");
				expect.fail("Should have thrown");
			} catch (e) {
				expect(e).toBeInstanceOf(InvalidTransactionUrlError);
				expect((e as InvalidTransactionUrlError).name).toBe(
		...
		test("error includes context", () => {
			try {
				...
				expect(error).toBeInstanceOf(InvalidTransactionUrlError);
				const e = error as InvalidTransactionUrlError;
				expect(e.name).toBe("InvalidTransactionUrlError");
				expect(e.details?.url).toBe("http://example.com");
				expect(e.details?.scheme).toBe("http");
			}
		});

		test("error has correct code property", () => {
			const error = new InvalidTransactionUrlError("Test error", {
				url: "invalid",
			});
