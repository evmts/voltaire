...
describe("CallData", () => {
	...
	describe("encode()", () => {
		...
		it("throws ParameterCountMismatchError on parameter count mismatch", () => {
			try {
				CallData.encode("transfer(address,uint256)", ["0x70997970..."]);
				expect.fail("Should have thrown");
			} catch (e) {
				expect(e).toBeInstanceOf(ParameterCountMismatchError);
				expect((e as ParameterCountMismatchError).name).toBe(
					"ParameterCountMismatchError",
				);
				expect((e as ParameterCountMismatchError).message).toContain(
					"Parameter count mismatch",
				);
			}
		});

		it("throws InvalidSignatureError on invalid signature format", () => {
			try {
				CallData.encode("invalid", []);
				expect.fail("Should have thrown");
			} catch (e) {
				expect(e).toBeInstanceOf(InvalidSignatureError);
				expect((e as InvalidSignatureError).name).toBe("InvalidSignatureError");
				expect((e as InvalidSignatureError).message).toContain(
	...
	describe("decode()", () => {
		...
		it("decodes calldata with ABI", () => {
			...
			// biome-ignore lint/suspicious/noExplicitAny: ABI type coercion for test
		...
		it("throws AbiItemNotFoundError on unknown selector", () => {
			const calldata = CallData.from("0x12345678");
			try {
				// biome-ignore lint/suspicious/noExplicitAny: ABI type coercion for test
				CallData.decode(calldata, abi as any);
				expect.fail("Should have thrown");
			} catch (e) {
				expect(e).toBeInstanceOf(AbiItemNotFoundError);
				expect((e as AbiItemNotFoundError).name).toBe("AbiItemNotFoundError");
				expect((e as AbiItemNotFoundError).message).toContain(
