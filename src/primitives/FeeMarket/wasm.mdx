...
**WASM call overhead (~1-2μs) exceeds calculation time (`less than 1μs`)**
...
```
BaseFee:                 ~250ns per call   (~4M ops/sec)
BlobBaseFee:             ~10μs per call    (~100K ops/sec)
...
WASM beneficial for:
- Heavy computation (greater than 10μs per operation)
...
- `BaseFee`: ~4M ops/sec
- `BlobBaseFee`: ~100K ops/sec (Taylor series)
...
- [Address](/primitives/address) - Keccak-256, checksums
- [Keccak256](/crypto/keccak256) - Hash computations
- [RLP](/primitives/rlp) - Encoding/decoding
