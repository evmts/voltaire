...
describe("EventLog.matchesTopics", () => {
	...
	it("matches with topic array (OR logic)", () => {
		...
		expect(
			EventLog.matchesTopics(log, [[topic0, topic1], topic1, topic2]),
		).toBe(true);
		expect(
			EventLog.matchesTopics(log, [topic0, [topic1, topic3], topic2]),
		).toBe(true);
	});
	...
	it("fails when topic array does not include log topic", () => {
		...
		expect(
			EventLog.matchesTopics(log, [topic0, [topic2, topic3], topic2]),
		).toBe(false);
	});
});
...
describe("EventLog.matchesFilter", () => {
	...
	it("matches filter with topics only", () => {
		...
		expect(EventLog.matchesFilter(log, { topics: [topic0, topic1] })).toBe(
			true,
		);
	...
	it("matches filter with block number range", () => {
		...
		expect(EventLog.matchesFilter(log, { fromBlock: 50n, toBlock: 150n })).toBe(
			true,
		);
		expect(
			EventLog.matchesFilter(log, { fromBlock: 100n, toBlock: 100n }),
		).toBe(true);
		expect(
			EventLog.matchesFilter(log, { fromBlock: 101n, toBlock: 150n }),
		).toBe(false);
	});
...
describe("EventLog edge cases", () => {
	...
	it("handles zero block number", () => {
		...
		expect(EventLog.matchesFilter(log, { fromBlock: 0n, toBlock: 100n })).toBe(
			true,
		);
	});
