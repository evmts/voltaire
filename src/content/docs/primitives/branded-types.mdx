...
Type-level tags that make structurally identical types distinguishable to TypeScript's type checker.
...
getUser(productId) // ✓ TypeScript allows this (both are numbers)
```

This compiles but is logically wrong - product ID shouldn't be accepted as user ID.
...
Brand exists only in type system - erased at runtime. Values are still plain numbers.
...
const user = { id: 1, __brand: "UserId" } // runtime __brand field conflicts!
```
