...
<TabItem label="Technology Stack">
...
<Tabs>
<TabItem label="Ethereum Mainnet">
```typescript
...
const baseFee = Gwei.from(30n)     // Set by protocol
const priorityFee = Gwei.from(2n)  // User sets (tip)
const maxFee = Gwei.from(50n)      // User sets max
...
const costWei = Uint.times(effectivePrice, Uint.from(21_000n))
...
const gasPrice = Gwei.from(1n)  // Usually 0.1-1 Gwei
...
<TabItem label="Polygon (Sidechain)">
```typescript
import { Chain } from '@tevm/voltaire'
import * as Gwei from '@tevm/voltaire/Gwei'
import * as Wei from '@tevm/voltaire/Wei'
...
const gasPrice = Gwei.from(30n)  // 20-100 Gwei range
...
function calculateTransferCost(
  ...
  const gasPriceWei = Gwei.toWei(Gwei.from(gasPriceGwei))
  const costWei = Uint.times(gasPriceWei, Uint.from(21_000n))
...
function calculateSwapCost(
  ...
  const gasPriceWei = Gwei.toWei(Gwei.from(gasPriceGwei))
  const costWei = Uint.times(gasPriceWei, Uint.from(150_000n))
  const costEth = Number(costWei) / 1e18
...
<TabItem label="Arbitrum/Optimism/Base (L2s)">
**Use when:**
...
function estimateFees(
  ...
  const calculateCost = (gas: bigint) => {
    const costWei = Uint.times(
      Gwei.toWei(Gwei.from(baseGasPrice)),
      Uint.from(gas)
    )
    return {
      eth: (Number(costWei) / 1e18).toFixed(6),
      gwei: Uint.dividedBy(costWei, Uint.from(1_000_000_000n)).toString()
    }
  }

  return {
    network: chain.name,
    baseGasPrice: Gwei.from(baseGasPrice),
