...
function processHash(hex: BrandedHex) {
  ...
  // TypeScript now knows hex is Sized<32>
...
function createHash(hex: BrandedHex): Hash {
  return Hex.assertSize(hex, 32)  // Returns Sized<32>
}

function createAddress(hex: BrandedHex): Address {
  return Hex.assertSize(hex, 20)  // Returns Sized<20>
}
```
