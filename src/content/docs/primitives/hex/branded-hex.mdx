...
- **Zero-overhead** [branded type](/primitives/branded-types) wrapping hex strings with `0x` prefix
...
Runtime-validated hex string with `0x` prefix. [Brand](/primitives/branded-types) prevents accidental mixing with plain strings. `Sized<N>` adds compile-time size tracking.

Defined in: [primitives/Hex/BrandedHex/BrandedHex.js](https://github.com/evmts/primitives/blob/main/src/primitives/Hex/BrandedHex/BrandedHex.js)
...
// Namespace usage
const hex = BrandedHex.from("0x1234")
const bytes = BrandedHex.toBytes(hex)
```
...
**Bundle impact:** Only `from`, `toBytes`, and `concat` included. Unused functions excluded from bundle.
...
assertSize(hex, 32)      // Sized<32> (throws if wrong size)
```
...
- **Using many methods** (>5 functions)
...
Defined in: [primitives/Hex/BrandedHex/errors.js](https://github.com/evmts/primitives/blob/main/src/primitives/Hex/BrandedHex/errors.js)
