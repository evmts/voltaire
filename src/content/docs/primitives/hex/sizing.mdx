...
Defined in: [primitives/Hex/BrandedHex/BrandedHex.js:7-11](https://github.com/evmts/voltaire/blob/main/src/primitives/Hex/BrandedHex/BrandedHex.js#L7-L11)
...
if (Hex.isSized(hex, 2)) {
  // TypeScript narrows to Sizedless than 2>
  hex  // Type: Sizedless than 2>
}

// Common sizes
const maybeHash: BrandedHex = Hex.random(32)
if (Hex.isSized(maybeHash, 32)) {
  maybeHash  // Type: Sizedless than 32> (Hash)
}

const maybeAddr: BrandedHex = Hex.random(20)
if (Hex.isSized(maybeAddr, 20)) {
  maybeAddr  // Type: Sizedless than 20> (Address)
}
```
...
Defined in: [primitives/Hex/BrandedHex/isSized.js:15-17](https://github.com/evmts/voltaire/blob/main/src/primitives/Hex/BrandedHex/isSized.js#L15-L17)
Tests: [isSized.test.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Hex/BrandedHex/isSized.test.ts)
...
Defined in: [primitives/Hex/BrandedHex/assertSize.js:18-25](https://github.com/evmts/voltaire/blob/main/src/primitives/Hex/BrandedHex/assertSize.js#L18-L25)
Tests: [assertSize.test.ts](https://github.com/evmts/voltaire/blob/main/src/primitives/Hex/BrandedHex/assertSize.test.ts)
...
Hex.assertSize(hash, 32)             // Sizedless than 32>

const addr = Hex.fromNumber(0, 20)   // BrandedHex
Hex.assertSize(addr, 20)             // Sizedless than 20>

// Pad to size
const value = Hex("0x1234")
const padded = Hex.pad(value, 32)    // BrandedHex
Hex.assertSize(padded, 32)           // Sizedless than 32>
```
...
Hex.assertSize(randomHash, 32)  // Sizedless than 32>

const zeroAddr = Hex.zero(20)
Hex.assertSize(zeroAddr, 20)    // Sizedless than 20>
```
...
type Selector = Sizedless than 4>
type U256 = Sizedless than 32>
type U128 = Sizedless than 16>
type U64 = Sizedless than 8>
type Byte = Sizedless than 1>
...
```typescript
function processHash(hash: Sizedless than 32>): Uint8Array {
  // TypeScript knows hash is 32 bytes
  return Hex.toBytes(hash)
}

function processAddress(addr: Sizedless than 20>): string {
...
processHash(wrong)  // ✗ compile error: Type 'BrandedHex' not assignable to 'Sizedless than 32>'
```
...
const hash = toFixedBytes(255n, 32)    // Sizedless than 32>
const addr = toFixedBytes(0n, 20)      // Sizedless than 20>
const selector = toFixedBytes(0n, 4)   // Sizedless than 4>
```
...
function createHash(hex: BrandedHex): Hash {
  ...
  return hex  // TypeScript knows it's Sizedless than 32>
}
...
```typescript
type EthValue =
  | { type: "hash"; value: Sizedless than 32> }
  | { type: "address"; value: Sizedless than 20> }
  | { type: "selector"; value: Sizedless than 4> }
...
```typescript
// Array of hashes
const hashes: Sizedless than 32>[] = [
...
const sized: Sizedless than 32> = Hex.assertSize(Hex.random(32), 32)
const unsized: BrandedHex = sized  // Valid - upcasting

// Must re-assert to recover size type
Hex.assertSize(unsized, 32)  // Sizedless than 32> again
```
...
function create(size: 32): Sizedless than 32>
function create(size: 20): Sizedless than 20>
...
if (isSized(hex, 32)) {
  hex  // Sizedless than 32>
}

const hash: Sizedless than 32> = assertSize(hex, 32)
