...
Defined in: [BrandedBloomFilter/toHex.js:3-20](https://github.com/evmts/primitives/blob/main/src/primitives/BloomFilter/BrandedBloomFilter/toHex.js#L3-L20)
...
const restored = BloomFilter.fromHex(data.logsBloom, data.m, data.k)
...
function parseBlockBloom(block: BlockHeader): BloomFilter {
  return BloomFilter.fromHex(block.logsBloom, 2048, 3)
}
...
function createBlockBloom(logs: Log[]): string {
  ...
  for (const log of logs) {
    // Add address
    filter.add(Address.from(log.address))
...
Defined in: [BrandedBloomFilter/hash.js:3-16](https://github.com/evmts/primitives/blob/main/src/primitives/BloomFilter/BrandedBloomFilter/hash.js#L3-L16)
...
// Check collisions
const collisions = Array.from(dist.values()).filter(count => count > 1).length
...
async function loadFilter(id: number): Promise<BloomFilter> {
  ...
  return BloomFilter.fromHex(data.hex, data.m, data.k)
}
```
...
class SerializableBloomFilter {
  ...
  static fromJSON(json: any): BloomFilter {
    return BloomFilter.fromHex(json.hex, json.m, json.k)
  }
}
...
ws.on("message", (data: string) => {
  ...
  if (message.type === "bloom_filter") {
    const filter = BloomFilter.fromHex(
      message.data,
      message.params.m,
      message.params.k
    )
...
const item = new Uint8Array(100)  // 100-byte item
...
// Tree-shakeable conversions
const filter: BrandedBloomFilter = create(2048, 3)
const hex = toHex(filter)

// Tree-shakeable hash
const item = new TextEncoder().encode("Transfer")
const h0 = hash(item, 0, 2048)
const h1 = hash(item, 1, 2048)
```
