...
// Rounds: 12 (standard Blake2b)
new DataView(input1.buffer).setUint32(0, 12, false);

// State h: Blake2b-512 IV XOR parameter block
// IV values for Blake2b-512 (little-endian)
const h1 = [
  0x6a09e667f3bcc908n ^ 0x01010040n, // XOR with params: digest length 64, fanout 1, depth 1
  0xbb67ae8584caa73bn,
  0x3c6ef372fe94f82bn,
  0xa54ff53a5f1d36f1n,
  0x510e527fade682d1n,
...
h1.forEach((val, i) => view1.setBigUint64(4 + i * 8, val, true));

// Message m: all zeros (empty message block)
// Already zero-filled

// Offset counter t: [0, 0] (no bytes processed yet)
view1.setBigUint64(196, 0n, true);
view1.setBigUint64(204, 0n, true);

// Final flag f: 0x01 (this is the final block even though empty)
input1[212] = 0x01;
...
new DataView(input2.buffer).setUint32(0, 12, false);

// State h: Same IV as above
const view2 = new DataView(input2.buffer);
h1.forEach((val, i) => view2.setBigUint64(4 + i * 8, val, true));

// Message m: "abc" padded to 128 bytes (little-endian encoding)
input2[68] = 0x61; // 'a'
input2[69] = 0x62; // 'b'
input2[70] = 0x63; // 'c'

// Offset counter t: [3, 0] (3 bytes processed)
...
const input3 = new Uint8Array(213);
new DataView(input3.buffer).setUint32(0, 1, false); // Only 1 round

const view3 = new DataView(input3.buffer);
h1.forEach((val, i) => view3.setBigUint64(4 + i * 8, val, true));
...
const input4 = new Uint8Array(213);
new DataView(input4.buffer).setUint32(0, 10000, false); // 10000 rounds

const view4 = new DataView(input4.buffer);
h1.forEach((val, i) => view4.setBigUint64(4 + i * 8, val, true));
