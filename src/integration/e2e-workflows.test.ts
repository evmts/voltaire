...
class MockEthereumProvider {
	...
	// biome-ignore lint/suspicious/noExplicitAny: Mock receipts have dynamic shape
	private txReceipts: Map<string, any> = new Map();
	private contractCode: Map<string, Uint8Array> = new Map();
	// biome-ignore lint/suspicious/noExplicitAny: Mock logs have dynamic shape
	private logs: any[] = [];
	...
	// biome-ignore lint/suspicious/noExplicitAny: Ethereum JSON-RPC params/return vary by method
	...
	// biome-ignore lint/suspicious/noExplicitAny: Mock contract call tx has dynamic shape
	...
	// biome-ignore lint/suspicious/noExplicitAny: Mock log has dynamic shape
...
describe("E2E Workflows", () => {
	describe("1. Deploy Contract", () => {
		it("should compile, sign deployment tx, execute, and verify address", async () => {
			...
			// biome-ignore lint/suspicious/noExplicitAny: Mock provider expects flexible tx object
	...
	describe("4. Multi-Signature Workflow", () => {
		it("should collect signatures from multiple signers and verify", async () => {
			...
			// biome-ignore lint/style/noNonNullAssertion: test context - signer derived from valid seed
	...
	describe("5. HD Wallet Derivation", () => {
		it("should generate seed, derive accounts, and sign from each", async () => {
			...
			for (let i = 0; i < 5; i++) {
				...
				// biome-ignore lint/style/noNonNullAssertion: test context - account derived from valid seed
	...
	describe("Cross-workflow Integration", () => {
		it("should handle complete wallet → sign → submit → verify flow", async () => {
			...
			// biome-ignore lint/style/noNonNullAssertion: test assumes valid HD wallet derivation
			const privKey = HDWallet.getPrivateKey(account)!;
