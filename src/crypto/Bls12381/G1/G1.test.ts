...
describe("BLS12-381 G1 curve operations", () => {
	...
	describe("isZero", () => {
		...
		test("point with z=0 is zero", () => {
			const point = { x: 1n, y: 2n, z: 0n } as G1PointType;
	...
	describe("isOnCurve", () => {
		...
		test("verifies curve equation y^2 = x^3 + 4", () => {
			const gen = G1.generator();
			// In affine: y^2 = x^3 + b where b = 4
			...
			for (let i = 0; i < multiples.length; i++) {
				for (let j = i + 1; j < multiples.length; j++) {
					...
					if (pi !== undefined && pj !== undefined) {
						expect(G1.equal(pi, pj)).toBe(false);
					}
				}
			}
		});
	...
	describe("edge cases", () => {
		...
		test("handles zero coordinates", () => {
			const point = { x: 0n, y: 0n, z: 1n } as G1PointType;
			G1.isOnCurve(point);
		});
