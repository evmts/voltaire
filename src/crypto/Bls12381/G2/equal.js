...
export function equal(p1, p2) {
	...
	// X1*Z2 = X2*Z1
	const x1z2 = Fp2.mul(p1.x, p2.z);
	const x2z1 = Fp2.mul(p2.x, p1.z);
	if (!Fp2.equal(x1z2, x2z1)) return false;

	// Y1*Z2 = Y2*Z1
	const y1z2 = Fp2.mul(p1.y, p2.z);
	const y2z1 = Fp2.mul(p2.y, p1.z);
	return Fp2.equal(y1z2, y2z1);
}
