...
describe("ChaCha20Poly1305", () => {
	...
	describe("error cases", () => {
		...
		it("throws DecryptionError with correct name for wrong key", () => {
			const key1 = ChaCha20Poly1305.generateKey();
			const key2 = ChaCha20Poly1305.generateKey();
			const nonce = ChaCha20Poly1305.generateNonce();
			const plaintext = new TextEncoder().encode("Test");
			...
			try {
				ChaCha20Poly1305.decrypt(ciphertext, key2, nonce);
		...
		it("throws DecryptionError with correct name for ciphertext too short", () => {
			const key = ChaCha20Poly1305.generateKey();
			const nonce = ChaCha20Poly1305.generateNonce();
			const tooShort = new Uint8Array(15); // Less than 16-byte tag

			try {
				ChaCha20Poly1305.decrypt(tooShort, key, nonce);
				expect.fail("Should have thrown");
			} catch (e) {
				expect((e as Error).name).toBe("DecryptionError");
			}
		...
		it("throws InvalidNonceError with correct name for invalid nonce in encrypt", () => {
			const key = ChaCha20Poly1305.generateKey();
		...
		it("throws InvalidNonceError with correct name for invalid nonce in decrypt", () => {
			const key = ChaCha20Poly1305.generateKey();
		...
		it("throws InvalidKeyError with correct name for invalid key size on encrypt", () => {
