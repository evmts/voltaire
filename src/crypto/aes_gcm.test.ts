...
describe("AesGcm", () => {
  ...
  describe("Encryption/Decryption", () => {
    ...
    it("encrypt and decrypt should work with additional data", async () => {
      ...
      const ciphertext = await AesGcm.encrypt(plaintext, key, nonce, additionalData);
      const decrypted = await AesGcm.decrypt(ciphertext, key, nonce, additionalData);
      expect(decrypted).toEqual(plaintext);
    });
    ...
    it("decrypt should fail with wrong additional data", async () => {
      ...
      await expect(AesGcm.decrypt(ciphertext, key, nonce, ad2)).rejects.toThrow();
    });
    ...
    it("encrypt should throw for invalid nonce length", async () => {
      ...
      await expect(AesGcm.encrypt(plaintext, key, invalidNonce)).rejects.toThrow();
    });
