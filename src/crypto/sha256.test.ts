...
describe.each([
  ...
  describe("NIST test vectors", () => {
    ...
    test("448-bit message", () => {
      const hash = impl.hashString("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq");
      const expected = new Uint8Array([
    ...
    test("quick brown fox", () => {
      const hash = impl.hashString("The quick brown fox jumps over the lazy dog");
      const expected = new Uint8Array([
...
describe("Cross-validation: Noble === WASM", () => {
  ...
  test("quick brown fox", () => {
    const noble = Sha256.hashString("The quick brown fox jumps over the lazy dog");
    const wasm = Sha256Wasm.hashString("The quick brown fox jumps over the lazy dog");
    expect(wasm).toEqual(noble);
  });
