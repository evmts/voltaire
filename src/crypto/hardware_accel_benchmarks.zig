...
test "comprehensive_crypto_hardware_acceleration_benchmarks" {
    ...
    std.log.debug("  Address generation (10k operations): {} ns ({d:.2} addresses/sec)", .{ 
        addr_gen_time, 
        10000.0 / (@as(f64, @floatFromInt(addr_gen_time)) / 1_000_000_000.0) 
    });
    ...
    std.log.debug("  Function selector (100k operations): {} ns ({d:.2} selectors/sec)", .{ 
        selector_time, 
        100000.0 / (@as(f64, @floatFromInt(selector_time)) / 1_000_000_000.0) 
    });
    ...
    std.log.debug("  Overall speedup: {d:.2}x", .{
        @as(f64, @floatFromInt(mixed_std_time)) / @as(f64, @floatFromInt(mixed_hw_time))
    });
