...
results.push(
	benchmark("hash - empty (Noble)", () => Keccak256.hash(emptyData)),
);
results.push(
	benchmark("hash - 32 bytes (Noble)", () => Keccak256.hash(data32B)),
);
results.push(
	benchmark("hash - 256 bytes (Noble)", () => Keccak256.hash(data256B)),
);
...
results.push(
	benchmark("hash - empty (WASM)", () => Keccak256Wasm.hash(emptyData)),
);
results.push(
	benchmark("hash - 32 bytes (WASM)", () => Keccak256Wasm.hash(data32B)),
);
results.push(
	benchmark("hash - 256 bytes (WASM)", () => Keccak256Wasm.hash(data256B)),
);
results.push(
	benchmark("hash - 1 KB (WASM)", () => Keccak256Wasm.hash(data1KB)),
);
results.push(
	benchmark("hash - 4 KB (WASM)", () => Keccak256Wasm.hash(data4KB)),
);
results.push(
	benchmark("hash - 16 KB (WASM)", () => Keccak256Wasm.hash(data16KB)),
);
results.push(
	benchmark("hash - 64 KB (WASM)", () => Keccak256Wasm.hash(data64KB)),
);
...
results.push(
	benchmark("hashString - empty (Noble)", () => Keccak256.hashString("")),
);
results.push(
	benchmark("hashString - short (Noble)", () =>
		Keccak256.hashString(shortString),
	),
);
results.push(
	benchmark("hashString - medium (Noble)", () =>
		Keccak256.hashString(mediumString),
	),
);
results.push(
	benchmark("hashString - long (Noble)", () =>
...
console.log("\n--- hashString (WASM) ---");
results.push(
	benchmark("hashString - empty (WASM)", () => Keccak256Wasm.hashString("")),
);
results.push(
	benchmark("hashString - short (WASM)", () =>
		Keccak256Wasm.hashString(shortString),
	),
);
results.push(
	benchmark("hashString - medium (WASM)", () =>
		Keccak256Wasm.hashString(mediumString),
	),
);
results.push(
	benchmark("hashString - long (WASM)", () =>
...
results.push(
	benchmark("hashHex - short (Noble)", () => Keccak256.hashHex(shortHex)),
);
results.push(
	benchmark("hashHex - medium (Noble)", () => Keccak256.hashHex(mediumHex)),
);
...
console.log("\n--- hashHex (WASM) ---");
results.push(
	benchmark("hashHex - short (WASM)", () => Keccak256Wasm.hashHex(shortHex)),
);
results.push(
	benchmark("hashHex - medium (WASM)", () => Keccak256Wasm.hashHex(mediumHex)),
);
...
results.push(
	benchmark("selector - function sig (Noble)", () =>
...
console.log("\n--- selector (WASM) ---");
results.push(
	benchmark("selector - function sig (WASM)", () =>
...
console.log("\n--- topic (Noble) ---");
results.push(
	benchmark("topic - event sig (Noble)", () => Keccak256.topic(eventSig)),
);
...
console.log("\n--- topic (WASM) ---");
results.push(
	benchmark("topic - event sig (WASM)", () => Keccak256Wasm.topic(eventSig)),
);
...
results.push(
	benchmark("contractAddress (Noble)", () =>
		Keccak256.contractAddress(address20, 0n),
	),
);
...
results.push(
	benchmark("contractAddress (WASM)", () =>
		Keccak256Wasm.contractAddress(address20, 0n),
	),
);
...
const throughputTests = [
	{
		name: "32 bytes",
		size: 32,
		noble: results.find((r) => r.name === "hash - 32 bytes (Noble)"),
		wasm: results.find((r) => r.name === "hash - 32 bytes (WASM)"),
	},
	{
		name: "256 bytes",
		size: 256,
		noble: results.find((r) => r.name === "hash - 256 bytes (Noble)"),
		wasm: results.find((r) => r.name === "hash - 256 bytes (WASM)"),
	},
	{
		name: "1 KB",
		size: 1024,
		noble: results.find((r) => r.name === "hash - 1 KB (Noble)"),
		wasm: results.find((r) => r.name === "hash - 1 KB (WASM)"),
	},
	{
		name: "4 KB",
		size: 4096,
		noble: results.find((r) => r.name === "hash - 4 KB (Noble)"),
		wasm: results.find((r) => r.name === "hash - 4 KB (WASM)"),
	},
	{
		name: "16 KB",
		size: 16384,
		noble: results.find((r) => r.name === "hash - 16 KB (Noble)"),
		wasm: results.find((r) => r.name === "hash - 16 KB (WASM)"),
	},
	{
		name: "64 KB",
		size: 65536,
		noble: results.find((r) => r.name === "hash - 64 KB (Noble)"),
		wasm: results.find((r) => r.name === "hash - 64 KB (WASM)"),
	},
].filter((t) => t.noble && t.wasm) as Array<{
	name: string;
	size: number;
	noble: BenchmarkResult;
...
for (const test of throughputTests) {
	...
	console.log(
		`  ${test.name}: ${speedup.toFixed(2)}x (${percentage}% ${speedup > 1 ? "faster" : "slower"})`,
	);
}
...
if (nobleFirst && nobleLast) {
	...
	console.log(
		`Noble - Slowest: ${nobleLast.name} - ${nobleLast.opsPerSec.toFixed(0)} ops/sec`,
	);
}
...
if (wasmFirst && wasmLast) {
	...
	console.log(
		`WASM - Slowest: ${wasmLast.name} - ${wasmLast.opsPerSec.toFixed(0)} ops/sec`,
	);
}

// Overall speedup
const avgNobleOps =
	nobleResults.reduce((sum, r) => sum + r.opsPerSec, 0) / nobleResults.length;
const avgWasmOps =
	wasmResults.reduce((sum, r) => sum + r.opsPerSec, 0) / wasmResults.length;
...
console.log(
	`  Speedup: ${overallSpeedup.toFixed(2)}x (${((overallSpeedup - 1) * 100).toFixed(1)}% ${overallSpeedup > 1 ? "faster" : "slower"})`,
);
