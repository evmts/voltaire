...
const BenchmarkResult = struct {
    ...
    pub fn format(self: BenchmarkResult, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.print("{s}: avg={d}ns, min={d}ns, max={d}ns, mem={d}bytes", .{ self.name, self.avg_time_ns, self.min_time_ns, self.max_time_ns, self.memory_used });
    }
};
...
fn benchmark_cross_contract_calls(allocator: Allocator) !void {
    ...
    var caller_contract = Evm.Contract.init_at_address(Address.ZERO, caller_addr, 0, // value
        1000000, // gas_limit
        &CALLER_CONTRACT_BYTECODE, &[_]u8{}, // empty input
        false // not static
    );
...
fn benchmark_error_propagation(allocator: Allocator) !void {
    ...
    var contract = Evm.Contract.init_at_address(Address.ZERO, target, 0, // value
        100, // low gas limit to trigger error
        &COMPLEX_CONTRACT_BYTECODE, &[_]u8{}, // empty input
        false // not static
    );
...
fn benchmark_component_integration(allocator: Allocator) !void {
    ...
    for (addresses) |addr| {
        ...
        var contract = Evm.Contract.init_at_address(Address.ZERO, addr, 100, // value
            1000000, // gas_limit
            &COMPLEX_CONTRACT_BYTECODE, &[_]u8{}, // empty input
            false // not static
        );
...
fn benchmark_memory_expansion(allocator: Allocator) !void {
    ...
    var contract = Evm.Contract.init_at_address(Address.ZERO, target, 0, // value
        1000000, // gas_limit
        &memory_test_bytecode, &[_]u8{}, // empty input
        false // not static
    );
    defer contract.deinit(allocator, null);
...
fn benchmark_stack_overflow_error(allocator: Allocator) !void {
    ...
    var contract = Evm.Contract.init_at_address(Address.ZERO, target, 0, // value
        1000000, // gas_limit
        overflow_bytecode.items, &[_]u8{}, // empty input
        false // not static
    );
    defer contract.deinit(allocator, null);
...
fn benchmark_invalid_opcode_error(allocator: Allocator) !void {
    ...
    var contract = Evm.Contract.init_at_address(Address.ZERO, target, 0, // value
        1000000, // gas_limit
        &invalid_bytecode, &[_]u8{}, // empty input
        false // not static
    );
    defer contract.deinit(allocator, null);
...
fn benchmark_memory_bounds_error(allocator: Allocator) !void {
    ...
    var contract = Evm.Contract.init_at_address(Address.ZERO, target, 0, // value
        1000000, // gas_limit
        &bounds_test_bytecode, &[_]u8{}, // empty input
        false // not static
    );
    defer contract.deinit(allocator, null);
