...
test "opcode 0xd2 differential test" {
    const allocator = std.testing.allocator;
    
    // Build bytecode for this opcode
    const bytecode = try common.build_bytecode(allocator, 0xd2);
    ...
    var revm_result = revm_vm.execute(caller_address, null, 0, bytecode, 1_000_000) catch |err| {
        ...
        if (guillotine_result.success) {
            std.debug.print("REVM failed but Guillotine succeeded for opcode 0xd2\n", .{});
            return err;
        }
