...
test "LOG0: emit log with no topics" {
    ...
    // Execute LOG0
    _ = try test_helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    
    // Check that log was emitted
    try testing.expectEqual(@as(usize, 1), test_vm.evm.state.logs.items.len);
    const emitted_log = test_vm.evm.state.logs.items[0];
...
test "LOG1: emit log with one topic" {
    ...
    // Execute LOG1
    _ = try test_helpers.executeOpcode(0xA1, test_vm.evm, test_frame.frame);
...
test "LOG2: emit log with two topics" {
    ...
    // Execute LOG2
    _ = try test_helpers.executeOpcode(0xA2, test_vm.evm, test_frame.frame);
...
test "LOG3: emit log with three topics" {
    ...
    // Execute LOG3
    _ = try test_helpers.executeOpcode(0xA3, test_vm.evm, test_frame.frame);
...
test "LOG4: emit log with four topics" {
    ...
    // Execute LOG4
    _ = try test_helpers.executeOpcode(0xA4, test_vm.evm, test_frame.frame);
...
test "LOG0: write protection in static call" {
    ...
    const result = test_helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
...
test "LOG1: write protection in static call" {
    ...
    const result = test_helpers.executeOpcode(0xA1, test_vm.evm, test_frame.frame);
    try testing.expectError(test_helpers.ExecutionError.Error.WriteProtection, result);
}
...
test "LOG4: stack underflow" {
    ...
    const result = test_helpers.executeOpcode(0xA4, test_vm.evm, test_frame.frame);
    try testing.expectError(test_helpers.ExecutionError.Error.StackUnderflow, result);
}
