...
test "Control: JUMP basic operations" {
    ...
    const result = helpers.executeOpcode(0x56, test_vm.evm, test_frame.frame);
    ...
    const result2 = helpers.executeOpcode(0x56, test_vm.evm, test_frame.frame);
    ...
    const result3 = helpers.executeOpcode(0x56, test_vm.evm, test_frame.frame);
...
test "Control: JUMPI conditional jump" {
    ...
    const result = helpers.executeOpcode(0x57, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.InvalidJump, result);
...
test "Control: PC returns program counter" {
    ...
    _ = try helpers.executeOpcode(0x58, test_vm.evm, test_frame.frame);
...
test "Control: RETURN with data" {
    ...
    const result2 = helpers.executeOpcode(0xF3, test_vm.evm, test_frame.frame);
    ...
    const result3 = helpers.executeOpcode(0xF3, test_vm.evm, test_frame.frame);
...
test "Control: REVERT with data" {
    ...
    const result2 = helpers.executeOpcode(0xFD, test_vm.evm, test_frame.frame);
    ...
    const result3 = helpers.executeOpcode(0xFD, test_vm.evm, test_frame.frame);
...
test "Control: INVALID always fails" {
    ...
    const result2 = helpers.executeOpcode(0xFE, test_vm.evm, test_frame.frame);
...
test "Control: SELFDESTRUCT basic operation" {
    ...
    _ = try test_vm.evm.access_list.access_address(helpers.TestAddresses.BOB);
    try test_frame.pushStack(&[_]u256{beneficiary});

    const result2 = helpers.executeOpcode(0xFF, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.STOP, result2);
    ...
    const result3 = helpers.executeOpcode(0xFF, test_vm.evm, test_frame.frame);
...
test "Control: Stack underflow errors" {
    ...
    const jump_result = helpers.executeOpcode(0x56, test_vm.evm, test_frame.frame);
