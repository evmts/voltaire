...
test "CREATE: create new contract" {
    ...
    // Execute CREATE
    _ = try test_helpers.executeOpcode(0xF0, test_vm.evm, test_frame.frame);
...
test "CREATE: write protection in static call" {
    ...
    const result = test_helpers.executeOpcode(0xF0, test_vm.evm, test_frame.frame);
...
test "CREATE2: create with deterministic address" {
    ...
    // Execute CREATE2
    _ = try test_helpers.executeOpcode(0xF5, test_vm.evm, test_frame.frame);
...
test "CALL: basic call behavior" {
    ...
    // Execute CALL
    _ = try test_helpers.executeOpcode(0xF1, test_vm.evm, test_frame.frame);
...
test "CALL: value transfer in static call fails" {
    ...
    const result = test_helpers.executeOpcode(0xF1, test_vm.evm, test_frame.frame);
    try testing.expectError(ExecutionError.Error.WriteProtection, result);
}
...
test "DELEGATECALL: execute code in current context" {
    ...
    _ = try test_helpers.executeOpcode(0xF4, test_vm.evm, test_frame.frame);
...
test "STATICCALL: read-only call" {
    ...
    // Execute STATICCALL
    _ = try test_helpers.executeOpcode(0xFA, test_vm.evm, test_frame.frame);
...
test "CREATE: memory expansion for init code" {
    ...
    try test_vm.evm.state.set_balance(test_helpers.TestAddresses.CONTRACT, 1000000);
...
test "CREATE2: EIP-3860 initcode size limit" {
    ...
    const result = test_helpers.executeOpcode(0xF5, test_vm.evm, test_frame.frame);
    try testing.expectError(ExecutionError.Error.MaxCodeSizeExceeded, result);
}
