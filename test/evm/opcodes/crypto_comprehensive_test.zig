...
test "KECCAK256: Error conditions" {
    ...
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x20, test_vm.evm, test_frame.frame));
    ...
    try test_frame.pushStack(&[_]u256{ large_size, 0 });
    try testing.expectError(helpers.ExecutionError.Error.OutOfGas, helpers.executeOpcode(0x20, test_vm.evm, test_frame.frame));
    test_frame.frame.stack.clear();
    ...
    try testing.expectError(helpers.ExecutionError.Error.OutOfOffset, helpers.executeOpcode(0x20, test_vm.evm, test_frame.frame));
    test_frame.frame.stack.clear();
