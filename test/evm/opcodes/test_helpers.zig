...
pub const Evm = EvmModule; // Export the Evm module itself
...
pub const Address = EvmModule.Address;
pub const EVM = EvmModule.Evm;
pub const Operation = EvmModule.Operation;
pub const OperationModule = EvmModule.Operation;
pub const ExecutionError = EvmModule.ExecutionError;
pub const opcodes = EvmModule.opcodes;
pub const Hardfork = EvmModule.Hardfork.Hardfork;
pub const JumpTable = EvmModule.JumpTable;
pub const ChainRules = EvmModule.chain_rules;
pub const MemoryDatabase = EvmModule.MemoryDatabase;
pub const DatabaseInterface = EvmModule.DatabaseInterface;
pub const StorageKey = EvmModule.Evm.StorageKey;
...
pub const TestVm = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        const evm_instance = try allocator.create(EvmModule.Evm);
        const memory_db = try allocator.create(MemoryDatabase);
        memory_db.* = MemoryDatabase.init(allocator);
        const db_interface = memory_db.to_database_interface();
        evm_instance.* = try EvmModule.Evm.init(allocator, db_interface, null, null);
        return Self{
            .evm = evm_instance,
            .memory_db = memory_db,
        };
    ...
    pub fn init_with_hardfork(allocator: std.mem.Allocator, hardfork: EvmModule.Hardfork.Hardfork) !Self {
        const evm_instance = try allocator.create(EvmModule.Evm);
        const memory_db = try allocator.create(MemoryDatabase);
        memory_db.* = MemoryDatabase.init(allocator);
        const db_interface = memory_db.to_database_interface();
        evm_instance.* = try EvmModule.Evm.init_with_hardfork(allocator, db_interface, hardfork);
        return Self{
            .evm = evm_instance,
            .memory_db = memory_db,
        };
...
pub fn executeOpcode(
    opcode_byte: u8,
    evm_instance: *EvmModule.Evm,
    frame: *Frame,
) ExecutionError.Error!OperationModule.ExecutionResult {
    const interpreter_ptr: *OperationModule.Interpreter = @ptrCast(evm_instance);
    const state_ptr: *OperationModule.State = @ptrCast(frame);

    // Debug: Check if jump table has the opcode
    // const operation = evm_instance.table.get_operation(opcode_byte);

    // Use the Evm's jump table to execute the opcode
    ...
    const pc_value = if (opcode_byte == 0x58 or (opcode_byte >= 0x60 and opcode_byte <= 0x7F))
        ...
        0;
    return try evm_instance.table.execute(pc_value, interpreter_ptr, state_ptr, opcode_byte);
}
...
/// Helper function to run bytecode for testing (replaces evm.run_bytecode)
...
pub fn runBytecode(
    ...
    gas: u64,
    input: ?[]const u8,
) RunBytecodeError!EvmModule.RunResult {
