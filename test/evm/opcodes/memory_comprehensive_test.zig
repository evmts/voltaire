...
test "MSTORE: Stack underflow protection" {
    ...
    // Test 1: Empty stack
    const result1 = evm.table.execute(0, interpreter_ptr, state_ptr, 0x52);
    try testing.expectError(ExecutionError.Error.StackUnderflow, result1);

    // Test 2: Only one value on stack (need two)
    try frame.stack.append(42);
    const result2 = evm.table.execute(0, interpreter_ptr, state_ptr, 0x52);
...
test "MSTORE8: Memory expansion and gas costs" {
    ...
    const result = evm.table.execute(0, interpreter_ptr, state_ptr, 0x53);
    try testing.expectError(ExecutionError.Error.OutOfOffset, result);
}
...
test "MSTORE8: Stack underflow protection" {
    ...
    // Test 1: Empty stack
    const result1 = evm.table.execute(0, interpreter_ptr, state_ptr, 0x53);
    try testing.expectError(ExecutionError.Error.StackUnderflow, result1);

    // Test 2: Only one value on stack (need two)
    try frame.stack.append(42);
    const result2 = evm.table.execute(0, interpreter_ptr, state_ptr, 0x53);
    try testing.expectError(ExecutionError.Error.StackUnderflow, result2);
}
