...
test "SWAP1 (0x90): Swap top two stack items" {
    ...
    // Execute SWAP1
    const result = try helpers.executeOpcode(0x90, test_vm.evm, test_frame.frame);
...
test "SWAP2 (0x91): Swap 1st and 3rd stack items" {
    ...
    // Execute SWAP2
    const result = try helpers.executeOpcode(0x91, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
...
test "SWAP3-SWAP5: Various swaps" {
    ...
    var result = try helpers.executeOpcode(0x92, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
    ...
    // Before SWAP4
    result = try helpers.executeOpcode(0x93, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
    // After SWAP4
    ...
    test_frame.frame.pc = 2;
    // Before SWAP5
    result = try helpers.executeOpcode(0x94, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
    // After SWAP5
...
test "SWAP6-SWAP10: Mid-range swaps" {
    ...
    // Execute SWAP6
    test_frame.frame.pc = 0;
    const result6 = try helpers.executeOpcode(0x95, test_vm.evm, test_frame.frame);
    ...
    // Execute SWAP7
    test_frame.frame.pc = 1;
    _ = try helpers.executeOpcode(0x96, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, 0x103); // Was at position 7

    // Execute SWAP8
    test_frame.frame.pc = 2;
    _ = try helpers.executeOpcode(0x97, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, 0x102); // Was at position 8

    // Execute SWAP9
    test_frame.frame.pc = 3;
    _ = try helpers.executeOpcode(0x98, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, 0x101); // Was at position 9

    // Execute SWAP10
    test_frame.frame.pc = 4;
    _ = try helpers.executeOpcode(0x99, test_vm.evm, test_frame.frame);
...
test "SWAP11-SWAP16: High-range swaps" {
    ...
    // Execute SWAP11
    test_frame.frame.pc = 0;
    _ = try helpers.executeOpcode(0x9A, test_vm.evm, test_frame.frame);
...
test "SWAP16 (0x9F): Swap with 16th position (maximum)" {
    ...
    const result = try helpers.executeOpcode(0x9F, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
...
test "SWAP1-SWAP16: Gas consumption" {
    ...
    for (0..16) |i| {
        ...
        const result = try helpers.executeOpcode(opcode, test_vm.evm, test_frame.frame);
...
test "SWAP operations: Stack underflow" {
    ...
    test_frame.frame.pc = 0;
    var result = helpers.executeOpcode(0x90, test_vm.evm, test_frame.frame);
    ...
    result = helpers.executeOpcode(0x90, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
    ...
    test_frame.frame.pc = 1;
    result = helpers.executeOpcode(0x91, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
    ...
    test_frame.frame.pc = 2;
    _ = try helpers.executeOpcode(0x95, test_vm.evm, test_frame.frame);

    // SWAP16 should fail (have 7 items, need 17)
    test_frame.frame.pc = 3;
    result = helpers.executeOpcode(0x9F, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
}
...
test "SWAP operations: Sequential swaps" {
    ...
    _ = try helpers.executeOpcode(0x91, test_vm.evm, test_frame.frame);
...
test "SWAP operations: Pattern verification" {
    ...
    test_frame.frame.pc = 1;
    _ = try helpers.executeOpcode(0x94, test_vm.evm, test_frame.frame);
...
test "SWAP operations: Boundary test with exact stack size" {
    ...
    const result = helpers.executeOpcode(0x9F, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
}
...
test "SWAP operations: No side effects" {
    ...
    // Execute SWAP3
    _ = try helpers.executeOpcode(0x92, test_vm.evm, test_frame.frame);
