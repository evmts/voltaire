...
test "JUMPI (0x57): Conditional jump with true condition" {
    ...
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 1
    
    // PUSH1 8 (destination)
    test_frame.frame.pc = 2;
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 8
...
test "PC: Stack overflow protection" {
    ...
    const result = helpers.executeOpcode(0x58, test_vm.evm, test_frame.frame);
...
test "GAS: After consuming gas with operations" {
    ...
    _ = try helpers.executeOpcode(0x02, test_vm.evm, test_frame.frame); // MUL (costs 5)
    _ = try test_frame.popStack();
...
test "Control Flow: Complex jump sequences" {
    ...
    test_frame.frame.pc = 0;
    _ = try helpers.executeOpcode(0x5B, test_vm.evm, test_frame.frame); // JUMPDEST
    ...
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 0x42
    ...
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 17

    // 8. JUMPI (should jump because condition is 1)
    test_frame.frame.pc = 15;
    _ = try helpers.executeOpcode(0x57, test_vm.evm, test_frame.frame); // JUMPI
...
test "Control Flow: Out of gas scenarios" {
    ...
    for (test_cases) |case| {
        ...
        const result = helpers.executeOpcode(case.opcode, test_vm.evm, test_frame.frame);
        try testing.expectError(helpers.ExecutionError.Error.OutOfGas, result);
    }
}
