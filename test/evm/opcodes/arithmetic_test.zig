...
test "Arithmetic: ADD basic operations" {
    ...
    _ = try helpers.executeOpcodeWithGas(&jump_table, 0x01, test_vm.evm, test_frame.frame); // 0x01 = ADD
...
test "Arithmetic: Stack underflow errors" {
    ...
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x01, test_vm.evm, test_frame.frame));
    ...
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x08, test_vm.evm, test_frame.frame));
}
