...
test "Comparison: Stack underflow errors" {
    ...
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x10, test_vm.evm, test_frame.frame));
    ...
    test_frame.frame.stack.clear();
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x15, test_vm.evm, test_frame.frame));
}
...
test "Comparison: Gas consumption verification" {
    ...
    inline for (operations) |op_info| {
        ...
        _ = try helpers.executeOpcode(op_info.opcode, test_vm.evm, test_frame.frame);
        try helpers.expectGasUsed(test_frame.frame, 1000, helpers.opcodes.gas_constants.GasFastestStep);
    }
}
