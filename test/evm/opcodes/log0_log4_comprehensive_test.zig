...
test "LOG0 (0xA0): Emit log with no topics" {
    ...
    // Execute LOG0
    const result = try helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);

    // Check that log was emitted
    try testing.expectEqual(@as(usize, 1), test_vm.evm.state.logs.items.len);
    const log = test_vm.evm.state.logs.items[0];
...
test "LOG1 (0xA1): Emit log with one topic" {
    ...
    test_frame.frame.pc = 0;
    _ = try helpers.executeOpcode(0x7F, test_vm.evm, test_frame.frame); // PUSH32 topic
    test_frame.frame.pc = 33;
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 size
    test_frame.frame.pc = 35;
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // PUSH1 offset
    test_frame.frame.pc = 37;

    // Execute LOG1
    const result = try helpers.executeOpcode(0xA1, test_vm.evm, test_frame.frame);
    try testing.expectEqual(@as(usize, 1), result.bytes_consumed);
...
test "LOG2-LOG4: Multiple topics" {
    ...
    _ = try helpers.executeOpcode(0xA2, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0xA4, test_vm.evm, test_frame.frame);

    // Verify all logs
    try testing.expectEqual(@as(usize, 3), test_vm.evm.state.logs.items.len);

    // Check LOG2
    const log2 = test_vm.evm.state.logs.items[0];
    ...
    // Check LOG3
    const log3 = test_vm.evm.state.logs.items[1];
    ...
    // Check LOG4
    const log4 = test_vm.evm.state.logs.items[2];
...
test "LOG0-LOG4: Gas consumption" {
    ...
    _ = try helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0xA1, test_vm.evm, test_frame.frame);
...
test "LOG operations: Static call protection" {
    ...
    test_frame.frame.pc = 4;
    const result = helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    ...
    for (log_opcodes) |opcode| {
        ...
        const res = helpers.executeOpcode(opcode, test_vm.evm, test_frame.frame);
...
test "LOG operations: Stack underflow" {
    ...
    test_frame.frame.pc = 0;
    var result = helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);

    try test_frame.pushStack(&[_]u256{0});
    result = helpers.executeOpcode(0xA0, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
    ...
    test_frame.frame.pc = 4;
    result = helpers.executeOpcode(0xA4, test_vm.evm, test_frame.frame);
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, result);
}
...
test "LOG operations: ERC20 Transfer event pattern" {
    ...
    test_frame.frame.pc = 0;
    _ = try helpers.executeOpcode(0x73, test_vm.evm, test_frame.frame); // to address
    test_frame.frame.pc = 21;
    _ = try helpers.executeOpcode(0x73, test_vm.evm, test_frame.frame); // from address
    test_frame.frame.pc = 42;
    _ = try helpers.executeOpcode(0x7F, test_vm.evm, test_frame.frame); // signature
    test_frame.frame.pc = 75;
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // size
    test_frame.frame.pc = 77;
    _ = try helpers.executeOpcode(0x60, test_vm.evm, test_frame.frame); // offset
...
test "LOG operations: Multiple logs in sequence" {
    ...
    // First LOG0
    try testing.expectEqual(@as(usize, 0), test_vm.evm.state.logs.items[0].topics.len);
    try testing.expectEqualSlices(u8, &[_]u8{ 0xAA, 0xBB, 0xCC, 0xDD }, test_vm.evm.state.logs.items[0].data);

    // LOG1
    try testing.expectEqual(@as(usize, 1), test_vm.evm.state.logs.items[1].topics.len);
    try testing.expectEqual(@as(u256, 0x99), test_vm.evm.state.logs.items[1].topics[0]);
    try testing.expectEqualSlices(u8, &[_]u8{ 0x11, 0x22, 0x33, 0x44 }, test_vm.evm.state.logs.items[1].data);

    // Second LOG0
    try testing.expectEqual(@as(usize, 0), test_vm.evm.state.logs.items[2].topics.len);
    try testing.expectEqualSlices(u8, &[_]u8{ 0xFF, 0xEE, 0xDD, 0xCC }, test_vm.evm.state.logs.items[2].data);
}
