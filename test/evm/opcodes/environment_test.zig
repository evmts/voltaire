...
test "Environment: BALANCE opcode" {
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.ALICE, test_balance);
...
test "Environment: EXTCODESIZE opcode" {
    ...
    try test_vm.evm.state.set_code(helpers.TestAddresses.BOB, &test_code);
...
test "Environment: SELFBALANCE opcode (Istanbul)" {
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.CONTRACT, contract_balance);
...
test "Environment: Cold/Warm address access (EIP-2929)" {
    ...
    // Set up account
    try test_vm.evm.state.set_balance(helpers.TestAddresses.BOB, 1000);
...
test "Environment: Stack underflow errors" {
    ...
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x31, test_vm.evm, test_frame.frame));

    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x3B, test_vm.evm, test_frame.frame));

    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x3F, test_vm.evm, test_frame.frame));

    // EXTCODECOPY needs 4 stack items
    try test_frame.pushStack(&[_]u256{ 1, 2, 3 }); // Only 3 items
    try testing.expectError(helpers.ExecutionError.Error.StackUnderflow, helpers.executeOpcode(0x3C, test_vm.evm, test_frame.frame));
}
