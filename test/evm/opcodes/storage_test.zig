...
test "SLOAD: load value from storage" {
    ...
    // Set storage value
    try test_vm.evm.state.set_storage(test_helpers.TestAddresses.CONTRACT, 0x123, 0x456789);
    
    // Push storage slot
    try test_frame.pushStack(&[_]u256{0x123});
    
    // Execute SLOAD
    _ = try test_helpers.executeOpcode(0x54, test_vm.evm, test_frame.frame);
...
test "SSTORE: store value to storage" {
    ...
    // Execute SSTORE
    _ = try test_helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame);
    
    // Check that value was stored
    const stored_value = test_vm.evm.state.get_storage(test_helpers.TestAddresses.CONTRACT, 0x555);
...
test "SSTORE: overwrite existing value" {
    ...
    // Set initial value
    try test_vm.evm.state.set_storage(test_helpers.TestAddresses.CONTRACT, 0x100, 0x111);
    ...
    const stored_value = test_vm.evm.state.get_storage(test_helpers.TestAddresses.CONTRACT, 0x100);
...
test "SSTORE: write protection in static call" {
    ...
    const result = test_helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame);
...
test "TLOAD: load value from transient storage" {
    ...
    try test_vm.evm.state.set_transient_storage(test_helpers.TestAddresses.CONTRACT, 0xAAA, 0xBBBBBB);
    
    // Push storage slot
    try test_frame.pushStack(&[_]u256{0xAAA});
    
    // Execute TLOAD
    _ = try test_helpers.executeOpcode(0x5C, test_vm.evm, test_frame.frame);
...
test "TLOAD: transient storage is separate from regular storage" {
    ...
    try test_vm.evm.state.set_storage(test_helpers.TestAddresses.CONTRACT, 0x100, 0x111);
    try test_vm.evm.state.set_transient_storage(test_helpers.TestAddresses.CONTRACT, 0x100, 0x222);
...
test "TSTORE: store value to transient storage" {
    ...
    // Execute TSTORE
    _ = try test_helpers.executeOpcode(0x5D, test_vm.evm, test_frame.frame);
    
    // Check that value was stored
    const stored_value = test_vm.evm.state.get_transient_storage(test_helpers.TestAddresses.CONTRACT, 0x777);
...
test "TSTORE: overwrite existing transient value" {
    ...
    try test_vm.evm.state.set_transient_storage(test_helpers.TestAddresses.CONTRACT, 0x200, 0x333);
    ...
    // Check that value was updated
    const stored_value = test_vm.evm.state.get_transient_storage(test_helpers.TestAddresses.CONTRACT, 0x200);
...
test "TSTORE: write protection in static call" {
    ...
    const result = test_helpers.executeOpcode(0x5D, test_vm.evm, test_frame.frame);
    try testing.expectError(ExecutionError.Error.WriteProtection, result);
}
...
test "TSTORE: does not affect regular storage" {
    ...
    try test_vm.evm.state.set_storage(test_helpers.TestAddresses.CONTRACT, 0x300, 0x555);
    ...
    const regular_value = test_vm.evm.state.get_storage(test_helpers.TestAddresses.CONTRACT, 0x300);
    try testing.expectEqual(@as(u256, 0x555), regular_value);
    
    // Transient storage should have new value
    const transient_value = test_vm.evm.state.get_transient_storage(test_helpers.TestAddresses.CONTRACT, 0x300);
...
test "SLOAD: stack underflow" {
    ...
    const result = test_helpers.executeOpcode(0x54, test_vm.evm, test_frame.frame);
...
test "TLOAD: stack underflow" {
    ...
    const result = test_helpers.executeOpcode(0x5C, test_vm.evm, test_frame.frame);
    try testing.expectError(ExecutionError.Error.StackUnderflow, result);
}
