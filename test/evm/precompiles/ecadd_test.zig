...
test "ECADD point at infinity addition" {
    ...
    const result = Evm.precompiles.execute_precompile(ECADD_ADDRESS, &input, &output, 1000, chain_rules);
...
test "ECADD out of gas" {
    ...
    const result = Evm.precompiles.execute_precompile(ECADD_ADDRESS, &input, &output, 149, chain_rules);
...
test "ECADD insufficient output buffer" {
    ...
    const result = Evm.precompiles.execute_precompile(ECADD_ADDRESS, &input, &small_output, 1000, chain_rules);
...
test "ECADD gas cost hardfork consistency" {
    ...
    try testing.expectEqual(try evm.precompiles.estimate_gas(ECADD_ADDRESS, 128, istanbul_rules), try evm.precompiles.estimate_gas(ECADD_ADDRESS, 128, london_rules));

    try testing.expectEqual(try evm.precompiles.estimate_gas(ECADD_ADDRESS, 128, london_rules), try evm.precompiles.estimate_gas(ECADD_ADDRESS, 128, cancun_rules));
}
...
test "ECADD EIP-196 test vector - inverse points" {
    ...
    const p_minus_2_bytes = [_]u8{ 0x30, 0x64, 0x4e, 0x72, 0xe1, 0x31, 0xa0, 0x29, 0xb8, 0x50, 0x45, 0xb6, 0x81, 0x81, 0x58, 0x5d, 0x97, 0x81, 0x6a, 0x91, 0x68, 0x71, 0xca, 0x8d, 0x3c, 0x20, 0x8c, 0x16, 0xd8, 0x7c, 0xfd, 0x45 };
...
test "ECADD EIP-196 test vector - large coordinates" {
    ...
    const x1_bytes = [_]u8{ 0x2b, 0xd3, 0xe6, 0xd0, 0xf3, 0xb1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39, 0x4b, 0xd1, 0x42, 0x39 };
    const y1_bytes = [_]u8{ 0x1d, 0x86, 0x45, 0x15, 0x37, 0x7f, 0x21, 0x63, 0x7e, 0x76, 0x4a, 0x29, 0x5e, 0x94, 0xf4, 0x48, 0x1d, 0x86, 0x45, 0x15, 0x37, 0x7f, 0x21, 0x63, 0x7e, 0x76, 0x4a, 0x29, 0x5e, 0x94, 0xf4, 0x48 };
...
test "ECADD EIP-196 edge case - maximum field values" {
    ...
    const max_field_bytes = [_]u8{ 0x30, 0x64, 0x4e, 0x72, 0xe1, 0x31, 0xa0, 0x29, 0xb8, 0x50, 0x45, 0xb6, 0x81, 0x81, 0x58, 0x5d, 0x97, 0x81, 0x6a, 0x91, 0x68, 0x71, 0xca, 0x8d, 0x3c, 0x20, 0x8c, 0x16, 0xd8, 0x7c, 0xfd, 0x47 };
...
test "ECADD boundary test - exact gas limit" {
    ...
    const result = Evm.precompiles.execute_precompile(ECADD_ADDRESS, &input, &output, 150, // Exactly the required amount
        chain_rules);
...
test "ECADD large input stress test" {
    ...
    var output = [_]u8{0} ** 64;
    const result = Evm.precompiles.execute_precompile(ECADD_ADDRESS, &large_input, &output, 1000, chain_rules);
