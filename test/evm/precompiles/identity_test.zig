...
const ChainRules = @import("../../../src/evm/execution_context.zig").ChainRules;
...
test "precompile dispatcher execute identity" {
    ...
    const chain_rules = ChainRules.DEFAULT;
    const result = precompiles.execute_precompile(identity_address, &input_data, output_buffer, 100, chain_rules);
...
test "precompile dispatcher invalid address" {
    ...
    const result = precompiles.execute_precompile(invalid_address, &[_]u8{ 1, 2, 3 }, &output_buffer, 100, chain_rules);
