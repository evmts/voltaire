...
test "Integration: Logging with topics and data" {
    ...
    try testing.expectEqual(@as(usize, 1), test_vm.evm.logs.items.len);
    const log = test_vm.evm.logs.items[0];
...
test "Integration: LOG operations with multiple topics" {
    ...
    test_vm.evm.logs.clearRetainingCapacity();
    ...
    // Verify LOG0
    try testing.expectEqual(@as(usize, 1), test_vm.evm.logs.items.len);
    try testing.expectEqual(@as(usize, 0), test_vm.evm.logs.items[0].topics.len);
}
...
test "Integration: Log in static context fails" {
    ...
    const result = opcodes.log.op_log0(0, test_vm.evm, test_frame.frame);
    ...
    const result1 = opcodes.log.op_log1(0, test_vm.evm, test_frame.frame);
