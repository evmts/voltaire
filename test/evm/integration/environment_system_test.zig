...
test "Integration: Contract deployment simulation" {
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.ALICE, deployer_balance);
    ...
    _ = helpers.executeOpcode(0xF0, test_vm.evm, test_frame.frame) catch |err| {
...
test "Integration: Call with value transfer" {
    ...
    // Set up accounts
    try test_vm.evm.state.set_balance(helpers.TestAddresses.ALICE, helpers.TestValues.ONE_ETHER);
    try test_vm.evm.state.set_balance(helpers.TestAddresses.BOB, 0);
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.CONTRACT, helpers.TestValues.ONE_ETHER);
    ...
    _ = helpers.executeOpcode(0xF1, test_vm.evm, test_frame.frame) catch |err| {
...
test "Integration: Environment data access" {
    ...
    // Test ADDRESS
    _ = try helpers.executeOpcode(0x30, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, helpers.toU256(helpers.TestAddresses.CONTRACT));
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame);

    // Test ORIGIN
    _ = try helpers.executeOpcode(0x32, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, helpers.toU256(helpers.TestAddresses.ALICE));
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame);

    // Test CALLER
    _ = try helpers.executeOpcode(0x33, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, helpers.toU256(helpers.TestAddresses.BOB));
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame);

    // Test CALLVALUE
    _ = try helpers.executeOpcode(0x34, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, helpers.TestValues.ONE_GWEI);
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame);

    // Test GASPRICE
    _ = try helpers.executeOpcode(0x3A, test_vm.evm, test_frame.frame);
...
test "Integration: Log emission with topics" {
    ...
    const initial_log_count = test_vm.evm.state.logs.items.len;
    _ = try helpers.executeOpcode(0xA3, test_vm.evm, test_frame.frame);

    // Verify log was emitted
    try testing.expectEqual(initial_log_count + 1, test_vm.evm.state.logs.items.len);

    const emitted_log = test_vm.evm.state.logs.items[test_vm.evm.state.logs.items.len - 1];
...
test "Integration: External code operations" {
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.BOB, 0);
    try test_vm.evm.state.set_code(helpers.TestAddresses.BOB, &external_code);
    ...
    _ = try helpers.executeOpcode(0x3B, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0x3C, test_vm.evm, test_frame.frame);
    ...
    // Test EXTCODEHASH
    try test_frame.pushStack(&[_]u256{helpers.toU256(helpers.TestAddresses.BOB)});
    _ = try helpers.executeOpcode(0x3F, test_vm.evm, test_frame.frame);
...
test "Integration: Calldata operations" {
    ...
    // Test CALLDATASIZE
    _ = try helpers.executeOpcode(0x36, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, calldata.len);
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame);

    // Test CALLDATALOAD at offset 0 (function selector)
    try test_frame.pushStack(&[_]u256{0});
    _ = try helpers.executeOpcode(0x35, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0x37, test_vm.evm, test_frame.frame);
...
test "Integration: Self balance and code operations" {
    ...
    try test_vm.evm.state.set_balance(helpers.TestAddresses.CONTRACT, helpers.TestValues.ONE_ETHER);
    try test_vm.evm.state.set_code(helpers.TestAddresses.CONTRACT, &contract_code);
    ...
    // Test CODESIZE
    _ = try helpers.executeOpcode(0x38, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0x39, test_vm.evm, test_frame.frame);
