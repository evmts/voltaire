...
test "Integration: Token balance check pattern" {
    ...
    _ = try helpers.executeOpcode(0x80, test_vm.evm, test_frame.frame); // Duplicate slot
    try test_frame.pushStack(&[_]u256{initial_balance}); // Stack: [slot_hash, slot_hash, initial_balance]
    _ = try helpers.executeOpcode(0x90, test_vm.evm, test_frame.frame); // SWAP1: [slot_hash, initial_balance, slot_hash]
    _ = try helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame); // SSTORE: pops slot_hash, then initial_balance

    // Load balance
    _ = try helpers.executeOpcode(0x54, test_vm.evm, test_frame.frame); // SLOAD using the remaining slot_hash
    ...
    _ = try helpers.executeOpcode(0x90, test_vm.evm, test_frame.frame); // SWAP1. Stack: [100, balance]
    _ = try helpers.executeOpcode(0x10, test_vm.evm, test_frame.frame); // LT pops balance, then 100. Compares 100 < balance.
...
test "Integration: Packed struct storage" {
    ...
    _ = try helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame); // SSTORE: pops slot, then packed_value
...
test "Integration: Dynamic array length update" {
    ...
    // Store new length
    _ = try helpers.executeOpcode(0x80, test_vm.evm, test_frame.frame); // Duplicate new length
    try test_frame.pushStack(&[_]u256{array_slot}); // Stack: [old_length, new_length, array_slot]
    _ = try helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame); // SSTORE: pops array_slot, then new_length
...
test "Integration: Reentrancy guard pattern" {
    ...
    _ = try helpers.executeOpcode(0x50, test_vm.evm, test_frame.frame); // Remove old value from stack
    try test_frame.pushStack(&[_]u256{ ENTERED, guard_slot }); // Stack: [ENTERED, guard_slot]
    _ = try helpers.executeOpcode(0x55, test_vm.evm, test_frame.frame); // SSTORE: pops guard_slot, then ENTERED
...
test "Integration: Multi-sig wallet threshold check" {
    ...
    _ = try helpers.executeOpcode(0x54, test_vm.evm, test_frame.frame);
    try test_frame.pushStack(&[_]u256{1});
    _ = try helpers.executeOpcode(0x01, test_vm.evm, test_frame.frame); // Add 1
    ...
    _ = try helpers.executeOpcode(0x10, test_vm.evm, test_frame.frame); // LT: confirmations < required
    _ = try helpers.executeOpcode(0x15, test_vm.evm, test_frame.frame); // NOT(confirmations < required) = confirmations >= required
