...
test "Integration: Call with value transfer and balance check" {
    ...
    try test_vm.evm.balances.put(helpers.TestAddresses.ALICE, 1000);
    try test_vm.evm.balances.put(helpers.TestAddresses.BOB, 500);
    ...
    try test_vm.evm.balances.put(helpers.TestAddresses.ALICE, 900);
    try test_vm.evm.balances.put(helpers.TestAddresses.BOB, 600);
...
test "Integration: Environment opcodes in context" {
    ...
    test_vm.evm.tx_origin = helpers.TestAddresses.ALICE;
    test_vm.evm.gas_price = 20_000_000_000; // 20 gwei
    test_vm.evm.block_number = 15_000_000;
    test_vm.evm.timestamp = 1_650_000_000;
    test_vm.evm.block_coinbase = helpers.TestAddresses.CHARLIE;
    test_vm.evm.chain_id = 1; // Mainnet
...
test "Integration: DELEGATECALL preserves context" {
    ...
    try test_vm.evm.balances.put(helpers.TestAddresses.BOB, 0);
    try test_vm.evm.code.put(helpers.TestAddresses.BOB, &delegate_code);
    ...
    _ = try helpers.executeOpcode(0xF4, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, 1); // Success
...
test "Integration: STATICCALL prevents state changes" {
    ...
    _ = try helpers.executeOpcode(0xFA, test_vm.evm, test_frame.frame);
    try helpers.expectStackValue(test_frame.frame, 0, 1); // Success
...
test "Integration: Return data handling across calls" {
    ...
    test_frame.frame.stack.clear();
    _ = try helpers.executeOpcode(0x3D, test_vm.evm, test_frame.frame);
    ...
    _ = try helpers.executeOpcode(0x3E, test_vm.evm, test_frame.frame);
