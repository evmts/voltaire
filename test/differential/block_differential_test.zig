...
test "COINBASE opcode returns block coinbase" {
    ...
    if (revm_succeeded and guillotine_succeeded) {
        ...
        // COINBASE should return a valid address
...
test "GASLIMIT opcode returns block gas limit" {
    const allocator = testing.allocator;
    const bytecode = [_]u8{
        0x45, // GASLIMIT
        0x60, 0x00, // PUSH1 0 (memory offset)
    ...
    if (revm_succeeded and guillotine_succeeded) {
        ...
        try testing.expectEqual(revm_value, guillotine_value);
        // GASLIMIT should return a positive value
        try testing.expect(revm_value > 0);
    }
}
