...
test "fuzz_add_extensive_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x01); // ADD
...
test "fuzz_mul_comprehensive_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x02); // MUL
...
test "fuzz_sub_underflow_and_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x03); // SUB
...
test "fuzz_div_division_by_zero_and_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x04); // DIV
...
test "fuzz_sdiv_signed_division_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x05); // SDIV
...
test "fuzz_mod_modulo_by_zero_and_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x06); // MOD
...
test "fuzz_addmod_modular_arithmetic_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x08); // ADDMOD
...
test "fuzz_mulmod_modular_multiplication_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x09); // MULMOD
...
test "fuzz_exp_exponentiation_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x0A); // EXP
...
test "fuzz_signextend_sign_extension_edge_cases" {
    ...
    for (test_cases) |case| {
        ...
        const interpreter_ptr: *evm.Operation.Interpreter = @ptrCast(&ctx.vm);
        const state_ptr: *evm.Operation.State = @ptrCast(&ctx.frame);
        _ = try ctx.vm.table.execute(0, interpreter_ptr, state_ptr, 0x0B); // SIGNEXTEND
